...
These IDs and entity references also serve as the addressing basis for AI jobs. All AI operations reference entities by stable IDs (block ID, row ID, node ID, etc.), never by text offsets. See §AI-Job-Model.2.3 for the `EntityRef` structure used by the global AI job model.
...
**What**  
Defines artifact naming, integrity verification via SHA1, and the promotion gates for L3→L2→L1 flow.
...
This section defines a **profile** of the global AI Job Model (§AI-Job-Model). It inherits all core job fields and specialises:
...
Each job MUST track a `status` from the following set (aligned with the global job lifecycle in §AI-Job-Model.3):
...
> **Status Mapping Note:** This profile uses the global job lifecycle (§AI-Job-Model.3). The `awaiting_validation` and `awaiting_user` states are mandatory for this profile due to the non-destructive editing requirement.
...
This profile extends the global AI Job Model (§AI-Job-Model) with:
...
- ✓ **SHA1 integrity chain** prevents undetected corruption
