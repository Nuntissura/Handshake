...
- Task Board status labels in parentheses (Done / Ready for Dev / Stub Backlog) are taken from `docs/TASK_BOARD.md`.

---

## Addendum (v02.105) — Ready-for-Dev FAIL remediation stubs

`docs/TASK_BOARD.md` marks many Phase 1 items as FAIL (revalidation) and provides an additive stub revision under `docs/task_packets/stubs/`.
When selecting the WP to execute for an item below, prefer the additive stub revision and activate it (Technical Refinement Block → USER_SIGNATURE → official packet) before any implementation work.
