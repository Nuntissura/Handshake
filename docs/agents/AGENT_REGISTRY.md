# AGENT_REGISTRY

Mapping of contributing agents/models for traceability.

| AGENT_ID | Role | Model/Tooling | Version/Build | Contact/Notes |
| --- | --- | --- | --- | --- |
| AGENT_FRONTEND | Frontend Coder/Reviewer | TBD | TBD | Handles `app/` UI/React/Tauri |
| AGENT_SHELL | Tauri/IPC Coder/Reviewer | TBD | TBD | Handles `app/src-tauri/` orchestrator/IPC |
| AGENT_BACKEND | Backend Coder/Reviewer | TBD | TBD | Handles `src/backend/handshake_core/` |
| AGENT_SHARED | Shared Contracts | TBD | TBD | Handles `src/shared/` schemas/types |
| AGENT_DOCS | Docs Reviewer | TBD | TBD | Handles `/docs/` navigation pack updates |
| AGENT_CI | CI/Hygiene | TBD | TBD | Handles `just validate`/CI workflows |
| AGENT_VALIDATOR | Validator/Reviewer | Gemini CLI (local) | TBD | Runs `just ai-review` and validates changes |
