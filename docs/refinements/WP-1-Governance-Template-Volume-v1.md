...
- EXCERPT_ASCII_ESCAPED:
  ...
  **Purpose**
  Handshake MUST implement the project-agnostic Governance Kernel (7.5.4; `docs/GOV_KERNEL/*`) as a project-parameterized Governance Pack so the same strict workflow can be generated and enforced for arbitrary projects (not Handshake-specific).
  ...
  **Project identity (normative)**
  ...
  pub struct ProjectIdentity {
      ...
      pub role_mailbox_export_dir: String,      // MUST default to "docs/ROLE_MAILBOX/"
...
- EXCERPT_ASCII_ESCAPED:
  ...
  **Hard rule (Instantiation)**
  ...
  - Project-specific naming/layout/tool paths MUST live in `docs/PROJECT_INVARIANTS.md` (do not hardcode in templates).
