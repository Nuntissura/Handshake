...
- WP assignment is recorded in `docs/ORCHESTRATOR_GATES.json` as a `PREPARE` entry (via `just record-prepare ...`) with `branch` and `worktree_dir`.
...
- Record the coder assignment (writes `docs/ORCHESTRATOR_GATES.json`):
