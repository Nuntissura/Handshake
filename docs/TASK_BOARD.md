...
- **[WP-1-AppState-Refactoring-v3]** / READY (revalidation): Re-anchor to SPEC_CURRENT v02.103 + COR-701 packet format; revalidate that AppState does not expose raw `SqlitePool`/`DuckDbConnection` and no `state.pool`/`state.fr_pool` access exists outside storage (CX-DBP-030). [READY FOR DEV]
