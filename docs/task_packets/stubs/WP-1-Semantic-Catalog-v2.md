...
This is an additive remediation stub for `WP-1-Semantic-Catalog`.

It is created because the prior packet failed revalidation and/or has audit gaps.

## Prior packet
- Prior WP_ID: `WP-1-Semantic-Catalog`
- Prior packet: `docs/task_packets/WP-1-Semantic-Catalog.md`

## Known gaps (Task Board summary)
- / FAIL: Implement SemanticCatalog per A2.6.7. [READY FOR DEV]

## Activation checklist (before any coding)
1. In-chat Technical Refinement Block (per `docs/ORCHESTRATOR_PROTOCOL.md` Part 2.5.2).
2. USER_SIGNATURE.
3. Create `docs/refinements/WP-1-Semantic-Catalog-v2.md`.
4. Create official task packet via `just create-task-packet WP-1-Semantic-Catalog-v2`.
5. Update `docs/WP_TRACEABILITY_REGISTRY.md` to point `WP-1-Semantic-Catalog` -> `WP-1-Semantic-Catalog-v2`.
6. Update `docs/TASK_BOARD.md` to move `WP-1-Semantic-Catalog-v2` out of STUB when activated.
