...
- Why: Provide safe, auditable in-editor collaboration without silent edits outside the operator’s selected span.
...
- Risk: differing editor range semantics can create “off-by-one” or normalization surprises; must define canonical span model.
...
- [ ] Create `docs/refinements/WP-1-Atelier-Collaboration-Panel-v1.md` (approved/signed).
- [ ] Create the official task packet via `just create-task-packet WP-1-Atelier-Collaboration-Panel-v1` (in `docs/task_packets/`).
