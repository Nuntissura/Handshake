...
- Why: Provide operator-controlled projection without mutating stored raw artifacts and without “blur-but-revealable” leakage.
...
- OUT_OF_SCOPE:
  ...
  - “Collapsed but revealable” UX for non-sfw items in SFW mode (explicitly forbidden).
...
- [ ] Create `docs/refinements/WP-1-Lens-ViewMode-v1.md` (approved/signed).
- [ ] Create the official task packet via `just create-task-packet WP-1-Lens-ViewMode-v1` (in `docs/task_packets/`).
