...
- ROADMAP_POINTER: A§7.6.3 (Phase 1) -> Spec Router + governance session log (MVP); governance kernel adoption
- SPEC_ANCHOR_CANDIDATES (Main Body, not Roadmap):
  - A§2.6.8.10 Role Mailbox (Normative)
  - A§2.6.8.8 Spec Session Log (Normative)
  - A§11.5 Flight Recorder (LAW)

## INTENT (DRAFT)
- What: Implement Role Mailbox threads/messages with transcription links and always-on deterministic repo export to `docs/ROLE_MAILBOX/`.
...
- GOV_STANDARD and GOV_STRICT maintain continuous export to `docs/ROLE_MAILBOX/` (no silent drift).
...
- [ ] Create `docs/refinements/WP-1-Role-Mailbox-v1.md` (approved/signed).
- [ ] Create the official task packet via `just create-task-packet WP-1-Role-Mailbox-v1` (in `docs/task_packets/`).
