...
Scope Inputs:
- Task Packet: docs/task_packets/WP-1-AppState-Refactoring.md (status: Ready for Dev; signature `<pending>`)
...
- Concrete pool leakage: Database trait still exposes `fn sqlite_pool(&self) -> Option<&SqlitePool>` (storage/mod.rs:288), violating the “no concrete pool on public surfaces” rule in A2.3.12 and this WP’s DONE_MEANS.
