...
## SCOPE
- What: Harden `scripts/validation/gate-check.mjs` so it only recognizes explicit phase markers (headings / dedicated marker lines) and ignores prose or fenced code blocks, preventing false-positive gate failures.
...
- OUT_OF_SCOPE:
  - Any changes to other scripts in scripts/validation/
  - Any changes to src/, app/, tests/
  - Any changes to docs/TASK_BOARD.md (coordination rule for two-coder parallel work: Orchestrator updates the board to avoid file overlap)
...
node scripts/validation/gate-check.mjs WP-1-Gate-Check-Tool-v2
node scripts/validation/gate-check.mjs WP-1-Workflow-Engine-v4

# Negative check (expected FAIL: implementation evidence without a prior SKELETON APPROVED marker):
node scripts/validation/gate-check.mjs WP-1-Tokenization-Service-20251228
...
### DONE_MEANS
- `scripts/validation/gate-check.mjs` no longer matches phase markers inside prose paragraphs or inside fenced code blocks.
...
## IMPLEMENTATION
- Hardened `scripts/validation/gate-check.mjs` by switching to line-based parsing that ignores marker-like strings inside fenced code blocks and only recognizes explicit heading lines / marker lines per this packet's DONE_MEANS.

## HYGIENE
- node scripts/validation/gate-check.mjs WP-1-Gate-Check-Tool-v2: PASS
- node scripts/validation/gate-check.mjs WP-1-Workflow-Engine-v4: PASS
- node scripts/validation/gate-check.mjs WP-1-Tokenization-Service-20251228: FAIL (expected negative check)
- just post-work WP-1-Gate-Check-Tool-v2: PASS (after manifest filled; warnings possible if git subprocess checks unavailable)

## VALIDATION
- Target File: `scripts/validation/gate-check.mjs`
- Start: 6
- End: 133
- Line Delta: 59
...
Scope Inputs:
- Task Packet: `docs/task_packets/WP-1-Gate-Check-Tool-v2.md` (**Status:** Done)
- Spec (SPEC_CURRENT): `docs/SPEC_CURRENT.md` -> `Handshake_Master_Spec_v02.99.md` (A2.7.5, A2.9)
- Codex: `Handshake Codex v1.4.md`
...
- `node scripts/validation/gate-check.mjs WP-1-Gate-Check-Tool-v2`: PASS
- `node scripts/validation/gate-check.mjs WP-1-Workflow-Engine-v4`: PASS
- `node scripts/validation/gate-check.mjs WP-1-Tokenization-Service-20251228`: FAIL (expected)
