...
- **SPEC_ANCHOR:** §6.3.0 Mechanical Tool Bus Contract + §11.8 MEX v1.2 Specification (Main Body)
...
1. **No-Bypass:** Engines MUST NOT be invokable outside the orchestrator/runtime (§6.3.0)
2. **Artifact-First I/O:** Payloads >32KB MUST use artifact handles, never inline (§6.3.0)
3. **SHA-256 Hashing:** All artifacts MUST use SHA-256 with sidecar provenance (§11.8 §7)
...
### DONE_MEANS
1. [ ] `PlannedOperation` struct with all §11.8 §4.1 fields (`schema_version`, `op_id`, `engine_id`, `operation`, `inputs`, `params`, `capabilities_requested`, `budget`, `determinism`, `evidence_policy`, `output_spec`)
2. [ ] `EngineResult` struct with all §11.8 §4.2 fields (`op_id`, `status`, `started_at`, `ended_at`, `outputs`, `evidence`, `provenance`, `errors`, `logs_ref`)
...
6. [ ] Engine registry loader parses `mechanical_engines.json` (engine_id → ops, caps, determinism ceiling)
...
- `Handshake_Master_Spec_v02.96.md` §6.3.0 (Mechanical Tool Bus Contract)
- `Handshake_Master_Spec_v02.96.md` §11.8 (MEX v1.2 full specification)
...
```
[§11.8 §4.1] PlannedOperation envelope -> src/backend/handshake_core/src/mex/envelope.rs:L??
[§11.8 §4.2] EngineResult envelope -> src/backend/handshake_core/src/mex/envelope.rs:L??
[§11.8 §6] Global gates (G-SCHEMA..G-DET) -> src/backend/handshake_core/src/mex/gates.rs:L??
[§11.8 §8] Engine registry -> src/backend/handshake_core/src/mex/registry.rs:L??
[§11.8 §9] Conformance suite -> src/backend/handshake_core/src/mex/conformance.rs:L??
[§6.3.0] No-bypass invariant -> src/backend/handshake_core/src/mex/runtime.rs:L??
[§6.3.0] Artifact-first I/O (32KB) -> src/backend/handshake_core/src/mex/envelope.rs:L??
```
...
### Open Questions
- None (spec §6.3.0 + §11.8 is comprehensive)
...
## VALIDATION REPORT — WP-1-MEX-v1.2-Runtime-v2
**Verdict: FAIL (Process) / PASS (Logic)**

**Scope Inputs:**
- **Task Packet:** docs/task_packets/WP-1-MEX-v1.2-Runtime-v2.md (Status: Ready-for-Dev)
- **Spec:** Handshake_Master_Spec_v02.96.md (§6.3.0, §11.8)
...
**Findings:**
- **Requirement [A11.8 §4.1/4.2]:** Envelopes fully implemented in `envelope.rs`. All fields present and correctly typed.
- **Requirement [A11.8 §6] Global Gates:** All 6 gates implemented in `gates.rs`. 
    - `G-INTEGRITY` correctly enforces the **32KB Artifact-First Rule** (§6.3.0) at line 131.
    - `G-DET` correctly checks determinism against the registry ceiling at line 236.
- **Requirement [A11.8 §8/9]:** Registry loader and Conformance Harness implemented.
...
- **Hygiene (Task Board):** **FAIL.** The Coder failed to update `docs/TASK_BOARD.md` to "In Progress" before starting work. This is a protocol violation [CX-217].
...
**REASON FOR PASS:**
1. **[Logic]** 100% implementation of MEX v1.2 runtime contract (§11.8) including envelopes, gates, and registry.
