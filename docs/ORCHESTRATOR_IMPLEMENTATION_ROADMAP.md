...
## PHASE 1: Critical Foundations (87 → 93/100) — 2-3 hours
...
1. ✅ Appears in Main Body (not Roadmap, not aspirational)
2. ✅ Explicitly named (reader immediately finds it)
3. ✅ Specific (not "SHOULD be portable" but "MUST implement X trait")
4. ✅ Measurable acceptance criteria (verifiable yes/no)
5. ✅ No ambiguity (single valid interpretation)

PASS: All 5 ✅ → Proceed without enrichment
FAIL: Any ❌ → Ask user for clarification OR enrich spec

Examples:
CLEARLY COVERS: "§2.3.12.1: Database trait MUST have async fn get_blocks(&self, id: &str)"
- All 5 criteria met; unambiguous
- Proceed without enrichment

DOES NOT CLEARLY COVER: "§2.3.12: Storage abstraction SHOULD be portable"
...
- [ ] All phase-critical WPs: VALIDATED verdict (✅ PASS)
- [ ] Spec regression check: `just validator-spec-regression` → PASS
- [ ] Supply chain audit: `cargo deny check` + `npm audit` → 0 violations
...
3. If ERRATA: Add read-only ERRATA section to locked packet:
   ```
   ## ERRATA
   - Original SPEC_ANCHOR: §X was incorrect
   - Correct SPEC_ANCHOR: §Y
...
## PHASE 2: Quality Systems (93 → 96/100) — 2-3 hours
...
→ Result: **HIGH tier** (max of all dimensions)
```
...
## PHASE 3: Edge Cases & Polish (96 → 99/100) — 2-3 hours
...
# Checks:
- SPEC_ANCHOR exists in SPEC_CURRENT.md ✓
- Signature format correct ✓
- Signature not used before (grep) ✓
- Blocker status in TASK_BOARD ✓
- TASK_BOARD = packet STATUS ✓
- Pre-Delegation checklist items present ✓
```

**Where:** Create scripts/validation/orchestrator-audit.sh
...
### Week 3 (P2: Edge Cases) — 2-3 hours
