...
Each project MUST define a precedence order and keep it stable. A canonical order (highest → lowest):
...
3. **Master Spec** (`<PROJECT>_Master_Spec_vNN.NNN.md`, repo root) + pointer (`docs/SPEC_CURRENT.md`)
   - The authoritative product/architecture specification.
   - `docs/SPEC_CURRENT.md` MUST be the single pointer to the current spec file.
4. **Role Protocols** (`docs/*_PROTOCOL.md`)
...
6. **Work authorization artifacts** (`docs/task_packets/*.md`, `docs/refinements/*.md`, `docs/TASK_BOARD.md`)
   - Make “what is allowed” explicit and auditable.
7. **Gate tooling** (`scripts/validation/*`, `scripts/hooks/*`, `.github/workflows/*`, `justfile`)
   - Mechanical enforcement; tools MUST not silently change the law stack.

Kernel rule: when two sources conflict, the **higher** source wins. Overrides MUST be explicit and logged (see `docs/GOV_KERNEL/02_ARTIFACTS_AND_CONTRACTS.md`).
...
Mandatory rules:
- One work packet (WP) → one feature branch (e.g., `feat/WP-<ID>`).
...
Recommended rules:
- One role → one default worktree (e.g., `wt-orchestrator`, `wt-validator`) plus per-WP worktrees as needed.
