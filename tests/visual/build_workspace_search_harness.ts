...
const harnessEntry = path.join(appRoot, "src", "components", "workspace_search_visual_harness.tsx");
...
export function buildWorkspaceSearchHarness(): Promise<HarnessBundle> {
  ...
  cached = (async (): Promise<HarnessBundle> => {
    ...
    const result = (await build({
      ...
      build: {
        ...
        minify: false,
        cssCodeSplit: false,
        lib: { entry: harnessEntry, formats: ["iife"], name: "HsWorkspaceSearchHarness" },
      },
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } as any)) as any;
    ...
    if (!jsChunk) throw new Error("workspace search harness build produced no JS chunk");
    const css = cssAsset
