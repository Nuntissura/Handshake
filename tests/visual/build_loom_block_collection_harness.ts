...
const harnessEntry = path.join(
  ...
  "src",
  "components",
  "loom_block_collection_visual_harness.tsx",
);
...
export function buildLoomBlockCollectionHarness(): Promise<HarnessBundle> {
  ...
  cached = (async (): Promise<HarnessBundle> => {
    ...
    const result = (await build({
      ...
      build: {
        ...
        minify: false,
        cssCodeSplit: false,
        lib: { entry: harnessEntry, formats: ["iife"], name: "HsLoomBlockCollectionHarness" },
      },
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } as any)) as any;
    ...
    if (!jsChunk) throw new Error("loom block collection harness build produced no JS chunk");
    const css = cssAsset
