...
const harnessEntry = path.join(appRoot, "src", "components", "debug_session_visual_harness.tsx");
...
export function buildDebugSessionHarness(): Promise<HarnessBundle> {
  ...
  cached = (async (): Promise<HarnessBundle> => {
    ...
    const result = (await build({
      ...
      build: {
        ...
        minify: false,
        cssCodeSplit: false,
        lib: { entry: harnessEntry, formats: ["iife"], name: "HsDebugSessionHarness" },
      },
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } as any)) as any;
    ...
    if (!jsChunk) throw new Error("debug session harness build produced no JS chunk");
    const css = cssAsset
