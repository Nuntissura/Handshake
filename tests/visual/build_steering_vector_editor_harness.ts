// Builds the REAL <SteeringVectorEditor> into a self-contained IIFE bundle for
// the MT-098 Playwright visual smoke. The rendered DOM is the shipped React
// component; deterministic Tauri IPC mocks live in the harness entry.
...
const harnessEntry = path.join(
  ...
  "components",
  "inference_lab",
  "steering_vector_editor_visual_harness.tsx",
);
...
export function buildSteeringVectorEditorHarness(): Promise<HarnessBundle> {
  ...
  cached = (async (): Promise<HarnessBundle> => {
    ...
    const result = (await build({
      ...
      build: {
        ...
        lib: {
          entry: harnessEntry,
          formats: ["iife"],
          name: "HsSteeringVectorEditorHarness",
        },
      },
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } as any)) as any;
    ...
    if (!jsChunk) throw new Error("steering vector editor harness build produced no JS chunk");
    const css = cssAsset
