...
const harnessEntry = path.join(appRoot, "src", "components", "loom_ai_review_visual_harness.tsx");
...
export function buildLoomAiReviewHarness(): Promise<HarnessBundle> {
  ...
  cached = (async (): Promise<HarnessBundle> => {
    ...
    const result = (await build({
      ...
      build: {
        ...
        minify: false,
        cssCodeSplit: false,
        lib: { entry: harnessEntry, formats: ["iife"], name: "HsLoomAiReviewHarness" },
      },
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } as any)) as any;
    ...
    if (!jsChunk) throw new Error("loom ai review harness build produced no JS chunk");
    const css = cssAsset
