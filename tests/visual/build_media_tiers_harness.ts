...
const harnessEntry = path.join(appRoot, "src", "components", "media_tiers_visual_harness.tsx");
...
export function buildMediaTiersHarness(): Promise<HarnessBundle> {
  ...
  cached = (async (): Promise<HarnessBundle> => {
    ...
    const result = (await build({
      ...
      build: {
        ...
        minify: false,
        cssCodeSplit: false,
        lib: { entry: harnessEntry, formats: ["iife"], name: "HsMediaTiersHarness" },
      },
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } as any)) as any;
    ...
    if (!jsChunk) throw new Error("media tiers harness build produced no JS chunk");
    const css = cssAsset
