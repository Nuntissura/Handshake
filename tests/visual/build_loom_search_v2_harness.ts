...
const harnessEntry = path.join(appRoot, "src", "components", "loom_search_v2_visual_harness.tsx");
...
export function buildLoomSearchV2Harness(): Promise<HarnessBundle> {
  ...
  cached = (async (): Promise<HarnessBundle> => {
    ...
    const result = (await build({
      ...
      build: {
        ...
        minify: false,
        cssCodeSplit: false,
        lib: { entry: harnessEntry, formats: ["iife"], name: "HsLoomSearchV2Harness" },
      },
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } as any)) as any;
    ...
    if (!jsChunk) throw new Error("loom search v2 harness build produced no JS chunk");
    const css = cssAsset
