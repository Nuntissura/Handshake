...
//
// Spawns the mt253_source_control_fixture (real temp git repo + real
// source-control routes against isolated embedded SurrealDB), serves the BUILT
...
// and that write ops appended EventLedger receipts to real SurrealDB. The
...
function startFixture(): Promise<FixtureHandle> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      void terminateFixtureProcess(child).then(
        ...
        (teardownError) =>
          reject(
            new Error(
              `MT-253 fixture did not become ready within 600s. Startup teardown failed: ${String(teardownError)}. stderr:\n${stderr}`,
            ),
          ),
      );
    }, 600_000);
    ...
    child.once("exit", (code, signal) => {
      ...
      reject(
        new Error(
          `MT-253 fixture exited before ready with code ${code} and signal ${signal}. stderr:\n${stderr}`,
        ),
      );
    });
  });
