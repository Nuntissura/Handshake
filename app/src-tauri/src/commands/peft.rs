...
pub async fn start_peft_training_job(
    ...
    start_peft_training_job_inner(request, python_path, script_path, cpu_only_default).await
}
...
pub async fn start_peft_training_job_inner(
    ...
    let executor =
        PythonPeftTrainerExecutor::new(python_path, script_path).with_cpu_only(cpu_only);
    ...
    exec_result.map_err(|err| err.to_string())?;
