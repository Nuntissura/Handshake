...
[CX-117] HARD_GIT_TOPOLOGY_TERMINOLOGY (HARD): The assistant MUST use precise topology terms in instructions and approvals. `local branch` means a branch ref in the local checkout on disk. `remote branch` / `GitHub branch` means a branch at `origin/<name>` on GitHub. `worktree` means a working directory on disk attached to a local checkout. The assistant MUST NOT blur these terms when asking for approval or reporting actions.
