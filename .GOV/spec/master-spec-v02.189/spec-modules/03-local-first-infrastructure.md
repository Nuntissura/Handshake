...
body_sha256: "80a99f00538da8e0174505f273442a0a12e9ccfcd67229ceea16a5bd231818a9"
...
### 3.5.2 Tier-1 adapter implementations (container / process-jail)
...
- **`WSL2PodmanAdapter` (DEFAULT)** — rootless Podman inside WSL2. Cross-machine portable (any Win11/WSL2 host), near-native GPU on NVIDIA, supports bind-mount for large GGUF without copy. Chosen as default for the disk-agnostic portability requirement ([GLOBAL-PORTABILITY-001]).
...
- Operator default = `WSL2PodmanAdapter`.
...
- `cross_machine_portable`: bool — whether the adapter is portable across hosts (true for `WSL2PodmanAdapter` and `DockerAdapter`; false for `WindowsNativeJailAdapter`).
...
- **Tier 1 — container / process-jail (shared host kernel).** `WSL2PodmanAdapter` (default), `DockerAdapter`, `WindowsNativeJailAdapter`. Suitable for trusted or already-reviewed code and for GPU-bound local-model boxing (the proven WSL2+Podman NVIDIA path, §3.6). NOT sufficient for untrusted agent-written code: a shared-kernel escape is a host compromise.
...
**Windows-host reality (NORMATIVE constraint).** Cloud Hypervisor, libkrun, and gVisor all require Linux + KVM. On a Windows 11 host the substrate is **WSL2** (the same substrate the default `WSL2PodmanAdapter` already uses); the strong-isolation adapters run *inside* WSL2 over nested KVM. There is no Win32 user-mode hosting path for these engines. Therefore:
...
**GPU caveat (NORMATIVE).** Nested GPU passthrough from a Tier-3 microVM through WSL2 is not yet proven reliable. Until it is, GPU-bound local-model boxing (§3.6) MUST stay on the Tier-1 `WSL2PodmanAdapter` NVIDIA path; Tier-3 microVM isolation is reserved for code execution that does not require the GPU. Adapters declare GPU support truthfully via `gpu_passthrough`; the selection policy enforces the GPU guard in §3.5.3.
