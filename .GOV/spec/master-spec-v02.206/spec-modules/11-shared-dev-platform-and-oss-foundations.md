...
body_sha256: "ebb0fdc28f012d2b6c6c0b06d7963a8860e4633b21c18cdeffb894a20865118a"
metadata_rule: "frontmatter is machine metadata; body follows after this block"
---
# 11. Shared Dev Platform & OSS Foundations

## 11.0 SurrealDB Authority Override (v02.205) [OSS-SDB-001]

Shared development-platform state follows the v02.205 exclusive database law. Handshake-managed SurrealDB is the only durable database authority and persists EventLedger semantics; PostgreSQL and SQLite are forbidden as MCP targets, transactional state, schema tools, caches, fallbacks, imports, reconciliation, fixtures, tests, and proof paths. Later PostgreSQL/SQLite/sqlite-utils material is comparative research or non-executable historical inventory only. Implementations use the official SurrealDB Rust SDK, typed store boundaries, `SCHEMAFULL` SurrealQL, authenticated record-user permissions plus ResourceBroker, and SurrealKit rollouts. DuckDB may exist only as an explicitly rebuildable, bounded analytics/diagnostic projection.
...
- [9. SurrealDB Authority Opportunities](#9-surrealdb-authority-opportunities-refine-v02204)
...
- A **SurrealDB authority plan** showing how Handshake's typed record, graph, search, live-query, and permission architecture can implement these patterns at a level Heaper's own stack cannot reach.
...
|------|------|-------|----------------|
| **Tier 0: Metadata** | LoomBlock record + title + tags + links + timestamps | SurrealDB authority + CRDT-derived local projection | Always; offline projection is read-only until governed reconnect/promotion |
...
#### 9. SurrealDB Authority Opportunities [REFINE v02.204]

##### 9.1 Overview

Heaper's historical PostgreSQL document-store design remains useful comparative research. Handshake uses Handshake-managed SurrealDB as its sole runtime database authority for local and shared workspaces, with EventLedger retaining mutation-history semantics.

All authority features below MUST be implemented behind the typed storage boundary (Â§2.3.13.3) using the official SurrealDB Rust SDK. EventLedger persistence and authority records initialize from an empty SurrealDB schema and cold activation. PostgreSQL and SQLite connectivity, dependencies, configuration, importers, reconciliation tools, fallbacks, caches, test substitutes, proof paths, and dual authorities are forbidden.

##### 9.1.1 WP-KERNEL-009 authority supersession [ADD v02.192]

The imported Loom research in this Section 11 module preserves useful behavior and terminology, but its older SQLite/PostgreSQL roadmap and fallback language is superseded for WP-KERNEL-009 and all later runtime work. When Loom is used as part of the ProjectKnowledgeIndex, rich editor, retrieval trace, UserManual, or backend navigation surface, SurrealDB authority with EventLedger persisted inside SurrealDB is the only authority path. SQLite/PostgreSQL references in historical research and prior packet names are provenance only; they authorize no connectivity, import, reconciliation, authority, cache, offline, compatibility, bootstrap, test, or proof path.

For WP-KERNEL-009:

- LoomBlock, LoomEdge, Loom view filters, search results, backlinks, graph traversal, CRDT operation history, and AI tag/link proposals MUST resolve to ProjectKnowledgeIndex records or other SurrealDB authority records, including EventLedger entries persisted in SurrealDB.
...
- Search tiers MUST be expressed as product capabilities over SurrealDB authority records, including EventLedger entries, and derived indexes. Direct load and exact lookup take precedence over hybrid retrieval when the target source, LoomBlock, asset, claim, or relationship id is known.
...
- Old Phase 1 rows and packet names that name SQLite or PostgreSQL are retained only as historical provenance. They do not authorize connectivity, import, reconciliation, or runtime implementation choices.

##### 9.2 Queryable CRDT History

**Opportunity:** Store Yjs update vectors as typed fields in `SCHEMAFULL` SurrealDB records, enabling server-side queries over operation history without opaque JSON authority.

```surql
DEFINE TABLE loom_crdt_op SCHEMAFULL PERMISSIONS NONE;
DEFINE FIELD workspace_id ON loom_crdt_op TYPE record<workspace>;
DEFINE FIELD block_id ON loom_crdt_op TYPE record<loom_block>;
DEFINE FIELD site_id ON loom_crdt_op TYPE string;
DEFINE FIELD op_type ON loom_crdt_op TYPE string;
DEFINE FIELD op_payload ON loom_crdt_op FLEXIBLE TYPE object;
DEFINE FIELD created_at ON loom_crdt_op TYPE datetime;
DEFINE INDEX loom_crdt_op_site_created ON loom_crdt_op FIELDS site_id, created_at;
```
...
There is no SQLite or PostgreSQL connectivity, importer, reconciliation tool, or fallback. Derived client projections may support disconnected viewing, but cannot accept authoritative writes.

##### 9.3 Three-Tier Search Architecture

| Tier | Engine | Available On | Capability |
|------|--------|-------------|------------|
| Tier 1 (exact) | SurrealDB exact/indexed lookup | SurrealDB authority | Stable-id and keyword lookup before semantic retrieval |
| Tier 2 (search/graph) | SurrealDB search and graph queries | SurrealDB authority | Ranked results filtered by typed graph relationships |
| Tier 3 (semantic) | SurrealDB vector index + Shadow Workspace embeddings (Â§2.3.8) | SurrealDB authority | Vector similarity feeding AI/RAG with replayable traces |

**[LM-SEARCH-001]** The search API MUST be authority-boundary-agnostic. The storage trait exposes `search_loom_blocks(query, filters) -> Vec<LoomBlockSearchResult>` and its sole runtime implementation uses the official SurrealDB Rust SDK.

**[LM-SEARCH-002]** SurrealDB search results MUST be filterable by graph relationships (tags, mentions, backlink depth) within the query. Every search emits a replayable RetrievalTrace.

##### 9.4 Graph Queries via SurrealDB Relations

**[LM-GRAPH-001]** Graph traversal MUST use typed SurrealDB relation records and bounded traversal depth. The performance target is <50ms for 3-hop traversal on 10K blocks under the canonical test profile.

##### 9.5 Real-Time Collaboration via SurrealDB Live Queries

When multi-user workspaces are enabled:

**[LM-RT-001]** LoomBlock creation, linking, and tagging events MUST be observed through scoped SurrealDB live queries and broadcast to authorized WebSocket clients.
...
##### 9.6 SurrealDB Permissions for Shared Workspaces

**[LM-RLS-001]** Multi-user workspace isolation MUST use authenticated record-user table/field permissions intersected with ResourceBroker scope. A `viewer` can read but not create/edit LoomBlocks; a `member` can create and edit; an `admin` can delete and manage workspace settings.

**[LM-RLS-002]** Authority tables default to `PERMISSIONS NONE`; every grant is explicit at the database layer and rechecked by ResourceBroker. Privacy proof MUST execute as authenticated record users because root, namespace, database, and other system users bypass record-user table permissions.
...
##### 11.1 Schema (Typed SurrealDB Authority)

All schemas follow Â§2.3.13 Storage Backend Portability requirements:
- Use typed `SCHEMAFULL` SurrealDB records and SDK-bound query parameters; relational database syntax and import translators are prohibited.
...
##### Phase 1 â€” Foundation (Single-User, SurrealDB)

| Item | Description | Dependencies |
|------|-------------|--------------|
| **[ADD LM-P1-001][REFINE v02.204]** LoomBlock entity + storage | Implement typed `SCHEMAFULL` LoomBlock records, CRUD operations, and the storage trait through the official SurrealDB Rust SDK. | Â§2.3.13 SurrealDB authority closure WPs |
...
| **[ADD LM-P1-007][REFINE v02.204]** SurrealDB search for Loom | Wire LoomBlock content into governed SurrealDB search indexes. | LM-P1-001, Shadow Workspace (Â§2.3.8) |
...
| **[ADD LM-P2-005][REFINE v02.204]** SurrealDB search (Tier 2) | Implement ranked search behind the storage trait with graph-aware filtering and RetrievalTrace receipts. | Â§2.3.13 SurrealDB authority |
| **[ADD LM-P2-006]** Cache tier configuration UI | Per-device storage tier settings (Tier 0-3 selection). | LM-P1-005 |

##### Phase 3 â€” Multi-User + SurrealDB

| Item | Description | Dependencies |
|------|-------------|--------------|
| **[ADD LM-P3-001][REFINE v02.204]** SurrealDB Loom authority | Implement the LoomStorage trait through the official SurrealDB Rust SDK. | Â§2.3.13 SurrealDB-only authority |
| **[ADD LM-P3-002][REFINE v02.204]** CRDT op history in SurrealDB | Store Yjs update vectors in typed `SCHEMAFULL` records for server-side queries. | LM-P3-001, Â§2.3.9 |
| **[ADD LM-P3-003][REFINE v02.204]** Real-time live queries | Broadcast authorized Loom events via scoped SurrealDB live queries to WebSocket clients. | LM-P3-001, WebSocket infrastructure |
| **[ADD LM-P3-004][REFINE v02.204]** Record-user permissions | Implement default-deny workspace isolation and role-based access with authenticated record-user permissions plus ResourceBroker. | LM-P3-001, workspace roles (Â§2.3.15.8) |
| **[ADD LM-P3-005][REFINE v02.204]** Graph traversal optimization | Bounded SurrealDB relation traversal for multi-hop links; <50ms on 100K blocks. | LM-P3-001 |
...
- All authority schemas MUST be typed SurrealDB `SCHEMAFULL` definitions per Â§2.3.13.2; SQLite/PostgreSQL schema shapes and import translators MUST NOT ship or participate in tests or proof.
...
- **[LM-NOTES-003]** Notes is explicitly NOT a markdown-vault clone. Per Section 9.1.1, Notes resolves to SurrealDB authority records with EventLedger persisted in SurrealDB; markdown files, generated wiki pages, exports, and browser/view state are projections and MUST NOT become authority.
...
| PostgreSQL as flat doc store | **SKIP** | Historical research provenance only; PostgreSQL connectivity, importing, reconciliation, and runtime use are completely retired. Handshake authority is SurrealDB (Â§9). |
...
| Â§9 SurrealDB Authority | Â§2.3.13 Storage Authority, Â§2.3.15.5 Locus Storage | Typed records, graph/search indexes, live queries, and permissions |
...
Handshake Stage is a **governed browser/webviewer + trusted HTML/Stage App host + capture/import surface** that allows users and models to navigate the External Web, launch embedded Stage Apps, inspect trusted HTML artifacts, test front-end projects, and convert web/PDF/3D inputs into workspace artifacts using the AI Job Model + Workflow Engine + Mechanical Tool Bus (no bypass). Stage is not the visual creation suite; the Studio Module owns Figma-like collaboration, canvas/photo/painting/compositing, Affinity/Photoshop-class work, and future full visual authoring tools.
...
**Scope:** Handshake Stage (built-in browser/webviewer + embedded Stage Apps + trusted HTML artifact host), Stage Bridge API, Mechanical Tool integration, lightweight persisted tab/session records, front-end test and capture workflows, and 3D Mechanical Assist Pack inspection/import/validate/canonicalize/optimize/physics checks. Full visual authoring belongs to the Studio Module.
...
10. 3D in Stage (Viewport + Studio handoff)  
...
2. **Stage Apps** â€” packaged internal modules rendered in a WebView, used for structured interactions (inspectors, forms, artifact tools, review surfaces). Stage Apps can request Handshake actions **only via Stage Bridge**, a governed API.
...
- **Stage/Studio boundary**: Stage owns web I/O, trusted HTML projection, browser automation, capture/import, inspection, validation, and provenance review. The Studio Module owns visual creation and collaboration surfaces, including Figma/Spline-like authoring, canvas, photo/painting/compositing, Affinity/Photoshop-class tools, and future 3D visual authoring.
- **3D Mechanical Assist Pack**: open-format 3D ingestion (glTF/GLB) + typed scene IR + validator/optimizer/physics reports. Stage Viewport ships early as an inspector/review surface; full scene authoring belongs to Studio Module.
...
- **Studio Module**: native Handshake visual creation module family for canvas/Figma-like collaboration, photo/painting/compositing, Affinity/Photoshop-class tools, and future 3D authoring; it may consume Stage-captured artifacts but is not hosted by Stage.
...
- Lightweight tab/session model for large backlog usage: cold tabs are persisted records with URL, title, favicon/snapshot, extracted text where allowed, folder/tag/todo state, and provenance; only the active working set instantiates WebViews.
- Trusted HTML artifact viewing for model-authored plans, mockups, interactive queries, and front-end test surfaces, with sanitized/provenance-preserving import/export.
...
- Stage is not the Studio Module: no Figma/Spline/Photoshop/Affinity-class visual authoring is owned by Stage. Stage may review, inspect, capture, and hand off artifacts to Studio through governed jobs.

##### 3.3 Build order (Handshake phases; Stage-specific prioritization)

Stage is intentionally front-loaded because it is a **high-leverage operator and model surface** for research, verification, trusted HTML projection, evidence capture, front-end testing, browser automation, and job-triggered mechanical tooling. A Figma/Spline/Photoshop/Affinity-like editor UX is explicitly **not** a Stage dependency; visual authoring belongs to Studio Module. The early 3D value in Stage comes from **mechanical assists** that turn geometry/physics into machine-checkable facts.
...
- Lightweight persisted session/tab records with cold-tab unloading so thousands of saved tabs/backlog items do not imply thousands of live WebViews.
- Trusted HTML artifact viewer for model-authored plans, mockups, interactive query outputs, and local front-end project test surfaces.
...
**Phase 3 (MAY) â€” Stage-to-Studio visual review and handoff**
- Stage review surfaces can open 3D assets + reports, preview `scene_ir` diffs, and request governed patchset application jobs.
- Studio Module owns any visual authoring baseline; Stage hands off artifact handles, reports, snapshots, and patchsets instead of embedding a parallel editor.

**Phase 4 (MAY) â€” Studio visual authoring expansion**
- Advanced authoring UX (constraints, snapping/layout tools, animation timeline, interaction authoring), if justified, belongs to Studio Module and reuses Stage provenance/capture/import artifacts.
...
- This allowance is import/provenance only; it does not make Stage the authoring owner. First-party Spline-class UX belongs to Studio Module.
...
**Phase 3+ (MAY)**
- `com.handshake.stage.visual_review` â€” provenance-first visual/3D review surface for asset reports, `scene_ir` diffs, and Studio handoff; it is not a full visual authoring editor.
...
##### 10.4 Stage-to-Studio visual handoff direction (Phase 3/4 MAY)

Stage does not own the full visual creation editor. Stage may provide provenance-first review/inspection apps and hand off validated artifacts, reports, snapshots, and IR patchsets to the Studio Module. Studio Module owns visual authoring and collaboration.

**Constraints**
- MUST operate on artifact handles (no raw paths)
- MUST persist proposed changes as IR patchsets + job records (no silent edits)
- MUST export governed handoff/apply job outputs as GLB/glTF deterministically (or label non-determinism)
- MUST not require any proprietary vendor service
- MUST NOT silently mutate Studio state; Stage-to-Studio changes flow through artifact handles, IR patchsets, jobs, approvals, and Flight Recorder evidence.

Studio visual authoring MAY reuse open components, but packaging a full editor as a Stage App is no longer the default architecture. Stage Apps in this area are inspectors, reviewers, projection hosts, or handoff controllers.

[ADD v02.158] The Stage-to-Studio boundary remains shell-level in Phase 1. Backend lineage between Stage media artifacts, ASR transcript artifacts, and later Lens/Studio time-span consumption MUST remain explicit and stub-backed rather than being implied by the shell alone.
...
The adoption of MCP addresses the combinatorial explosion known as the "NÃ—M" problem. Without a standard protocol, integrating M models with N tools requires M \\times N custom connectors. By standardizing on MCP, the Handshake Rust Host can connect not just to its own internal Python Orchestrator, but to governed MCP-compliant servers such as a Git repository or a remote Slack instance. Database authority is not an arbitrary MCP target: Handshake durable product state uses only the typed Handshake-managed SurrealDB boundary, and PostgreSQL connectivity is forbidden.  
...
Handshake introduces a persistent **Job Queue** backed by Handshake-managed SurrealDB. Its EventLedger-linked records are the source of truth for the UI, decoupled from the active network connection.

#### 11.3.4.2 SurrealDB Schema Design
...
1. **Task Initiation**: When a tool is called, the Host creates a typed SurrealDB job record with status `PENDING` and generates a progressToken. This token is injected into the \_meta field of the MCP request.  
...
3. **Throttled Writes**: To bound high-frequency mutation volume (e.g., a download reporting every kilobyte), the ProgressWatcher buffers updates and flushes typed mutations at a maximum rate (e.g., 2Hz or 500ms).  
4. **UI Reactivity**: The UI consumes an authorized projection driven by SurrealDB live queries. On reload it fetches canonical state and resumes the projection; live-query notifications never become authority.
...
* **Detection**: On startup, the Rust Host loads canonical job records whose status is `RUNNING` through the typed storage boundary.  
...
* **Resumability (Advanced)**: If a Handshake-managed worker/service process is still running under ProcessOwnershipLedger after UI restart, the Host can attempt to **Re-bind**. It lists active tasks from the managed worker and correlates them with SurrealDB authority state, including EventLedger entries. Detached/manual daemon authority is not accepted. The default remains the simpler "Fail and Retry" model, facilitated by the stored payload in the database which allows the user to click "Retry" to re-submit the exact same tool call.

#### 11.3.4.5 MCP Implementation Snippets (Durable Progress Mapping)

### Target 3: Durable Progress Mapping (notifications â†’ SurrealDB jobs)

Assumptions:

* AI jobs and their EventLedger mutation history live in SurrealDB authority under the global Job Model.
...
#### Schema additions in SurrealDB

Minimal tables (names indicative):

```surql
DEFINE TABLE ai_job SCHEMAFULL PERMISSIONS NONE;
DEFINE FIELD job_kind ON ai_job TYPE string;
DEFINE FIELD status ON ai_job TYPE string;
DEFINE FIELD created_at ON ai_job TYPE datetime;
DEFINE FIELD updated_at ON ai_job TYPE datetime;
DEFINE FIELD error_code ON ai_job TYPE option<string>;
DEFINE FIELD error_message ON ai_job TYPE option<string>;
DEFINE FIELD mcp_server_id ON ai_job TYPE option<string>;
DEFINE FIELD mcp_call_id ON ai_job TYPE option<string>;
DEFINE FIELD mcp_progress_token ON ai_job TYPE option<string>;
DEFINE INDEX ai_job_progress_token ON ai_job FIELDS mcp_progress_token;

DEFINE TABLE job_progress_snapshot SCHEMAFULL PERMISSIONS NONE;
DEFINE FIELD job_id ON job_progress_snapshot TYPE record<ai_job>;
DEFINE FIELD progress_token ON job_progress_snapshot TYPE string;
DEFINE FIELD progress_pct ON job_progress_snapshot TYPE float;
DEFINE FIELD message ON job_progress_snapshot TYPE option<string>;
DEFINE FIELD updated_at ON job_progress_snapshot TYPE datetime;
DEFINE INDEX job_progress_identity ON job_progress_snapshot FIELDS job_id, progress_token UNIQUE;
```
...
async fn start_mcp_job(job_id: &JobId, server_id: &str, tool_name: &str) {
    ...
    // Persist the typed mapping through the SurrealDB-backed storage boundary before tools/call.
    storage
        .mark_mcp_job_running(job_id, server_id, &progress_token)
        .await?;
...
The important part: the same `progress_token` is known to both host and server and is durable in SurrealDB authority.
...
On startup, the host inspects SurrealDB authority through the typed storage boundary:
...
#### 11.7.6.13 Studio Module Canvas & Design OSS Matrix
...
## 11.20 Shared Workspace, SaaS, and MCP Authorization Foundation [ADD v02.202]

Shared development and external-service integrations use Handshake’s Principal/ProtectedResource/ResourceGrant model end to end. Provider identity, organization membership, project membership, MCP connection establishment, or possession of a remote token does not by itself authorize local content.

Capability evaluation is the intersection of authenticated Principal, session, selected AccessSpace, exact ResourceGrant, MembershipRole, ToolGate/CapabilityProfile, task delegation, provider/service audience, expiry/revocation, and resource policy. Any missing input denies.

MCP servers and clients expose only resources, prompts, tools, sampling, and elicitation paths granted to their service/client Principal. Tokens use audience restriction, least privilege, short bounded lifetime, revocation, and protected storage. Remote calls preserve correlation, delegation, provenance, source-resource scope, and result-resource scope.

The planned Handshake SaaS/MCP service may create moodboards, project starters, and inspiration collections. Sync/import contracts carry stable remote IDs, hashes, provenance, visibility policy, tenant/account audience, deletion/revocation state, and an explicit requested local destination. Local download/inspection occurs through quarantine and ResourceBroker; publishing back to the service requires a separate explicit grant.

Required typed event families include account/session lifecycle, Space/Persona switch, resource link/unlink, grant create/change/revoke/expire, membership change, visitor-pass issue/use/revoke, MCP client fingerprint/authorize/deny, ToolGate decision, external identity bind/unbind, service connect/disconnect, remote manifest inspect/import/quarantine/promote, sync conflict, and break-glass use.
