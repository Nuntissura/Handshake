...
body_sha256: "15e239c15c3b16efcc43818253c578a399e38398c1bbbf24a31997f053d66f51"
metadata_rule: "frontmatter is machine metadata; body follows after this block"
---
# 10. Product Surfaces

## 10.0 SurrealDB Authority Override (v02.205) [SURF-SDB-001]

This override is current product law and takes precedence over all later Calendar, Catalog, Dev Command Center, Notes, Settings, and other product-surface storage text. All durable surface state and EventLedger persistence use Handshake-managed SurrealDB through typed stores. PostgreSQL and SQLite connectivity, dependencies, configuration, credentials, local authority databases, caches, fallbacks, importers, reconciliation, fixtures, mocks, tests, and proof paths are forbidden. Existing product features, fields, filters, recovery behavior, offline UX, exports, and operator workflows survive; their storage implementation is translated to `SCHEMAFULL` SurrealQL, authenticated record-user permissions plus ResourceBroker, and SurrealKit rollouts.

The later Calendar SQLite DDL, `.hs.catalog.db`, `.handshake/devcc.db`, `devcc.db` migrations, SQL up/down files, and SQLite recovery instructions are non-executable historical semantic inventories. Calendar records, catalog records, DCC projects/sessions/anchors/handoffs/approvals/conversation state, and their EventLedger links are SurrealDB records. ArtifactStore owns materialized artifacts. DuckDB may be retained only as a bounded rebuildable diagnostic/analytics projection with no authority or fallback role. A clean environment initializes SurrealDB and EventLedger genesis without reading or contacting PostgreSQL or SQLite.
...
## 10.15 UserManual Surface [ADD v02.186; REFINED v02.201]

**Why**
[GLOBAL-BUILD-002]..[GLOBAL-BUILD-011] require every app to ship a built-in manual that a no-context model can read to operate the product. Handshake's canonical manual is the **UserManual** -- typed Rust records that hold purpose / workflows / startup / commands / expected I/O / navigation / safety / failure modes / recovery steps for every wired surface. Section 10.15 formalizes the authority location, native registered actions and handlers, optional compatibility adapters, the on-demand md projection, the in-app browsing surface, the self-consistency check, and the CI gate. `ModelManual`, `model_manual`, and `kernel.model_manual.*` are legacy compatibility names only; they MUST NOT identify a second authority surface.

**What**
Defines the seven sub-elements: authority location, native action/handler access, on-demand projection, browsing surface, self-consistency check, GLOBAL-BUILD coverage, same-commit CI hook.

---

### 10.15.1 Authority location

The UserManual authority lives at `src/backend/handshake_core/src/user_manual/mod.rs` (CX-200 BACKEND root). Typed Rust records (e.g., `UserManualRecord`, `UserManualCommand`, `UserManualWorkflow`, `UserManualSafetyConstraint`) hold the canonical text. There is no separate `USER_MANUAL.md` file that competes for authority. During migration, `src/backend/handshake_core/src/model_manual/mod.rs` MAY remain as a compatibility adapter only when every legacy record maps deterministically to exactly one canonical `UserManualRecord` and legacy use emits a compatibility receipt.

### 10.15.2 Native action and handler surface

UserManual operations MUST be registered native kernel actions backed by canonical backend handlers:

- `kernel.user_manual.get()` -- returns the full typed manifest.
- `kernel.user_manual.list_commands()` -- returns the command index.
- `kernel.user_manual.search(query)` -- performs substring + tag search.

The Handshake native frontend, backend model/agent routes, CLI or MCP adapters, and tests MUST call the same registered handlers through `KernelActionCatalogV1`; no frontend-local or transport-local implementation may become authority. A Tauri IPC bridge MAY expose these actions during compatibility migration, but the bridge is an adapter over the registered kernel handlers. Legacy `kernel.model_manual.*` aliases MAY remain only while they resolve one-to-one to the corresponding `kernel.user_manual.*` action, return the same typed result, and emit a compatibility receipt.

### 10.15.3 On-demand md projection

The Markdown projection (`USER_MANUAL.md`) is rendered **on demand** by the canonical UserManual projector. This matches CX-908 (deterministic-atomic-governance-files) and `feedback_no_default_md_files` (do not auto-emit .md files). The projection is a build artifact, not an authority surface; deleting it never affects the manual content. A legacy `MODEL_MANUAL.md` projection or `generate-model-manual-md` alias MAY exist only as an explicitly labelled compatibility output over the same UserManual records.

### 10.15.4 In-app browsing surface

Operators and no-context models browse the UserManual via the Diagnostics panel (Section 10.12), which embeds a dedicated 'Manual' tab. The tab dispatches the registered actions in Section 10.15.2; no separate browsing app or GUI-local manual store is shipped. Backend agents MUST be able to discover and read the same entries without opening or screen-scraping the GUI.

### 10.15.5 Self-consistency check (HBR-MAN-003)

A self-consistency check (`hbr-man-003-scan.mjs`) verifies every registered action id, backend handler id, compatibility channel id, schema field, config key, and CLI flag named by the UserManual against the canonical native registries and Rust symbols. A name in the UserManual that does not resolve to a registered source symbol is a HBR-MAN-003 violation. A native action or handler with no UserManual entry is the inverse violation. This prevents the manual, native backend, and compatibility transports from drifting apart.
...
A WP that adds a new wired surface without populating the corresponding UserManual fields fails the coverage gate.

### 10.15.7 Same-commit CI hook (HBR-MAN-001)

A CI hook (`hbr-man-001-paired-diff.mjs`) enforces:

- Any wired surface change (registered kernel action, backend handler, native control action, compatibility IPC channel, schema field, config key, CLI flag, or backend model/agent route) MUST be accompanied by a corresponding UserManual diff in the same commit.
- `USER_MANUAL_VERSION` in the canonical UserManual authority MUST bump. A legacy `MANUAL_VERSION` MAY mirror it during compatibility migration but MUST NOT version a separate corpus.

A commit that adds or changes an invocable native action or handler without updating the UserManual fails `just gov-check`; adding only a Tauri compatibility command does not create an exemption.

**Cross-references:** HBR-MAN-001 / HBR-MAN-002 / HBR-MAN-003; [GLOBAL-BUILD-002]..[GLOBAL-BUILD-011]; CX-200 BACKEND root; CX-908 deterministic-atomic-governance-files; `feedback_no_default_md_files`; Section 10.12 Diagnostics panel (embeds the Manual tab); Section 10.19 Command Corpus and Action Catalog Parity; historical WP-KERNEL-004 acceptance ids `AC-MODEL-MANUAL-*` (legacy names, retained for traceability).

### 10.15.8 UserManual migration bridge [ADD v02.192]

UserManual is the canonical no-context manual product surface. Existing `ModelManual` terminology, `model_manual` Rust modules, `kernel.model_manual.*` IPC names, tests, and projections are legacy compatibility paths until the owning implementation migrates them. During the bridge, a legacy ModelManual path MAY remain in code only if it maps deterministically to a `UserManualRecord` authority entry and emits a compatibility receipt when used. New product law, UI labels, action descriptors, backend routes, and manual entries MUST use UserManual terminology.
...
1. AnnotationLayer and AnnotationRegion records MUST be persisted only through the canonical authority stack: Handshake-managed SurrealDB for committed authority and EventLedger mutation history, ArtifactStore for materialized sidecars and renders, and the CRDT/write-box path for pre-promotion draft edits.
...
4. Promotion of a draft annotation set into SurrealDB authority, including its SurrealDB-persisted EventLedger entries, MUST require validation, idempotency, and stale-state checks, and MUST emit a replayable receipt.
5. The read-only display projection (the rendered overlay shown in the MediaPane) is a Display-layer projection rebuilt from authority; it MUST NOT be treated as a source of truth and MUST be reconstructable from SurrealDB authority, including EventLedger, plus ArtifactStore alone.
...
3. Notes anchored to regions MAY be surfaced to Project Brain and Loom as retrieval-shaping signals, but the canonical annotation authority remains SurrealDB, including EventLedger persistence.
...
- SET-STORE-001 (Canonical storage). The authoritative preference store MUST live in Handshake-managed SurrealDB (TECH-SURREALDB) as typed `SCHEMAFULL` records. Every mutation MUST emit an EventLedger entry; durable change evidence MUST be Flight-Recorder-backed (PRIM-FlightRecorder / PRIM-FlightEvent). Any externalized snapshot/export MUST materialize through the ArtifactStore (PRIM-ArtifactService) with a PRIM-ArtifactManifest. PostgreSQL is neither a runtime dependency nor a fallback or dual authority.
...
- SET-INV-001. SurrealDB, including EventLedger persistence, plus ArtifactStore are the only storage authorities for this domain; PostgreSQL and SQLite connectivity, dependencies, configuration, import, replay, reconciliation, fixtures, tests, and proof paths are forbidden everywhere.
...
The Stealth Reference Window state is a governed registry, not ad-hoc window handles. The registry is held in the kernel and persisted in Handshake-managed SurrealDB (TECH-SURREALDB); concurrent edits to a registry entry's annotation/layout fields go through the CRDT write-box (TECH-CRDT). PostgreSQL and SQLite connectivity, dependencies, configuration, imports, reconciliation, tests, fixtures, mocks, examples, fallbacks, caches, adapters, and temporary harnesses are forbidden.
...
- **Storage authority** -- registry state lives in Handshake-managed SurrealDB (TECH-SURREALDB); concurrent annotation/layout edits flow through the CRDT write-box (TECH-CRDT); event history lives in EventLedger persisted by SurrealDB; materialized captures/exports live in ArtifactStore. PostgreSQL and SQLite connectivity, dependencies, configuration, authorities, caches, fallbacks, imports, reconciliation, fixtures, mocks, compatibility adapters, tests, and proof paths are FORBIDDEN without exception.
...
**Why**
Handshake exposes a large operator/model command surface through registered native kernel actions and backend handlers, with native frontend controls, model/agent APIs, CLI/MCP routes, preload bridges, and legacy Tauri `#[tauri::command]` channels as consumers or compatibility adapters. Section 10.11.5.28 [ADD v02.185] already requires the Dev Command Center to project `KernelActionCatalogV1` rows, and Section 10.15 requires every wired surface to carry UserManual coverage. Section 10.19 defines the single normative parity contract that (a) enumerates the FULL native action/handler corpus into one typed action-catalog schema, (b) proves that every adapter resolves to that corpus, (c) cross-checks every catalog entry against the no-context UserManual, and (d) makes a missing product anchor an explicit, durable BLOCKED marker rather than a silent gap. This is a backend-authority + projection contract; it defines records, IDs, events, concurrency semantics, and gates, not product code.

**What**
Defines the canonical native action/handler corpus, the per-entry action-catalog descriptor parity record, compatibility-adapter mapping, the UserManual parity cross-check, the BLOCKED-anchor record, backend parallel-agent invocation semantics, the receipt/event evidence shape, and the CI parity gate. The source-of-truth is the native kernel action/handler registry; preload, Tauri IPC, CLI, MCP, GUI, and backend-agent surfaces project or invoke those same registered entries and MUST NOT define competing commands.

> NOTE (numbering): this normative section is canonically numbered 10.19. Historical references to Section 10.14 within the command-corpus contract are compatibility references and MUST resolve to Section 10.19.

---

### 10.19.1 LAW: Command corpus is a single typed catalog (Normative)

LAW-CORPUS-PARITY-001. There MUST be exactly one canonical command-corpus catalog for the product. It is the `KernelActionCatalogV1` (Section 10.11.5.28, registry `PRIM-KernelActionCatalogV1`) extended to enumerate EVERY registered native kernel action and backend handler that an operator, model, agent, native frontend, CLI, MCP route, or compatibility adapter can invoke. There MUST NOT be a second competing command list in prose, in a sidecar `.md`, in a transport registry, or in a UI-local table that claims authority.

LAW-CORPUS-PARITY-002. The corpus source-of-truth is mechanical: the canonical native kernel action/handler registry and its registered Rust handler symbols. A registered action or handler missing from the catalog is a parity defect (Section 10.19.6). A catalog entry that resolves to no registered native source symbol is a parity defect. Preload names, Tauri `#[tauri::command]` channels, CLI commands, MCP tools, and frontend control actions MUST declare the canonical `action_id` they adapt; an adapter with no matching native entry, or a native entry reachable only through one adapter, is a parity defect.
...
LAW-CORPUS-PARITY-005. Native GUI controls and backend model/agent operation MUST converge on the same registered handler, validation, authorization, idempotency, receipt, and evidence path. A GUI-only mutation, transport-only permission path, or backend-agent shortcut that bypasses the catalog is forbidden. Read/query actions MUST be usable headlessly through typed backend navigation without screen scraping; mutating actions MUST expose the same preview/draft, conflict, cancellation, recovery, and promotion semantics to every eligible actor.
...
- `action_id` -- stable kebab/dotted action id (e.g., `media.download`, `asr.transcribe`, `version.commit`); MUST resolve to one registered native kernel action or handler symbol.
- `corpus_source` -- enum set drawn from `{kernel_action, kernel_handler, cli_handler, compatibility_adapter}`; `kernel_action` and/or `kernel_handler` is mandatory, while adapter values describe reachability and never authority.
- `adapter_bindings` -- zero or more typed preload, Tauri IPC, CLI, MCP, native-control, or other transport bindings; each binding resolves to this `action_id` and handler version.
...
- `actor_eligibility` -- which actor classes MAY invoke (operator, model/coder, parallel backend agent, mechanical orchestrator); drives projection visibility per Section 10.11.5.28.
- `invocation_context` -- required actor, session, correlation, workspace/project, target-resource, and expected-version fields; mutating actions additionally declare idempotency-key and lease/claim requirements.
...
- `manual_anchor` -- the UserManual command id (Section 10.15) this entry maps to, or the sentinel `BLOCKED` (Section 10.19.4).
- `evidence_class` -- the EventLedger event type(s) and Flight Recorder span(s) the command emits (Section 10.19.5).
...
### 10.19.3 Normative: UserManual coverage cross-check

PARITY-MANUAL-001. For every catalog descriptor, exactly one of the following MUST hold: (a) `manual_anchor` resolves to a live UserManual command entry (Section 10.15.1 `UserManualRecord`) whose purpose / inputs / outputs / safety / failure / recovery fields are populated; or (b) `manual_anchor = BLOCKED` with a recorded BLOCKED record (Section 10.19.4). A legacy `PRIM-ModelManual` or `ModelManual` identifier MAY resolve only as a compatibility alias to that same UserManual entry.

PARITY-MANUAL-002. A UserManual command entry that names an `action_id` absent from the catalog is a coverage defect: the manual MUST NOT advertise a command the corpus does not expose. This is the inverse of HBR-MAN-003 (Section 10.15.5) and is enforced alongside it.
...
BLOCKED-001. A command MAY exist in the corpus with no valid product anchor (e.g., a handler that currently performs ungoverned external execution, or a command with no UserManual coverage and no governed execution path). Such a command MUST carry a durable BLOCKED record (`PRIM-CommandCorpusBlockedRecordV1`), not be silently omitted.
...
EVIDENCE-001. Every catalog command whose `execution_class` is `write_box`, `workflow_job`, or `ai_job` MUST, on invocation, emit (a) an EventLedger event of the type declared in `evidence_class` and persist it in SurrealDB, (b) a Flight Recorder span (`PRIM-FlightRecorder` / `PRIM-FlightRecorderEntry`) linkable by `action_id` and run id, and (c) a recoverable typed receipt of the declared `receipt_shape`. SurrealDB-persisted EventLedger remains the replay/promotion authority per Section 10.5 P-06.
...
EVIDENCE-004. Storage authority for all of the above is SurrealDB, including EventLedger persistence, plus ArtifactStore and CRDT/write-box only. No catalog descriptor, receipt, parity report, or BLOCKED record MAY connect to, read from, write to, fall back to, import from, reconcile with, or cache through PostgreSQL, SQLite, or any other relational authority; such a path is a hard parity defect. EventLedger persistence begins from a fresh SurrealDB genesis state and cold authority activation.

---

### 10.19.6 Normative: Parity CI gate (HBR-CORPUS)

GATE-001. A CI parity gate (HBR-CORPUS-001) MUST run on `just gov-check`. It (a) enumerates the corpus from the registered native kernel action/handler registry, (b) diffs the corpus against the catalog, (c) verifies every preload, Tauri IPC, CLI, MCP, native-control, and backend-agent adapter binding, (d) runs the UserManual cross-check (Section 10.19.3), and (e) validates per-descriptor invariants (typed params, invocation context, capabilities present-or-explicitly-empty, governed execution for external work, foreground discipline, typed receipt, event evidence).

GATE-002. The gate FAILS the build on any of: a registered native action or handler missing from the catalog; a catalog entry resolving to no native source symbol; an adapter binding resolving to no catalog action; a descriptor missing a required field; an external-execution command without a Workflow-Engine/AI-Job route or capability gate; a `foreground_flag` set for routine model invocation; a UserManual command naming a non-existent `action_id`; or a command lacking both a valid `manual_anchor` and a BLOCKED record.

GATE-003. A WP that adds, renames, or removes a registered native action, backend handler, or compatibility adapter MUST update the catalog, the UserManual coverage, and (if applicable) the BLOCKED records in the same commit, consistent with the same-commit discipline of Section 10.15.7 (HBR-MAN-001). Corpus/catalog drift is a project-quality defect.
...
### 10.19.7 Tailor and parallel backend-agent binding (Normative) [ADD v02.201]

TAILOR-CORPUS-001. Every Cloth and BodyKit tool named by Section 13, including its core garment and body authoring, simulation, validation, posing, configuration, export, and promotion-adjacent operations, MUST register exactly one canonical `action_id` and backend handler in `KernelActionCatalogV1`. Native Tailor controls, MCP tools, backend-agent routes, CLI/debug routes, and any temporary Tauri bridge MUST bind to those entries; no Tailor workflow may exist only in a GUI callback or private transport handler.

TAILOR-CORPUS-002. A Tailor action invoked by a backend agent MUST accept typed actor/session/correlation identity, project/workspace and garment/body/draft identity, expected input version, idempotency key, and lease/claim context where the action mutates shared state. It MUST return a typed result or error plus authoritative receipt and evidence links. Agents MUST be able to discover current state, dependency/lease conflicts, validation findings, cancellation state, and recovery instructions through registered read actions without opening or screen-scraping the Tailor GUI.

TAILOR-CORPUS-003. Parallel Tailor actors MUST obey the Section 13 CRDT, separate-draft, lease, job, cancellation, validation, and PromotionGate laws. Conflicting edits MUST remain attributable and recoverable; cancellation MUST release leases/claims; duplicate submissions MUST collapse only under the declared idempotency contract; no agent may self-promote or bypass operator-bound promotion. The GUI and backend-agent paths MUST surface the same conflict, denial, stale-version, partial-success, and recovery states.

TAILOR-CORPUS-004. Every Tailor catalog entry MUST resolve to a task-oriented UserManual entry covering purpose, native GUI path when one exists, backend/model invocation path, inputs/outputs, capability and foreground posture, concurrency/conflict behavior, failure/recovery, EventLedger/Flight Recorder evidence, and ArtifactStore outputs. Legacy `ModelManual` references remain compatibility aliases only.

**Cross-references:** Section 10.5 Operator Consoles (P-04 redaction, P-06 kernel authority linkability); Section 10.11.5.28 Kernel Action Catalog and Write Box Projections [ADD v02.185]; Section 10.15 UserManual Surface (HBR-MAN-001/002/003, [GLOBAL-BUILD-002]..[GLOBAL-BUILD-011]); Section 13 Tailor Cloth and BodyKit model-first APIs, swarm law, CRDT, jobs, validation, and PromotionGate; FEAT-ASR (transcript artifacts); FEAT-KERNEL-WORKSPACE-WRITE-BOX (`PRIM-KernelActionCatalogV1`, `PRIM-KernelActionDescriptorV1`, `PRIM-WriteBoxV1`, `PRIM-WriteBoxPromotionReceiptV1`); HBR-SWARM; HBR-QUIET; WP-KERNEL-004 / WP-KERNEL-009 governance lineage.
...
## 10.18 Account and Resource Privacy Surfaces [ADD v02.202]

The native GUI provides:

- a login/lock/account switcher and first-run Owner setup;
- an always-visible current account + AccessSpace indicator and a separate Persona indicator;
- account administration that separates AccountRole from content access;
- Space management and explicit resource-link management;
- project membership plus per-resource sharing/grant inspection;
- protected folder/file/database/asset/artifact lineage and visibility inspection;
- visitor-pass, MCP client, ToolGate, service-connection, token audience/expiry/revocation, and import-quarantine management;
- a privacy inspector showing why access is allowed or denied without revealing restricted resource metadata;
- filtered EventLedger/Flight Recorder/diagnostic views that show actor/delegation and resource scope only when authorized.

All surfaces consume backend authorization APIs. The GUI may narrow results for usability but cannot widen them. Stable Argus `author_id` targets and no-context UserManual routes are mandatory for every security-significant action and visible state.

Dev Command Center task/WP/MT/resource views carry `local_account_id`, `principal_id`, `session_id`, `access_space_id`, resource refs, and delegation refs where product resources are touched. Governance records do not become a hidden content-sharing path.
