...
body_sha256: "6da722d56736af19d673797dc6ed6ddedec98b63fd57a6bad0de598d44ec3398"
metadata_rule: "frontmatter is machine metadata; body follows after this block"
---
# 8. Reference

## 8.0 SurrealDB Authority Override (v02.205) [REF-SDB-001]

Every current reference model in this module is interpreted through the v02.205 exclusive authority contract: durable records and EventLedger persistence live in Handshake-managed SurrealDB, schema is typed `SCHEMAFULL` SurrealQL deployed through SurrealKit, and authenticated record-user permissions intersect with ResourceBroker. Files and sidecars are imports, exports, source artifacts, or rebuildable projections only; they are never product authority. Later SQLite-schema headings, SQLite index/database language, PostgreSQL examples, and relational DDL are non-executable historical semantic inventories whose entities and behavior must be translated to the SurrealDB model. No PostgreSQL or SQLite connectivity, import, reconciliation, cache, fallback, fixture, test, or proof path is authorized.
...
| **Storage** | File system + Handshake-managed SurrealDB (including EventLedger persistence) + ArtifactStore | Data persistence |
...
| Official SurrealDB Rust SDK | SurrealDB authority and EventLedger access, typed records, live queries, and authenticated record-user sessions |
...
**Key Takeaways**  
- Core stack: Tauri + React + Rust coordinator + Handshake ModelRuntime + Handshake-managed SurrealDB with EventLedger persistence + Yjs/CRDT write-box.
...
|   - Database: SurrealDB (including EventLedger)        |
...
- **Load:** write deterministic artifacts + upsert typed SurrealDB authority records and SurrealDB-persisted EventLedger entries + update derived indexes (search + vector + graph + role lanes)
...
## 7. Persistence contract (SurrealDB authority with EventLedger persistence)
...
**SurrealDB authority notes:**
- model vectors as typed record fields and build governed SurrealDB vector indexes; opaque blob-only authority is forbidden
- define authority records and queryable fields as `SCHEMAFULL`; untyped JSON bags may be derived projections but are never database-import inputs or authority
...
## 8.8 Account-Resource Privacy Gap Resolution and Glossary [ADD v02.202]

The Section 8.3 statement that Handshake has no user-account, team/sharing, or sensitive-data protection design is superseded by Section 2.3.13.12 and the v02.202 projections. Implementation remains scheduled through WP-KERNEL-006/007/008, so runtime proof is not claimed by this specification change. Enterprise identity providers, public-cloud operations, pricing, and generalized remote administration remain future scope.

Canonical terms:

- **LocalAccount:** login/credential holder on a Handshake installation.
- **AuthenticatedSession:** bounded authenticated account/Principal/Space context.
- **Principal:** auditable human, model, agent, MCP, service, visitor, or break-glass actor.
- **AccountRole:** installation administration role; not blanket content access.
- **MembershipRole:** project/workspace collaboration role; not blanket resource access.
- **AccessSpace:** selectable work context over already-authorized resources.
- **ProtectedResource:** any primary or derived product object with visibility consequences.
- **ResourceGrant:** durable action authorization over an exact resource or bounded set.
- **ResourceBroker:** executable filesystem/artifact authorization boundary.
- **Persona:** account-owned personalization/preferences; never authorization.
- **ExternalIdentityBinding:** local-to-remote identity link.
- **RemoteServiceConnection:** audience-bound, revocable SaaS/MCP connection.

Primary field/research basis: SurrealDB record authentication and table/field permissions, PostgreSQL Row Security Policies as legacy comparative research, OWASP Authorization and Password Storage guidance, Google Zanzibar, AWS SaaS tenant isolation and authorization guidance, MCP authorization, OpenFGA, Hugging Face organization/resource-group/token patterns, and Windows Data Protection API. SurrealDB privacy validation MUST use an authenticated record user plus the Handshake ResourceBroker; root/system identities bypass record-user permission enforcement and therefore cannot prove privacy. Civitai collection/API behavior is useful inspiration for remote moodboard/collection UX but is not an authorization authority and must be wrapped by the Handshake resource/grant model.
