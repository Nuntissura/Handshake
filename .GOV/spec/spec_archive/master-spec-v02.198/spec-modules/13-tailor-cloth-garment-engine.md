...
body_sha256: "51e079eb3b326389806e297bd5a6e55ed8b9e68278feba77f7539a4ec4ced8d9"
...
Tailor attaches to the kernel like the atelier module: a `handshake_core::tailor` domain module bound to PostgreSQL/EventLedger authority, CRDT collaboration, the sandbox->validation->promotion lifecycle, and model lanes, plus a standalone UI-agnostic `tailor-solver` Rust crate (XPBD on WGSL/wgpu; `parry` rigid proxies) reached only through the `ClothSolver` trait. This section is product LAW. The canonical authority for every Cloth type, field, unit, event, schema-id, table, migration, validation check, and promotion-equivalence rule is sub-section 13.14 (Canonical Tailor Authority Contracts); where any other sub-section conflicts with 13.14, 13.14 wins. BodyKit canonical contracts live in 13.16 and extend (never fork) the 13.14 contract surface. The research packages `.GOV/reference/cloth_engine_research/` and `.GOV/reference/tailor_bodykit_research/` are non-normative provenance only; operator-stated body requirements (OBR-001..004) bind through the normative clauses in 13.16-13.27.
...
The feature ceiling for "fully implemented" MUST be Marvelous Designer 2026.0, verified against
TWO inventories: the original research source `cloth_engine_research/02-md-feature-map.md`
(T-MD-FEATURES) and the independent second-pass external inventory
`tailor_bodykit_research/04-md-feature-inventory.md` (full MD manual corpus; v02.198). The
second-pass gap matrix (`tailor_bodykit_research/06-gap-matrix-and-mt-plan.md` Part A, CG-01..29)
is closed by microtasks MT-449..MT-477; its features are normative under this ceiling clause with
these contract deltas owned by 13.14: PanelSpec internal Hole contours (CG-01), SeamKind::Turned
(CG-08), SeamSpec.sublayer (CG-09), trim category 'piping' (CG-11), per-face material assignment
fields (CG-19), PrintLayer repeat kind (CG-20), tape-measure table `tailor_tape_measures` TPM-
(CG-21), and ExportTopologyOptions on export presets (CG-25). Features the second pass verified
as CLO-only (DXF-AAMA import, grading, seam-allowance/notch/annotation tools, colorways, offline
render, print layout, down fill) are NOT part of the MD ceiling; existing clauses that exceed MD
(DXF R12 export, SeamSpec allowance, wetness physics) remain in scope as deliberate exceedances.
All eight D4 moat features identified in T-MD-FEATURES MUST be addressed by a corresponding
normative requirement in the sub-section that owns them; MOAT-7 (EveryWear-equivalent automated
game rigging) remains post-v1 EXCEPT its core garment skin-weight-transfer path, which is in
scope per CG-28 (MT-476).
...
Reconciled 2026-07-08 (v02.198, second-pass MD parity): three items formerly deferred here are
now IN SCOPE as feature-gated deliverables with dedicated microtasks; the remaining items stay
deferred and MUST NOT be implemented in v1:

Feature-gated in scope (no longer deferred):
...
Still deferred (MUST NOT be implemented in v1):
...
- **UDIM tile support:** Each panel occupying a separate `[0,1]^2` UV tile requires an
  ...
  column in v1. (BodyKit skin UDIM law in 13.22 is a separate, in-scope surface and does not
  lift this Cloth deferral.)
...
- **Fur strand material:** Out of scope for MVP (MT-384 remains a deferred stub).
...
The following 16 tables plus the v02.198 additions listed after the block are the complete and canonical Cloth-submodule Postgres authority (BodyKit tables are owned by 13.16 under the same rules). All MUST use `TEXT PRIMARY KEY` with the stated id prefixes. Every row MUST carry an `event_ledger_event_id TEXT NOT NULL` FK to `kernel_event_ledger.event_id`. Every INSERT MUST call `guard_authority_write(AuthorityMode::Postgres)` first (the `no_sqlite_tripwire`). SQLite is NOT permitted.
...
A model-authored `BodySpec` MUST NOT be written directly to a Postgres authority row; it MUST
...
the LLM's primary output surface, the solver's primary input surface, and the PostgreSQL
authority JSONB column (`bodykit_bodies.spec_json`). It MUST derive `schemars::JsonSchema` so
...
### BK-16.3 Storage Authority and Canonical DDL

**[TAI-BK-006]** BodyKit authority is PostgreSQL ONLY. Every INSERT or mutating UPDATE to a
`bodykit_*` table MUST be preceded by `guard_authority_write(AuthorityMode::Postgres)`
(the `no_sqlite_tripwire`). All tables use `TEXT PRIMARY KEY` prefixed ids
(`BDY-{uuid_v7}` bodies, `BRC-{uuid_v7}` recipes, `BMA-{uuid_v7}` morph assets,
`BCH-{uuid_v7}` channel registry, `BSK-{uuid_v7}` skeleton defs); `UUID PRIMARY KEY DEFAULT
gen_random_uuid()` is forbidden ([TAI-OVR-013]). Every row carries
`event_ledger_event_id TEXT NOT NULL` ([TAI-OVR-014]). Migrations use the dated convention
`2026_MM_DD_tailor_bodykit_<topic>.sql` + `.down.sql` (date assigned at implementation);
numbered `0NNN_*` migrations are forbidden. This sub-section owns the canonical DDL for the
five foundation tables below. `bodykit_skin_materials` (`BSM-`) is authored canonically in
13.22 and `bodykit_export_profiles` (`BXP-`) in 13.26; both follow the identical conventions.

**[TAI-BK-007]** The canonical foundation DDL (dependency order):
...
`MeshComparator`-class tolerance equivalence on the evaluated mesh (default `epsilon_mm = 0.1`,
[TAI-OVR-006]) and content-hash idempotency for same-machine reruns only. No self-approval: the
...
pose set, solver settings) produce comparator-equivalent outputs (`MeshComparator`,
`epsilon_mm = 0.1`) across GPU backends; content-hash equality is reserved for same-machine
idempotency ([TAI-OVR-006] law).
...
`guard_authority_write(AuthorityMode::Postgres)` first.
...
### 22.11 Groom Hooks (Governed Deferral)

**[TAI-BK-139]** BodyKit MUST ship groom HOOKS now, with strand authoring itself deferred:
canonical hair-region density masks (`pubic`, `chest`, `arms`, `brows` at minimum, extensible)
stored against the region-mask authority, plus groom attachment metadata (barycentric surface
anchors that survive morphs and HD deltas; PREQ-021/022). **DEFER-BK-GROOM-001 (governed
deferral):** strand/curve authoring, groom simulation, and hair-card generation are deferred to a
dedicated future groom work packet; the hooks above MUST land in BodyKit v1 so grooms bind later
without a base-mesh or UV version change. This deferral MUST be recorded in the WP stub's open
questions (Q11) and MUST NOT be silently dropped.
...
### 23.5 Viseme Track Type (Solver Deferred)
...
**[TAI-BK-150]** **DEFER-BK-LIPSYNC-001 (governed deferral):** the audio-to-viseme lip-sync
SOLVER (Audio2Face-class or equivalent) is deferred; BodyKit v1 ships the typed `VisemeTrack`
contract, the ingest hook (accept externally produced tracks), and the mapping table only. The
deferral MUST be recorded in the WP stub open questions (Q12). Implementations MUST NOT ship an
untyped stopgap solver in place of the contract.
...
**[TAI-BK-171]** The BodyKit model API MUST be the API (parity with 13.13 §1 and [TAI-OVR-001]):
no separate model shim over a human-facing surface; GUI sliders (13.27) are projections of the
same typed contracts. All model inputs/outputs are typed JSON; every body mutation emits an
EventLedger event; the MCP gate (`src/mcp/gate.rs`) is the model's sole entry point.
...
### 25.6 Swarm Law and Model Manual
...
**[TAI-BK-194]** Built-in model manual (13.13 §12 parity; [GLOBAL-BUILD] alignment): the MCP
capability advertisement and `ContextBundle` for `task = "author_body"` MUST embed the tool call
...
`guard_authority_write(AuthorityMode::Postgres)` on every write:
...
### 26.9 DSON Read-Only Import Lane

**[TAI-BK-217]** BodyKit MUST provide a READ-ONLY DSON import lane (format-strategy P2) for
user-owned Daz library migration, with these hard constraints: (a) reads `.duf`/`.dsf` JSON
against the user's own configured Daz content roots only; (b) produces channel-mapping PROPOSALS
(Daz dial name -> BodyKit channel id + confidence + value translation) that the operator reviews
before apply — never silent auto-apply; (c) MUST NOT read `.dhdm` (undocumented proprietary
binary); (d) MUST NOT import Genesis mesh geometry, morph delta payloads, UVs, or any
Genesis-derived data as BodyKit authority (Daz EULA; delta payloads are topologically
meaningless on the BodyKit mesh anyway); (e) user-owned texture assets MAY import with explicit
operator license attestation recorded in `license_tag`; (f) the 13.16 `LICENSE_GUARD` tripwire
runs fail-closed on every import artifact. The lane kills Daz-user migration friction without
inheriting Daz IP.
...
BodyKit activation closes only on the end-to-end lifecycle proof (MT-653): NL description ->
...
model using only the typed contracts and the built-in manual ([TAI-BK-194]).
