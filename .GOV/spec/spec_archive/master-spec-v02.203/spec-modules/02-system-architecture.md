...
body_sha256: "a34f8fe42c8fb190f2a92450125d7260bec69bff62c9909e03228966d57a59b6"
...
- **Local document store** backed by Handshake-managed PostgreSQL/EventLedger authority plus CRDT/write-box structures for collaborative draft state.
...
- Stores state and history in **PostgreSQL/EventLedger**, with Temporal-inspired durable execution and resumability.
...
â”‚ ModelRuntime      â”‚ â”‚ PostgreSQL+CRDTâ”‚ â”‚  (Sandboxed code)    â”‚
â”‚                   â”‚ â”‚                â”‚ â”‚                      â”‚
...
The Janitor service MUST NOT hold a direct reference to a database pool. It MUST interact with the storage layer exclusively via the `Database` trait or a dedicated `JanitorStorage` interface. This ensures maintenance tasks remain isolated from pool ownership while preserving PostgreSQL/EventLedger as the product authority path.
...
**Why**
Handshake's local-first philosophy (Â§1.1.4) now targets PostgreSQL/EventLedger authority from the start. Building storage-boundary constraints prevents rework while avoiding stale SQLite, Docker, or outside-app fallback paths.

**What**
Defines four mandatory architectural pillars for PostgreSQL/EventLedger authority: single storage API, PostgreSQL-safe schema/migrations, rebuildable indexes, and live PostgreSQL/EventLedger testing.
...
- **PostgreSQL-safe Schema**: Migrations written for the PostgreSQL/EventLedger authority path without SQLite fallbacks or Docker proof shortcuts.
- **Rebuildable Index**: Optimization artifact that can be regenerated (not migrated row-by-row).
- **Authority-path Testing**: Validating code against real PostgreSQL/EventLedger through Handshake-managed PostgreSQL or an explicit real PostgreSQL URL.

---

#### 2.3.13.1 Four Portability Pillars [CX-DBP-002]

This section defines four mandatory architectural constraints that keep storage behavior centralized, replayable, and testable on the PostgreSQL/EventLedger authority path.
...
- REQUIRED: AppState MUST NOT expose raw `PgPool`, legacy `SqlitePool`, or `DuckDbConnection`

**Rationale:**
If database access is scattered across handlers, routes, and services, authority, replay, and isolation become impossible to prove. Centralizing through a single API module keeps PostgreSQL/EventLedger behavior auditable and prevents fallback drift.
...
All migrations MUST target PostgreSQL/EventLedger authority. SQLite-specific syntax, SQLite compatibility shims, and Docker-dependent proof assumptions are forbidden.
...
- REQUIRED: Migrations use a version-managed PostgreSQL migration framework (compatible with sqlx::migrate or similar)
- REQUIRED: Schema definitions are pure DDL (no data transforms)

**Rationale:**
Migrations are expensive to rewrite. Using one PostgreSQL/EventLedger authority path prevents dual-backend drift. Application-layer mutation tracking remains required where database triggers would obscure replay semantics.
...
**Pillar 4: PostgreSQL/EventLedger Testing [CX-DBP-013] [UPDATED v02.190]**

PostgreSQL/EventLedger is the Handshake authority path. Test infrastructure MUST prove storage behavior against real PostgreSQL/EventLedger through Handshake-managed PostgreSQL or an explicit real PostgreSQL URL. SQLite dual-backend expectations and Docker-launched PostgreSQL are stale pre-Kernel-V1 guidance for Handshake product code.

- REQUIRED: Storage layer tests target PostgreSQL/EventLedger authority.
- REQUIRED: CI pipeline includes a real PostgreSQL test variant launched through Handshake-native managed tooling or an explicit service URL, not Docker as a default.
- REQUIRED: New storage features are tested against PostgreSQL/EventLedger before merge.
- REQUIRED: Failure in the PostgreSQL/EventLedger authority path blocks PR merge.

**Rationale:**
Testing against SQLite, mocks, or Docker-only PostgreSQL launch paths hides authority-path bugs. Testing real PostgreSQL/EventLedger through Handshake-managed PostgreSQL or an explicit real URL catches product failures before merge.
...
**Authority Path (PostgreSQL/EventLedger):**
```sql
-- âœ… RECOMMENDED
CREATE TABLE ai_jobs (
...
pub struct PostgresDatabase {
    // PostgreSQL/EventLedger authority implementation
}

impl Database for PostgresDatabase { /* ... implementation logic ... */ }
```
...
- REQUIRED: Migrations tested against PostgreSQL/EventLedger through Handshake-managed PostgreSQL or an explicit real PostgreSQL URL before merge

**Rationale:**
This pattern is standard across mature database-backed products. Using it from Phase 1 means PostgreSQL/EventLedger authority is the tested path rather than a later migration project.
...
4. **WP-1-PostgreSQL-EventLedger-Tests** [UPDATED v02.190]
   - Add PostgreSQL/EventLedger to the test matrix through Handshake-managed PostgreSQL or an explicit real PostgreSQL URL.
   - Remove SQLite dual-backend and Docker-launched PostgreSQL assumptions from Handshake product tests.
   - Block PRs that fail on the PostgreSQL/EventLedger authority path.
...
- **Future authority expansion**: Reuse existing storage traits + migration framework for new PostgreSQL/EventLedger domains without introducing SQLite or Docker-backed proof paths.
...
[ADD v02.156] LoomBlock/LoomEdge records, LoomViewFilters, LoomSearchFilters, LoomBlockSearchResult, and LoomSourceAnchor are canonical backend library contracts. Their meaning MUST survive PostgreSQL/EventLedger storage, export, and replay instead of being hidden behind view-only adapters.

**PostgreSQL/EventLedger authority schema**
...
#### 2.3.13.8 PostgreSQL/EventLedger Handshake-Native Authority Foundation [ADD v02.182] [UPDATED v02.190]

Handshake treats PostgreSQL/EventLedger as the runtime authority for the self-hosted control plane and Kernel V1. This supersedes earlier "PostgreSQL later", "dual backend", SQLite cache/offline, and developer-container guidance for Handshake product authority. Local-first ergonomics remain product requirements, but authority, replay, validation, and proof come from product-managed PostgreSQL/EventLedger state.

Normative requirements:

- The self-hosted control plane MUST support an explicit `postgres_eventledger_authority` mode for Handshake product authority.
- In `postgres_eventledger_authority`, authoritative control-plane records MUST be written to PostgreSQL/EventLedger. This includes software-delivery runtime truth, ModelSession scheduling/queue state, workflow durable execution state, Locus work-tracking projection authority, Role Mailbox linked-control-plane state, Dev Command Center runtime projections, FEMS memory authority, and governed action/claim/lease/backpressure records as those feature slices are implemented.
- If PostgreSQL/EventLedger authority is required and no valid product-managed PostgreSQL connection is available, startup or the attempted authoritative write MUST fail closed. The runtime MUST NOT silently fall back to SQLite, Docker, outside apps, mock stores, fixture stores, provider chat history, or repo-governance artifacts.
- Handshake core operation MUST use Handshake-native integrated tooling based on open-source implementations: managed PostgreSQL lifecycle, product-managed subprocesses, bundled or runtime-discovered native binaries, in-process libraries, and explicit operator-configured adapters. Docker Desktop, Docker Compose, third-party model-server daemons, and manually launched support apps are compatibility-only opt-ins and are not core runtime prerequisites.
- The foundation slice for `WP-1-PostgreSQL-EventLedger-Authority-Foundation-v1` is limited to the spec law, storage-mode/default configuration, fail-closed behavior, managed bootstrap/health proof, and downstream candidate-WP handoff. It MUST NOT implement SQLite fallback, developer-container matrix surfaces, Docker proof shortcuts, or outside-app prerequisites.

Downstream activation split:

- `WP-1-Handshake-Managed-PostgreSQL-Test-Tooling-v1` owns reproducible Handshake-managed PostgreSQL startup, reset, fixtures, migration smoke profiles, and CI-ready test matrix behavior without Docker or outside-app defaults.
...
- Any prior `WP-1-SQLite-Cache-Offline-Boundaries-v1` wording is stale for Handshake product authority. Owning packets must migrate affected surfaces to PostgreSQL/EventLedger or explicitly classify them as non-authoritative legacy product debt.
...
WP-KERNEL-009 introduces the ProjectKnowledgeIndex as a Kernel V1 product authority surface. It is not a wiki folder, markdown vault, sidecar paper set, browser cache, or repo-governance mirror. It is a PostgreSQL/EventLedger-backed authority schema that lets Handshake index selected project roots, source files, media, rich documents, Loom blocks, claims, retrieval traces, UserManual entries, and validation receipts into one replayable product substrate.
...
All ProjectKnowledgeIndex authority records MUST be durable PostgreSQL/EventLedger records. Generated markdown, wiki pages, HTML, context bundles, trace reports, visual-debug exports, and operator summaries are projections only. Deleting or editing a projection MUST NOT mutate authority records. A projection MAY contain stable refs into authority records but MUST NOT become the only location where knowledge, claims, edges, or manual instructions exist.
...
##### 2.3.13.12.4 PostgreSQL enforcement

PostgreSQL/EventLedger is authorization authority for database-backed resources:

- ownership and grants use stable UUID v7 identifiers and indexed foreign keys;
- Row-Level Security is enabled and `FORCE ROW LEVEL SECURITY` is required for protected tables;
- the product runtime role MUST NOT own protected tables and MUST NOT have `SUPERUSER` or `BYPASSRLS`;
- transaction-scoped, server-derived account/principal/session/Space context is set before protected queries and reset on transaction completion;
- policies cover `SELECT`, `INSERT`, `UPDATE`, `DELETE`, conflict/upsert paths, bulk operations, background jobs, maintenance/rebuild jobs, and failure recovery;
- policy functions fail closed for missing, malformed, expired, revoked, or mismatched context;
...
Application authorization remains required for action semantics and capability checks; RLS is the non-bypassable data boundary, not the only policy layer.
...
- RLS non-bypass by table owner/runtime role/background worker;
...
Normative research basis: PostgreSQL Row Security Policies; OWASP Authorization and Password Storage Cheat Sheets; Google Zanzibar; AWS SaaS Tenant Isolation and multi-tenant authorization guidance; the Model Context Protocol authorization specification; OpenFGA; Hugging Face organization/resource-group/token patterns; and Windows Data Protection API. These sources inform the architecture but do not replace the Handshake-specific rules above.
