...
body_sha256: "e28f5c229d39befa6b1d813d55173411badb28b7712fda6bdb2771dc4fc2cccb"
...
**Why**
Pre-v02.186 Handshake had no canonical answer to "who spawned this process?" -- a problem that compounds under multi-engine model runtimes (Section 4.6), per-job sandbox adapters (Section 3.5), ASR workers, ComfyUI workers, plugin processes, and helper subprocesses. Orphan-process audits, post-mortem reconstructions, and reclaim hooks all need a single ledger. This section [ADD v02.186] introduces `ProcessOwnershipLedger` as that ledger, backed by Postgres (no SQLite, per CX-503R).

**What**
Defines the ledger primitive, the Postgres-backed schema, the HARD scope (every Handshake-spawned process), the perf-mitigated write path, the validator query surface, and the reclaim-hook integration.

---

### 5.7.1 Primitive + schema

Primitive: `ProcessOwnershipLedger`, backed by the Postgres table `kernel_process_lifecycle`. Columns:
...
- **Batched async writes** via a bounded mpsc channel (the spawn-thread never blocks on a Postgres round-trip).
...
Authorization is default-deny and checked for every request. PostgreSQL Row-Level Security and ResourceBroker enforcement protect data even when UI filters, API handlers, model prompts, background workers, or tool adapters are defective. AccountRole, MembershipRole, AccessSpace, ResourceGrant, Persona, WorkProfile, and CapabilityProfile are distinct inputs and cannot substitute for one another.
...
Required adversarial proof includes wrong account, wrong Space, project member without resource grant, disabled account, expired/revoked session, grant changed during use, stale search/cache/context, background worker without context, table-owner/RLS bypass attempt, forged client scope, path traversal/junction/symlink escape, mixed-scope derivation, log/count/error inference, export/backup restore under a different account, and remote token audience/expiry/revocation.
