...
body_sha256: "dde0ff22f54de69ca20b78231f020fdbe9b5eaa24d9659304bcb2e7e716e3b92"
...
### 3.5.2 Three required adapter implementations

KERNEL-004 ships three adapters:
...
- If no adapter satisfies the declared capabilities, the spawn MUST fail loudly with a typed `SandboxSelectionFailure` event; silent fallback is forbidden.
...
**Cross-references:** §5.2.5 Mechanical Runner Sandbox (high-level scope); §5.7 ProcessOwnershipLedger (consumes adapter id per spawned process); §4.6 ModelRuntime (delegates engine boxing to SandboxAdapter); CX-503R (no SQLite); CX-503D (terminal hygiene & reclaim); KERNEL-003 DocketAdapter (DockerAdapter preserves this integration); WP-KERNEL-004 refinement acceptance criteria AC-SANDBOX-ADAPTER-TRAIT, AC-SANDBOX-ADAPTER-IMPLS, AC-SANDBOX-CAP-DECL.
