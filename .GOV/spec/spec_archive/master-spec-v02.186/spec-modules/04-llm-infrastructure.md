...
body_sha256: "8d9309fcceffcd0128581357910671b960e5c292752b6639ad40307e37c9dfb3"
...
- `load(&self, model_id: &ModelId, opts: LoadOpts) -> Result<ModelHandle>` -- load a model into VRAM/RAM under the configured `SandboxAdapter` (Section 3.5).
- `unload(&self, handle: ModelHandle) -> Result<()>` -- release VRAM/RAM and fire ProcessOwnershipLedger STOP row (Section 5.7).
- `generate(&self, handle: &ModelHandle, prompt: Prompt, opts: GenOpts) -> Result<TokenStream>` -- streaming token API; returns a `TokenStream` consumable by upstream callers without blocking.
- `score(&self, handle: &ModelHandle, prompt: Prompt, completion: &str) -> Result<f32>` -- log-likelihood scoring for evaluation lanes.
- `embed(&self, handle: &ModelHandle, text: &str) -> Result<Embedding>` -- embedding vector (for adapters that ship an embedding head).
- `capabilities(&self, handle: &ModelHandle) -> ModelCapabilities` -- see Section 4.6.3.
- `kv_cache(&self, handle: &ModelHandle) -> Option<KvCacheHandle>` -- for prefix-cache + KV-quant operations (LlamaCppRuntime).
- `lora_stack(&self, handle: &ModelHandle) -> Option<LoraStackHandle>` -- for LoRA hot-swap (both adapters).
- `steering_hooks(&self, handle: &ModelHandle) -> Option<SteeringHookHandle>` -- for activation steering / refusal-vector / CAA (CandleRuntime).
- `cancel(&self, handle: &ModelHandle, token: &CancellationToken)` -- cooperative cancellation, must unwind any in-flight `generate`.
