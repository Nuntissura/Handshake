...
body_sha256: "4c1d4c00271e4789ef14ff0d3b45470148b37f51a042189f85a22a9f3399bac5"
...
[ADD v02.164] Phase 1 MUST treat repository management as a single declared backend policy surfaced through governed metadata rather than an implementation detail. The default backend remains the `git` command-line client behind an `external_process` boundary until an explicit decision artifact records a different backend. Silent fallback among the `git` command-line client, `go-git`, and `libgit2` is forbidden.
