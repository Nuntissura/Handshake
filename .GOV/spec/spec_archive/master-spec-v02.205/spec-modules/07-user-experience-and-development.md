...
body_sha256: "69b560f380e2fb02d765dd37802008dba96130624a53ccc447f24e93588dfc75"
metadata_rule: "frontmatter is machine metadata; body follows after this block"
---
# 7. User Experience & Development

## 7.0 SurrealDB Authority Override (v02.205) [UXD-SDB-001]

This override is current product law and takes precedence over every later database-specific template, example, validator snippet, path, migration guide, checklist, `DONE_MEANS`, or rollback example in this module. Handshake-managed SurrealDB is the exclusive durable authority and persists EventLedger records, idempotency claims, stream heads, replay metadata, workflow/session state, collaboration state, and diagnostic authority. PostgreSQL and SQLite connectivity, clients, pools, dependencies, configuration, credentials, stubs, schema runners, importers, reconciliation, fallbacks, caches, fixtures, tests, and proof paths are forbidden.

Later `SqliteDatabase`, `PostgresDatabase`, `SqlitePool`, `PgPool`, `POSTGRES_TEST_DB`, `.sql`, `.down.sql`, portable-SQL, numbered-SQL-migration, trigger, or relational-DDL text is retained only as non-executable historical semantic inventory. Implementing work MUST translate the described entities and behavior into typed `SCHEMAFULL` SurrealQL definitions, official-SDK typed stores, authenticated record-user permissions intersected with ResourceBroker, and ordered SurrealKit rollouts. Privacy proof uses record-user sessions, never root/system credentials. Real-runtime proof begins from an empty SurrealDB namespace/database with a verified EventLedger genesis and cold activation.
...
The editor MUST NOT store authoritative document state in browser local storage, hidden markdown files, generated wiki pages, provider chat history, or unversioned DOM/editor state. Those stores MAY be transient caches or projections only when replay from SurrealDB-persisted EventLedger and CRDT records reconstructs the same product state.

#### 7.1.1.9 WP-KERNEL-009 Unified Work Surface (Notes + Loom + Project Wiki + Rich Editor) [ADD v02.193]

For WP-KERNEL-009, the Notes surface (Section 10.12 Loom spec, Section 16), the Loom engine (LoomBlock/LoomEdge substrate, Sections 2.2.1.14 and 2.3.7.1), the project wiki compile layer (Section 10.12 Loom spec, Section 17), and the Tiptap/Monaco rich editor (Section 7.1.1.8) form ONE combined rich-text work surface: a full VS Code-class and Obsidian-class replacement in a single surface, for operators AND models to understand large projects and co-author work. The unified work surface is one product surface over one authority substrate; it MUST NOT ship as separate apps, separate stores, or separate operator-facing naming.

The unified work surface MUST provide at minimum (this list is explicitly non-exhaustive; later packets extend it without renaming or splitting the surface):

- code editing across many coding formats/languages via Monaco-backed code nodes;
- automatic code blocks: fenced/indented paste detection, language hints, and reversible prose-to-code conversion;
- save-to-format export per Section 7.1.1.10;
- hyperlinks;
- embedded videos;
- embedded albums;
- embedded slideshows via typed [[HS_slideshow]] links;
- picture insertion via typed [[HS_images]] links;
- file linking through typed wikilinks that resolve to backend entities;
- folder linking with folder color labels;
- project linking.

Embedded media and link nodes MUST resolve real backend assets fail-closed: an unresolvable embed renders a typed, visible error state - never a blank node, never mock data.

One surface serves two audiences. Operators work through the GUI; models work through the backend knowledge/Loom/retrieval/navigation APIs over the same SurrealDB authority records, including EventLedger entries persisted in SurrealDB. Both audiences MUST observe the same canonical state; the unified surface introduces no parallel store and no markdown-vault authority.

#### 7.1.1.10 WP-KERNEL-009 Rich Document Save-to-Format: HTML Primary Projection [ADD v02.193]

Authority is unchanged: the versioned RichDocument record and its EventLedger mutation history persisted in SurrealDB (Section 2.3.13.11) are the only authority for rich documents. An export file is never authority.

- HTML is the PRIMARY file projection for rich documents. Exported HTML MUST carry data-hs-* attributes encoding the typed link/embed/code-node semantics so that export-then-import reproduces an identical document structure (near-lossless round-trip).
- Save-to-format MUST offer two HTML export modes: self-contained (assets base64-inlined; the exported file opens standalone in a browser) and reference-linked (assets referenced by stable ids).
- Markdown export remains a deliberately lossy extractor for interop. It MUST be documented as lossy and MUST NOT be treated as a round-trip or authority format.
- Plain-text and ProseMirror JSON exports MUST be available through the same save-to-format surface.
- HTML import MUST sanitize fail-closed: scripts, event handlers, and unrecognized active content are stripped; an import that cannot be sanitized produces a typed, visible error instead of a silent partial document.
- Retrieval MAY serve a rich document as markdown (cheap), HTML (structure-aware), or JSON (programmatic) per task cost; the chosen projection MUST be recorded in the RetrievalTrace and never substitutes for the authority record.
...
**Naming note [ADD v02.193]**  
The Loom views above surface to operators inside the **Notes** surface (Section 10.12 Loom spec, Section 16) as part of the WP-KERNEL-009 unified work surface (Section 7.1.1.9). "Loom" remains the engine term; "Notes" is the operator-facing name.
...
- `{{SURREALDB_TEST_NAMESPACE}}`, `{{SURREALDB_TEST_DATABASE}}`: isolated CI/test SurrealDB namespace and database names (project-specific).
...
jobs:
  ...
  backend-storage:
    ...
    steps:
      ...
      - name: Configure Handshake-managed SurrealDB env
        if: matrix.backend == 'surrealdb_eventledger'
        run: |
          # Use Handshake-managed SurrealDB tooling and an authenticated record-user test identity.
          # Root/system credentials cannot prove resource privacy; no PostgreSQL or SQLite fallback is permitted.
          echo "HANDSHAKE_SURREALDB_ENDPOINT={{HANDSHAKE_MANAGED_SURREALDB_ENDPOINT}}" >> $GITHUB_ENV
          echo "HANDSHAKE_SURREALDB_NAMESPACE={{SURREALDB_TEST_NAMESPACE}}" >> $GITHUB_ENV
          echo "HANDSHAKE_SURREALDB_DATABASE={{SURREALDB_TEST_DATABASE}}" >> $GITHUB_ENV
...
- SURREALDB_TEST_ENDPOINT: {{SURREALDB_TEST_ENDPOINT}}  # disposable managed-SurrealDB test endpoint
- SURREALDB_TEST_NAMESPACE: {{SURREALDB_TEST_NAMESPACE}}  # isolated test namespace
- SURREALDB_TEST_DATABASE: {{SURREALDB_TEST_DATABASE}}  # isolated test database
...
- Goal: Initialize SurrealDB authority, including EventLedger persistence, from a fresh schema and cold authority activation; PostgreSQL connectivity, dependencies, configuration, migration tools, and proof paths are completely retired
...
- **DONE_MEANS**:
  ...
  * âœ… AppState refactored to use the typed SurrealDB authority-store boundary (not a concrete relational pool)
  * âœ… SurrealDatabase implements the trait with all 6 methods (Â§2.3.13.2 as refined by v02.204)
  * âœ… PostgreSQL and SQLite authority implementations, stubs, features, fixtures, and connection settings are absent
  * âœ… All existing tests pass (5 units + 3 integration tests)
  * âœ… All NEW tests pass (2 trait tests + 2 real-SurrealDB implementation tests)
...
âœ… RIGHT: IN_SCOPE_PATHS:
  - {{BACKEND_SRC_DIR}}/storage/mod.rs
  - {{BACKEND_SRC_DIR}}/storage/surreal.rs
  - {{BACKEND_SRC_DIR}}/api/jobs.rs
```
...
- "MUST: Implement the typed SurrealDatabase authority boundary"
- "MUST: Remove PostgreSQL and SQLite authority/runtime paths"

DONE_MEANS (mapped):
- [ ] MUST: Database trait defined (Â§2.3.13.3, requirement 1)
- [ ] MUST: SurrealDatabase implemented (Â§2.3.13.3 as refined by v02.204, requirement 2)
- [ ] MUST: PostgreSQL and SQLite authority/runtime paths absent (v02.204 requirement 3)
- [ ] All tests pass
...
- CX-DBP-VAL-011: SurrealQL safety. Require official SurrealDB Rust SDK query binding and reject interpolated query strings.
- CX-DBP-VAL-012: Trait boundary. No concrete SurrealDB client types cross the product API surface; require the typed storage interface.
- CX-DBP-VAL-013: SurrealKit rollout hygiene. Check ordered start/app-cutover/complete/rollback stages, idempotency, and version continuity.
- CX-DBP-VAL-014: SurrealDB-only authority. Tests MUST use Handshake-managed SurrealDB and prove that PostgreSQL/SQLite unavailability cannot select a fallback or dual authority.
...
- Query safety: no string-concatenated SurrealQL; use official SurrealDB Rust SDK query bindings and typed records.
...
// CX-DBP-VAL-011: SurrealQL safety and authenticated permissions
{
  const interpolated = runRg("query\\(format!|query\\(&format!", [backendSrc]);
  const permissions = runRg("PERMISSIONS|DEFINE ACCESS|AUTHENTICATE", [backendSrc, migrationsDir]);
  if (interpolated) {
...
// CX-DBP-VAL-012: Typed storage boundary (concrete client leakage)
{
...
{
  const rollout = runRg("SurrealKit|app-cutover|rollout", [backendSrc, migrationsDir]);
  const stages = runRg("start|app-cutover|complete|rollback", [backendSrc, migrationsDir]);
  if (!rollout || !stages) {
    failures.push("CX-DBP-VAL-013 (SurrealKit rollout hygiene): rollout implementation or required stages missing.");
  }
}

// CX-DBP-VAL-014: SurrealDB-only authority (SurrealDB present; legacy relational backends absent)
{
  const surreal = runRg("surrealdb|SurrealDB|Surreal<", [backendSrc, migrationsDir]);
  const legacy = runRg("postgres|Postgres|PgPool|PgConnection|sqlx", [backendSrc, migrationsDir]);
  if (!surreal) {
...
âœ… RIGHT: "That conflicts with Master Spec v02.204 and cannot be added to Handshake.
           Keep the packet on the SurrealDB-only authority path and record the rejected request."
```
...
Intent: Required schema-rollout law for the Handshake-managed SurrealDB authority.
````md
# MIGRATION_GUIDE (LAW) â€” SurrealDB Schema Rollouts with SurrealKit
...
- Apply ordered SurrealKit rollouts with explicit `start`, `app-cutover`, `complete`, and `rollback` stages.
...
- PostgreSQL connectivity is forbidden even for migration, compatibility, reconciliation, fixtures, tests, or proof. Retire its runtime dependencies, configuration keys, credentials, launch tooling, schema files, and database-specific validation paths.
...
- [ ] Start, app-cutover, complete, rollback, interruption recovery, and idempotent rerun behavior are tested against a real disposable SurrealDB.
...
## 7.13 Account, Space, Persona, and Sharing UX [ADD v02.202]

Handshake UI must always make the current LocalAccount and AccessSpace visible on protected product surfaces. Persona is shown separately because it changes personalization, not access. WorkProfile and CapabilityProfile remain execution/tool settings and must not be presented as login or visibility controls.

Required flows include first-run Owner creation, login/logout/lock, account switching, account administration, AccessSpace creation/selection, Persona creation/selection, explicit project/resource linking, per-resource sharing, membership management, grant review/revocation, visitor-pass management, MCP/service connection management, import quarantine, and break-glass audit review.

Every folder/file/resource picker, search result, recent item, breadcrumb, graph, preview, thumbnail, activity feed, log viewer, export dialog, backup browser, and model-context inspector must query the authorized backend set. Empty and denial states must not reveal restricted existence. Sharing dialogs show the exact resource set, recipient/Principal, allowed actions, inheritance behavior, expiry, remote audience, and derivative consequences before confirmation.

Argus-visible stable identifiers are required for account, Space, Persona, resource-visibility, share, revoke, service-link, quarantine, and denial state. Visual proof covers two accounts on the same PC, two members in one project with different private resources, concurrent grant revocation, restricted search/count behavior, Persona switching without visibility change, and service import without automatic publication.
