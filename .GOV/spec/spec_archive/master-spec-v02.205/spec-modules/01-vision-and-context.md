...
body_sha256: "6b5341a6dfe1928f0091386125780158cbbf787c97cb6a31952a58a92ece09d0"
...
> 
> **Tech Stack Decision (v02.195 native-GUI pivot, refined v02.197 for Stage):** Native Rust GUI (no webview-hosted application shell; not Tauri/Electron as the app shell) + Rust coordinator/backend + Python (AI backend). Controlled embedded WebView/browser islands are allowed only for the Stage browser/webviewer surface and Stage Apps; they are not the general Handshake UI substrate. SUPERSEDED: the prior Tauri + React + TypeScript front-end is retained only as migration/parity reference.
>
> **Data Authority Decision (v02.205):** Handshake-managed SurrealDB is the exclusive durable product authority, accessed through the official Rust SDK and typed Handshake storage boundaries. EventLedger remains the canonical append/replay semantic contract and is persisted in SurrealDB. PostgreSQL is completely retired: Handshake does not connect to it, read it, import from it, reconcile it, fall back to it, or use it as a validation target. SQLite is not an authority, cache, fallback, fixture, or proof path. Authority begins from a clean SurrealDB genesis because the current PostgreSQL database contains no valuable live information.
> 
...
| Decision | Choice | Why |
|----------|--------|-----|
| Desktop Shell | **Native Rust GUI** (no webview-hosted shell; not Tauri/Electron as app shell) | one bundled installer, fully user-owned; native rendering; supersedes the prior Tauri shell [§0A CX-008-VIS]; permits controlled Stage WebView/browser islands only as feature-local browser surfaces |
| Frontend | **Native Rust GUI** (egui/GPUI-class; toolkit decided by WP-KERNEL-011 spike) | handmade primitives, model-steerable via AccessKit; supersedes React + TypeScript [§0A CX-004/005-VIS] |
...
- [ADD v02.197] The native Rust rule forbids a webview-hosted app shell, not a governed in-app browser. Stage may own controlled WebView/browser islands for External Web and trusted Stage Apps; those islands MUST NOT become the general Handshake UI substrate.
...
| **Implementation** | Rust coordinator + **native Rust GUI** desktop shell (supersedes Tauri + React), Model Runtime Layer, embedded local stores |
...
## 1.9 Account-Bound Privacy and Collaboration Principle [ADD v02.202]

Handshake is local-first and private-by-default, not permanently single-user. A person must be able to use different LocalAccounts and AccessSpaces on the same PC without exposing private projects, files, assets, artifacts, database rows, logs, memory, indexes, previews, or model context to another account. This includes cases where coworkers share one project but do not share every resource in it.

Local-first means local authority and an offline-capable product-managed path. It does not forbid explicitly configured remote models, collaborators, sync, or the planned Handshake SaaS/MCP service. Remote access is opt-in, resource-bounded, attributable, expiring/revocable, and unable to widen local authority.

“Everything uses everything” is constrained to authorized interoperability. A surface, model, tool, or collaborator may use another resource only after the authenticated Principal, session, action, exact resource, ResourceGrant, and applicable capability policy authorize it. Privacy boundaries are product architecture, not a future enterprise add-on.
