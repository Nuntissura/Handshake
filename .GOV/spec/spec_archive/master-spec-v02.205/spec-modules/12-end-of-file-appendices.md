...
body_sha256: "e9fd6782e33e3d3a8d24fdf1c2c3551bfd24f29b9e24ca4c932f8ceac960ab9e"
...
## 12.0 SurrealDB Authority Override (v02.205) [APP-SDB-001]

All active appendix registries, feature links, runtime-visibility rows, ownership matrices, coverage maps, and technology assignments are governed by exclusive Handshake-managed SurrealDB/EventLedger authority. PostgreSQL and SQLite primitive or technology assignments, runtime postures, gap targets, source paths, and proof paths are retired from the active registry. Historical names may remain only in changelog/provenance strings or completed-packet identifiers and never authorize implementation. DuckDB is permitted only as a bounded rebuildable projection under the explicit projection boundary. Conflicts later in this module resolve to this override and the corrected v02.205 structured registry values.
...
[ADD v02.198] The v02.198 bump grew Section 13 (Tailor) into the two-submodule Cloth + BodyKit creative module (Main Body law in 13.1-13.27, anchors TAI-BK-001..240). The FEATURE_REGISTRY, PRIMITIVE_TOOL_TECH_MATRIX, UI_GUIDANCE, and INTERACTION_MATRIX appendix blocks below carry the matching `FEAT-TAILOR-CLOTH` and `FEAT-TAILOR-BODYKIT` rows for that Main-Body growth (Cloth rows are also a backfill of the v02.194 Section 13 activation). This appendix growth follows, and does not lead, the Section 13 Main Body.

[ADD v02.199] The v02.199 bump added Section 14 (Studio -- Unified Creative Suite): the full deduped feature research corpus for Photoshop, Illustrator, InDesign, the Affinity suite, and the Figma family merged into the Master Spec as sole normative authority (no sidecars), across 24 sub-sections 14.1-14.24 with ~626 `[STU-*]` normative anchors. The FEATURE_REGISTRY, PRIMITIVE_TOOL_TECH_MATRIX, and INTERACTION_MATRIX appendix blocks below carry the matching `FEAT-STUDIO-*` (12 domain features), `PRIM-Studio*` (38 primitives), and `IMX-STUDIO-*` (3 force-multiplier edges) rows for that Main-Body growth. This appendix growth follows, and does not lead, the Section 14 Main Body law defined in module 14.

[ADD v02.200] The v02.200 bump hardens Section 14 (Studio) with the universal command contract [STU-CON-007] (every Studio tool/command/primitive MUST be model-invokable + parallel-safe + deterministic + visually-verifiable, so parallel model agents deterministically edit photos/vector-masks/layouts across multiple files at once with the visual tools), the no-operator-only-by-default rule [STU-MDL-006], and two scope clarifications ([STU-RAS-051] focus-stack/seamless-blend op; [STU-OVR-015] video-footage clip-editing out of scope). This is Main-Body-only hardening; the appendix blocks below are re-stamped to v02.200 but add no new FEAT/PRIM/IMX rows, so this appendix change follows, and does not lead, the Section 14 Main Body.

[ADD v02.201] The v02.201 Tailor build-readiness enrichment makes the existing Cloth and BodyKit entries implementation-grade: one versioned capability registry, command/operation authority, native surface registry, project lifecycle, task-oriented in-product UserManual, parallel-agent occupancy and recovery, and explicit Flight Recorder, shared diagnostics, Locus, Loom, and Calendar edges. `PRIM-TailorMixGraph` may consume immutable PoseKit evidence only after the PoseKit source commit is contained in merged main and the pinned WP-KERNEL-012 native-shell compatibility receipt passes; this appendix does not represent an unmerged PoseKit surface as current product authority. This appendix growth follows, and does not lead, the matching Section 13 Main Body law.

[ADD v02.203] The v02.203 Tailor/BodyKit-v2 enrichment replaces Python, generated Blender scripts and cloud-capable core paths with a Rust-owned offline runtime; makes GNM v3 the default provider behind provider-neutral body/head evaluation; adds source/assembly, head-package, corrective/residual, performance, conditioning, dependency and interchange primitives; defines GLB/restricted-USD/OpenPBR/MaterialX/EXR paths and honest Blender/Unreal qualification; applies account-resource privacy; and specifies the professional native workspaces, top-bar actions, parallel-project state and movie-production waist-up proof. The rows below follow the matching 13.37-13.45 Main Body law.
...
```json
{
  ...
  "features": [
    ...
    },
    {
      ...
      "notes": "[ADD v02.144] ASR is a governed transcription surface that turns media into timestamped transcript artifacts for Loom, retrieval, and operator evidence. [ADD v02.158] Transcript payloads, source-media hashes, ffprobe-derived media facts, timing anchors, and bounded failure/progress events are canonical backend portability and Flight Recorder seams instead of UI-only output. [ADD v02.189] Media transcript and caption pipeline (Section 6.11): ffmpeg/ffprobe media-probe job, transcript-format + caption/subtitle artifacts, timing anchors, source-media hash; Workflow-Engine jobs -> ArtifactStore + receipts. Extends the ASR subsystem.",
    ...
    },
    {
      ...
      "notes": "Job-driven photo pipeline surface; exports follow the unified export contract. [ADD v02.144][REFINE v02.197] Photo Studio is a Studio Module visual-work surface whose job graph and sidecar render path must stay explicit. [ADD v02.189] ComfyUI custom-node intake happy-path (Section 6.9): governed bridge-node presence detection, capability registration, and output routing into ArtifactStore/EventLedger inside engine.comfyui job context; SaveImage directory-scan fallback preserved."
    },
    {
    ...
    },
    {
      ...
      "primitives": [
        "PRIM-Database",
        "PRIM-SurrealDatabase",
        "PRIM-SurrealPrimaryControlPlane",
      ...
      "tools_tech": [
        "TECH-SURREALDB",
        "TECH-SURREALDB-RUST-SDK",
        "TECH-SURREALKIT"
      ],
      "notes": "[REFINE v02.204] Handshake-managed SurrealDB is the exclusive runtime database authority and persists EventLedger semantics. The official SurrealDB Rust SDK provides typed access; authority tables are SCHEMAFULL; privacy uses authenticated record-user table/field permissions intersected with ResourceBroker; live queries drive authorized projections; SurrealKit governs staged schema rollouts. Authority initializes from an empty SurrealDB schema, a verified EventLedger genesis event, and cold activation. PostgreSQL and SQLite connectivity, dependencies, configuration, credentials, launch tooling, importers, reconciliation tools, fixtures, tests, and proof paths are completely retired. [ADD v02.147] Portable artifact manifests, bundle indexes, retention policy, and prune reports remain part of the storage portability contract. [ADD v02.148] RetentionReport preserves evidence across SurrealDB schema rollouts. [ADD v02.150-v02.153] Stage, Media Downloader, workflow, mailbox, AI-ready index, Spec Router, MCP/MEX, consent, and cloud-escalation evidence retains stable manifest/hash/retention semantics."
    },
    {
      ...
      "notes": "[ADD v02.144] Studio is the cross-surface creative shell that must make runtime orchestration visible across Canvas, Lens, Photo, and later design modules. [ADD v02.158] In Phase 1, Studio remains shell-level while Stage/ASR/Lens backend lineage and job identity stay explicit and stub-backed rather than hidden in the shell. [ADD v02.197] Studio Module owns visual creation: canvas/Figma-like collaboration, photo/painting/compositing, Affinity/Photoshop-class tools, and future 3D visual authoring; Stage supplies captured/provenance artifacts and web/HTML projections but not a parallel visual editor.",
      "capability_slice_ids": [
        "CAP-007"
      ],
      "runtime_visibility_ids": [
    ...
    },
    {
      ...
      "notes": "[ADD v02.185][REFINE v02.204] Kernel V1 pre-promotion CRDT workspace edits route through KernelActionCatalogV1 and WriteBoxV1; direct authoritative edits are denied with durable evidence; promotion into SurrealDB authority, including EventLedger entries persisted in SurrealDB, requires explicit validation, idempotency, stale-state checks, and replayable receipts."
    },
    {
      "feature_id": "FEAT-SOURCING-SPEC-MATRIX",
      "title": "Sourcing-Spec Schema and Handler Version Matrix",
      "spec_anchor": "6.12",
      "surfaces": [
        "backend",
        "operator_consoles"
      ...
      "notes": "[ADD v02.189] YAML sourcing-spec schema parsing/validation + multi-version handler compatibility matrix on the workflow spec registry: version pinning, handler routing, idempotent ingestion, version-mismatch rejection with receipts."
    },
    {
      "feature_id": "FEAT-MEDIA-ANNOTATION",
      "title": "Media Annotation Overlays",
      "spec_anchor": "10.16",
      "surfaces": [
        "ui",
        "backend"
      ...
      "notes": "[ADD v02.189] DAM-level per-image annotation overlays: typed regions/boxes/points/notes layered over a media asset, decoupled from pose keypoints, stored as data + read-only projection, surviving export."
    },
    {
      "feature_id": "FEAT-SETTINGS-PREFERENCES",
      "title": "Settings and Preferences Domain",
      "spec_anchor": "10.17",
      "surfaces": [
        "ui",
        "backend",
      ...
      "notes": "[ADD v02.189] Governed general settings/preferences domain: typed preference records (data roots, view defaults, retention, feature toggles), validation, defaults, redacted projection; distinct from LLM config."
    },
    {
      "feature_id": "FEAT-STEALTH-REFERENCE-WINDOW",
      "title": "Stealth Reference Window",
      "spec_anchor": "10.18",
      "surfaces": [
        "ui",
        "backend",
      ...
      "notes": "[ADD v02.189] Stealth reference-window state model + read-only projection honoring the non-intrusive operation policy (HBR-QUIET): never steals focus, never foreground, consumable by diagnostics/automation without OS input injection."
    },
    {
      "feature_id": "FEAT-COMMAND-CORPUS-PARITY",
      "title": "Command Corpus and Action Catalog Parity",
      "spec_anchor": "10.19",
      "surfaces": [
        "backend",
        "operator_consoles"
      ...
      "notes": "[ADD v02.189] Command-corpus parity contract: full ~100+ operator/model command set enumerated into the kernel action catalog schema, cross-checked against the no-context model manual, with BLOCKED markers where no product anchor exists."
    },
    {
      "feature_id": "FEAT-TAILOR-CLOTH",
      "title": "Tailor::Cloth — Garment Authoring & Cloth Simulation Engine",
      "spec_anchor": "#13-tailor (13.1-13.15)",
      "surfaces": [
        "backend",
        "ui",
        "operator_consoles"
      ...
      "primitives": [
        "PRIM-GarmentSpec",
        "PRIM-ClothSolver",
        "PRIM-SimulationReceipt",
        "PRIM-TailorValidationDescriptor",
        "PRIM-ClothBodyProxy",
        "PRIM-TailorModelAdapter",
        "PRIM-GarmentAnimationDraft",
        "PRIM-TailorExportPreset",
        "PRIM-TailorCapabilityRegistry",
        "PRIM-TailorOperationContract",
        "PRIM-TailorSurfaceDescriptor",
        "PRIM-TailorMixGraph",
        "PRIM-TailorProject",
        "PRIM-TailorDependencyBundle",
        "PRIM-TailorInterchangeProfile",
        "PRIM-TailorActiveToolState"
      ],
      "tools_tech": [
        "TOOL-MCP",
        "TECH-RUST",
      ...
      "notes": "[ADD v02.198] Appendix backfill: Section 13 (Tailor) activated in v02.194 without appendix rows — recorded now as part of the v02.198 two-submodule expansion. MD-parity garment engine: XPBD GPU solver, pattern/seam authoring, fabric models, collision (exaggerated proportions), trims, UV/texture, animation, auto-fit, and model-first MCP surface. Second-pass parity CG-01..29 closed by MT-449..477. [ADD v02.201] Build-readiness adds version-pinned parity claims, one command/operation authority for native GUI and headless agents, stable TailorSurfaceDescriptor routes and control ids, recoverable TailorProject lifecycle, task-oriented UserManual coverage, and explicit shared-pillar projection. [ADD v02.203] Rust-owned offline dependency and interchange profiles supersede Python/Blender-script Alembic and FBX requirements; canonical export is deterministic GLB plus a restricted qualified USD path, with optional DCC qualification and explicit loss/unsupported receipts."
    },
    {
      "feature_id": "FEAT-TAILOR-BODYKIT",
      "title": "Tailor::BodyKit — Parametric Body Engine (Daz-replacement)",
      "spec_anchor": "#13-tailor (13.16-13.27)",
      "surfaces": [
        "backend",
        "ui",
        "operator_consoles"
      ...
      "primitives": [
        ...
        "PRIM-BodyChannel",
        "PRIM-BodyRecipe",
        "PRIM-BodyCorrectiveEngine",
        "PRIM-BodySoftTissue",
        "PRIM-BodyGenitalModules",
        "PRIM-BodyExportProfile",
        "PRIM-BodyValidationReceipt",
        "PRIM-TailorCapabilityRegistry",
        "PRIM-TailorOperationContract",
        "PRIM-TailorSurfaceDescriptor",
        "PRIM-TailorMixGraph",
        "PRIM-TailorProject",
        "PRIM-TailorDependencyBundle",
        "PRIM-BodyProviderPackage",
        "PRIM-BodyEvaluator",
        "PRIM-BodyHeadPackage",
        "PRIM-HeadBodyAssembly",
        "PRIM-BodyResidualLayer",
        "PRIM-BodyPerformanceTrack",
        "PRIM-BodyConditioningBundle",
        "PRIM-TailorInterchangeProfile",
        "PRIM-TailorActiveToolState",
        "PRIM-BodyCinematicQualificationProfile"
      ],
      "tools_tech": [
        "TOOL-MCP",
        "TECH-RUST",
      ...
      "notes": "[ADD v02.198] BodyKit submodule: decoupled body channels, extreme ranges, native genital modules, per-body correctives, unified-solver soft tissue, UDIM skin, face controls, Cloth targets, model API and archetype qualification. [ADD v02.201] Build-readiness adds the version-pinned parity gate, command/operation authority, native surface descriptors, project lifecycle, UserManual, pillar links, and parallel-agent conflict/recovery law. [ADD v02.203] BodyKit-v2 adds local GNM-v3-default provider-neutral Rust evaluation, high-resolution head packages, source/assembly and reversible residual/corrective layers, performance/capture and conditioning bundles, OpenRigLogic-compatible Rust evaluation, OpenPBR/MaterialX/EXR, Rust GLB/restricted USD, Unreal native qualification, manual Blender qualification, dependency/SBOM/airgap control, privacy, parallel projects, full operator/model workspaces, and cinematic waist-up frame/sequence proof."
    },
    {
      "feature_id": "FEAT-STUDIO-RASTER",
      "title": "Studio::Raster — Imaging & Photo Editing",
      "spec_anchor": "#14-studio (14.4)",
      "surfaces": [
        "backend",
        "ui",
        "operator_consoles"
      ...
      "primitives": [
        "PRIM-StudioDocument",
        "PRIM-StudioLayer",
        "PRIM-StudioSelectionSet",
        "PRIM-StudioMask",
        "PRIM-StudioRasterTile",
        "PRIM-StudioAdjustment",
        "PRIM-StudioLiveFilter",
        "PRIM-StudioBlendMode"
      ],
      "tools_tech": [
        "TECH-RUST",
        "TECH-WGSL",
      ...
      "notes": "[ADD v02.199] Deduped raster/photo surface (Photoshop+Camera-Raw pixel side+Affinity Photo+Figma raster fills): pixel/adjustment/live-filter layer model, unified selection+masking, brush/clone/heal/inpaint retouch, content-aware transforms, ~29 adjustments + 30 blend modes, channels/8-16-32-bit. Native ML primitives; generative (Firefly/Neural cloud) = optional adapter."
    },
    {
      "feature_id": "FEAT-STUDIO-VECTOR",
      "title": "Studio::Vector — Graphics & Illustration",
      "spec_anchor": "#14-studio (14.5)",
      "surfaces": [
        "backend",
        "ui",
        "operator_consoles"
      ...
      "primitives": [
        "PRIM-StudioVectorPath",
        "PRIM-StudioVectorNetwork",
        "PRIM-StudioGradient",
        "PRIM-StudioPattern",
        "PRIM-StudioStyleRegistry",
        "PRIM-StudioEffectStack"
      ],
      "tools_tech": [
        "TECH-RUST",
        "TECH-WGSL",
      ...
      "notes": "[ADD v02.199] Deduped vector surface (Illustrator+Affinity Designer+Figma/Draw): unified path+vector-network geometry, pen/curvature/node/width tools, parametric shapes, boolean/geometry ops, appearance model (multi fill/stroke), brush families, artboards, transforms/distort/blend/repeat, live paint, gradient mesh, image trace. Text-to-Vector/Generative Recolor = optional adapter."
    },
    {
      "feature_id": "FEAT-STUDIO-LAYOUT",
      "title": "Studio::Layout — Page Layout & Publishing",
      "spec_anchor": "#14-studio (14.6)",
      "surfaces": [
        "backend",
        "ui",
        "operator_consoles"
      ...
      "primitives": [
        "PRIM-StudioPageSpread",
        "PRIM-StudioLayer",
        "PRIM-StudioTextStory",
        "PRIM-StudioStyleRegistry",
        "PRIM-StudioLayoutGrid",
        "PRIM-StudioExportRecipe"
      ],
      "tools_tech": [
        "TECH-RUST",
        "TECH-WGSL",
      ...
      "notes": "[ADD v02.199] Deduped layout/publishing surface (InDesign+Affinity Publisher): page/spread+parent-page model, threaded text frames, placed graphics + links, tables, full style system (para/char/object/table/cell/TOC, GREP/nested styles), long-document (books/TOC/index/footnotes/cross-refs/conditional-text/variables), grids/guides, prepress (preflight/package/print/PDF-X/ink-manager/flattener/separations/data-merge)."
    },
    {
      "feature_id": "FEAT-STUDIO-TYPOGRAPHY",
      "title": "Studio::Typography — Text Engine",
      "spec_anchor": "#14-studio (14.7)",
      "surfaces": [
        "backend",
        "ui",
        "operator_consoles"
      ...
      "notes": "[ADD v02.199] Deduped native-Rust typography engine (cosmic-text/rustybuzz-class, no platform/subscription dep) across all five suites: point/area/frame/artistic text + text-on-path + threading, full character/paragraph H&J controls, composers, balance-ragged-lines, optical-margin, OpenType feature exposure, variable fonts, glyph browser, special-char catalogs."
    },
    {
      "feature_id": "FEAT-STUDIO-COLOR",
      "title": "Studio::Color — Management & Pipeline",
      "spec_anchor": "#14-studio (14.8)",
      "surfaces": [
        "backend",
        "ui",
        "operator_consoles"
      ...
      "notes": "[ADD v02.199] Deduped color pipeline (native ColorEngine CMM): RGB/CMYK/Lab/Gray, 8/16/32-bit, ICC/OCIO, wide-gamut, swatches (process/global/spot/mixed-ink), gradients, patterns, soft-proof/gamut-warning/rendering-intents/overprint/ink-manager/separations, harmony/recolor. Pantone/branded libraries = optional license-gated adapter; native spot-color first-class."
    },
    {
      "feature_id": "FEAT-STUDIO-EFFECTS",
      "title": "Studio::Effects — Filters, Effects & Adjustments",
      "spec_anchor": "#14-studio (14.9)",
      "surfaces": [
        "backend",
        "ui",
        "operator_consoles"
      ...
      "notes": "[ADD v02.199] Deduped non-destructive effect engine (~226 filter/effect rows across Photoshop filters+Illustrator live-effects+Affinity live-filters+Figma effects collapsed to one StudioEffectStack+StudioLiveFilter model): blur/distort/noise/pixelate/render/sharpen/stylize/artistic families, liquify, vector 3D-and-materials/pathfinder-effects/15-warp-presets, layer fx. GPU via studio-engine RenderEngine; generative filters = optional adapter."
    },
    {
      "feature_id": "FEAT-STUDIO-DESIGN-SYSTEM",
      "title": "Studio::DesignSystem — Components & Variables",
      "spec_anchor": "#14-studio (14.10)",
      "surfaces": [
        "backend",
        "ui",
        "operator_consoles"
      ...
      "primitives": [
        "PRIM-StudioComponent",
        "PRIM-StudioComponentInstance",
        "PRIM-StudioVariable",
        "PRIM-StudioVariableCollection",
        "PRIM-StudioStyleRegistry",
        "PRIM-StudioAutoLayout",
        "PRIM-StudioConstraint",
        "PRIM-StudioLayoutGrid"
      ],
      "tools_tech": [
        "TECH-RUST",
      ...
      "notes": "[ADD v02.199] Deduped design-system surface (Figma components/variants/variables + Illustrator symbols + InDesign styles): components/instances/variants + component properties, override model, libraries/publish/swap, variables (color/number/string/boolean + collections/modes/scoping/aliasing/expressions), styles-vs-variables, full auto-layout contract, constraints, layout grids."
    },
    {
      "feature_id": "FEAT-STUDIO-PROTOTYPE",
      "title": "Studio::Prototype — Prototyping, Motion & Interaction",
      "spec_anchor": "#14-studio (14.11)",
      "surfaces": [
        "backend",
        "ui",
        "operator_consoles"
      ...
      "primitives": [
        "PRIM-StudioPrototypeFlow",
        "PRIM-StudioMotionTimeline",
        "PRIM-StudioVariable",
        "PRIM-StudioComponentInstance"
      ],
      "tools_tech": [
      ...
      "notes": "[ADD v02.199] Deduped prototyping/motion surface (Figma prototyping+Motion + InDesign interactive/EPUB): flows/triggers/actions, smart-animate + easing (bezier+spring), overlays, scroll behaviors, interactive components, motion timeline (keyframes/animatable props/motion-paths/playback), interactive documents (buttons/forms/MSO/media/transitions). Lottie/hosted viewers = optional adapter."
    },
    {
      "feature_id": "FEAT-STUDIO-RAW",
      "title": "Studio::Raw — Camera Raw / Develop Pipeline",
      "spec_anchor": "#14-studio (14.12)",
      "surfaces": [
        "backend",
        "ui",
        "operator_consoles"
      ...
      "notes": "[ADD v02.199] Deduped raw-develop pipeline (Photoshop Camera Raw + Affinity Develop persona -> one StudioRawDevelop): 9 panel groups (basic/curve/detail/color-mixer/color-grading/optics/geometry/effects/calibration), full mask-source set via shared StudioMask, deterministic enhance (denoise/demosaic/super-resolution) native; ML-enhance = optional local adapter, raw-as-re-editable-object linkage to raster."
    },
    {
      "feature_id": "FEAT-STUDIO-INTEROP",
      "title": "Studio::Interop — Import/Export & Format Compatibility",
      "spec_anchor": "#14-studio (14.13)",
      "surfaces": [
        "backend",
        "ui",
        "operator_consoles"
      ...
      "notes": "[ADD v02.199] Deduped import/export surface (all five suites' format matrices -> one 47-row compatibility matrix): raster (PSD/TIFF/PNG/JPEG/WebP/HEIC/EXR/HDR/JPEG-XL/FITS), vector (AI/SVG/EPS/PDF/DWG-DXF), layout (IDML/PDF-X/tagged-PDF), Affinity native (.af family), Figma local-copy (.fig/.jam/.deck/.buzz/.site/.make + Sketch import). No invented interchange format; five-artifact promotion gate (fixtures/round-trip/unsupported-receipts/recovery). Account-gated formats = adapter/omitted."
    },
    {
      "feature_id": "FEAT-STUDIO-AUTOMATION",
      "title": "Studio::Automation — Scripting & Plugin/API",
      "spec_anchor": "#14-studio (14.14)",
      "surfaces": [
        "backend",
        "ui",
        "operator_consoles"
      ...
      "primitives": [
        "PRIM-StudioDocument",
        "PRIM-StudioModelAdapter",
        "PRIM-StudioEditProposal"
      ],
      "tools_tech": [
        "TECH-RUST",
        "TECH-WGSL",
      ...
      "notes": "[ADD v02.199] Deduped automation surface (all five suites' scripting DOMs -> one typed Studio command API = the same contract models call): actions/macros/batch, data-driven graphics/data-merge, find/change+GREP, sandboxed plugin layer under kernel capability/consent gates, MCP-native inspect/codegen (Dev-Mode-class). Affinity no-scripting posture rejected as target; hosted REST = optional adapter."
    },
    {
      "feature_id": "FEAT-STUDIO-WHITEBOARD",
      "title": "Studio::Whiteboard — Whiteboard & Diagramming",
      "spec_anchor": "#14-studio (14.15)",
      "surfaces": [
        "backend",
        "ui",
        "operator_consoles"
      ...
      "notes": "[ADD v02.199] Deduped whiteboard surface (FigJam-class, proportionate): StudioDocument in whiteboard mode, board objects (sticky notes/shapes/smart-connectors/stamps/tables/mindmaps/sections/media/code-blocks/drawing) as StudioLayer kinds, templates, widgets-as-plugins, facilitation (voting/timer/cursor-chat via 14.17), diagram generation via 14.14/14.18."
    }
  ],
  "capability_slices": [
    {
  ...
  "runtime_visibility_map": [
    {
      ...
      "storage_posture": "SURREALDB_EXCLUSIVE_AUTHORITY",
...
```json
{
  ...
  "primitives": [
    ...
    {
      "primitive_id": "PRIM-SurrealDatabase",
      "title": "SurrealDatabase",
      "kind": "rust_struct"
    },
    {
    ...
    {
      "primitive_id": "PRIM-SurrealPrimaryControlPlane",
      "title": "SurrealPrimaryControlPlane",
      "kind": "spec_schema"
    },
    {
    ...
    {
      "primitive_id": "PRIM-FemsSurrealMemoryStore",
      "title": "FemsSurrealMemoryStore",
      "kind": "spec_schema"
    },
    {
      "primitive_id": "PRIM-WorkflowSurrealDurableExecution",
      "title": "WorkflowSurrealDurableExecution",
      "kind": "spec_schema"
    },
    {
      "primitive_id": "PRIM-DccSurrealProjection",
      "title": "DccSurrealProjection",
      "kind": "spec_schema"
    },
    {
    ...
    {
      "primitive_id": "PRIM-KernelActionCatalogV1",
      "title": "KernelActionCatalogV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-KernelActionDescriptorV1",
      "title": "KernelActionDescriptorV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-WriteBoxV1",
      "title": "WriteBoxV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-WriteBoxDirectEditDeniedV1",
      "title": "WriteBoxDirectEditDeniedV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-WriteBoxPromotionRequestV1",
      "title": "WriteBoxPromotionRequestV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-WriteBoxPromotionReceiptV1",
      "title": "WriteBoxPromotionReceiptV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-CrdtWorkspaceDraftV1",
      "title": "CrdtWorkspaceDraftV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-CrdtWorkspaceSnapshotV1",
      "title": "CrdtWorkspaceSnapshotV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-CandleRuntime",
      "title": "CandleRuntime",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-DiagnosticsPanel",
      "title": "DiagnosticsPanel",
      "kind": "ts_interface"
    },
    {
      "primitive_id": "PRIM-DistillationCandidateReview",
      "title": "DistillationCandidateReview",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-DockerAdapter",
      "title": "DockerAdapter",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-ExecPolicyExtended",
      "title": "ExecPolicyExtended",
      "kind": "spec_schema"
    },
    {
      "primitive_id": "PRIM-HBRGate",
      "title": "HBRGate",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-HBRViolationReceipt",
      "title": "HBRViolationReceipt",
      "kind": "spec_schema"
    },
    {
      "primitive_id": "PRIM-InferenceLabUI",
      "title": "InferenceLabUI",
      "kind": "ts_interface"
    },
    {
      "primitive_id": "PRIM-LlamaCppRuntime",
      "title": "LlamaCppRuntime",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-LocalModelAdapter",
      "title": "LocalModelAdapter",
      "kind": "rust_trait"
    },
    {
      "primitive_id": "PRIM-MemoryCapsule",
      "title": "MemoryCapsule",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-ModelManual",
      "title": "ModelManual",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-ModelRuntime",
      "title": "ModelRuntime",
      "kind": "rust_trait"
    },
    {
      "primitive_id": "PRIM-ModelRuntimeControlPanel",
      "title": "ModelRuntimeControlPanel",
      "kind": "ts_interface"
    },
    {
      "primitive_id": "PRIM-ProcessOwnershipLedger",
      "title": "ProcessOwnershipLedger",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-SandboxAdapter",
      "title": "SandboxAdapter",
      "kind": "rust_trait"
    },
    {
      "primitive_id": "PRIM-HandshakeNativeSandboxAdapter",
      "title": "HandshakeNativeSandboxAdapter",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-SelfImprovementLoop",
      "title": "SelfImprovementLoop",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-SwarmTestHarness",
      "title": "SwarmTestHarness",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-WSL2PodmanAdapter",
      "title": "WSL2PodmanAdapter",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-WindowsNativeJailAdapter",
      "title": "WindowsNativeJailAdapter",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-ComfyBridgeNodeProbeV1",
      "title": "ComfyBridgeNodeProbeV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-ComfyBridgeCapabilityRecordV1",
      "title": "ComfyBridgeCapabilityRecordV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-ComfyIntakeOutputRecordV1",
      "title": "ComfyIntakeOutputRecordV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-ComfyIntakeReceiptV1",
      "title": "ComfyIntakeReceiptV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-ComfyBridgeFakeAdapterV1",
      "title": "ComfyBridgeFakeAdapterV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-OutputRootConfigV2",
      "title": "OutputRootConfigV2",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-MdDownloadSessionV2",
      "title": "MdDownloadSessionV2",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-MdItemStateV2",
      "title": "MdItemStateV2",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-MdCheckpointV2",
      "title": "MdCheckpointV2",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-MdAuthContextV2",
      "title": "MdAuthContextV2",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-MdAllowlistPolicyV2",
      "title": "MdAllowlistPolicyV2",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-MdSessionReceiptV2",
      "title": "MdSessionReceiptV2",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-MediaProbeReportV1",
      "title": "MediaProbeReportV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-TranscriptArtifactV1",
      "title": "TranscriptArtifactV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-TimingAnchor",
      "title": "TimingAnchor",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-CaptionArtifactV1",
      "title": "CaptionArtifactV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-MediaProbeReceiptV1",
      "title": "MediaProbeReceiptV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-TranscribeReceiptV1",
      "title": "TranscribeReceiptV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-CaptionRenderReceiptV1",
      "title": "CaptionRenderReceiptV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-SourcingSpecRecordV1",
      "title": "SourcingSpecRecordV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-HandlerVersionMatrixEntryV1",
      "title": "HandlerVersionMatrixEntryV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-SourcingBindingDecisionV1",
      "title": "SourcingBindingDecisionV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-VersionMismatchReceiptV1",
      "title": "VersionMismatchReceiptV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-SourcingIngestionReceiptV1",
      "title": "SourcingIngestionReceiptV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-AnnotationLayer",
      "title": "AnnotationLayer",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-AnnotationRegion",
      "title": "AnnotationRegion",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-AnnotationRegionKind",
      "title": "AnnotationRegionKind",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-AnnotationExportMode",
      "title": "AnnotationExportMode",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-AnnotationEvent",
      "title": "AnnotationEvent",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-PreferenceRecord",
      "title": "PreferenceRecord",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-PreferenceValueType",
      "title": "PreferenceValueType",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-PreferenceScope",
      "title": "PreferenceScope",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-PreferenceSchemaEntry",
      "title": "PreferenceSchemaEntry",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-PreferenceChangedEvent",
      "title": "PreferenceChangedEvent",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-PreferenceChangeReceipt",
      "title": "PreferenceChangeReceipt",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-PreferenceProjection",
      "title": "PreferenceProjection",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StealthReferenceWindow",
      "title": "StealthReferenceWindow",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-ContentRef",
      "title": "ContentRef",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-VisibilityFlag",
      "title": "VisibilityFlag",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-QuietFlags",
      "title": "QuietFlags",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StealthRefEvent",
      "title": "StealthRefEvent",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-CommandCorpusEntryV1",
      "title": "CommandCorpusEntryV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-CommandCorpusBlockedRecordV1",
      "title": "CommandCorpusBlockedRecordV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-CommandCorpusParityReportV1",
      "title": "CommandCorpusParityReportV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-GarmentSpec",
      "title": "GarmentSpec",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-ClothSolver",
      "title": "ClothSolver",
      "kind": "rust_trait"
    },
    {
      "primitive_id": "PRIM-SimulationReceipt",
      "title": "SimulationReceipt",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-TailorValidationDescriptor",
      "title": "TailorValidationDescriptor",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-ClothBodyProxy",
      "title": "ClothBodyProxy",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-TailorModelAdapter",
      "title": "TailorModelAdapter",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-GarmentAnimationDraft",
      "title": "GarmentAnimationDraftV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-TailorExportPreset",
      "title": "ExportPreset (Tailor)",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-TailorCapabilityRegistry",
      "title": "TailorCapabilityRegistryV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-TailorOperationContract",
      "title": "TailorOperationContractV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-TailorSurfaceDescriptor",
      "title": "TailorSurfaceDescriptorV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-TailorMixGraph",
      "title": "TailorMixGraphV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-TailorProject",
      "title": "TailorProjectV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-TailorDependencyBundle",
      "title": "TailorDependencyBundleV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-BodyProviderPackage",
      "title": "BodyProviderPackageV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-BodyEvaluator",
      "title": "BodyEvaluatorV1",
      "kind": "rust_trait"
    },
    {
      "primitive_id": "PRIM-BodyHeadPackage",
      "title": "BodyHeadPackageV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-HeadBodyAssembly",
      "title": "HeadBodyAssemblyV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-BodyResidualLayer",
      "title": "BodyResidualLayerV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-BodyPerformanceTrack",
      "title": "BodyPerformanceTrackV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-BodyConditioningBundle",
      "title": "BodyConditioningBundleV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-TailorInterchangeProfile",
      "title": "TailorInterchangeProfileV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-TailorActiveToolState",
      "title": "TailorActiveToolStateV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-BodyCinematicQualificationProfile",
      "title": "BodyCinematicQualificationProfileV1",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-BodySpec",
      "title": "BodySpec",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-BodyChannel",
      "title": "BodyChannel (decoupled region channel)",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-BodyRecipe",
      "title": "BodyRecipe",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-BodyCorrectiveEngine",
      "title": "RBF pose-space corrective engine + sim-to-corrective baking",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-BodySoftTissue",
      "title": "BodySoftTissue (unified-solver region cages)",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-BodyGenitalModules",
      "title": "Genital modules (vulva/penis/anus, arousal controller)",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-BodyExportProfile",
      "title": "BodyExportProfile (BXP-)",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-BodyValidationReceipt",
      "title": "BodyValidationReceipt",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioDocument",
      "title": "StudioDocument (unified creative document)",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioArtboard",
      "title": "StudioArtboard",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioPageSpread",
      "title": "StudioPageSpread",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioLayer",
      "title": "StudioLayer (kind-tagged layer node)",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioLayerGraph",
      "title": "StudioLayerGraph",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioSelectionSet",
      "title": "StudioSelectionSet (one selection model)",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioMask",
      "title": "StudioMask",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioRasterTile",
      "title": "StudioRasterTile",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioVectorPath",
      "title": "StudioVectorPath",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioVectorNetwork",
      "title": "StudioVectorNetwork",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioTextStory",
      "title": "StudioTextStory",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioTypeStyle",
      "title": "StudioTypeStyle",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioColorProfile",
      "title": "StudioColorProfile",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioSwatch",
      "title": "StudioSwatch",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioGradient",
      "title": "StudioGradient",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioPattern",
      "title": "StudioPattern",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioEffectStack",
      "title": "StudioEffectStack",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioAdjustment",
      "title": "StudioAdjustment",
      "kind": "rust_enum"
    },
    {
      "primitive_id": "PRIM-StudioLiveFilter",
      "title": "StudioLiveFilter",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioBlendMode",
      "title": "StudioBlendMode",
      "kind": "rust_enum"
    },
    {
      "primitive_id": "PRIM-StudioComponent",
      "title": "StudioComponent",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioComponentInstance",
      "title": "StudioComponentInstance",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioVariable",
      "title": "StudioVariable",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioVariableCollection",
      "title": "StudioVariableCollection",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioStyleRegistry",
      "title": "StudioStyleRegistry",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioLayoutGrid",
      "title": "StudioLayoutGrid",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioConstraint",
      "title": "StudioConstraint",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioAutoLayout",
      "title": "StudioAutoLayout",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioPrototypeFlow",
      "title": "StudioPrototypeFlow",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioMotionTimeline",
      "title": "StudioMotionTimeline",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioRawDevelop",
      "title": "StudioRawDevelop",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioExportRecipe",
      "title": "StudioExportRecipe",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioImportProfile",
      "title": "StudioImportProfile",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioHistoryEntry",
      "title": "StudioHistoryEntry",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioEditProposal",
      "title": "StudioEditProposal",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioModelAdapter",
      "title": "StudioModelAdapter",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioValidationDescriptor",
      "title": "StudioValidationDescriptor",
      "kind": "rust_struct"
    },
    {
      "primitive_id": "PRIM-StudioRenderHarness",
      "title": "StudioRenderHarness",
      "kind": "rust_struct"
    }
  ],
  "tools": [
  ...
  "technologies": [
    ...
    {
      "technology_id": "TECH-SURREALDB",
      "title": "SurrealDB",
      "notes": "[REFINE v02.204] Exclusive Handshake-managed runtime database authority."
    },
    {
    ...
    {
      "technology_id": "TECH-SURREALDB-RUST-SDK",
      "title": "SurrealDB Rust SDK",
      "notes": "[REFINE v02.204] Official typed Rust access, authenticated record-user sessions, query bindings, and live queries."
    },
    {
      "technology_id": "TECH-SURREALKIT",
      "title": "SurrealKit",
      "notes": "[REFINE v02.204] Ordered start/app-cutover/complete/rollback schema rollouts."
    },
    {
    ...
    {
      "technology_id": "TECH-WGPU",
      "title": "wgpu",
      "notes": "[ADD v02.198] GPU compute/render abstraction for the tailor-solver and native viewport."
    },
    {
      "technology_id": "TECH-WGSL",
      "title": "WGSL",
      "notes": "[ADD v02.198] Compute shader language for XPBD solver passes."
    }
  ],
  "feature_links": [
    {
    ...
    },
    {
      ...
      "coverage_refs": [
        ...
        ".GOV/spec/master-spec-v02.193/spec-modules/06-mechanical-integrations.md Section 6.11"
      ],
      "gap_stub_ids": [
    ...
    },
    {
      ...
      "coverage_refs": [
        ...
        "src/backend/handshake_core/src/api/jobs.rs",
        "src/backend/handshake_core/src/api/workspaces.rs",
        "Section 2.3.13.10 [ADD v02.185]",
        "Section 10.11.5.28 [ADD v02.185]"
      ],
      "gap_stub_ids": [
    ...
    },
    {
      ...
      "coverage_refs": [
        ...
        ".GOV/spec/master-spec-v02.193/spec-modules/06-mechanical-integrations.md Section 6.10"
      ],
      "gap_stub_ids": [
        "WP-1-Media-Downloader-Loom-Bridge-v1",
        "WP-1-Video-Archive-Loom-Integration-v1",
    ...
    },
    {
      ...
      "coverage_refs": [
        ...
        ".GOV/spec/master-spec-v02.193/spec-modules/06-mechanical-integrations.md Section 6.9"
      ],
      "gap_stub_ids": [
    ...
    },
    {
      ...
      "coverage_status": "PARTIAL",
      "coverage_refs": [
        "master-spec-v02.197/spec-modules/11-shared-dev-platform-and-oss-foundations.md 10.13 Handshake Stage (Built-in Browser + Stage Apps)",
        "master-spec-v02.197/spec-modules/12-end-of-file-appendices.md Appendix 12.3 FEAT-STAGE",
        "app/src-tauri/src/lib.rs",
        "src/backend/handshake_core/src/workflows.rs",
        "app/src/components/MediaDownloaderView.tsx",
        "app/src/lib/mediaDownloader.ts"
    ...
    },
    {
      ...
      "coverage_refs": [
        ...
        "src/backend/handshake_core/src/storage/surreal.rs",
        "src/backend/handshake_core/src/storage/surreal_runtime.rs",
        "src/backend/handshake_core/src/bundles/schemas.rs",
        "src/backend/handshake_core/src/bundles/validator.rs",
        "src/backend/handshake_core/src/ai_ready_data/embedding.rs",
        "src/backend/handshake_core/src/ai_ready_data/indexing.rs",
        ...
        "src/backend/handshake_core/src/workflows.rs",
        "src/backend/handshake_core/src/mcp/gate.rs",
        ".GOV/task_packets/WP-KERNEL-017-Agent-Steering-Roles-Worktree-Merge-Notes-Annotations-IDE-Parity-v1/MT-331.json",
        ".GOV/task_packets/WP-KERNEL-017-Agent-Steering-Roles-Worktree-Merge-Notes-Annotations-IDE-Parity-v1/MT-346.json"
      ],
      "gap_stub_ids": [
        "WP-1-Stage-Media-Artifact-Portability-v1",
        "WP-1-AIReady-Index-Evidence-Export-v1",
        ...
        "WP-1-MCP-MEX-Evidence-Export-v1",
        "WP-1-Cloud-Consent-Evidence-Portability-v1",
        "WP-KERNEL-017-Agent-Steering-Roles-Worktree-Merge-Notes-Annotations-IDE-Parity-v1"
      ]
    },
    {
      "feature_id": "FEAT-STUDIO",
      ...
      "coverage_refs": [
        ...
        "master-spec-v02.197/spec-modules/11-shared-dev-platform-and-oss-foundations.md 10.4 Stage-to-Studio visual handoff direction (Phase 3/4 MAY)",
        "app/src/components/CanvasView.tsx",
        "app/src/components/AtelierCollaborationPanel.tsx",
    ...
    },
    {
      ...
      "coverage_status": "PARTIAL",
      "coverage_refs": [
        "Section 2.3.13.10 Kernel V1 CRDT Workspace, Write Box, and Promotion Bridge [ADD v02.185]",
        "Section 10.11.5.28 Kernel Action Catalog and Write Box Projections [ADD v02.185]"
      ],
      "gap_stub_ids": [
        "WP-KERNEL-002-CRDT-Workspace-Write-Box-Preuse-Hardening-v1"
      ]
    },
    {
      ...
      "coverage_status": "GAP",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.193/spec-modules/06-mechanical-integrations.md Section 6.12"
      ],
      "gap_stub_ids": [
        "WP-KERNEL-005"
      ]
    },
    {
      ...
      "coverage_status": "GAP",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.193/spec-modules/10-product-surfaces.md Section 10.16"
      ],
      "gap_stub_ids": [
        "WP-KERNEL-005"
      ]
    },
    {
      ...
      "coverage_status": "GAP",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.193/spec-modules/10-product-surfaces.md Section 10.17"
      ],
      "gap_stub_ids": [
        "WP-KERNEL-005"
      ]
    },
    {
      ...
      "coverage_status": "GAP",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.193/spec-modules/10-product-surfaces.md Section 10.18"
      ],
      "gap_stub_ids": [
        "WP-KERNEL-005"
      ]
    },
    {
      ...
      "coverage_status": "GAP",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.193/spec-modules/10-product-surfaces.md Section 10.19"
      ],
      "gap_stub_ids": [
        "WP-KERNEL-005"
      ]
    },
    {
      ...
      "coverage_status": "SEEDED",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.203/spec-modules/13-tailor-cloth-garment-engine.md (Cloth plus BodyKit-v2 native-runtime hardening)",
        ".GOV/task_packets/WP-KERNEL-010-Tailor-Cloth-Garment-Engine-v1/_MT_INDEX.json (MT-001..842; ownership is contract-defined)",
        ".GOV/task_packets/WP-KERNEL-010-Tailor-Cloth-Garment-Engine-v1/WP-KERNEL-010-Tailor-Cloth-Garment-Engine-v1.build-readiness-prework.json"
      ],
      "gap_stub_ids": []
    },
    {
      ...
      "coverage_status": "SEEDED",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.203/spec-modules/13-tailor-cloth-garment-engine.md (BodyKit plus v2 native-runtime, provider, cinematic and interchange authority)",
        ".GOV/task_packets/WP-KERNEL-010-Tailor-Cloth-Garment-Engine-v1/_MT_INDEX.json (MT-478..842; ownership is contract-defined)",
        ".GOV/task_packets/WP-KERNEL-010-Tailor-Cloth-Garment-Engine-v1/WP-KERNEL-010-Tailor-Cloth-Garment-Engine-v1.build-readiness-prework.json",
        ".GOV/reference/tailor_bodykit_research/index.yaml"
      ],
      "gap_stub_ids": []
    },
    {
      ...
      "coverage_status": "SEEDED",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.199/spec-modules/14-studio-creative-suite.md (14.4)"
      ],
      "gap_stub_ids": []
    },
    {
      ...
      "coverage_status": "SEEDED",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.199/spec-modules/14-studio-creative-suite.md (14.5)"
      ],
      "gap_stub_ids": []
    },
    {
      ...
      "coverage_status": "SEEDED",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.199/spec-modules/14-studio-creative-suite.md (14.6)"
      ],
      "gap_stub_ids": []
    },
    {
      ...
      "coverage_status": "SEEDED",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.199/spec-modules/14-studio-creative-suite.md (14.7)"
      ],
      "gap_stub_ids": []
    },
    {
      ...
      "coverage_status": "SEEDED",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.199/spec-modules/14-studio-creative-suite.md (14.8)"
      ],
      "gap_stub_ids": []
    },
    {
      ...
      "coverage_status": "SEEDED",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.199/spec-modules/14-studio-creative-suite.md (14.9)"
      ],
      "gap_stub_ids": []
    },
    {
      ...
      "coverage_status": "SEEDED",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.199/spec-modules/14-studio-creative-suite.md (14.10)"
      ],
      "gap_stub_ids": []
    },
    {
      ...
      "coverage_status": "SEEDED",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.199/spec-modules/14-studio-creative-suite.md (14.11)"
      ],
      "gap_stub_ids": []
    },
    {
      ...
      "coverage_status": "SEEDED",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.199/spec-modules/14-studio-creative-suite.md (14.12)"
      ],
      "gap_stub_ids": []
    },
    {
      ...
      "coverage_status": "SEEDED",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.199/spec-modules/14-studio-creative-suite.md (14.13)"
      ],
      "gap_stub_ids": []
    },
    {
      ...
      "coverage_status": "SEEDED",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.199/spec-modules/14-studio-creative-suite.md (14.14)"
      ],
      "gap_stub_ids": []
    },
    {
      ...
      "coverage_status": "SEEDED",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.199/spec-modules/14-studio-creative-suite.md (14.15)"
      ],
      "gap_stub_ids": []
    }
  ],
...
```json
{
  ...
  "ui_guidance": [
    ...
    {
      "feature_id": "FEAT-STAGE",
      "user_goal": "Use a governed browser/webviewer, trusted HTML/Stage Apps host, model-operated web/front-end test surface, and capture/import lane that emits governed artifacts via jobs (no UI bypass).",
      "entry_points": [
        "Stage surface",
    ...
    {
      "feature_id": "FEAT-STUDIO",
      "user_goal": "Operate the Studio Module visual creation shell with explicit runtime visibility across Figma-like collaboration/canvas, Lens, Photo, painting/compositing, Affinity/Photoshop-class tools, and future 3D visual authoring.",
      "entry_points": [
        "Studio shell",
    ...
    {
      "feature_id": "FEAT-KERNEL-WORKSPACE-WRITE-BOX",
      "user_goal": "Inspect and safely promote Kernel V1 draft workspace edits without confusing CRDT draft state, denied direct edits, and EventLedger authority.",
      "entry_points": [
        "Dev Command Center > Kernel write boxes",
        "Dev Command Center > action catalog",
        "Kernel promotion preview"
      ],
      "required_surfaces": [
        "ui",
        "backend"
      ...
      "interaction_contract": {
        ...
        "errors": [
          "action_not_cataloged",
          "actor_ineligible",
          "stale_state_vector",
          "duplicate_promotion",
          "schema_invalid",
          "eventledger_write_failed"
        ]
      },
      "capability_gates": [
        "Write-capable actions must expose action id, actor eligibility, target authority class, and approval/capability posture before execution."
      ],
      "telemetry": [
        "Write-box creation, denial, validation, promotion request, promotion result, and projection rebuild events remain linked to EventLedger or durable pre-promotion receipts."
      ],
      "tests": [
        "DCC rows expose stable ids for action catalog entries, write boxes, denial receipts, promotion previews, and stale projection badges.",
        "A denied direct edit shows recovery instructions and can be normalized into an advisory write box without mutating authority."
      ]
    },
    {
      "feature_id": "FEAT-SOURCING-SPEC-MATRIX",
      "user_goal": "Sourcing-Spec Schema and Handler Version Matrix available as governed product surface with full provenance.",
      "entry_points": [
        "Operator Consoles",
        "Atelier/Lens surfaces"
      ],
      "required_surfaces": [
        "backend",
        "operator_consoles"
      ...
      "capability_gates": [
        "Mutating actions require capability profile + consent state per FEAT-CAPABILITIES-CONSENT."
      ],
      "telemetry": [
        "EventLedger + Flight Recorder evidence for every mutation/job."
      ]
    },
    {
      "feature_id": "FEAT-MEDIA-ANNOTATION",
      "user_goal": "Media Annotation Overlays available as governed product surface with full provenance.",
      "entry_points": [
        "Operator Consoles",
        "Atelier/Lens surfaces"
      ],
    ...
    {
      "feature_id": "FEAT-SETTINGS-PREFERENCES",
      "user_goal": "Settings and Preferences Domain available as governed product surface with full provenance.",
      "entry_points": [
        "Operator Consoles",
        "Atelier/Lens surfaces"
      ],
    ...
    {
      "feature_id": "FEAT-STEALTH-REFERENCE-WINDOW",
      "user_goal": "Stealth Reference Window available as governed product surface with full provenance.",
      "entry_points": [
        "Operator Consoles",
        "Atelier/Lens surfaces"
      ],
    ...
    {
      "feature_id": "FEAT-COMMAND-CORPUS-PARITY",
      "user_goal": "Command Corpus and Action Catalog Parity available as governed product surface with full provenance.",
      "entry_points": [
        "Operator Consoles",
        "Atelier/Lens surfaces"
      ],
    ...
    {
      "feature_id": "FEAT-TAILOR-CLOTH",
      "user_goal": "Create or recover a Tailor project, author and sew garments, calibrate fabric, simulate and repair drape, validate, render, and publish through one task-oriented native workspace whose GUI and model commands share the same typed authority.",
      "entry_points": [
        "Native module switcher > Tailor > Projects",
        "Tailor task switcher > Cloth, Sewing/Arrangement, Fabric Lab, Simulation, Retopology, Materials, Render, or Publish",
        "Garment/fabric/trim/style asset browser with compatibility and provenance filters",
        "Command palette, MCP, or backend action by canonical stable Tailor action_id",
        "Deep links by tailor_project_id, garment_id, run_id, finding_id, or artifact_id from Locus, Loom, Calendar, Flight Recorder, and Diagnostics",
        "Context Help > Tailor UserManual at the current surface/control/error anchor"
      ],
      "required_surfaces": [
        "ui",
        "backend",
      ...
      "interaction_contract": {
        ...
        "errors": [
          ...
          "object_lease_conflict",
          "validation_blocking_finding",
          "sim_run_rejected",
          "device_lost_recovery_required",
          "project_recovery_checkpoint_required",
          "manual_anchor_missing",
          "export_lane_unavailable_fbx_bridge_missing"
        ]
      },
      "capability_gates": [
        "Every native control, shortcut, command-palette action, MCP call, replay action, and semantic undo resolves through the same versioned Tailor command and operation contract; projection-local mutation is prohibited.",
        "Required vendor-parity rows must be QUALIFIED before full-parity wording or dependent publish actions are available; missing, stale, uninspected, or unsupported-required rows fail closed.",
        "Authoritative changes require expected revisions and object-scoped leases; previews are non-authoritative until apply succeeds and produces an attributable receipt.",
        "promote_garment requires consent (13.13); interactive-session receipts are exploratory and rejected by the PromotionGate."
      ],
      "telemetry": [
        "Every authoritative mutation and job emits one EventLedger identity projected unchanged into Flight Recorder with event_id, run_id, actor_id, correlation_id, action_id, object ids, revisions, and artifact refs.",
        "The native status strip and Diagnostics inspector expose job progress, solver residual/contact/VRAM state, warnings, failures, recovery actions, and Palmistry watcher state without requiring console scraping.",
        "Locus receives optional QA/repair/production projections; Loom receives stable asset and evidence refs/backlinks; Calendar receives ActivitySpan/SessionSpan projections and optional production-block association. None becomes Tailor authority.",
        "Parallel-agent occupancy shows actor, leased object scope, lease expiry, base revision, current command, conflict state, and recover/resume action.",
        "Captures are registered in tailor_captures and publish/export manifests are hashed and diffable."
      ],
      "tests": [
        "Argus/AccessKit enumerates every TailorSurfaceDescriptor route, pane, control, dynamic entity key, command binding, help anchor, permission, undo policy, and receipt link without foreground focus theft.",
        "Native visual capture matrix covers clean, empty, loading, dirty, preview, leased, stale, blocked, device-lost, recoverable-failure, validation, and publish states at supported window sizes; text does not overlap and critical state remains visible.",
        "The same seeded garment workflow produces equivalent commands, receipts, revisions, artifacts, and diagnostics through native GUI, command palette, MCP/headless operation, replay, and manual-only no-context operation.",
        "Two agents editing disjoint objects succeed concurrently; same-object contention, stale revisions, cancellation, crash recovery, and lease expiry preserve authoritative state and attributable receipts.",
        "No Tailor product action depends on a WebView-local store, hidden session context, fragile screen coordinates, or an operator-only route."
      ]
    },
    {
      "feature_id": "FEAT-TAILOR-BODYKIT",
      "user_goal": "Create, shape, assemble, rig, perform, light, render, condition, validate, interchange, and publish high-resolution lifelike production humans, including movie-ready waist-up shots, through one Rust-native offline GUI/model command authority usable across parallel projects.",
      "entry_points": [
        "Native module switcher > Tailor > Projects > BodyKit",
        "BodyKit task switcher > Shape, Measurements/Scan Fit, Geometry/Morph, Anatomy, Rig/Figure Setup, Weights/Transfer, Pose/Contact, Face/Performance, Skin/Materials, Groom, Animation, Conditioning, Render, Interchange, or Publish",
        "Provider/package, archetype, recipe, source/assembly, and smart-content browsers with versions, hashes, compatibility, dependencies, privacy, qualification, and provenance",
        "Command palette, MCP, or backend action by canonical stable Tailor action_id",
        "Deep links by tailor_project_id, body_id, recipe_id, pose_id, mix_graph_id, contact_run_id, finding_id, or artifact_id from Locus, Loom, Calendar, Flight Recorder, and Diagnostics",
        "Context Help > Tailor UserManual at the current surface/control/error anchor"
      ],
      "required_surfaces": [
        "ui",
      ...
      "interaction_contract": {
        "states": [
          ...
          "adapter_removed",
          "interchange_loss_review",
          "manual_qualification_pending",
          "validated",
          "publish_ready",
          "rejected",
          "archived"
        ...
        "errors": [
          ...
          "object_lease_conflict",
          "decouple_violation",
          "license_guard_rejected_payload",
          "python_dependency_prohibited",
          "network_access_prohibited",
          "dependency_digest_missing",
          "provider_package_incompatible",
          "residual_invalidated",
          "corrective_regeneration_required",
          "unsupported_interchange_direction",
          "pose_range_corrective_missing",
          "contact_feasibility_failed",
          "posekit_dependency_not_ready",
          "project_recovery_checkpoint_required",
          "manual_anchor_missing",
      ...
      "capability_gates": [
        "Every native control, shortcut, command-palette action, MCP call, replay action, and semantic undo resolves through the same versioned Tailor command and operation contract; projection-local mutation is prohibited.",
        "Required vendor-parity rows must be QUALIFIED before full-parity wording or dependent publish actions are available; missing, stale, uninspected, or unsupported-required rows fail closed.",
        "Model-authored bodies pass sandbox->validate->promote (13.25); coupling links only via the inspectable coupling-graph panel.",
        "PoseKit/character-sheet mixing is disabled until the source commit is contained in merged main and the pinned WP-KERNEL-012 native-shell compatibility receipt passes; imported PoseKit/OpenPose data remains immutable evidence or a proposal and never silently becomes 3D pose, camera, contact, anatomy, cloth, or body authority.",
        "Authoritative changes require expected revisions and object-scoped leases; previews and contact-stage sweeps are non-authoritative until apply succeeds and produces an attributable receipt."
        ,"GNM v3 is the default local provider; Rust-owned core and UI, the no-Python/no-cloud firewall, immutable dependency bundles, runtime precedence, adapter removal, and resource privacy fail closed before dependent actions run."
        ,"GLB and the qualified restricted USD profile are canonical interchange; Blender qualification is manual, Unreal automation is a native C++ commandlet, and FBX/Alembic directions remain visible unsupported capabilities until separately qualified."
      ],
      "telemetry": [
        "Every authoritative mutation and job emits one EventLedger identity projected unchanged into Flight Recorder with event_id, run_id, actor_id, correlation_id, action_id, object ids, revisions, and artifact refs.",
        "The native status strip and Diagnostics inspector expose dependency/provider/package, corrective/residual, soft-tissue, collision/contact, capture/performance, renderer, conditioning, interchange, recovery, privacy, and Palmistry watcher state without requiring console scraping.",
        "Locus receives optional QA/repair/production-work projections; Loom receives stable body, head, provider, recipe, material, performance, conditioning, qualification, capture, and evidence refs/backlinks; Calendar receives ActivitySpan/SessionSpan projections and optional production-block association. None becomes BodyKit authority.",
        "Parallel-agent occupancy shows actor, leased object scope, lease expiry, base revision, current command, conflict state, and recover/resume action.",
        "TAILOR_BODY_* events cover dependency, provider/package, head, source/assembly, residual/corrective, channel, recipe, anatomy, contact, performance/capture, mix, material, groom, animation, conditioning, render, interchange, privacy, recovery, and export operations; QA matrices persist as validation records and captures."
      ],
      "tests": [
        "Argus/AccessKit enumerates every TailorSurfaceDescriptor route, pane, control, dynamic entity key, command binding, help anchor, permission, undo policy, and receipt link without foreground focus theft.",
        "Native visual capture matrix covers clean, empty, loading, dirty, preview, leased, stale, blocked, device-lost, recoverable-failure, anatomy/contact diagnostics, validation, and publish states at supported window sizes; text does not overlap and critical state remains visible.",
        "The same seeded BodyKit workflow produces equivalent commands, receipts, revisions, artifacts, and diagnostics through native GUI, command palette, MCP/headless operation, replay, and manual-only no-context operation.",
        "Two agents editing disjoint regions or artifacts succeed concurrently; same-object contention, stale revisions, cancellation, crash recovery, and lease expiry preserve authoritative state and attributable receipts.",
        "PoseKit-unavailable and compatibility-receipt-failed tests keep mix actions disabled and explain the exact dependency; immutable 2D evidence never mutates 3D authority.",
        "Archetype, decouple, anatomy/contact, deformation, expression, material, render, and export qualification matrices are visually inspected against pinned references before PASS.",
        "An air-gapped clean-state test proves no Python process/library/file, no cloud/runtime download, frozen dependency resolution, hash/SBOM/license inventory, missing-package recovery, and core usability with every optional adapter removed.",
        "A no-context two-project proof creates a GNM-v3 assembled character, performs high-resolution face/rig/material/groom/animation work, fits Cloth, renders and directly inspects a lifelike waist-up still and sequence plus EXR conditioning channels, reloads GLB/restricted USD, runs Unreal native and manual Blender qualification, injects/recover failures, and replays authoritative state."
      ]
    }
  ]
}
...
```json
{
  ...
  "edges": [
    ...
    },
    {
      ...
      "notes": "[ADD v02.155][REFINE v02.204] Calendar source sync state, write policy, export mode, and EventLedger mutation history are canonical SurrealDB contracts initialized through fresh authority activation and preserving stable meaning across SurrealKit rollouts and later export tooling."
    },
    {
      "edge_id": "IMX-079",
      "from_kind": "feature",
    ...
    },
    {
      ...
      "to_kind": "feature",
      "to_id": "FEAT-WORKFLOW-ENGINE",
      "kind": "surreal_primary_storage_mode_bounds_workflow_runtime_authority",
      "scope": "backend_runtime",
      "tokens": [
        "[ADD v02.182]",
        "surreal_primary",
      ...
      "notes": "[ADD v02.182][REFINE v02.204] SurrealDB-primary storage mode is the exclusive authority boundary for workflow-backed control-plane records; when Handshake-managed SurrealDB is unavailable, workflow/control-plane writes fail closed without PostgreSQL or SQLite fallback."
    },
    {
      "edge_id": "IMX-134",
      "from_kind": "feature",
      ...
      "to_kind": "feature",
      "to_id": "FEAT-MODEL-SESSION-ORCHESTRATION",
      "kind": "surreal_primary_storage_mode_bounds_model_session_queue_authority",
      "scope": "backend_runtime",
      "tokens": [
        "[ADD v02.182]",
        "surreal_primary",
      ...
      "notes": "[ADD v02.182][REFINE v02.204] ModelSession scheduling, queue state, persisted messages, checkpoints, cancellation, provider-profile persistence, and EventLedger entries resolve through SurrealDB authority."
    },
    {
      "edge_id": "IMX-135",
      "from_kind": "feature",
      ...
      "to_kind": "feature",
      "to_id": "FEAT-DEV-COMMAND-CENTER",
      "kind": "surreal_primary_authority_projects_into_dcc_with_source_freshness",
      "scope": "backend_runtime",
      "tokens": [
        "[ADD v02.182]",
        "SurrealDB",
      ...
      "notes": "[ADD v02.182][REFINE v02.204] Dev Command Center projections over sessions, queues, leases, workflows, memory jobs, and dead-letter state must expose SurrealDB authority plus source/freshness metadata instead of treating legacy relational stores or UI state as authority."
    },
    {
      "edge_id": "IMX-136",
      "from_kind": "feature",
      "from_id": "FEAT-KERNEL-WORKSPACE-WRITE-BOX",
      "to_kind": "feature",
      "to_id": "FEAT-WORKFLOW-ENGINE",
      "kind": "promotes_validated_write_boxes",
      "scope": "normative",
      "tokens": [
        "WriteBoxV1",
        "EventLedger",
      ...
      "notes": "[ADD v02.185] CRDT draft state can request promotion only through validated write boxes and workflow/event-ledger backed promotion receipts."
    },
    {
      "edge_id": "IMX-137",
      "from_kind": "feature",
      "from_id": "FEAT-DEV-COMMAND-CENTER",
      "to_kind": "feature",
      "to_id": "FEAT-KERNEL-WORKSPACE-WRITE-BOX",
      "kind": "projects_write_box_state",
      "scope": "normative",
      "tokens": [
      ...
      "notes": "[ADD v02.185] DCC projects catalog actions, write boxes, denials, promotion previews, and stale badges without becoming authority."
    },
    {
      "edge_id": "IMX-138",
      "from_kind": "primitive",
      "from_id": "PRIM-ModelRuntime",
      "to_kind": "primitive",
      "to_id": "PRIM-SandboxAdapter",
      "kind": "boxes_model_process",
      "scope": "normative",
      "tokens": [
        "every model process boxed"
      ],
      "spec_refs": [
        "Section 4.6",
        "Section 3.5"
      ],
      "notes": "[ADD v02.186] Every model process spawned via ModelRuntime is boxed by a SandboxAdapter implementation."
    },
    {
      "edge_id": "IMX-139",
      "from_kind": "primitive",
      "from_id": "PRIM-ModelRuntime",
      "to_kind": "primitive",
      "to_id": "PRIM-ProcessOwnershipLedger",
      "kind": "writes_lifecycle_rows",
      "scope": "normative",
      "tokens": [
        "start row",
        "stop row"
      ...
      "notes": "[ADD v02.186] ModelRuntime writes a start and stop row to ProcessOwnershipLedger for every spawned model process."
    },
    {
      "edge_id": "IMX-140",
      "from_kind": "primitive",
      "from_id": "PRIM-LlamaCppRuntime",
      "to_kind": "primitive",
      "to_id": "PRIM-ModelRuntime",
      ...
      "notes": "[ADD v02.186] LlamaCppRuntime is the default LocalModelAdapter implementing the ModelRuntime trait."
    },
    {
      "edge_id": "IMX-141",
      "from_kind": "primitive",
      "from_id": "PRIM-CandleRuntime",
      "to_kind": "primitive",
      "to_id": "PRIM-ModelRuntime",
      "kind": "implements_trait",
      "scope": "normative",
      "tokens": [
        "ModelRuntime impl",
        "hook-requiring techniques"
      ],
      "spec_refs": [
        "Section 4.6.2"
      ],
      "notes": "[ADD v02.186] CandleRuntime implements ModelRuntime for hook-requiring inference techniques and subquadratic architectures."
    },
    {
      "edge_id": "IMX-142",
      "from_kind": "primitive",
      "from_id": "PRIM-InferenceLabUI",
      "to_kind": "primitive",
      "to_id": "PRIM-ExecPolicyExtended",
      "kind": "edits_policy_to_runtime",
      "scope": "normative",
      "tokens": [
        "exec_policy",
        "Work Profile knob"
      ...
      "notes": "[ADD v02.186] InferenceLabUI mutates ExecPolicyExtended which then flows to ModelRuntime as the active per-job runtime policy."
    },
    {
      "edge_id": "IMX-143",
      "from_kind": "primitive",
      "from_id": "PRIM-MemoryCapsule",
      "to_kind": "primitive",
      "to_id": "PRIM-SelfImprovementLoop",
      "kind": "feeds_self_improvement",
      "scope": "normative",
      "tokens": [
        "Memory V0+",
      ...
      "notes": "[ADD v02.186] MemoryCapsule feeds SelfImprovementLoop, which routes through DistillationCandidateReview into Skill Bank promotion (extends existing Skill Bank edge)."
    },
    {
      "edge_id": "IMX-144",
      "from_kind": "primitive",
      "from_id": "PRIM-DiagnosticsPanel",
      "to_kind": "feature",
      "to_id": "FEAT-DEV-COMMAND-CENTER",
      "kind": "tauri_ipc_visual_debug",
      "scope": "normative",
      "tokens": [
        "kernel.visual_debug.*",
        "WebView2 CDP"
      ...
      "notes": "[ADD v02.186] DiagnosticsPanel calls kernel.visual_debug.* Tauri IPC which drives Playwright/CDP capture against WebView2."
    },
    {
      "edge_id": "IMX-145",
      "from_kind": "primitive",
      "from_id": "PRIM-SwarmTestHarness",
      "to_kind": "primitive",
      "to_id": "PRIM-KernelActionCatalogV1",
      "kind": "mutates_via_catalog_reads_inspector",
      "scope": "normative",
      "tokens": [
        "KernelActionCatalogV1 mutation",
        "Inspector Plane read"
      ],
      "spec_refs": [
        "Section 6.7",
        "Section 6.5",
      ...
      "notes": "[ADD v02.186] SwarmTestHarness performs mutations through KernelActionCatalogV1 envelopes and observes results through the read-only Backend Inspector Plane."
    },
    {
      "edge_id": "IMX-146",
      "from_kind": "primitive",
      "from_id": "PRIM-HBRGate",
      "to_kind": "feature",
      "to_id": "FEAT-WORKFLOW-ENGINE",
      "kind": "gates_handoff_via_receipt",
      "scope": "normative",
      "tokens": [
        "WP_RECEIPT",
        "handoff envelope",
      ...
      "notes": "[ADD v02.186] HBRGate emits its verdict into the WP_RECEIPT / handoff envelope stream, blocking handoff on violation per CX-131."
    },
    {
      "edge_id": "IMX-147",
      "from_kind": "primitive",
      "from_id": "PRIM-HBRViolationReceipt",
      "to_kind": "feature",
      "to_id": "FEAT-FLIGHT-RECORDER",
      "kind": "typed_receipt_family",
      "scope": "normative",
      "tokens": [
        "CX-130 typed-receipt family",
        "HBR_VIOLATION"
      ],
      "spec_refs": [
        "Section 5.6.3",
      ...
      "notes": "[ADD v02.186] HBRViolationReceipt is a typed-receipt family member governed by CX-130 inter-role wire discipline."
    },
    {
      "edge_id": "IMX-148",
      "from_kind": "primitive",
      "from_id": "PRIM-ModelManual",
      "to_kind": "feature",
      "to_id": "FEAT-DEV-COMMAND-CENTER",
      "kind": "on_demand_md_projection",
      "scope": "normative",
      "tokens": [
      ...
      "notes": "[ADD v02.186] ModelManual exposes Tauri IPC kernel.model_manual.get for on-demand .md projection so no-context models can self-bootstrap."
    },
    {
      "edge_id": "IMX-149",
      "from_kind": "primitive",
      "from_id": "PRIM-SandboxAdapter",
      "to_kind": "primitive",
      "to_id": "PRIM-HandshakeNativeSandboxAdapter",
      "kind": "trait_default_and_compat_impls",
      "scope": "normative",
      "tokens": [
        "HandshakeNativeSandboxAdapter default",
        "WSL2PodmanAdapter compat",
        "WindowsNativeJailAdapter strict",
        "DockerAdapter compat"
      ],
      "spec_refs": [
        "Section 3.5",
        "Section 3.5.2"
      ],
      "notes": "[UPDATED v02.190] SandboxAdapter default is HandshakeNativeSandboxAdapter. WSL2PodmanAdapter and DockerAdapter are explicit compatibility adapters only; WindowsNativeJailAdapter remains the strict Win32-native path."
    },
    {
      "edge_id": "IMX-K005-01",
      "from_kind": "feature",
      "from_id": "FEAT-PHOTO-STUDIO",
      "to_kind": "feature",
      "to_id": "FEAT-CAPABILITIES-CONSENT",
      "kind": "enforce_capabilities",
      "scope": "normative",
      ...
      "notes": "WP-KERNEL-005 CKC fold-in: 6.9 executes only under capability-gated governed jobs."
    },
    {
      "edge_id": "IMX-K005-02",
      "from_kind": "feature",
      ...
      "notes": "WP-KERNEL-005 CKC fold-in: 6.10 executes only under capability-gated governed jobs."
    },
    {
      "edge_id": "IMX-K005-03",
      "from_kind": "feature",
      ...
      "notes": "WP-KERNEL-005 CKC fold-in: 6.11 executes only under capability-gated governed jobs."
    },
    {
      "edge_id": "IMX-K005-04",
      "from_kind": "feature",
      "from_id": "FEAT-SOURCING-SPEC-MATRIX",
      "to_kind": "feature",
      "to_id": "FEAT-CAPABILITIES-CONSENT",
      "kind": "enforce_capabilities",
      "scope": "normative",
      ...
      "notes": "WP-KERNEL-005 CKC fold-in: 6.12 executes only under capability-gated governed jobs."
    },
    {
      "edge_id": "IMX-K005-05",
      "from_kind": "feature",
      "from_id": "FEAT-MEDIA-ANNOTATION",
      "to_kind": "feature",
      "to_id": "FEAT-CAPABILITIES-CONSENT",
      "kind": "enforce_capabilities",
      "scope": "normative",
      ...
      "notes": "WP-KERNEL-005 CKC fold-in: 10.16 executes only under capability-gated governed jobs."
    },
    {
      "edge_id": "IMX-K005-06",
      "from_kind": "feature",
      "from_id": "FEAT-SETTINGS-PREFERENCES",
      "to_kind": "feature",
      "to_id": "FEAT-CAPABILITIES-CONSENT",
      "kind": "enforce_capabilities",
      "scope": "normative",
      ...
      "notes": "WP-KERNEL-005 CKC fold-in: 10.17 executes only under capability-gated governed jobs."
    },
    {
      "edge_id": "IMX-K005-07",
      "from_kind": "feature",
      "from_id": "FEAT-STEALTH-REFERENCE-WINDOW",
      "to_kind": "feature",
      "to_id": "FEAT-CAPABILITIES-CONSENT",
      "kind": "enforce_capabilities",
      "scope": "normative",
      ...
      "notes": "WP-KERNEL-005 CKC fold-in: 10.18 executes only under capability-gated governed jobs."
    },
    {
      "edge_id": "IMX-K005-08",
      "from_kind": "feature",
      "from_id": "FEAT-COMMAND-CORPUS-PARITY",
      "to_kind": "feature",
      "to_id": "FEAT-CAPABILITIES-CONSENT",
      "kind": "enforce_capabilities",
      "scope": "normative",
      ...
      "notes": "WP-KERNEL-005 CKC fold-in: 10.19 executes only under capability-gated governed jobs."
    },
    {
      "edge_id": "IMX-TAILOR-01",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-BODYKIT",
      "to_kind": "feature",
      "to_id": "FEAT-TAILOR-CLOTH",
      "kind": "provides_body_targets",
      "scope": "normative",
      "tokens": [
        "parametric tailor_avatars rows",
        "channel-derived collision proxies",
        "grow-into-garment refit"
      ],
      "spec_refs": [
        "13-tailor 13.24"
      ],
      "notes": "[ADD v02.198] BodyKit publishes parametric avatars + auto-updating capsule/sphere/SDF proxies that Cloth drapes against; garments carry fit data across the channel space."
    },
    {
      "edge_id": "IMX-TAILOR-02",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-CLOTH",
      "to_kind": "feature",
      "to_id": "FEAT-TAILOR-BODYKIT",
      "kind": "drapes_and_conforms_garments",
      "scope": "normative",
      "tokens": [
        "volume-aware auto-follow",
        "garment fix dials",
        "conform-wardrobe batch"
      ],
      "spec_refs": [
        "13-tailor 13.24"
      ],
      "notes": "[ADD v02.198] Cloth drapes/refits wardrobes onto BodyKit bodies incl. extreme archetypes; rigidity masks honored across projection and sim."
    },
    {
      "edge_id": "IMX-TAILOR-03",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-CLOTH",
      "to_kind": "feature",
      "to_id": "FEAT-AI-JOB-MODEL",
      ...
      "scope": "normative",
      "tokens": [
        "sim/export/refit as Handshake JOBS",
        "leases/backpressure/cancellation"
      ],
      "spec_refs": [
        "13-tailor 13.11"
      ],
      "notes": "[ADD v02.198] Every Tailor simulation, refit, bake, and export run executes on the kernel job scheduler with Flight Recorder lifecycle events."
    },
    {
      "edge_id": "IMX-TAILOR-04",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-BODYKIT",
      "to_kind": "feature",
      "to_id": "FEAT-AI-JOB-MODEL",
      ...
      "scope": "normative",
      "tokens": [
        "corrective bakes, archetype QA matrices, exports as JOBS"
      ],
      "spec_refs": [
        "13-tailor 13.27"
      ],
      "notes": "[ADD v02.198] BodyKit bake/QA/export pipelines are scheduler jobs; QA matrices persist as validation records."
    },
    {
      "edge_id": "IMX-STUDIO-01",
      "from_kind": "feature",
      "from_id": "FEAT-STUDIO-RASTER",
      "to_kind": "feature",
      "to_id": "FEAT-STUDIO-VECTOR",
      "kind": "shared_unified_document",
      "scope": "normative",
      "tokens": [
        "one StudioDocument",
        "one selection/history/color/export"
      ],
      "spec_refs": [
        "14-studio 14.3",
        "14-studio 14.21"
      ],
      "notes": "[ADD v02.199] Raster and vector layers live in one StudioDocument sharing one selection, history, color pipeline, and export surface (unified operator suite, no per-app silo)."
    },
    {
      "edge_id": "IMX-TAILOR-05",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-CLOTH",
      "to_kind": "feature",
      "to_id": "FEAT-FLIGHT-RECORDER",
      "kind": "project_authoritative_lifecycle_evidence",
      "scope": "normative",
      "tokens": [
        "EventLedger identity projection",
        "event_id/run_id/actor_id/correlation_id",
        "command and revision receipts"
      ],
      "spec_refs": [
        "13.30 [TAI-ACT-001..010]",
        "13.33 [TAI-INT-001..006]",
        "13.35 [TAI-QA-001..007]"
      ],
      "notes": "[ADD v02.201] Every authoritative Cloth mutation, simulation, recovery, render, and publish lifecycle projects the same EventLedger identity into Flight Recorder; the projection is bounded metadata plus artifact references and never becomes a second authority."
    },
    {
      "edge_id": "IMX-TAILOR-06",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-BODYKIT",
      "to_kind": "feature",
      "to_id": "FEAT-FLIGHT-RECORDER",
      ...
      "notes": "[ADD v02.201] Every authoritative BodyKit shape, anatomy, pose, contact, material, recovery, render, and publish lifecycle projects the same EventLedger identity into Flight Recorder; the projection is bounded metadata plus artifact references and never becomes a second authority."
    },
    {
      "edge_id": "IMX-TAILOR-07",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-CLOTH",
      "to_kind": "feature",
      "to_id": "FEAT-DIAGNOSTICS-SCHEMA",
      "kind": "uses_shared_three_tier_diagnostics",
      "scope": "normative",
      "tokens": [
        "Flight Recorder",
      ...
      "spec_refs": [
        "13.33 [TAI-INT-001..006]",
        "13.34 [TAI-UX-001..005]",
        "13.35 [TAI-QA-001..007]"
      ],
      "notes": "[ADD v02.201] Cloth classifies operations DIRECT, INHERITED, or NOT_APPLICABLE across the shared three-tier diagnostics contract; solver, GPU/device, job, bridge, validation, and recovery evidence is typed and inspectable without console scraping or a Tailor-only watcher."
    },
    {
      "edge_id": "IMX-TAILOR-08",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-BODYKIT",
      "to_kind": "feature",
      "to_id": "FEAT-DIAGNOSTICS-SCHEMA",
      ...
      "notes": "[ADD v02.201] BodyKit classifies operations DIRECT, INHERITED, or NOT_APPLICABLE across the shared three-tier diagnostics contract; corrective, soft-tissue, contact, renderer, job, bridge, validation, and recovery evidence is typed and inspectable without console scraping or a Tailor-only watcher."
    },
    {
      "edge_id": "IMX-TAILOR-09",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-CLOTH",
      "to_kind": "feature",
      "to_id": "FEAT-LOCUS-WORK-TRACKING",
      "kind": "projects_optional_production_work",
      "scope": "normative",
      "tokens": [
        "QA and repair assignments",
        "production plan",
        "claim/occupancy/resume",
        "tailor:// references"
      ],
      "spec_refs": [
        "13.28 [TAI-PRO-001..006]",
        "13.33 [TAI-INT-001..006]"
      ],
      "notes": "[ADD v02.201] Cloth may project QA assignments, repair queues, production plans, occupancy, checkpoints, and stable tailor:// references into Locus. Locus records coordination and readiness only; TailorProject, EventLedger, Workflow, and ArtifactStore remain product/job authority."
    },
    {
      "edge_id": "IMX-TAILOR-10",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-BODYKIT",
      "to_kind": "feature",
      "to_id": "FEAT-LOCUS-WORK-TRACKING",
      ...
      "notes": "[ADD v02.201] BodyKit may project QA assignments, corrective/contact repairs, production plans, occupancy, checkpoints, and stable tailor:// references into Locus. Locus records coordination and readiness only; TailorProject, EventLedger, Workflow, and ArtifactStore remain product/job authority."
    },
    {
      "edge_id": "IMX-TAILOR-11",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-CLOTH",
      "to_kind": "feature",
      "to_id": "FEAT-LOOM-LIBRARY",
      "kind": "publishes_stable_asset_and_evidence_refs",
      "scope": "normative",
      "tokens": [
        "garment/material/style refs",
        "captures and validation reports",
        "backlinks/tags/search/navigation"
      ],
      "spec_refs": [
        "13.31 [TAI-CLP-001..007]",
        "13.33 [TAI-INT-001..006]"
      ],
      "notes": "[ADD v02.201] Cloth publishes stable references and bounded metadata for garments, fabrics, styles, captures, manifests, diagnostics, and validation reports into Loom/Notes/ProjectKnowledgeIndex. Loom provides discovery and backlinks but cannot mutate Tailor authority through prose, tags, or cached previews."
    },
    {
      "edge_id": "IMX-TAILOR-12",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-BODYKIT",
      "to_kind": "feature",
      "to_id": "FEAT-LOOM-LIBRARY",
      ...
      "scope": "normative",
      "tokens": [
        "body/recipe/pose/mix refs",
        "captures and validation reports",
        "backlinks/tags/search/navigation"
      ],
      "spec_refs": [
        "13.32 [TAI-BKP-001..011]",
        "13.33 [TAI-INT-001..006]"
      ],
      "notes": "[ADD v02.201] BodyKit publishes stable references and bounded metadata for bodies, recipes, materials, poses, mix graphs, captures, manifests, diagnostics, and validation reports into Loom/Notes/ProjectKnowledgeIndex. Loom provides discovery and backlinks but cannot mutate BodyKit authority through prose, tags, or cached previews."
    },
    {
      "edge_id": "IMX-TAILOR-13",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-CLOTH",
      "to_kind": "feature",
      "to_id": "FEAT-CALENDAR",
      "kind": "projects_activity_spans_and_optional_production_blocks",
      "scope": "normative",
      "tokens": [
        "ActivitySpan/SessionSpan",
        "optional production block",
        "CalendarMutation via WorkflowRun/calendar-sync",
      ...
      "notes": "[ADD v02.201] Cloth jobs project ActivitySpan/SessionSpan timing and may associate an operator-approved production block. Calendar mutations route through WorkflowRun/calendar-sync and Calendar policy is a scheduling hint only; it cannot cancel, reprioritize, or authorize Tailor jobs."
    },
    {
      "edge_id": "IMX-TAILOR-14",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-BODYKIT",
      "to_kind": "feature",
      "to_id": "FEAT-CALENDAR",
      ...
      "notes": "[ADD v02.201] BodyKit jobs project ActivitySpan/SessionSpan timing and may associate an operator-approved production block. Calendar mutations route through WorkflowRun/calendar-sync and Calendar policy is a scheduling hint only; it cannot cancel, reprioritize, or authorize Tailor jobs."
    },
    {
      "edge_id": "IMX-TAILOR-15",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-BODYKIT",
      "to_kind": "primitive",
      "to_id": "PRIM-TailorMixGraph",
      "kind": "dependency_gated_posekit_evidence_mix",
      "scope": "dependency_gated",
      "tokens": [
        "immutable PoseKit evidence",
        "BodyPose proposal",
        "source commit merged-main containment",
        "WP-KERNEL-012 compatibility receipt",
        "fail-closed availability"
      ],
      "spec_refs": [
        "13.32 [TAI-BKP-001..011]",
        "13.33 [TAI-INT-001..006]",
        "13.36 [TAI-GATE-001..005]"
      ],
      "notes": "[ADD v02.201] TailorMixGraph may mix immutable PoseKit evidence with BodyPose, BodyKit recipe/channel, garment, material, and animation layers only after the PoseKit source commit is contained in merged main and the pinned WP-KERNEL-012 native-shell compatibility receipt passes. Before that gate, commands and GUI controls remain disabled with `posekit_dependency_not_ready`. PoseKit/OpenPose output is 2D evidence, never 3D pose, camera, contact, anatomy, cloth, or body authority; apply remains previewable, selective, revision-checked, leased, attributable, and reversible."
    },
    {
      "edge_id": "IMX-TAILOR-16",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-BODYKIT",
      "to_kind": "primitive",
      "to_id": "PRIM-TailorDependencyBundle",
      "kind": "resolves_offline_content_addressed_dependencies",
      "scope": "normative",
      "tokens": ["Rust acquisition", "exact hashes", "licenses and SBOM", "offline frozen runtime", "rollback"],
      "spec_refs": ["13.37 [TAI-V2-RUN-001..010]"],
      "notes": "[ADD v02.203] BodyKit models, runtimes, SDKs, shaders and fixtures resolve only from immutable dependency bundles; missing content never causes a runtime download."
    },
    {
      "edge_id": "IMX-TAILOR-17",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-BODYKIT",
      "to_kind": "primitive",
      "to_id": "PRIM-BodyEvaluator",
      "kind": "evaluates_provider_neutral_body_and_head",
      "scope": "normative",
      "tokens": ["GNM-v3 default", "Rust evaluator", "provider package", "measurements and landmarks", "typed diagnostics"],
      "spec_refs": ["13.38 [TAI-V2-BDY-001..012]"],
      "notes": "[ADD v02.203] Provider schemas map explicitly into canonical BodyKit recipes and artifacts; no provider becomes project authority."
    },
    {
      "edge_id": "IMX-TAILOR-18",
      "from_kind": "primitive",
      "from_id": "PRIM-BodyHeadPackage",
      "to_kind": "primitive",
      "to_id": "PRIM-HeadBodyAssembly",
      "kind": "assembles_versioned_high_resolution_character",
      "scope": "normative",
      "tokens": ["source head and body", "neck seam", "rig and materials", "LOD correspondence", "reversible residuals"],
      "spec_refs": ["13.38 [TAI-V2-BDY-002A..010]"],
      "notes": "[ADD v02.203] Source packages remain linked and replaceable; assembly, residual and corrective decisions are previewable, versioned and recoverable."
    },
    {
      "edge_id": "IMX-TAILOR-19",
      "from_kind": "primitive",
      "from_id": "PRIM-BodyPerformanceTrack",
      "to_kind": "feature",
      "to_id": "FEAT-TAILOR-BODYKIT",
      "kind": "drives_native_rig_expression_and_animation",
      "scope": "normative",
      "tokens": ["manual and capture producers", "Audio2Face optional", "timecode", "editable layers", "temporal validation"],
      "spec_refs": ["13.39 [TAI-V2-PERF-001..008]"],
      "notes": "[ADD v02.203] All performance producers emit one provider-neutral timestamped track; original captures remain immutable evidence."
    },
    {
      "edge_id": "IMX-TAILOR-20",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-BODYKIT",
      "to_kind": "primitive",
      "to_id": "PRIM-BodyConditioningBundle",
      "kind": "publishes_generation_and_scene_conditioning",
      "scope": "normative",
      "tokens": ["EXR typed AOVs", "camera and lights", "depth normals motion IDs", "artifact hashes", "Rust reload"],
      "spec_refs": ["13.39 [TAI-V2-PERF-009..010]"],
      "notes": "[ADD v02.203] Lifelike renders and machine-consumable conditioning share explicit provenance and channel semantics."
    },
    {
      "edge_id": "IMX-TAILOR-21",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-BODYKIT",
      "to_kind": "primitive",
      "to_id": "PRIM-TailorInterchangeProfile",
      "kind": "exports_and_qualifies_loss_bounded_interchange",
      "scope": "normative",
      "tokens": ["deterministic GLB", "restricted USD", "OpenPBR MaterialX", "Unreal native commandlet", "manual Blender"],
      "spec_refs": ["13.40 [TAI-V2-IO-001..010]"],
      "notes": "[ADD v02.203] Direction-specific capability rows make FBX/Alembic limitations and every material/scene loss visible instead of routing through generated Python."
    },
    {
      "edge_id": "IMX-TAILOR-22",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-BODYKIT",
      "to_kind": "feature",
      "to_id": "FEAT-RESOURCE-PRIVACY",
      "kind": "enforces_account_resource_privacy",
      "scope": "normative",
      "tokens": ["SurrealDB authenticated record-user permissions", "PERMISSIONS NONE", "ResourceBroker", "derived-scope intersection", "HBR-PRIV-001..008", "negative non-disclosure tests", "no root/system-user privacy proof"],
      "spec_refs": ["13.41 [TAI-V2-PRJ-001..010]", "2.3.13.12"],
      "notes": "[ADD v02.203] Bodies, heads, captures, models, residuals, renders, logs and exports remain default-private and cannot widen access through derivation or projection."
    },
    {
      "edge_id": "IMX-TAILOR-23",
      "from_kind": "feature",
      "from_id": "FEAT-TAILOR-CLOTH",
      "to_kind": "primitive",
      "to_id": "PRIM-TailorActiveToolState",
      "kind": "shares_native_professional_tool_state",
      "scope": "normative",
      "tokens": ["2D and 3D tools", "sliders plus numeric", "gizmos and snapping", "one semantic command", "GUI model parity"],
      "spec_refs": ["13.42 [TAI-V2-ACT-001..007]", "13.43 [TAI-V2-UX-001..012]"],
      "notes": "[ADD v02.203] Cloth and BodyKit share discoverable tool, viewport, outliner, inspector, timeline, jobs, Problems and history contracts."
    },
    {
      "edge_id": "IMX-STUDIO-02",
      "from_kind": "feature",
      "from_id": "FEAT-STUDIO-AUTOMATION",
      "to_kind": "feature",
      "to_id": "FEAT-STUDIO-DESIGN-SYSTEM",
      "kind": "one_command_api_all_domains",
      "scope": "normative",
      "tokens": [
        "typed command contract",
        "model API = product API"
      ],
      "spec_refs": [
        "14-studio 14.14",
        "14-studio 14.16"
      ],
      "notes": "[ADD v02.199] All five suites' scripting DOMs collapse into one typed Studio command API that both operators/plugins and model lanes call; no per-domain DOM fork."
    },
    {
      "edge_id": "IMX-STUDIO-03",
      "from_kind": "feature",
      "from_id": "FEAT-STUDIO-INTEROP",
      "to_kind": "feature",
      "to_id": "FEAT-TAILOR-CLOTH",
      "kind": "texture_and_asset_pipeline",
      "scope": "normative",
      "tokens": [
        "PBR/texture handoff",
        "exported studio assets"
      ],
      "spec_refs": [
        "14-studio 14.13",
        "13-tailor 13.9"
      ],
      "notes": "[ADD v02.199] Studio exports (textures, graphics, layouts) feed Tailor garment/body texturing and vice-versa; both are kernel-native creative modules over shared EventLedger authority."
    }
  ]
}
```
...
## 12.7 WP-KERNEL-009 Primitive and Interaction Addendum [ADD v02.192]

This addendum registers the WP-KERNEL-009 primitive family until the next full appendix generator pass folds these rows into the machine appendix blocks above.

Primitive additions:

- `PRIM-ProjectKnowledgeIndex`: SurrealDB authority substrate, including EventLedger persistence, for indexed project roots, sources, spans, entities, relationships, claims, passages, rich documents, UserManual entries, retrieval traces, validation receipts, and projections.
- `PRIM-KnowledgeSource`: stable source registration record with provenance, source kind, content hash, redaction state, permissions, and index receipts.
- `PRIM-KnowledgeSpan`: minimum citeable evidence unit anchored to source bytes, AST nodes, rich-document ranges, media timestamps, PDF pages, or table cells.
- `PRIM-KnowledgeEntity`: typed symbol, concept, file, folder, project, task, API, schema, command, media object, manual entry, or product primitive.
- `PRIM-KnowledgeEdge`: typed relationship record with source spans, extractor version, lifecycle state, confidence, and stable `relationship_id`.
- `PRIM-KnowledgeClaim`: lifecycle-managed assertion with evidence spans, conflict refs, and resolution receipts.
- `PRIM-MemoryPassage`: bounded passage eligible for model context with retrieval mode, freshness, ranking features, and compaction policy.
- `PRIM-RetrievalTrace`: replayable retrieval-plan and retrieval-result record for none/direct_load/exact_lookup/graph_traversal/hybrid_rag.
- `PRIM-RichDocument`: versioned ProseMirror/Tiptap document authority record connected to CRDT snapshots and EventLedger promotions.
- `PRIM-EditorCodeNode`: Monaco-backed code block embedded in a RichDocument with language id, diagnostics, worker bundling, and round-trip hash.
- `PRIM-UserManual`: canonical no-context operator/model manual surface; legacy ModelManual code paths are compatibility shims until migrated.
- `PRIM-BackendNavigationReceipt`: typed receipt for model/operator backend navigation actions with actor, session, correlation, target ref, action, and replay refs.
- `PRIM-VisualDebugEvidence`: screenshot, UI snapshot, state dump, or trace artifact linked to the authority records it verifies.

Interaction additions:

- `PRIM-ProjectKnowledgeIndex` consumes `PRIM-KnowledgeSource`, `PRIM-KnowledgeSpan`, `PRIM-KnowledgeEntity`, `PRIM-KnowledgeEdge`, `PRIM-KnowledgeClaim`, `PRIM-MemoryPassage`, `PRIM-RetrievalTrace`, `PRIM-RichDocument`, `PRIM-UserManual`, EventLedger, ArtifactStore, CRDT/write-box records, and Visual Debugger evidence.
- `PRIM-RichDocument` uses ProseMirror/Tiptap schema state for structure and `PRIM-EditorCodeNode` for Monaco-backed code blocks; authority changes promote through WriteBoxV1 and EventLedger.
- Loom views, backlinks, sorted groups, pins, graph traversal, and Obsidian-like navigation are projections over ProjectKnowledgeIndex and Loom authority records.
- UserManual coverage is required for every new ProjectKnowledgeIndex route, rich editor command, backend navigation action, retrieval trace view, graph view, visual-debug action, and recovery path.
- HBR rows applicable at packet hydration include HBR-INT-001, HBR-INT-003, HBR-INT-004, HBR-INT-006, HBR-INT-007, HBR-SWARM-001, HBR-SWARM-003, HBR-SWARM-004, HBR-VIS-001, HBR-VIS-002, HBR-QUIET-001, HBR-QUIET-002, HBR-MAN-001, HBR-MAN-002, and HBR-MAN-003.

### Appendix 12 Account-Resource Privacy Registry Additions [ADD v02.202]

Feature additions:

- `FEAT-LOCAL-ACCOUNT-AUTH`: local credentials, first-run Owner, login/session/account lifecycle, re-authentication, key-vault binding.
- `FEAT-ACCESS-SPACE`: named authorized-work-context selection without grant amplification.
- `FEAT-RESOURCE-PRIVACY`: default-private ProtectedResources, ResourceGrant evaluation, same-project private resources, revocation, non-disclosure.
- `FEAT-RESOURCE-SHARING`: explicit project/resource membership and per-resource grants.
- `FEAT-VISITOR-MCP-GATE`: visitor passes, MCP fingerprints/Principals, ToolGate, audience/expiry/revocation.
- `FEAT-PERSONA`: account-owned preferences/presentation/memory personalization without authorization semantics.
- `FEAT-REMOTE-INSPIRATION-SERVICE`: future moodboard/project-starter/inspiration SaaS/MCP manifest, quarantine, import, and sync seams.

Primitive additions:

- `PRIM-LocalAccount`, `PRIM-AuthenticatedSession`, `PRIM-Principal`, `PRIM-AccountRole`, `PRIM-MembershipRole`
- `PRIM-AccessSpace`, `PRIM-ProtectedResource`, `PRIM-ResourceLink`, `PRIM-ResourceGrant`, `PRIM-ResourceBroker`
- `PRIM-Persona`, `PRIM-ExternalIdentityBinding`, `PRIM-RemoteServiceConnection`, `PRIM-RemoteResourceManifest`

Interaction additions:

- LocalAccount authenticates into AuthenticatedSession; every action resolves to a Principal and delegation chain.
- AccessSpace selects a subset of resources already allowed by ResourceGrant; it never grants.
- AccountRole governs installation administration, MembershipRole contributes project/workspace policy, and ResourceGrant governs exact content access.
- Persona modifies preferences and memory personalization only after resource authorization.
- ResourceBroker resolves protected filesystem/artifact handles and enforces canonical path/root policy.
- All primary and derived resource producers attach owner, actor, session, Space, grant, lineage, and EventLedger refs.
- Derivation computes source-scope intersection; imports begin quarantined; revocation invalidates caches/contexts/handles.
- HBR-PRIV-001 through HBR-PRIV-008 are applicable by default whenever a WP/MT touches a product resource, context, account, sharing, authorization, model/tool retrieval, diagnostic, export, backup, or remote-service path.

UI guidance:

- show current account and AccessSpace persistently and Persona separately;
- make exact sharing scope, actions, expiry, audience, inheritance, and derivative impact reviewable before commit;
- never use hidden rows as the security boundary or leak restricted existence through empty/error/count/search/preview states;
- expose stable Argus identifiers and backend navigation for login, Space, Persona, sharing, revocation, visitor/MCP, service import, privacy inspection, and break-glass audit.
