...
body_sha256: "dee69105ad1ff1c66ed922fd71db57b45c9ade3a93c2b61fa0d0ff2c5def61c3"
...
**Why**
Handshake Build Rules (HBRs) are the cross-cutting acceptance gates that hold every WP to the same minimum bar for sandbox quietness, visual debugging, multi-model parallel coordination, and ModelManual self-consistency. Pre-v02.186 they were enforced ad-hoc through validator prose. This section [ADD v02.186] makes `HANDSHAKE_BUILD_RULES.json` the single source-of-truth and pins HBR gate evaluation to two concrete enforcement points (build time + every governed inter-role handoff). The cross-anchor in the Handshake Codex is CX-131 (added Stage 3).
...
`HANDSHAKE_BUILD_RULES.json` is the single source-of-truth for HBR-IDs across the INT, SWARM, VIS, QUIET, and MAN families (23 active rules at v1.1.0). Authority lives in `.GOV/roles_shared/records/HANDSHAKE_BUILD_RULES.json`. Any prose mention of an HBR-ID in spec, refinement, or work-packet documents resolves against this file.
...
- The new CX-131 anchor in `Handshake_Codex_v1.4.md` (Stage 3 will add CX-131; rule changes after Stage 3 must reference it).
...
**Cross-references:** CX-130 typed-receipt discipline; CX-131 new Codex anchor (added Stage 3); CX-105A spec edit law; `.GOV/roles_shared/records/HANDSHAKE_BUILD_RULES.json`; HBR families INT/SWARM/VIS/QUIET/MAN; WP-KERNEL-004 refinement acceptance criteria AC-HBR-AUTHORITY, AC-HBR-HANDOFF-GATE, AC-HBR-TYPED-RECEIPT.
