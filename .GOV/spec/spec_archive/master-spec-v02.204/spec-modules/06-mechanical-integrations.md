...
body_sha256: "6e3d08cd61da235f974fcfce30a610e2239825017336c70dc5f92e4f7418120a"
metadata_rule: "frontmatter is machine metadata; body follows after this block"
---
# 6. Mechanical Integrations

## 6.0.0 SurrealDB authority supersession [ADD v02.204]

Every current-normative PostgreSQL physical-storage commitment in this module is superseded by the SurrealDB/EventLedger authority defined in Section 2.3.13.0. Mechanical integrations MUST persist typed authority records through the official SurrealDB Rust SDK and the shared storage boundary; no feature may retain PostgreSQL as runtime, fallback, compatibility, fixture, cache, test, migration source, or dual authority. EventLedger semantics transfer fully to fresh SurrealDB persistence and ArtifactStore relationships remain unchanged. Schema changes use SurrealKit rollouts; live consumers use SurrealDB live queries as non-authoritative wakeups. PostgreSQL rows MUST NOT be read, imported, replayed, reconciled, translated, or connected; PostgreSQL dependencies, configuration, credentials, migration tooling, fixtures, tests, proof paths, and network connectivity are retired.
...
2. Each runtime-visible capability slice MUST declare, directly or via linked runtime visibility rows:
   ...
   - storage posture (`SurrealDB/EventLedger required`; SQLite/PostgreSQL runtime paths forbidden).
...
- **Load:** write deterministic artifacts + upsert typed SurrealDB/EventLedger authority records + update derived indexes (FTS + vector + graph + role lanes)
...
###### 6.3.3.5.7.18 Persistence contract (SurrealDB/EventLedger authority) [ADD v02.123] [UPDATED v02.204]
...
**Addendum: 7.2 SurrealDB/EventLedger authority schema [UPDATED v02.204]**

The physical schema may evolve through SurrealKit rollouts, but authoritative typed `SCHEMAFULL` records persist through SurrealDB/EventLedger and the logical tables and keys are normative. SQLite and PostgreSQL are not runtime, fixture, cache, fallback, compatibility, or proof paths for this persistence contract.
...
**Authority notes:**
- store vectors through the SurrealDB/EventLedger authority path using versioned SurrealDB vector indexes/search, with index contents rebuildable from authoritative source records
...
LAW-MDV2-RESUME-001: A session MUST be resumable after process restart with NO loss of completed-item work. Resume MUST reconstruct session and per-item stage from the EventLedger event stream plus ArtifactStore manifests; the in-memory queue is rebuilt, never read back from a process-local store. SQLite (TECH-SQLITE) and PostgreSQL MUST NOT be used as a session/checkpoint store in runtime, tests, fixtures, mocks, examples, fallbacks, cache, or compatibility paths; canonical session/checkpoint state lives in SurrealDB + EventLedger + ArtifactStore.
...
Job state, lineage, and evidence persist ONLY to fresh SurrealDB + EventLedger + ArtifactStore + CRDT/write-box authority. SQLite and PostgreSQL are FORBIDDEN for this feature in runtime, tests, fixtures, mocks, examples, fallbacks, cache, compatibility adapters, temporary harnesses, imports, migration tooling, and proof. PostgreSQL MUST NOT be connected or read. No new TECH-SQLITE or PostgreSQL dependency is introduced by 6.10.
...
> Normative rule: Sourcing-spec records, handler matrix entries, binding decisions, and receipts persist through SurrealDB + EventLedger and materialize derived artifacts through ArtifactStore. SQLite and PostgreSQL MUST NOT be used in any runtime, test, fixture, mock, example, cache, fallback, or compatibility path for this feature.
...
- **Storage authority:** records persist through SurrealDB + EventLedger + ArtifactStore + CRDT/write-box only; SQLite and PostgreSQL are forbidden across runtime, tests, fixtures, mocks, examples, fallbacks, cache, and compatibility adapters for this feature.
...
LAW-COMFY-INTAKE-004: Storage authority for this feature is SurrealDB (`TECH-SURREALDB`) + EventLedger + ArtifactStore + CRDT/write-box (`TECH-CRDT` / `PRIM-WriteBoxV1`) ONLY. SQLite and PostgreSQL are FORBIDDEN for intake state, fixtures, mocks, caches, fallbacks, and the fake adapter. No intake record, manifest index, or test fixture may be backed by either legacy backend.
...
- Fixtures, fake-adapter state, and materialized test artifacts use ArtifactStore + SurrealDB/EventLedger fixtures ONLY. SQLite and PostgreSQL are FORBIDDEN in the fake adapter and its fixtures (LAW-COMFY-INTAKE-004).
...
## 6.13 Palmistry (External Out-of-Process Diagnostic Watcher) [ADD v02.196]

**Why**
An in-process diagnostics tool (`internal_diagnostics`, Section 5.8) cannot reliably capture the moment a process freezes or dies: when the UI thread hangs, when the process is pinned under heavy CPU, or when it crashes, the in-process tool is exactly the thing that has stopped running. Field crash-reporting systems (the Google Crashpad model) solve this by running a SEPARATE watcher process that survives the parent and captures the moment of death from outside. This section [ADD v02.196] introduces **Palmistry** as the third diagnostic tier: an EXTERNAL, out-of-process watcher that survives Handshake freezes, crashes, and heavy-CPU stalls, and captures the state an in-process tool structurally cannot.

**What**
Defines the Palmistry watcher executable, its lifecycle inversion (launched-with-Handshake, survives parent death, closes only on explicit command), the shared-memory ring-buffer reader, freeze detection, crash detection with out-of-process minidump capture, the durable survivor store, the recovery-time forwarding path into the Flight Recorder, and the typed-allowlist privacy boundary.

---

### 6.13.1 Purpose: capture the moment of death

Palmistry is an external watcher whose job is to observe Handshake from outside its process boundary and capture freeze and crash evidence that an in-process tool cannot. It is the Tier 3 surface in the three-tier diagnostic decoupling (Tier 1 Flight Recorder business-event ledger; Tier 2 `internal_diagnostics` in-process; Tier 3 Palmistry external) canonical in HBR-INT-009 v1.5.0 and Codex CX-981.

### 6.13.2 Architecture: the Google-Crashpad model

Palmistry follows the Google-Crashpad architecture: a **separate `palmistry` executable**, distinct from the Handshake process, launched alongside Handshake. It is a sibling watcher, not a worker thread and not an in-process subsystem.

### 6.13.3 Lifecycle inversion (HARD)

The watcher's lifecycle is INVERTED relative to an ordinary child process:

- It is **launched WITH Handshake** at startup.
- It **CLOSES ONLY on explicit command** (a deliberate shutdown signal from Handshake or the operator).
- It **SURVIVES parent death** -- if Handshake freezes or crashes, Palmistry keeps running so it can capture and persist the evidence.

Critically, Palmistry MUST NOT be a kill-on-job-close child (it must not be tied to the parent via an OS job object / process-group that terminates children when the parent dies). A watcher that dies with its parent cannot record the parent's death; that defeats the entire purpose and is a spec defect.

### 6.13.4 Shared-memory ring-buffer reader

Palmistry reads Handshake's last UI-thread heartbeat and last-N diagnostic events from the shared-memory ring buffer that `internal_diagnostics` (Section 5.8.6) writes every frame. The reader requires **zero cooperation** from Handshake: it reads the ring even when the UI thread is frozen, because the ring is plain shared memory and the reader never asks the (possibly hung) writer to do anything. This is why a blocking socket / request-response channel would be the wrong design -- a frozen Handshake could never answer, so liveness MUST be observed passively from shared memory, not requested over a blocking channel.

### 6.13.5 Freeze detection

A freeze is detected when the heartbeat in the ring buffer goes **stale** -- the frame-loop counter / monotonic timestamp stops advancing for longer than a threshold (default ~5 seconds). Staleness alone is corroborated by an **OS hung-window probe** (asking the window manager whether the application's top-level window is not responding) before Palmistry declares a freeze, to avoid false positives during legitimate long frames.

### 6.13.6 Crash detection and out-of-process minidump

A crash is detected via a **process-handle wait** (the watcher waits on Handshake's process handle and is signalled on exit) and/or an `UnhandledExceptionFilter` registration. On crash, Palmistry captures a **minidump using an out-of-process minidump writer** (the crashing process cannot reliably dump itself; the external writer captures the dump from outside). The minidump records the moment of death -- thread stacks, registers, loaded modules -- that no in-process path could capture.

### 6.13.7 Durable survivor store and recovery-time forwarding

Palmistry persists captured freeze and crash records (heartbeat snapshot, last-N events, minidump pointer, OS probe result) into a **durable survivor store** on local disk that outlives the Handshake process. When Handshake recovers (restarts after a crash, or unfreezes), Palmistry **forwards the captured freeze/crash records into the Flight Recorder** (Tier 1), so the out-of-process evidence rejoins the governed business-event ledger and shows up in replay/audit and in the Diagnostics Panel (Section 10.12).

### 6.13.8 Privacy: typed allowlist; minidumps local-only by default

Palmistry records **NO project content and NO sensitive data**. Captured records are constrained to a **typed allowlist** of mechanical fields, and the standard mechanism names (`heartbeat`, `minidump`, `watchdog`, `ring-buffer`) are kept verbatim as field-standard terms, not project data. **Minidumps are local-only by default**; any upload (e.g., to a crash-aggregation endpoint) is **consent-gated** and never happens automatically.

Built by WP-KERNEL-015 (the strong base) and retrofitted onto already-shipped work by WP-KERNEL-016.

**Cross-references:** Section 5.8 internal_diagnostics (ring-buffer producer; Tier 2); Section 5.3 AI Observability / Flight Recorder (Tier 1 business-event ledger; recovery-time forward target); Section 5.7 ProcessOwnershipLedger (process lifecycle rows); Section 10.12 Diagnostics Panel (surfaces Palmistry freeze/crash/debris records); HBR-INT-009 v1.5.0 (three-tier decoupling, mandatory); Codex CX-981; HBR-QUIET-003 non-hijacking; HBR-SWARM-001 / HBR-SWARM-002; WP-KERNEL-015 (build), WP-KERNEL-016 (retrofit).
