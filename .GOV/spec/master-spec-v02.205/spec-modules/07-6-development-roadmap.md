...
body_sha256: "b9154093178c68df30087728b02e45efb72aeae1fc294286de5382f7e37af149"
...
### 7.6.0 SurrealDB Authority Override (v02.205) [ROAD-SDB-001]

Every planned durable workspace, job, workflow, Loom, Locus, Dev Command Center, model-session, catalog, memory, or governance record MUST use Handshake-managed SurrealDB through the typed authority boundary and MUST persist EventLedger semantics in SurrealDB. Any later SQLite, PostgreSQL, `devcc.db`, sqlite-vec, LanceDB-authority, local database file, SQL migration, or relational-store roadmap item is superseded as an implementation choice while its product behavior, fields, workflows, recovery semantics, and user-facing objectives remain required. PostgreSQL and SQLite are not runtime, cache, fallback, import, fixture, test, or proof paths. Vector retrieval uses typed SurrealDB vector fields/indexes. DuckDB may survive only as a bounded rebuildable diagnostic/analytics projection that is never authority.
...
- **[ADD v02.131][REFINE v02.197] Handshake Stage MVP (governed browser/webviewer + capture surface)**  
  Deliver Handshake Stage as a first-class, governed browser/webviewer surface with isolated sessions/tabs, cold persisted tab records, trusted HTML artifact viewing, embedded Stage Apps, model-operated front-end test paths, and a Stage Bridge API that can enqueue evidence-backed capture/import jobs (web/PDF/3D) through the Workflow Engine + Mechanical Tool Bus (no bypass). Stage does not own Figma/Spline/Photoshop/Affinity-class visual authoring; that belongs to Studio Module.
...
- [ADD v02.79] Studio Module / Photo Studio scope explosion (Lightroom/Affinity-class) â†’ enforce Phase 1 boundary: â€œskeleton only; no RAW/masks/layers/AIâ€.
...
**Explicitly OUT of scope**
- [ADD v02.130][REFINE v02.204] Loom: AI auto-tagging/auto-caption, semantic/hybrid search (Tierâ€‘3), multi-user Loom collaboration/sync, and SurrealDB-backed Loom query engines are Phase 2+ / Phase 4.
- [ADD v02.131][REFINE v02.197] Stage: browser-extension ecosystem, arbitrary third-party Stage Apps/plugin marketplace, bulk crawling/mirroring, and advanced visual/3D authoring are outside Stage. Studio Module owns Figma/Spline/Photoshop/Affinity-class visual authoring and collaboration.
...
- [ADD v02.136][REFINE v02.197] Begin **Design Studio shell/IA** alignment (see `Handshake_Design_Studio_Overhaul_v0.1.md`): treat worksurfaces as modules in one shell (rail + inspector + bottom drawers). This is **recontextualization** only; storage/layout/capability boundaries remain unchanged. Studio owns Figma-like collaboration, canvas, photo/painting/compositing, and future 3D visual authoring; Stage supplies captured/provenance artifacts and web/HTML projections into that shell.
...
**Explicitly OUT of scope**
- [ADD v02.130][REFINE v02.204] Loom: real-time multi-user collaboration, cross-device sync semantics, and SurrealDB-backed Loom view engines remain Phase 4.
- [ADD v02.131][REFINE v02.197] Stage future items: third-party Stage App marketplace/extensions and cross-device/multi-user Stage sessions remain Phase 3+ / Phase 4.
- [ADD v02.197] Studio Module future items: full visual authoring and advanced 3D editing (Spline-class) belong to Studio Module rather than Stage.
...
- [ADD v02.123][REFINE v02.204] Implement Lens persistence contract extensions against freshly initialized SurrealDB authority, including EventLedger persistence: deterministic ArtifactStore layout for Derived Fact projections, SurrealKit schema rollouts, and replayable QueryPlan/Trace storage. Retire SQLite/PostgreSQL connectivity, dependencies, configuration, schema paths, and proof paths completely; no database migration or reconciliation input exists.
...
- **[ADD v02.131][REFINE v02.197] Stage-to-Studio provenance review baseline**  
  Extend Stage from capture-only into provenance-first visual/3D review: deterministic analysis/validation/canonicalization jobs produce reports and patch sets; a Stage App can review patch sets and request governed apply/handoff jobs into workspace artifacts or Studio Module state (no silent asset edits, no embedded full authoring editor).
...
10. **[ADD v02.131][REFINE v02.197] Stage-to-Studio baseline (provenance-first 3D review)**  
   - Provide a Stage visual review surface (Stage App) that can open 3D assets + reports, show `scene_ir` diffs, and review patch sets before requesting governed apply/handoff jobs; Studio owns visual edit state.  
...
- [ADD v02.186][REFINE v02.204] KERNEL-004 cluster D â€” Memory V0+ self-improvement loop on fixed ~30-item HBR corpus (Â§4.8.3 + FEMS V1); distillation pipeline opt-in + content review (Â§4.8); ProcessOwnershipLedger SurrealDB table for ALL Handshake-spawned processes (Â§5.7); ModelManual surface (Â§10.15).
- [ADD v02.196] KERNEL-015 builds the diagnostic three-tier base: a Handshake-native in-process self-diagnostics tool (internal_diagnostics, Â§5.8 -- panic record, UI-thread heartbeat into a shared-memory ring, frame-time + CPU/RSS/GPU counters, open typed-allowlist diagnostic-event API) plus an external out-of-process Palmistry watcher (Â§6.13 -- Crashpad-model sibling that survives parent death, reads the heartbeat ring with zero cooperation, detects freezes via ~5s heartbeat staleness + OS hung-window probe, captures out-of-process minidumps, persists a durable survivor store, forwards recovered records into the Flight Recorder), and the backend-down freeze fix (the native app degrades, never freezes the frame loop, when a backend route is unreachable). The Flight Recorder stays the Tier 1 business-event ledger, unchanged.
- [ADD v02.196] KERNEL-016 retrofits the base: hook up EVERY already-built feature, back-end call site, front-end widget, and GUI action to internal_diagnostics + Palmistry so nothing already shipped is left un-instrumented. Governance enforcement (HBR-INT-009 v1.5.0 + Codex CX-981) makes the growth and use of internal_diagnostics + Palmistry MANDATORY for all future builds.
...
- [ADD v02.131][REFINE v02.197] Stage-to-Studio review: a 3D report + patchset can be reviewed in a Stage App; an approved apply/handoff job produces a new derived asset or Studio handoff artifact with before/after hashes and provenance; allow/deny and mutations are logged in Flight Recorder/Job History.
...
- [ADD v02.131][REFINE v02.197] Studio Module owns full authoring (Spline/Figma-class), complex rigging/animation editors, and collaborative real-time 3D editing; Stage remains the browser/webviewer, capture, inspection, provenance, and handoff surface.
...
- [ADD v02.116] **Locus Work Tracking System (Â§2.3.15) - Phase 3:**
  - Implement SurrealDB authority with workspace multi-tenancy enforced by authenticated record-user permissions intersected with ResourceBroker scope (Â§2.3.15.5, Â§2.3.15.8)
  - Implement CRDT operation-based conflict resolution with vector clocks (Â§2.3.15.6)
  - Implement WebSocket real-time collaboration (SurrealDB live queries + WebSocket broadcast) (Â§2.3.15.8)
...
- **[ADD v02.131][REFINE v02.197] Make Stage collaboration-safe + extension-ready**  
  Stage sessions, Stage Apps, and Stage Bridge must work in multi-user workspaces: per-user capability grants, audit logs, hash-pinned Stage App bundles, and handoff records into Studio Module. Spline/Figma-class authoring evolves under Studio Module, not inside Stage.
...
12. **Loom collaboration + shared libraries** [ADD v02.130]  
   ...
   - Scale Loom view/search for shared workspaces using SurrealDB search/vector indexes while preserving local-first projections when offline.
...
- [ADD v02.79][REFINE v02.197] Studio Module / Photo Studio Phase 4: full Affinity/Illustrator parity; advanced compositor; full marketplace.
...
- [ADD v02.130][REFINE v02.204] Loom shared-workspace query scaling: SurrealDB search/vector indexes and background rebuild jobs for metrics/backlinks at scale.
...
## 7.6.10 Account-Resource Privacy Delivery Sequence [ADD v02.202]

Account and resource privacy starts in Kernel V1 rather than waiting for cloud collaboration:

1. **WP-KERNEL-006** establishes LocalAccount credentials/sessions, Principal identity, AccountRole, actor attribution, SurrealDB authenticated record-user context, foundational ownership fields, default-deny table/field permissions intersected with ResourceBroker scope, and break-glass/key-vault seams while retaining the original Principal/KernelActor/flight-recording scope. Root/system users are administration-only and cannot serve as privacy proof.
2. **WP-KERNEL-007** establishes AccessSpace, ProtectedResource, ResourceGrant, MembershipRole, ResourceBroker, same-project private resources, visitor passes, MCP ToolGate/fingerprint policy, and the ExternalIdentityBinding/remote-service import seam while retaining the original visitor-pass/MCP-gate scope.
3. **WP-KERNEL-008** establishes Persona and CRM/FEMS personalization that is strictly subordinate to LocalAccount and resource authorization; it retains preference/CRM/FEMS switching scope without using Persona as a grant boundary.
4. Later shared-workspace, network, and Handshake SaaS/MCP WPs reuse these identifiers, grants, lineage, revocation, and proof contracts rather than adding a second cloud-only authorization model.

No phase may ship a new ProtectedResource producer or consumer without stable resource linkage and executable privacy proof. Multi-user UI can arrive later; multi-user-safe data and authorization contracts cannot.
