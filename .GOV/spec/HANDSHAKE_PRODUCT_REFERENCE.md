...
- **Coverage basis:** 5,005 source-distilled feature rows — 2,730 help-TOC leaf rows plus a 2,275-row deep-delta pass (Photoshop 573, Illustrator 447, InDesign 415, Affinity 440, Figma 400) covering complete tool sets, menu trees, filter/effect catalogs, adjustment/blend-mode enumerations, Camera Raw/Develop control sets, panel inventories, dialog/preference catalogs, and scripting/plugin/REST API domains.
- **No double features:** shared capability across source apps maps to ONE Studio primitive (cross-app dedupe map + generated deep-delta overlap map, 159 overlap groups); source-app rows remain provenance variants only. Vendor names never become Studio tool names.
