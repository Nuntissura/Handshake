...
- **Coverage basis:** 5,034 source-distilled feature rows — 2,730 help-TOC leaf rows plus a 2,304-row deep-delta pass (Photoshop 576, Illustrator 447, InDesign 416, Affinity 440, Figma 425) covering complete tool sets, menu trees, filter/effect catalogs, adjustment/blend-mode enumerations, Camera Raw/Develop control sets, panel inventories, dialog/preference catalogs, and scripting/plugin/REST API domains. Every deep-delta row is source-confirmed VERIFIED except 4 documented-unreachable residuals (3 InDesign menu enumerations needing installed-app capture, 1 vendor-undocumented Figma Motion persistence posture), each carrying a `residual_reason`. Two completeness-audit rounds (Adobe-centric top-level classes, then app-unique + cross-cutting depth across 130+ surface classes) returned dry after filling Figma Motion, three Photoshop ecosystem-posture rows, and InDesign Balance Ragged Lines.
