...
| 18 | Studio | Visual creation module - ONE unified operator-facing creative surface targeting full Photoshop/Illustrator/InDesign/Affinity-suite/Figma-class parity (raster, vector, page layout, design systems/prototyping, whiteboard), plus Canvas, Lens, and future visual tools | Native-Rust shared primitives (one primitive per shared capability across source apps — no per-vendor silos), persona-style task modes over one document model, kernel binding per the Tailor creative-module pattern; research corpus + integration architecture in `.GOV/reference/studio_app_feature_research/` (see §5.2) | §6.3.3.5, §10.10 |
...
### 5.2 Studio — Unified Creative Suite **(RESEARCH/DESIGN STAGE — spec enrichment + WP pending)**

Studio (pillar #18) is being expanded into the Handshake-native unified creative suite: ONE operator-facing application with full feature parity against Photoshop + Camera Raw, Illustrator, InDesign, the Affinity V2 suite, and the Figma family — built as native Rust tools/primitives, offline, no external dependencies. Authority for this scope will come from a future Master Spec enrichment; the current basis is the research corpus at `.GOV/reference/studio_app_feature_research/` (reference only, not product law).

- **Coverage basis:** 5,060 source-distilled feature rows — 2,730 help-TOC leaf rows plus a 2,330-row deep-delta pass (Photoshop 577, Illustrator 448, InDesign 416, Affinity 464, Figma 425) covering complete tool sets, menu trees, filter/effect catalogs, adjustment/blend-mode enumerations, Camera Raw/Develop control sets, panel inventories, dialog/preference catalogs, and scripting/plugin/REST API domains. Every deep-delta row is source-confirmed VERIFIED except 4 documented-unreachable residuals (3 InDesign menu enumerations needing installed-app capture, 1 vendor-undocumented Figma Motion persistence posture), each carrying a `residual_reason`. Three completeness-audit rounds with distinct decompositions (top-level classes; app-unique + cross-cutting depth across 130+ classes; 2024–2026 recency) reached convergence after filling Figma Motion, three Photoshop posture rows, InDesign Balance Ragged Lines, Photoshop Adjustment Brush, Illustrator Objects on Path, and Affinity's v3.0 unified relaunch.
- **Field validation of the unification goal:** the recency audit found Affinity itself shipped a **v3.0 unified relaunch (Oct 2025)** that folds its former three apps into ONE app with Vector/Pixel/Layout studios over a single `.af` document — the same architecture Studio targets. Affinity gates that app behind a required online Canva account; Studio's unification stays fully local-first with no account or sign-in.
- **No double features:** shared capability across source apps maps to ONE Studio primitive (cross-app dedupe map + generated deep-delta overlap map, 160 overlap groups); source-app rows remain provenance variants only. Vendor names never become Studio tool names.
- **Operator unification:** persona-style task modes (photo / vector / page-layout / design-system) over one document model, one selection model, one undo/history surface, one color pipeline, one asset home (Loom), one export surface.
- **Kernel binding (Tailor pattern):** typed EventLedger `STUDIO_*` events before authority rows, per-file CRDT documents, sandbox → validation → PromotionGate for all model-authored edits, renders/exports as scheduler jobs, three-tier diagnostics (Flight Recorder wired; internal_diagnostics/Palmistry typed-DEFERRED until shipped), Argus targets on every surface.
- **Model workflow:** models see Studio in backend (typed document/command API with dry-run + receipts) and frontend (stable `author_id` targets, visual steering); parallel models across files and on one file (CRDT presence + leases); a propose-work system (StudioEditProposal → DCC Approval Inbox → promotion); mandatory visual inspection of both structured diffs and rendered output; per-file history/undo with ≥1-level revert-of-undo; all agent activity headless and QUIET — never steals focus or conflicts with concurrent operator work.
- **UserManual:** dual-audience entries for every tool/command/panel — operator layer (task-oriented, minimally technical) + model layer (command ids, typed I/O, receipts, undo semantics, Argus targets, failure/recovery) — generated from the corpus manual-handoff index and kept same-change current per HBR-MAN.
- **File compatibility:** existing creative formats (PSD, AI, IDML/PDF, Affinity natives, .fig family, SVG, EXR, …) are import/export/round-trip targets with fixtures and unsupported-feature receipts; no invented replacement interchange format.
