...
body_sha256: "60c391aaa46c76d031c4fe1f1a3c35fed5fd36f326f15d831b61eabb966b588e"
...
This section is product LAW. It attaches to the kernel exactly like the Tailor creative module (Section 13): a `handshake_core::studio` domain module bound to PostgreSQL/EventLedger authority, CRDT collaboration, the sandbox -> validation -> `PromotionGate` lifecycle, and model lanes, plus standalone UI-agnostic Rust engine crates reached only through typed traits. The canonical authority for every Studio type, field, unit, event, schema-id, table, migration, validation check, and promotion-equivalence rule is sub-section 14.23 (Canonical Studio Authority Contracts); where any other sub-section conflicts with 14.23, 14.23 wins.
...
[STU-ARC-004] PostgreSQL tables for Studio MUST use `TEXT PRIMARY KEY` with prefixed string IDs (e.g. `document_id = "SDOC-{uuid_v7}"`, `layer_id = "SLYR-{uuid_v7}"`, `artboard_id = "SART-{uuid_v7}"`), following the codebase convention. `UUID PRIMARY KEY DEFAULT gen_random_uuid()` is off-convention and MUST NOT be used for new Studio tables. Every Studio authority-table row MUST carry an `event_ledger_event_id TEXT NOT NULL` foreign key to `kernel_event_ledger.event_id`, and every INSERT or mutating UPDATE MUST be preceded by a `guard_authority_write(AuthorityMode::Postgres)` call enforcing the `no_sqlite_tripwire`. Migrations use the dated convention `YYYY_MM_DD_studio_<topic>.sql` + `.down.sql`; numbered `0NNN_*` migrations are forbidden. The canonical table set is defined in 14.23.

[STU-ARC-005] A model-authored Studio mutation (any command batch that changes document authority) MUST NOT be written directly to a Postgres authority row. It MUST enter the kernel sandbox (`SandboxAdapter`, process-tier by default), be validated by the `StudioValidationDescriptor` catalog (14.24), and pass the `PromotionGate` (`PromotionDecisionV1: Accepted`) before authority rows change. This lifecycle is not optional and MUST NOT be bypassed regardless of model confidence (mirrors [TAI-OVR-004]). See 14.18 for the propose-work system that carries model edits through this lifecycle.
...
This sub-section is the single canonical authority for every Studio type, field, unit, event variant, schema id, table, migration, and promotion rule. Where any other Studio sub-section conflicts with 14.23, 14.23 wins.
...
[STU-VAL-005] NATIVE-RUNTIME + POSTGRESQL/CRDT DUTY. Studio durable authority is Handshake-managed PostgreSQL/EventLedger; live collaborative state is CRDT with persistence/reconnect/replay/conflict-visibility and promotion into authority. No SQLite, no SQL-portability shim, no mock-only resource as default proof, and no Docker/third-party-daemon/outside-app as a core-operation dependency or acceptance shortcut. Test-only/fixture-only proof cannot satisfy a runtime/storage/EventLedger/UI/replay MUST.
