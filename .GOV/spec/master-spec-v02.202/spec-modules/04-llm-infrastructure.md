...
body_sha256: "29b7609fdac1515bd1c6a857d051f3c85cd60f54c3a9000dfe1f1d3b0715aba5"
...
##### 4.3.9.2.5 Model lane exchange contract (normative) [ADD v02.197]

Modern Handshake swarm execution treats every local model, cloud model, CLI model, human operator lane, validator, and subagent as a **ModelLane**. Dexterity is the internal model switching and launch kernel that owns this lane lifecycle. A ModelLane is not a peer-to-peer chat endpoint. It is a coordinator-owned execution lane that reads a declared ContextBundle, emits typed messages and artifacts, and can be resumed from canonical state. Dexterity is a kernel/manual name only; stable schema IDs remain `hsk.model_lane_run@1`, `hsk.model_lane@1`, and `hsk.model_lane_message@1`.
...
ModelLane:
  ...
  kind: ModelLaneKind
  role: string
  backend: ModelBackend | "cli" | "human" | "subagent" | "validator"
  model_id: ModelId | null
  session_id: string
  model_session_id: string
  adapter_id: string
  runtime_binding: "LOCAL" | "CLOUD" | "CLI_BRIDGE" | "HUMAN" | "SUBAGENT" | "VALIDATOR"
  launch_authority: "ModelRuntime" | "CloudLane" | "CliBridge" | "Operator" | "SubagentManager" | "ValidatorRunner"
  capability_token_ids: string[]
  effective_capability_snapshot_ref: string | null
  projection_plan_ref: string | null
  consent_receipt_ref: string | null
  status: "PLANNED" | "STARTING" | "READY" | "RUNNING" | "WAITING" | "BLOCKED" | "DONE" | "FAILED" | "CANCELLED"
  recovery_state: "NONE" | "RESTARTABLE" | "STALE_LEASE" | "ORPHANED" | "MISSING_PAYLOAD" | "REPLAY_BLOCKED" | "RECLAIMED"
  heartbeat_at_utc: string | null
  lease_expires_at_utc: string | null
  reclaim_after_utc: string | null
  restart_generation: integer
  cancellation_ref: string | null
  backpressure_ref: string | null
  loop_counter_ref: string | null
  last_runtime_status_ref: string | null
  last_recovery_event_ref: string | null
  failstate_code: string | null
  reason_ref: string | null
  recovery_hint_ref: string | null
  wp_id: string | null
  mt_id: string | null
  work_packet_id: string | null
  micro_task_id: string | null
  owner_session: string
  locus_binding_ref: string
...
ModelLaneRun:
  ...
  lane_ids: string[]
  event_ledger_scope: string
  event_ledger_stream_id: string
  event_ledger_append_id: string | null
  event_ledger_seq: integer | null
  event_stream_version: integer | null
  transaction_seq: integer | null
  ...
  replay_order_key: string
  replay_after_event_ledger_seq: integer | null
  recovery_state: "NONE" | "RESTARTABLE" | "STALE_LEASE" | "ORPHANED" | "MISSING_PAYLOAD" | "REPLAY_BLOCKED" | "RECLAIMED"
  failstate_code: string | null
  reason_ref: string | null
  recovery_hint_ref: string | null
...
ModelLaneMessage:
  ...
  run_id: string
  trace_id: string
  message_span_id: string
  parent_span_id: string | null
  linked_span_contexts: string[]
  from_lane_id: string
  to_lane_id: string | "BROADCAST" | "COORDINATOR"
  kind: "REQUEST" | "PROPOSAL" | "ANSWER" | "CRITIQUE" | "HANDOFF" | "VOTE" | "VALIDATION" | "ERROR" | "STATUS"
  payload_ref: string
  payload_sha256: string
  event_ledger_stream_id: string
  event_ledger_event_id: string
  event_ledger_seq: integer
  event_stream_version: integer
  transaction_seq: integer
  summary: string
  authority: "ADVISORY" | "PROMOTED_TO_CONTRACT" | "PROMOTED_TO_CODE" | "VALIDATOR_VERDICT"
  promotion_gate_ref: string | null
  promotion_receipt_ref: string | null
  validator_verdict_ref: string | null
  operator_decision_ref: string | null
  promoted_artifact_ref: string | null
  promoted_artifact_sha256: string | null
  promoted_artifact_version: string | null
  tool_gate_decision_refs: string[]
  coordinator_session_id: string
  work_packet_id: string | null
  micro_task_id: string | null
  task_board_id: string | null
  ...
  replay_order_key: string
  replay_after_event_ledger_seq: integer | null
  crdt_update_ref: string | null
  crdt_base_snapshot_ref: string | null
  crdt_state_vector: string | null
  crdt_proposal_ref: string | null
  crdt_stale_base_ref: string | null
  failstate_code: string | null
  reason_ref: string | null
  recovery_hint_ref: string | null
...
Launch requirements:
- Dexterity MUST record every model switch, launch, relaunch, and lane-status transition through ModelLaneRun, ModelLane, ModelLaneMessage, PostgreSQL, and EventLedger state. It MUST NOT rely on provider memory, terminal scrollback, or chat history for launch authority.
- Product request sources, including IPC commands, scheduled spin-ups, operator actions, and future native GUI controls, MAY request lane launch, but MUST NOT become launch authority. Before any request reaches runtime creation it MUST attach a coordinator-generated Dexterity launch contract, resolve through `SwarmCoordinator::spawn_session`, persist ModelLane/EventLedger state, and fail closed if the ModelLaneStore/PostgreSQL authority path is unavailable.
- Local model lanes MUST launch through `LlmClient -> ModelRuntime` for authoritative local execution. Compatibility endpoints remain `ExternalEngineImport` lanes and MUST NOT become hidden ModelRuntime authorities.
- Cloud model lanes MUST launch through the CloudLane/BYOK boundary and MUST carry the selected BYOK/cloud provider identity plus the applicable ProjectionPlan and ConsentReceipt when workspace data leaves the local boundary.
- CLI model lanes MUST use the CLI bridge boundary and MUST be observable as ModelLane records, not opaque background terminals.
- Human operator lanes MUST use `launch_authority="Operator"`, `runtime_binding="HUMAN"`, and `backend="human"` and MUST still emit ModelLane status and message records when they participate in a coordinated run.
- Subagents MUST be represented as ModelLane records with stable lane ids, session ids, role identity, and restart/reclaim status.
- Validator lanes MUST use `launch_authority="ValidatorRunner"`, `runtime_binding="VALIDATOR"`, and `backend="validator"` and MUST emit validator verdict refs instead of directly mutating advisory lane output into authority.

Communication requirements:
- Models MUST NOT communicate by private peer-to-peer side channels, hidden chat state, raw terminal scraping, or untracked clipboard/session context.
- All lane-to-lane exchange MUST flow through coordinator-mediated ModelLaneMessage records persisted in EventLedger. Non-status payload bytes MUST be addressed through `payload_ref` and `payload_sha256` against ArtifactStore and/or ContextBundle authority. Role Mailbox MAY carry a handoff notification that cites `message_id` and EventLedger refs, but it MUST NOT become payload or promotion authority.
- A model output remains advisory until a coordinator, operator, or validator promotes it into code, a WP/MT contract, a validation verdict, or another authoritative artifact.
- If two lanes disagree, the conflict MUST be represented as typed messages plus an explicit coordinator decision; later lanes MUST receive the selected decision through a ContextBundle rather than through conversational memory.
- Every ModelLaneRun MUST be recoverable from canonical records: run metadata, lane records, messages, payload artifact refs, status markers, and Flight Recorder correlation.
- Every lane run, lane, and message MUST preserve trace/span correlation (`trace_id`, `run_span_id`, `lane_span_id`, `message_span_id`, `parent_span_id` or `linked_span_contexts`) and task ownership (`work_packet_id`, `micro_task_id`, `owner_session`) without relying on Flight Recorder payload prose.
- Lane launch MUST resolve a validated Locus binding before side effects. The binding MUST connect the coordinator session, model session, owner session, work packet, microtask, task board when present, adapter identity, runtime binding, and effective capability snapshot.
- Cloud lanes MUST carry `projection_plan_ref` and `consent_receipt_ref` before any workspace data leaves the local boundary; local, CLI, human, subagent, and validator lanes MUST leave these fields null unless a projection/consent boundary is explicitly invoked.
- Message replay MUST be deterministic: `idempotency_key`, `payload_sha256`, `replay_order_key`, `payload_ref`, and any `tool_gate_decision_refs` MUST be durable enough for PostgreSQL/EventLedger/ArtifactStore replay without provider memory, terminal scrollback, or chat history.
- Recovery diagnostics MUST be structured, not inferred from prose: `recovery_state`, `failstate_code`, `reason_ref`, and `recovery_hint_ref` MUST be populated when a lane is stale, orphaned, missing payloads, blocked from replay, reclaimed, failed, or cancelled so Argus/UserManual/debug surfaces can render operator action without parsing logs.
- Any `ModelLaneMessage.authority` value other than `ADVISORY` MUST carry the applicable `promotion_gate_ref`, `promotion_receipt_ref`, `validator_verdict_ref`, or `operator_decision_ref`, and that ref MUST resolve to EventLedger-backed promotion evidence. Advisory messages MUST NOT write authority directly.
- Deterministic replay MUST order lane runs and messages by EventLedger stream identity and append order: `event_ledger_stream_id`, `event_ledger_append_id` or `event_ledger_event_id`, `event_ledger_seq`, `event_stream_version`, `transaction_seq`, `replay_order_key`, and `replay_after_event_ledger_seq` MUST be stable enough to replay concurrent lane events without using `created_at_utc` as the ordering authority.
- Promotion replay MUST recover both the advisory message and the authority artifact. Promoted messages MUST carry `promoted_artifact_ref`, `promoted_artifact_sha256`, and `promoted_artifact_version` when they create or update code, WP/MT contracts, validator verdicts, or other authority artifacts.
- CRDT-targeting messages MUST carry `crdt_update_ref`, `crdt_base_snapshot_ref`, `crdt_state_vector`, `crdt_proposal_ref`, and `crdt_stale_base_ref` as applicable. A shared editable-state payload without the needed CRDT base/proposal metadata MUST fail closed before promotion.
- Lane reclaim and restart MUST be explicit through `heartbeat_at_utc`, `lease_expires_at_utc`, `reclaim_after_utc`, `restart_generation`, `last_runtime_status_ref`, `last_recovery_event_ref`, `cancellation_ref`, `backpressure_ref`, and `loop_counter_ref`. Stale-lane recovery MUST NOT be inferred from process presence alone.
- Side-effecting messages MUST carry immutable Locus binding fields (`coordinator_session_id`, `task_board_id`, `work_packet_id`, `micro_task_id`, `owner_session`, and `locus_binding_ref`) on the message record itself, even when those values also exist on the run.

Cloud ProjectionPlan / ConsentReceipt requirements (MT-006):
- Cloud/BYOK ModelLane launch MUST resolve durable PostgreSQL/EventLedger `ModelLaneCloudProjectionPlanRecord` and `ModelLaneCloudConsentReceiptRecord` authority before provider runtime creation. String refs, provider memory, chat history, UI state, and prompt-only consent are not authority.
- A cloud ProjectionPlan record MUST bind `projection_plan_id`, `projection_plan_hash`, `run_id`, `lane_id`, `model_session_id`, `provider_kind`, `requested_model_id`, `scope_hash`, source artifact refs, payload artifact ref/hash, redaction policy, retention policy, export posture, provider profile, fan-out targets, Locus ownership, EventLedger event id/seq, and a UserManual behavior ref.
- A cloud ConsentReceipt record MUST bind `consent_receipt_id`, `projection_plan_id`, `projection_plan_hash`, `run_id`, `lane_id`, `model_session_id`, `provider_kind`, `requested_model_id`, `scope_hash`, consent scope, retention policy, export posture, fan-out targets, validity window, approval/revocation state, Locus ownership, EventLedger event id/seq, and a UserManual behavior ref.
- `SwarmCoordinator::spawn_session` MUST run the cloud consent/projection preflight before any cloud provider factory call. Missing, expired, mismatched, or revoked consent MUST fail closed as `CX-MM-007`, MUST append a `model_lane_cloud_consent_denial` EventLedger row with `provider_call_attempted=false` and `partial_authority_state_created=false`, and MUST NOT create partial lane authority.
- Revoking a ConsentReceipt MUST cancel covered non-terminal cloud lanes, mark them cancelled or blocked with `failstate_code="CX-MM-007"` and typed `consent_status`, and append terminal EventLedger evidence. Repeating the same denial or revocation MUST be idempotent and replayable, not a new authority mutation.
- Cloud ModelLane output MUST remain `ADVISORY` or `PROMOTION_CANDIDATE` unless an approved PromotionGate path writes promoted authority. Cloud lanes MUST NOT self-assert `OPERATOR_DECISION` or `VALIDATOR_VERDICT`, and hidden provider/session refs such as `provider-session://` or `provider-memory://` MUST NOT enter payload, proposal, CRDT, ContextBundle, or promoted-artifact authority paths.
- MT-006 runtime proof wires EventLedger evidence for projection, consent, denial, advisory cloud output, and revocation terminal state. Direct typed Flight Recorder `FR-EVT-CLOUD-*` emission remains a separate observability wiring requirement and MUST NOT be claimed wired until product code emits and tests those events.

The in-app visual/debug surface for inspecting ModelLaneRun state MUST use the project-approved internal viewing-tool name from the owning viewing-tool worktree; this ModelLane WP does not define that name. The surface renders TraceProjection evidence for HBR-VIS checks: lane topology, messages, payload refs, tool calls, artifacts, validation, promotion state, runtime errors, and recovery hints.
...
- `CX-MM-006` ModelLaneMessage payload missing, corrupt, or not recoverable
- `CX-MM-007` Cloud ModelLane attempted without required ProjectionPlan or ConsentReceipt
- `CX-MM-008` Unauthorized peer-to-peer model channel attempted outside the coordinator
- `CX-MM-009` Subagent ModelLane orphaned or unrecoverable from canonical state
...
- Dexterity model switching/launching kernel (ModelLaneRun, lane launch/relaunch, ModelLaneMessage, PostgreSQL/EventLedger replay).
- Project-named internal viewing tool (in-app visual/debug view for ModelLaneRun, lane messages, artifacts, validation, promotion, and recovery).
...
## 4.9 Role Execution Identity and Profile Naming [ADD v02.202]

Every model or agent run uses a `RoleExecutionIdentity` containing `principal_id`, `principal_kind`, `delegating_principal_id`, `delegation_chain`, `session_id`, `local_account_id`, `access_space_id`, `work_profile_id`, `capability_profile_id`, task/WP/MT refs, grant-set version, issue/expiry times, and revocation epoch.

Spawned agents and local/remote model sessions receive the intersection of the delegator’s ResourceGrants, task resource set, selected AccessSpace, ToolGate capabilities, WorkProfile policy, and service/provider constraints. They cannot request, inherit, cache, or export broader visibility than the delegator has at execution time. Long-running runs revalidate bounded leases and stop or redact when authority is revoked.

Naming is fixed:

- `WorkProfile`: model choice, budgets, autonomy, routing, validation, and execution behavior.
- `CapabilityProfile`: tool/capability bundle.
- `Persona`: account-owned preferences, tone, presentation, and memory personalization.
- `AccessSpace`: selected view/context over authorized resources.

Persona and WorkProfile changes may affect behavior but never authorization. Model retrieval, prompt assembly, context caching, tool calls, generated artifacts, logs, and distillation candidates remain ProtectedResources and preserve source-scope intersection.
