...
spec_version: "v02.202"
bundle_id: "master-spec-v02.202"
module_id: "12"
section_id: "12"
...
body_sha256: "102b601ef65729fc97ed9fe45fa0786408890700da51af6dd4cd9b5cf13fe699"
...
```json
{
  ...
  "features": [
    ...
    },
    {
      ...
      "notes": "[ADD v02.198] Appendix backfill: Section 13 (Tailor) activated in v02.194 without appendix rows — recorded now as part of the v02.198 two-submodule expansion. MD-parity garment engine: XPBD GPU solver, pattern/seam authoring, fabric models, collision (exaggerated proportions), trims, UV/texture, animation, auto-fit, model-first MCP surface (six core tools + registered extensions), export lanes (glTF/Alembic-shim/USD/OBJ; FBX via Blender-bridge only). Second-pass parity CG-01..29 closed by MT-449..477. [ADD v02.201] Build-readiness adds version-pinned parity claims, one command/operation authority for native GUI and headless agents, stable TailorSurfaceDescriptor routes and control ids, recoverable TailorProject lifecycle, task-oriented UserManual coverage, and explicit shared-pillar projection. Parallel agents act through object leases, expected revisions, preview/apply/revert, attributable receipts, and stale-state rejection; no viewport-local or model-session state is authority."
    },
    {
      ...
      "notes": "[ADD v02.198] BodyKit submodule (operator fold-in directive 2026-07-08): decoupled region morph channels (breast volume/ptosis/firmness/profile independent of shoulders/hips/thighs/midriff per OBR-001..004), extreme ranges by design, native genital modules (vulva/penis/anus/futa, gape+erection continuums, arousal controller), skeleton with per-body re-baked RBF correctives, unified-solver soft tissue, skin/texture (UDIM, wet layers, biophysical tone), ARKit-52 face baseline, Cloth bridge (parametric avatars + channel-derived proxies), model-first body API (author_body..export_body), Blender/UE export lanes, acceptance archetypes ARCH-A..H, license guards (SMPL/Genesis/dhdm prohibited). [ADD v02.201] Build-readiness adds the shared version-pinned parity gate, command/operation authority, native surface descriptors, project lifecycle, UserManual, pillar links, and parallel-agent conflict/recovery law. PoseKit/OpenPose and character-sheet inputs remain immutable evidence or proposals: TailorMixGraph activation is fail-closed until the PoseKit source commit is merged and the pinned WP-KERNEL-012 compatibility receipt passes, and those inputs never become 3D pose, camera, contact, anatomy, cloth, or body authority."
    },
    {
...
```json
{
  ...
  "feature_links": [
    ...
    },
    {
      ...
      "coverage_status": "SEEDED",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.201/spec-modules/13-tailor-cloth-garment-engine.md (Cloth plus build-readiness hardening)",
        ".GOV/task_packets/WP-KERNEL-010-Tailor-Cloth-Garment-Engine-v1/_MT_INDEX.json (MT-001..782; ownership is contract-defined)",
    ...
    },
    {
      ...
      "coverage_status": "SEEDED",
      "coverage_refs": [
        ".GOV/spec/master-spec-v02.201/spec-modules/13-tailor-cloth-garment-engine.md (BodyKit plus build-readiness hardening)",
        ".GOV/task_packets/WP-KERNEL-010-Tailor-Cloth-Garment-Engine-v1/_MT_INDEX.json (MT-478..782; ownership is contract-defined)",
        ".GOV/task_packets/WP-KERNEL-010-Tailor-Cloth-Garment-Engine-v1/WP-KERNEL-010-Tailor-Cloth-Garment-Engine-v1.build-readiness-prework.json",
...
```json
{
  ...
  "ui_guidance": [
    ...
    {
      "feature_id": "FEAT-TAILOR-BODYKIT",
      "user_goal": "Build, pose, stage, validate, render, and publish production bodies through independent region and anatomy controls, explicit contact/physics diagnostics, and one native GUI/headless command authority that remains usable by operators and parallel agents.",
      "entry_points": [
        "Native module switcher > Tailor > Projects > BodyKit",
        "BodyKit task switcher > Shape, Anatomy, Rig/Figure Setup, Pose, Face, Contact, Skin/Materials, Groom, Animation, Render, or Publish",
        "Archetype/recipe/smart-content browser with ARCH-A..H seeds, compatibility, dependencies, and provenance",
        "Command palette, MCP, or backend action by canonical stable Tailor action_id",
      ...
      "telemetry": [
        "Every authoritative mutation and job emits one EventLedger identity projected unchanged into Flight Recorder with event_id, run_id, actor_id, correlation_id, action_id, object ids, revisions, and artifact refs.",
        "The native status strip and Diagnostics inspector expose corrective, soft-tissue, collision/contact, strain/inversion, renderer, export, recovery, and Palmistry watcher state without requiring console scraping.",
        "Locus receives optional QA/repair/production projections; Loom receives stable body, recipe, material, pose, mix-graph, capture, and evidence refs/backlinks; Calendar receives ActivitySpan/SessionSpan projections and optional production-block association. None becomes BodyKit authority.",
        "Parallel-agent occupancy shows actor, leased object scope, lease expiry, base revision, current command, conflict state, and recover/resume action.",
        "TAILOR_BODY_* events cover channel, recipe, anatomy, contact, mix, material, animation, render, and export operations; QA matrices persist as validation records and captures."
      ],
      "tests": [
        "Argus/AccessKit enumerates every TailorSurfaceDescriptor route, pane, control, dynamic entity key, command binding, help anchor, permission, undo policy, and receipt link without foreground focus theft.",
