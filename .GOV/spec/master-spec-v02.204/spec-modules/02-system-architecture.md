...
body_sha256: "c8076bab97c8967311b4aad13498271efed91f1c43dcc73595fe28dab5238341"
...
**Jargon**  
- **Native Rust GUI** (current direction, Codex §0A [CX-008-VIS]): the desktop shell is a native Rust GUI with NO webview substrate, shipped as one bundled installer and fully user-owned. (**Tauri** — SUPERSEDED prior shell, retained only as reference — was a Rust webview framework using the system webview.)
- **Rust Coordinator**: The central Rust process managing data, CRDT state, workflows, and service connections.
- **Model Runtime Layer**: Handshake-native abstraction over in-process libraries, product-managed subprocesses, bundled/runtime-discovered open-source engines, and explicit operator-configured compatibility adapters.
...
- **Native Rust GUI** desktop shell (no webview substrate; supersedes the prior Tauri app + React front-end) [§0A CX-008-VIS].
...
- **Local document store** backed by Handshake-managed SurrealDB/EventLedger authority plus CRDT/write-box structures for collaborative draft state.
...
- Encapsulates calls from the coordinator into Handshake-owned ModelRuntime adapters and managed engines (for example llama.cpp/Candle/native ComfyUI-compatible graph execution), using in-process calls or product-owned IPC/HTTP/gRPC. Third-party model-server daemons are compatibility-only opt-ins.
...
- Stores state and history in **SurrealDB/EventLedger**, with Temporal-inspired durable execution and resumability.
...
â”‚ ModelRuntime      â”‚ â”‚ SurrealDB+CRDT â”‚ â”‚  (Sandboxed code)    â”‚
â”‚                   â”‚ â”‚                â”‚ â”‚                      â”‚
...
The Janitor service MUST NOT hold a direct reference to a database client. It MUST interact with the storage layer exclusively via the `Database` trait or a dedicated `JanitorStorage` interface. This ensures maintenance tasks remain isolated from client ownership while preserving SurrealDB/EventLedger as the product authority path.
...
#### 2.3.13.0 SurrealDB-exclusive authority supersession [ADD v02.204]

This v02.204 rule supersedes every earlier current-normative PostgreSQL physical-storage commitment in this module. Earlier PostgreSQL names, SQL examples, implementation types, packet names, and migration phases remain only as historical provenance until their contracts are remediated; they MUST NOT be interpreted as an allowed runtime, fallback, compatibility, fixture, test, cache, migration target, or second authority.

- Handshake runtime authority MUST be Handshake-managed SurrealDB accessed through the official SurrealDB Rust SDK. `EventLedger` remains the semantic append-only authority primitive; its durable records live in SurrealDB.
- The product MUST have no PostgreSQL runtime dependency, fallback, or dual-authority mode. Absence or failure of the configured SurrealDB authority MUST fail closed for startup and authoritative writes.
- Authority records MUST use typed records and `SCHEMAFULL` schema definitions. Storage clients, record IDs, query values, and transaction boundaries MUST remain encapsulated by the storage boundary rather than leaking through `AppState` or feature services.
- Protected tables MUST default-deny authenticated record users through SurrealDB table `PERMISSIONS`; field permissions MUST cover sensitive fields. Privacy proof MUST use authenticated record-user sessions and the Handshake ResourceBroker. Root, system, namespace, database-owner, or other privileged sessions bypass table permissions and MUST NOT be accepted as isolation proof.
- Durable change feeds and projection wakeups MUST use SurrealDB live queries where a live subscription is required. Recovery authority remains EventLedger replay; a live-query notification is never authority.
- Schema evolution MUST use SurrealKit rollout lifecycle (`start`, application cutover, `complete`, and tested rollback) with versioned, replay-safe transformations and independent schema/data verification.
- SurrealDB authority MUST be initialized fresh, with a cold EventLedger authority head and deterministic bootstrap records created directly in SurrealDB. Existing PostgreSQL state has no migration value and MUST NOT be read, imported, replayed, reconciled, replicated, dual-written, or connected during activation.
- PostgreSQL retirement is complete: remove its client/runtime dependencies, drivers, binaries, managed-process lifecycle, URLs, environment/config keys, credentials, schema/migration tooling, fixtures, health checks, tests, proof paths, and network connectivity. A PostgreSQL connection attempt from Handshake is a failing conformance defect.

All later references in this module to PostgreSQL/EventLedger authority, `PgPool`, `PostgresDatabase`, `sqlx`, JSONB, PostgreSQL RLS, `LISTEN/NOTIFY`, PostgreSQL migrations, PostgreSQL URLs, or PostgreSQL test provisioning resolve to the SurrealDB contracts above unless the text explicitly labels the reference as historical provenance.

**Why**
Handshake's local-first philosophy (Â§1.1.4) targets SurrealDB/EventLedger authority from the start. Building storage-boundary constraints prevents rework while avoiding stale SQLite, PostgreSQL, Docker, or outside-app fallback paths.

**What**
Defines four mandatory architectural pillars for SurrealDB/EventLedger authority: single storage API, typed schema rollouts, rebuildable indexes, and live SurrealDB/EventLedger testing.

**Jargon**
- **Storage API**: Single module (`storage::`) that all database access flows through.
- **Database Abstraction Layer (DAL)**: Interface hiding concrete pool ownership and connection details from business logic.
- **SurrealDB-safe Schema**: `SCHEMAFULL` definitions and SurrealKit rollouts written for the SurrealDB/EventLedger authority path without alternate-backend fallbacks.
- **Rebuildable Index**: Optimization artifact that can be regenerated (not migrated row-by-row).
- **Authority-path Testing**: Validating code against real Handshake-managed SurrealDB/EventLedger through the official Rust SDK.

---

#### 2.3.13.1 Four Portability Pillars [CX-DBP-002]

This section defines four mandatory architectural constraints that keep storage behavior centralized, replayable, and testable on the SurrealDB/EventLedger authority path.
...
- FORBIDDEN: Direct SurrealDB SDK queries in API handlers or feature services
...
- REQUIRED: AppState MUST NOT expose a raw SurrealDB client, legacy `PgPool`/`SqlitePool`, or `DuckDbConnection`

**Rationale:**
If database access is scattered across handlers, routes, and services, authority, replay, and isolation become impossible to prove. Centralizing through a single API module keeps SurrealDB/EventLedger behavior auditable and prevents fallback drift.
...
All schema changes MUST target SurrealDB/EventLedger authority. SQLite- and PostgreSQL-specific syntax, compatibility shims, and Docker-dependent proof assumptions are forbidden.

- FORBIDDEN: `strftime()`, SQLite datetime functions â†’ REQUIRED: Parameterized timestamps
- FORBIDDEN: Backend-specific positional placeholder or SQL dialect contracts outside the storage boundary
- FORBIDDEN: SQLite triggers with `OLD`/`NEW` semantics â†’ REQUIRED: Application-layer mutation tracking
- REQUIRED: Schema changes use SurrealKit rollout lifecycle with explicit start, application cutover, complete, rollback, and reconciliation proof
- REQUIRED: Authority tables use typed records and `SCHEMAFULL`; data transforms are versioned, bounded, replay-safe operations

**Rationale:**
Migrations are expensive to rewrite. Using one SurrealDB/EventLedger authority path prevents dual-backend drift. Application-layer mutation tracking remains required where datastore automation would obscure replay semantics.
...
**Rationale:**
If a migration requires preserving an index byte-for-byte across backend changes, the migration becomes brittle and hard to understand. Treating indexes as regenerable simplifies migrations and makes backend changes more maintainable.

---

**Pillar 4: SurrealDB/EventLedger Testing [CX-DBP-013] [UPDATED v02.204]**

SurrealDB/EventLedger is the Handshake authority path. Test infrastructure MUST prove storage behavior against real Handshake-managed SurrealDB. SQLite/PostgreSQL dual-backend expectations and Docker-launched database proof are stale guidance for Handshake product code.

- REQUIRED: Storage layer tests target SurrealDB/EventLedger authority.
- REQUIRED: CI includes a real Handshake-managed SurrealDB authority variant, not a mock, privileged-session-only, or alternate-backend substitute.
- REQUIRED: Privacy tests authenticate as record users and prove table/field permissions plus ResourceBroker scope; privileged sessions cannot supply this proof.
- REQUIRED: New storage features are tested against SurrealDB/EventLedger before merge.
- REQUIRED: Failure in the SurrealDB/EventLedger authority path blocks PR merge.

**Rationale:**
Testing against SQLite, PostgreSQL, mocks, or privileged-only SurrealDB sessions hides authority-path bugs. Testing real Handshake-managed SurrealDB/EventLedger catches product failures before merge.
...
**Authority Path (SurrealDB/EventLedger):**
```surql
-- Typed authority table; application code supplies timestamps and typed values.
DEFINE TABLE OVERWRITE ai_job SCHEMAFULL;
DEFINE FIELD OVERWRITE created_at ON TABLE ai_job TYPE datetime;
DEFINE FIELD OVERWRITE status ON TABLE ai_job TYPE string;
DEFINE INDEX OVERWRITE ai_job_status ON TABLE ai_job FIELDS status;
```
...
The `Database` trait MUST NOT expose any methods that return concrete, backend-specific types (for example a raw SurrealDB client, `SqlitePool`, `PgPool`, or `DuckDbConnection`). All implementations MUST encapsulate their internal clients and connection ownership. 
...
pub struct SurrealDatabase {
    // SurrealDB/EventLedger authority implementation using the official Rust SDK
}

impl Database for SurrealDatabase { /* ... implementation logic ... */ }
```
...
#### 2.3.13.4 SurrealKit Rollout Requirements [CX-DBP-022] [UPDATED v02.204]

Schema evolution MUST use versioned SurrealKit rollouts.

- REQUIRED: Every rollout declares start, application cutover, complete, and rollback procedures.
- REQUIRED: Every schema or data step is replay-safe and independently verified against the resulting SurrealDB schema and records.
- REQUIRED: `SCHEMAFULL` definitions, typed fields, indexes, events, and table/field permissions are version-controlled with the rollout.
- REQUIRED: Rollback is tested before merge and MUST preserve EventLedger ordering, stable record identifiers, and acknowledged writes.
- REQUIRED: Rollouts are tested against real Handshake-managed SurrealDB before merge.
- REQUIRED: SurrealDB begins from a fresh schema and cold EventLedger authority head; SurrealKit MUST NOT connect to, read, import, replay, reconcile, or translate a PostgreSQL database.
- REQUIRED: PostgreSQL migration tooling, drivers, URLs, configuration, credentials, fixtures, and connectivity are absent from the product and its proof environment.

**Rationale:**
This lifecycle makes SurrealDB/EventLedger authority the tested path and keeps application cutover, schema completion, and rollback explicit.
...
1. **WP-1-Storage-Abstraction-Layer**
   ...
   - **MANDATORY AUDIT**: The codebase MUST be scanned for raw SurrealDB SDK calls outside storage, and for legacy `sqlx::`, PostgreSQL, `PgPool`, and `SqlitePool` references.
     - **Acceptance Criteria**: Zero occurrences allowed outside of `src/backend/handshake_core/src/storage/`.
     - **Evidence**: targeted dependency and source scans return no legacy PostgreSQL/sqlx runtime path and no raw SurrealDB SDK access outside the storage boundary.
...
3. **WP-1-Migration-Framework** (historical packet name; SurrealDB remediation required)
   - Adopt SurrealKit rollout lifecycle
   - Translate authority schema into typed `SCHEMAFULL` SurrealQL definitions
   - Add rollout versioning and cutover state to the authority database
   - Enforce replay-safe migrations (heavy per-file) and tests (2.3.13.4.1)
   - Require concrete down migrations and tests (2.3.13.4.2)

4. **WP-1-PostgreSQL-EventLedger-Tests** [UPDATED v02.204] (historical packet name)
   - Supersede its completed PostgreSQL proof with real Handshake-managed SurrealDB/EventLedger test coverage.
   - Remove SQLite/PostgreSQL dual-backend and Docker-launched database assumptions from Handshake product tests.
   - Block PRs that fail on the SurrealDB/EventLedger authority path.
...
- **Future authority expansion**: Reuse existing storage traits + SurrealKit rollout framework for new SurrealDB/EventLedger domains without introducing SQLite, PostgreSQL, or Docker-backed proof paths.
...
[ADD v02.156] LoomBlock/LoomEdge records, LoomViewFilters, LoomSearchFilters, LoomBlockSearchResult, and LoomSourceAnchor are canonical backend library contracts. Their meaning MUST survive SurrealDB/EventLedger storage, export, and replay instead of being hidden behind view-only adapters.

**SurrealDB/EventLedger authority schema [UPDATED v02.204]**

```surql
DEFINE TABLE OVERWRITE loom_block SCHEMAFULL;
DEFINE FIELD OVERWRITE workspace ON TABLE loom_block TYPE record<workspace>;
DEFINE FIELD OVERWRITE content_type ON TABLE loom_block TYPE string;
DEFINE FIELD OVERWRITE document ON TABLE loom_block TYPE option<record<document>>;
DEFINE FIELD OVERWRITE asset ON TABLE loom_block TYPE option<record<asset>>;
DEFINE FIELD OVERWRITE title ON TABLE loom_block TYPE option<string>;
DEFINE FIELD OVERWRITE content_hash ON TABLE loom_block TYPE option<string>;
DEFINE FIELD OVERWRITE pinned ON TABLE loom_block TYPE bool DEFAULT false;
DEFINE FIELD OVERWRITE created_at ON TABLE loom_block TYPE datetime;
DEFINE FIELD OVERWRITE updated_at ON TABLE loom_block TYPE datetime;
DEFINE INDEX OVERWRITE loom_block_content_hash ON TABLE loom_block FIELDS workspace, content_hash;

DEFINE TABLE OVERWRITE loom_edge TYPE RELATION IN loom_block OUT loom_block SCHEMAFULL;
DEFINE FIELD OVERWRITE edge_type ON TABLE loom_edge TYPE string;
DEFINE FIELD OVERWRITE created_by ON TABLE loom_edge TYPE record<principal>;
DEFINE FIELD OVERWRITE crdt_site_id ON TABLE loom_edge TYPE option<string>;
DEFINE FIELD OVERWRITE source_anchor ON TABLE loom_edge FLEXIBLE TYPE option<object>;
DEFINE FIELD OVERWRITE created_at ON TABLE loom_edge TYPE datetime;

DEFINE TABLE OVERWRITE loom_block_metric SCHEMAFULL;
DEFINE FIELD OVERWRITE block ON TABLE loom_block_metric TYPE record<loom_block>;
DEFINE FIELD OVERWRITE backlink_count ON TABLE loom_block_metric TYPE int DEFAULT 0;
DEFINE FIELD OVERWRITE mention_count ON TABLE loom_block_metric TYPE int DEFAULT 0;
DEFINE FIELD OVERWRITE tag_count ON TABLE loom_block_metric TYPE int DEFAULT 0;
DEFINE FIELD OVERWRITE last_computed_at ON TABLE loom_block_metric TYPE datetime;
```
...
#### 2.3.13.8 SurrealDB/EventLedger Handshake-Native Authority Foundation [ADD v02.182] [UPDATED v02.204]

Handshake treats SurrealDB/EventLedger as the exclusive runtime authority for the self-hosted control plane and Kernel V1. This supersedes earlier PostgreSQL, dual-backend, SQLite cache/offline, and developer-container guidance for Handshake product authority. Local-first ergonomics remain product requirements, but authority, replay, validation, and proof come from Handshake-managed SurrealDB/EventLedger state.

Normative requirements:

- The self-hosted control plane MUST expose one `surrealdb_eventledger_authority` mode and make it the required product authority mode.
- Authoritative control-plane records MUST be written to SurrealDB/EventLedger through the official Rust SDK. This includes software-delivery runtime truth, ModelSession scheduling/queue state, workflow durable execution state, Locus work-tracking projection authority, Role Mailbox linked-control-plane state, Dev Command Center runtime projections, FEMS memory authority, and governed action/claim/lease/backpressure records.
- If no valid Handshake-managed SurrealDB connection is available, startup or the attempted authoritative write MUST fail closed. The runtime MUST NOT silently fall back to PostgreSQL, SQLite, Docker, outside apps, mock stores, fixture stores, provider chat history, or repo-governance artifacts.
- Handshake core operation MUST own the SurrealDB lifecycle, configuration, health, namespace/database selection, credentials by secret reference, and shutdown behavior. A manually launched database or third-party management app is not a core prerequisite.
- Historical packet names in the downstream list identify provenance only. Their completed PostgreSQL proof does not validate SurrealDB; affected behavior MUST be converted and reproved by the active SurrealDB authority packet without connecting to the old database.

Downstream activation split:

- `WP-1-Handshake-Managed-PostgreSQL-Test-Tooling-v1` is the historical packet name for the former PostgreSQL startup, reset, fixture, migration-smoke, and CI-test scope; none of those PostgreSQL surfaces remain allowed.
- `WP-1-Postgres-Control-Plane-Leases-Backpressure-v1` is the historical packet name for shared claim, lease, heartbeat, retry, dead-letter, and backpressure primitives.
- `WP-1-ModelSession-Postgres-Queue-Workers-v1` is the historical packet name for ModelSession queue workers, persisted messages, checkpoints, cancellation, and provider-profile persistence.
- `WP-1-FEMS-Postgres-Memory-Store-v1` is the historical packet name for FEMS memory records, memory packs, memory jobs, replay metadata, and parallel-safe memory writes.
- `WP-1-Workflow-Engine-Postgres-Durable-Execution-v1` is the historical packet name for workflow instance state, node execution checkpoints, retries, terminal outcomes, and crash-resume semantics.
- `WP-1-DCC-Postgres-Control-Plane-Projections-v1` is the historical packet name for Dev Command Center projections over runtime truth for sessions, queues, leases, workflows, memory jobs, and dead-letter states.
- Any prior SQLite or PostgreSQL authority wording is stale for Handshake product authority. Owning packets must convert affected surfaces to fresh SurrealDB/EventLedger authority or explicitly classify them as non-authoritative legacy product debt.
...
- A SurrealDB-backed append-only EventLedger for kernel task, session, tool, artifact, validation, and promotion events.
...
The Kernel V1 EventLedger is the product authority for the first kernel slice. SurrealDB live queries, process-local locks, UI state, and provider or framework traces MAY accelerate or display work, but recovery and validation MUST be possible from durable product records alone.

#### 2.3.13.10 Kernel V1 CRDT Workspace, Write Box, and Promotion Bridge [ADD v02.185]

Kernel V1 CRDT workspace state is pre-promotion working state. It MAY hold concurrent human and model drafts, advisory edits, review notes, and normalized proposed operations, but it is not authority until promotion commits through the SurrealDB EventLedger defined in Section 2.3.13.9.
...
#### 2.3.13.11 Project Knowledge Index and Rich Document Authority [ADD v02.192]

WP-KERNEL-009 introduces the ProjectKnowledgeIndex as a Kernel V1 product authority surface. It is not a wiki folder, markdown vault, sidecar paper set, browser cache, or repo-governance mirror. It is a SurrealDB/EventLedger-backed authority schema that lets Handshake index selected project roots, source files, media, rich documents, Loom blocks, claims, retrieval traces, UserManual entries, and validation receipts into one replayable product substrate.

The ProjectKnowledgeIndex MUST expose these canonical record families:

- `KnowledgeSource`: a registered project root, file, asset, rich document, Loom block, external import, or operator-provided artifact with stable source id, source kind, content hash, provenance, permission scope, redaction state, and last-index receipt.
- `KnowledgeSpan`: a byte, text, AST, media-time, page, cell, or rich-document range anchored to a KnowledgeSource. A span is the minimum citeable evidence unit for claims and graph edges.
- `KnowledgeEntity`: a typed symbol, concept, file, folder, project, person, role, task, API, schema, command, media object, manual entry, or product primitive detected from one or more spans.
- `KnowledgeEdge`: a typed relationship between entities or sources. Every edge MUST carry source span refs, extractor version, lifecycle state, confidence, and stable `relationship_id`.
- `KnowledgeClaim`: an assertion about a source, product behavior, task, or operator workflow. Claims MUST carry lifecycle state (`proposed`, `accepted`, `conflicted`, `retired`), evidence spans, conflict refs, and resolution receipts.
- `MemoryPassage`: a bounded passage eligible for model context. It is derived from sources and claims and MUST record ranking features, retrieval mode, freshness, and compaction policy.
- `RetrievalTrace`: a replayable record of `none`, `direct_load`, `exact_lookup`, `graph_traversal`, or `hybrid_rag` selection, including why broader retrieval was used or skipped.
- `RichDocument`: a versioned ProseMirror/Tiptap document JSON authority record, with schema version, CRDT snapshot refs, EventLedger promotion refs, and projection refs.
- `EditorCodeNode`: a Monaco-backed code block embedded in a RichDocument, with language id, worker/bundling requirements, source mapping, lint diagnostics, and round-trip hash.
- `UserManualRecord`: a no-context operator/model manual entry tied to real product commands, routes, IPC channels, schemas, recovery paths, and visual-debug anchors.

All ProjectKnowledgeIndex authority records MUST be durable SurrealDB/EventLedger records. Generated markdown, wiki pages, HTML, context bundles, trace reports, visual-debug exports, and operator summaries are projections only. Deleting or editing a projection MUST NOT mutate authority records. A projection MAY contain stable refs into authority records but MUST NOT become the only location where knowledge, claims, edges, or manual instructions exist.

SQLite, embedded local relational stores, browser storage, raw files under a generated wiki folder, Docker-only proof paths, external app state, external model-server state, and provider chat history MUST NOT satisfy ProjectKnowledgeIndex authority, replay, validation, cache, or fixture requirements. Earlier SQLite-compatible Loom/search/workflow examples are legacy references for unrelated product debt unless explicitly migrated through this section.

RichDocument and EditorCodeNode edits MAY use CRDT state for collaboration and pre-promotion drafting, but authority changes MUST flow through WriteBoxV1 and EventLedger promotion. AI edit proposals, graph mutation proposals, relationship extraction, auto-linking, auto-tagging, and manual edits MUST leave actor, source span, state-vector, validation, denial, or promotion receipts. A direct write to ProjectKnowledgeIndex authority records outside the catalog/write-box/promotion path is invalid.

ProjectKnowledgeIndex MUST provide backend navigation APIs for no-context local and cloud agents. A backend navigation call MUST carry actor id, session id, correlation id, target authority ref, intended operation, and a typed receipt. Screen-reading a generated wiki, relying on chat history, or scraping visible UI rows is not an acceptable backend navigation path when a structured authority route exists.

The first WP-KERNEL-009 vertical slice MUST prove: source registration, span extraction, stable relationship_id generation, claim lifecycle, direct/exact/graph retrieval traces, RichDocument round-trip with EditorCodeNode, UserManual lookup, projection rebuild, EventLedger replay, and visual-debug evidence for the operator-facing surfaces.

##### 2.3.13.11.1 Rich Document and Project Wiki Projection Contract [ADD v02.193]

WP-KERNEL-009 fixes the projection contract for rich documents and the project wiki compile layer over the authority records above:

- HTML is the PRIMARY file projection for RichDocument records. Exported HTML MUST carry data-hs-* attributes preserving typed link/embed/code-node semantics for near-lossless round-trip, with two export modes: self-contained (assets base64-inlined) and reference-linked. Markdown stays a deliberately lossy extractor; HTML import MUST sanitize fail-closed. Export files are projections and never authority. The editor-facing contract is Section 7.1.1.10.
- Compiled project wiki pages are typed projections compiled from ProjectKnowledgeIndex authority (code-index symbols/references/file lens, knowledge entities and edges, rich documents). Every compiled page MUST be stamped with the EventLedger source version and the exact cited-source set it compiled from, MUST cite entities/spans/symbols by id and content hash, and MUST be served with a machine-readable staleness verdict, fail-closed; pages without staleness stamping are forbidden to serve as fresh. Regeneration is incremental and budget-bounded with EventLedger receipts, and deleting every compiled page MUST leave authority byte-identical. The full contract is Section 10.12 Loom spec, Section 17.
- Retrieval MAY serve rich documents and wiki pages as markdown, HTML, or JSON per task cost; the serving projection MUST be recorded in RetrievalTrace and MUST NOT substitute for authority records.

#### 2.3.13.12 Local Account, Principal, Access Space, and Protected Resource Authority [ADD v02.202]

This section makes account-bound resource privacy a permanent Handshake architecture pillar. It governs local single-PC use now and must remain the same authorization foundation when Handshake adds networked co-authors, shared workspaces, remote models, and the planned Handshake SaaS/MCP service for moodboards, project starts, and inspiration collection. Later collaboration may add transports and identity providers; it MUST NOT replace the local resource identity or visibility model.

##### 2.3.13.12.1 Canonical concept separation

- **LocalAccount** is the credential holder who can authenticate on this Handshake installation. A LocalAccount is not a Persona, AccessSpace, project, or model role.
- **AuthenticatedSession** is a bounded login session pinned to one LocalAccount, one effective Principal, one selected AccessSpace, credential/authentication strength, issue/expiry times, and revocation epoch.
- **Principal** is the auditable actor on an operation. Principal kinds include `human_account`, `local_model`, `remote_model`, `spawned_agent`, `mcp_client`, `service_identity`, `visitor_pass`, and `break_glass_operator`.
- **AccountRole** is installation administration authority (`Owner`, `Admin`, or `Member`). AccountRole does not grant blanket content read access.
- **MembershipRole** is a role within a project, workspace, or collaboration boundary (`Owner`, `Manager`, `Editor`, `Commenter`, or `Viewer`). MembershipRole is an input to authorization, not proof that every resource inside the boundary is visible.
- **AccessSpace** is a named, selectable work context that filters and groups resources the current Principal is already allowed to access. Examples may include an all-authorized-resources master Space, a SFW Space, and a client-specific Space. AccessSpace MUST NOT grant authority by itself.
- **ProtectedResource** is any product object with visibility consequences.
- **ResourceGrant** is the durable authorization relationship that permits a Principal, LocalAccount, group, MembershipRole, service identity, or visitor pass to perform named actions on a ProtectedResource or an explicitly bounded resource set.
- **Persona** is an account-owned preference, behavior, presentation, and memory-personalization bundle. Persona selection MUST NOT widen resource visibility or replace a ResourceGrant.
- **WorkProfile** retains its established meaning for model routing, budgets, autonomy, and execution policy. **CapabilityProfile** retains its established meaning for tool/capability bundling. Neither term may be reused for login identity, content visibility, or Persona.
- **ExternalIdentityBinding** links a LocalAccount to a future remote Handshake/SaaS identity without changing local resource ownership. **RemoteServiceConnection** is a revocable, audience-bound connection to a remote service.

UI copy may call an AccessSpace a “Space” and a Persona a “Persona.” Product code, schemas, events, APIs, logs, manuals, and governance contracts MUST use the canonical terms above.

##### 2.3.13.12.2 Protected-resource envelope

Every durable or derived ProtectedResource MUST carry, directly or through a canonical registry row:

- stable `resource_id`, `resource_kind`, schema version, lifecycle state, and policy version;
- `owner_account_id`, `created_by_principal_id`, `created_in_session_id`, and current `access_space_id` or explicit Space links;
- project/workspace/client refs when applicable, without treating those refs as grants;
- parent/source resource refs and complete derivation lineage;
- ResourceGrant refs or a deterministic grant-set selector;
- storage locator or ArtifactStore manifest ref that does not expose unauthorized absolute paths;
- classification, retention/deletion state, and EventLedger/Flight Recorder correlation refs.

ProtectedResource includes, without limitation: projects, workspaces, folders, files, external roots, SurrealDB namespaces/databases/tables/records/record sets, CRDT documents and updates, rich documents, source records, assets, artifacts, previews, thumbnails, caches, indexes, embeddings, search results, retrieval traces, memory entries, model contexts, prompts, generated outputs, exports, backups, logs, traces, diagnostics, receipts, capabilities, tools, MCP resources/prompts, remote objects, and service-import manifests.

Missing owner or scope metadata MUST fail closed. “Unknown,” `NULL`, legacy, imported, orphaned, or failed-path resources are not public by default. Repair/quarantine tools may inspect them only through an explicit administrative capability that does not imply ordinary content access.

##### 2.3.13.12.3 Authorization law

1. Default visibility is private to the owning LocalAccount until an explicit ResourceGrant says otherwise.
2. Every request MUST authorize the authenticated Principal, current session, intended action, exact resource, current ResourceGrant state, MembershipRole where relevant, and any capability constraint. Client-supplied account, Space, project, role, tenant, path, or resource identifiers are selectors only and MUST NOT be trusted as authorization.
3. AccessSpace selection filters an already-authorized set. Creating, joining, or switching to a Space MUST NOT create ResourceGrants.
4. Project/workspace membership does not imply visibility of every folder, file, database row, asset, artifact, log, memory item, or derived object in that project. Same-project private resources are a mandatory supported case.
5. AccountRole administration and content access are separate. An Admin can manage accounts and policy without automatically reading private content. Owner emergency access requires a distinct break-glass path with reason, strong re-authentication, narrow scope, expiry, and immutable audit.
6. Authorization is deny-by-default and evaluated on every executable backend boundary. Hiding UI rows is a projection behavior, never the security boundary.
7. Denials MUST use constant-shape responses where practical so counts, identifiers, names, paths, thumbnails, timing, search facets, error variants, logs, or diagnostics do not reveal that a restricted resource exists.
8. Revocation, account disable, password/session reset, MembershipRole reduction, grant expiry, and Space switch MUST take effect within a bounded policy window and invalidate stale caches, model contexts, search results, open handles, exports, and remote delegation.

The “everything uses everything” principle means every authorized surface can interoperate through typed contracts. It never means every account, Principal, project member, Persona, Space, model, or service can read every resource.

##### 2.3.13.12.4 SurrealDB enforcement [UPDATED v02.204]

SurrealDB/EventLedger plus ResourceBroker is authorization authority for database-backed resources:

- ownership and grants use stable UUID v7 identifiers and typed record links;
- protected tables are `SCHEMAFULL` and default-deny record users with explicit table `PERMISSIONS`; sensitive fields carry explicit field permissions;
- authenticated record-user context is established before protected queries and is bound to account/principal/session/Space claims validated by ResourceBroker;
- permissions cover selection, creation, update, deletion, upsert/conflict paths, bulk operations, background jobs, maintenance/rebuild jobs, and failure recovery;
- permission expressions and ResourceBroker checks fail closed for missing, malformed, expired, revoked, or mismatched context;
- root, system, namespace-owner, database-owner, and other privileged SurrealDB sessions MUST NOT execute ordinary protected-resource flows or count as privacy/isolation proof because privileged users bypass table permissions;
- EventLedger, Flight Recorder, diagnostics, outbox, search/index, and projection tables are protected resources and cannot become side-channel bypasses.

Application authorization remains required for action semantics and capability checks; record-user table/field permissions plus ResourceBroker are the non-bypassable product data boundary, not the only policy layer.

##### 2.3.13.12.5 Filesystem, ArtifactStore, and derived-resource enforcement

All protected filesystem access MUST flow through a ResourceBroker/ArtifactStore boundary that resolves a `resource_id` under authenticated context, re-evaluates grants, canonicalizes paths, rejects traversal/junction/symlink escapes, and records attribution. Raw arbitrary paths MUST NOT be accepted from UI, models, tools, plugins, MCP clients, or remote services as proof of authority.

External folders require explicit root registration, owner/scope linkage, and a brokered locator. OS ACLs and Windows-bound encryption may add defense in depth, but application isolation is the portable baseline. A Windows account that can bypass application storage remains outside the app-level guarantee unless files/secrets are encrypted to an account-bound key.

Derived resources inherit the intersection of their source restrictions. A thumbnail, index row, embedding, summary, generated asset, model context, log, export, backup, or remote copy MUST NOT widen visibility beyond any restricting source. Mixed-scope derivation must either keep the most restrictive effective scope, split outputs by compatible scope, redact unauthorized sources with explicit provenance, or refuse generation.

##### 2.3.13.12.6 Login, credentials, sessions, and attribution

Local password verifiers MUST use a memory-hard password hashing algorithm with per-account salt and versioned parameters; plaintext or reversibly encrypted passwords are forbidden. Secrets, refresh material, and encryption keys MUST use an OS-bound secret store such as Windows Data Protection API or an equivalent product-managed vault. Optional Windows Hello/passkey integration may strengthen login without replacing LocalAccount identity.

First run creates the installation Owner through an explicit setup ceremony. Login, logout, lock, re-authentication, session renewal, account disable, password change, role change, break-glass, AccessSpace switch, Persona switch, grant mutation, visitor-pass use, MCP use, and remote-service link/use MUST emit typed EventLedger/Flight Recorder events with `account_id`, `principal_id`, `session_id`, `access_space_id`, delegation chain, resource refs, action, result, and policy version. Logs and event viewers MUST themselves filter by ResourceGrant.

Spawned agents and model Principals receive the intersection of the delegating Principal’s current ResourceGrants, requested task scope, tool capabilities, and policy constraints. Delegation MUST never amplify authority. Every child action retains the full delegation chain.

##### 2.3.13.12.7 Future shared workspaces and Handshake SaaS/MCP continuity

Future collaboration and the planned Handshake SaaS/MCP service MUST reuse the same stable resource, actor, grant, lineage, and revocation concepts:

- remote tokens are audience-bound, least-privilege, expiring, revocable, and linked to an ExternalIdentityBinding;
- remote moodboards, project starters, and inspiration items arrive as signed/versioned manifests with remote resource IDs, tenant/account audience, source provenance, content hashes, requested local scope, and import state;
- downloaded or inspected remote content is quarantined until the operator chooses the destination LocalAccount/project/AccessSpace and the local ResourceGrant set;
- remote ACLs cannot silently widen local grants, and local grants cannot silently widen remote publication;
- sync conflicts preserve the stricter visibility until an authorized resolution;
- offline operation uses cached authorization only within a bounded, recorded lease and fails closed after lease expiry;
- MCP tools, resources, prompts, sampling, elicitation, and callbacks run as explicit service/client Principals through ToolGate and ResourceBroker, not as the logged-in human by assumption.

##### 2.3.13.12.8 Required proof

Every implementation touching ProtectedResource production or consumption MUST prove:

- same-account authorized create/read/update/delete and intended sharing;
- cross-account and cross-Space denial;
- same-project private-resource denial;
- stale, expired, revoked, disabled-account, and post-switch denial;
- metadata/existence non-disclosure through list, count, search, index, preview, cache, error, log, trace, and diagnostic paths;
- mixed-source derived-scope non-widening;
- table/field-permission and ResourceBroker non-bypass by record user/background worker, with privileged-session use rejected from ordinary flows and proof;
- filesystem traversal/symlink/junction escape denial where file roots are touched;
- concurrent grant/revoke/use behavior;
- remote token audience/expiry/revocation and import-quarantine behavior where SaaS/MCP is touched;
- no-context UserManual operation and Argus-visible current account/Space/Persona/share state for GUI changes.

Normative research basis: SurrealDB record access and table/field permissions; OWASP Authorization and Password Storage Cheat Sheets; Google Zanzibar; AWS SaaS Tenant Isolation and multi-tenant authorization guidance; the Model Context Protocol authorization specification; OpenFGA; Hugging Face organization/resource-group/token patterns; and Windows Data Protection API. These sources inform the architecture but do not replace the Handshake-specific rules above.
...
| SurrealDB | Current supported release | Handshake-managed local/shared authority storage |
...
- Implementations MUST use SurrealDB/EventLedger as the exclusive storage engine. Legacy SQLite or PostgreSQL state MUST NOT be read, imported, replayed, reconciled, translated, or connected; initialization creates fresh SurrealDB authority. Canonical work-tracking state SHOULD also be materializable as structured records at stable paths:
...
**Multi-user SurrealDB authority [UPDATED v02.204]**

```surql
DEFINE TABLE OVERWRITE workspace SCHEMAFULL;
DEFINE FIELD OVERWRITE name ON TABLE workspace TYPE string;
DEFINE FIELD OVERWRITE owner ON TABLE workspace TYPE record<principal>;
DEFINE FIELD OVERWRITE created_at ON TABLE workspace TYPE datetime;
DEFINE FIELD OVERWRITE replica_id ON TABLE workspace TYPE string;
DEFINE INDEX OVERWRITE workspace_replica_unique ON TABLE workspace FIELDS replica_id UNIQUE;

DEFINE TABLE OVERWRITE work_packet SCHEMAFULL;
DEFINE FIELD OVERWRITE workspace ON TABLE work_packet TYPE record<workspace>;
DEFINE FIELD OVERWRITE version ON TABLE work_packet TYPE int;
DEFINE FIELD OVERWRITE vector_clock ON TABLE work_packet FLEXIBLE TYPE object;
DEFINE FIELD OVERWRITE last_modified_by ON TABLE work_packet TYPE option<record<principal>>;

DEFINE TABLE OVERWRITE locus_event SCHEMAFULL;
DEFINE FIELD OVERWRITE workspace ON TABLE locus_event TYPE record<workspace>;
DEFINE FIELD OVERWRITE replica_id ON TABLE locus_event TYPE string;
DEFINE FIELD OVERWRITE sequence ON TABLE locus_event TYPE int;
DEFINE FIELD OVERWRITE event_type ON TABLE locus_event TYPE string;
DEFINE FIELD OVERWRITE event_data ON TABLE locus_event FLEXIBLE TYPE object;
DEFINE FIELD OVERWRITE vector_clock ON TABLE locus_event FLEXIBLE TYPE object;
DEFINE FIELD OVERWRITE created_at ON TABLE locus_event TYPE datetime;
```
...
broadcastChange(workspace_id: string, event: LocusEvent) {
  // 1. Persist the typed EventLedger record through the storage boundary.
  storage.appendLocusEvent(workspace_id, event);

  // 2. SurrealDB live query delivers a non-authoritative change notification.
  surrealLiveQuery.subscribe("locus_event", workspace_id);

  // 3. Broadcast the authorized projection via WebSocket.
...
**Scale targets:**
- SurrealDB baseline: 10,000 WPs per workspace, 50 dependencies per WP
- SurrealDB multi-user target: 100,000 WPs per workspace, 500 dependencies per WP, 100 concurrent users
...
3. Implementations MAY use SurrealDB live queries for non-authoritative wakeups
...
Handshake distinguishes between the **coordinator** (Rust/Tauri host) and the **Model Runtime Layer** (Handshake-owned engines and explicit compatibility adapters chosen through product configuration). The coordinator never talks to individual models directly; it always uses a small, stable internal API.
...
- Falling back to alternate Handshake-managed runtimes when possible (for example from a GPU runtime to a smaller CPU local model).
...
- **Model Runtime Layer (Handshake-owned engines)**
  - One or more Handshake-native engines or adapters based on open-source implementations (for example llama.cpp, Candle, native image graph execution, or product-managed ASR binaries).
  - May run in-process or as product-managed subprocesses with owned lifecycle, health checks, ports, logs, teardown, and ProcessOwnershipLedger rows. Outside model-server daemons are explicit compatibility adapters only.
...
- The orchestrator executes jobs using Handshake-managed local models by default.  
...
2.  **SentencePiece:** For Llama3/Mistral models served through Handshake ModelRuntime.
