...
body_sha256: "347008adbe67dd87cb012b5d1a4c6c5f0fa5404c1f005f66dcac10f40c4eac9a"
...
Launch requirements:
- Dexterity MUST record every model switch, launch, relaunch, and lane-status transition through ModelLaneRun, ModelLane, ModelLaneMessage, SurrealDB, and EventLedger state. It MUST NOT rely on provider memory, terminal scrollback, or chat history for launch authority.
- Product request sources, including IPC commands, scheduled spin-ups, operator actions, and future native GUI controls, MAY request lane launch, but MUST NOT become launch authority. Before any request reaches runtime creation it MUST attach a coordinator-generated Dexterity launch contract, resolve through `SwarmCoordinator::spawn_session`, persist ModelLane/EventLedger state, and fail closed if the ModelLaneStore/SurrealDB authority path is unavailable.
...
- Message replay MUST be deterministic: `idempotency_key`, `payload_sha256`, `replay_order_key`, `payload_ref`, and any `tool_gate_decision_refs` MUST be durable enough for SurrealDB/EventLedger/ArtifactStore replay without provider memory, terminal scrollback, or chat history.
...
- Cloud/BYOK ModelLane launch MUST resolve durable SurrealDB/EventLedger `ModelLaneCloudProjectionPlanRecord` and `ModelLaneCloudConsentReceiptRecord` authority before provider runtime creation. String refs, provider memory, chat history, UI state, and prompt-only consent are not authority.
...
- Dexterity model switching/launching kernel (ModelLaneRun, lane launch/relaunch, ModelLaneMessage, SurrealDB/EventLedger replay).
