...
body_sha256: "923e2cb7c3258a4cd12639581f87a03f02398e92f1aaf9b3dd99ec1d19065dd2"
...
#### 3.3.0.2 SurrealDB/EventLedger Authority + CRDT Draft State [UPDATED v02.204]

**Kernel V1 boundary [ADD v02.184] [UPDATED v02.204]:** The earlier SQLite and PostgreSQL guidance in this section is superseded for Handshake product authority. Runtime authority, cache, offline mode, compatibility mode, local fallback, bootstrap convenience, and test fixtures MUST use SurrealDB/EventLedger authority, CRDT/write-box draft state, ArtifactStore materialization, or rebuildable projections derived from those authorities. Kernel V1 scheduling, promotion, replay, validation, session brokering, and trace reconstruction MUST use the SurrealDB/EventLedger authority defined in Section 2.3.13.8. Handshake MUST use the official SurrealDB Rust SDK, typed `SCHEMAFULL` records, and SurrealKit rollouts; it MUST NOT retain a PostgreSQL runtime dependency, fallback, or dual authority.

```
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚               SurrealDB/EventLedger Authority                 â”‚
â”œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¤
â”‚ Type:         Managed SurrealDB + EventLedger authority     â”‚
...
**Why SurrealDB/EventLedger for this project:**
...
â”‚        â”‚ â€¢ Update SurrealDB/EventLedger projections         â”‚
â”‚        â”‚ â€¢ Update indexes                                   â”‚
â”‚        â–¼                                                    â”‚
â”‚  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”                                              â”‚
â”‚  â”‚ SurrealDB â”‚  â—„â”€â”€â”€ Handles: Authority, Query, Events     â”‚
â”‚  â”‚           â”‚                                              â”‚
â”‚  â””â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”˜                                              â”‚
â”‚        â”‚                                                    â”‚
â”‚        â”‚ Query results                                      â”‚
...
In SurrealDB/EventLedger + rebuildable projections:
...
1. USER MAKES EDIT (Device A)
   ...
   â”‚
   â”œâ”€â”€â–º SurrealDB/EventLedger authority/projections updated
   â”‚
...
2. SYNC UPDATE RECEIVED (Device B)
   ...
   â”‚
   â”œâ”€â”€â–º SurrealDB/EventLedger projections reflect merged state
   â”‚
...
**Key Takeaways**  
- CRDTs handle sync/merge; SurrealDB/EventLedger handles authority, query, replay, and provenance.
- CRDT stores draft content that needs to sync; SurrealDB/EventLedger stores promoted metadata, search inputs, and derived authority.
- On every promoted CRDT change, update SurrealDB/EventLedger and rebuild projections from authority.
- Start with file-based sync (OneDrive/Dropbox + CRDT); add WebSocket server later for real-time collaboration.
- SurrealDB/EventLedger is the required authority path; local-first ergonomics come from Handshake-managed lifecycle and rebuildable projections, not SQLite or PostgreSQL fallback.

---

Kernel V1 is excluded from the old SQLite/PostgreSQL local-first recommendations: its authority path is SurrealDB/EventLedger only.

Kernel V1 CRDT workspace addendum [ADD v02.185] [UPDATED v02.204]: Kernel V1 may use CRDT updates, snapshots, and state vectors for draft workspace collaboration, but those records remain pre-promotion working state. SurrealDB remains mandatory for EventLedger authority, promotion receipts, replay, and validation. CRDT storage MUST be restart-replayable, snapshot-safe, and joinable to write-box and promotion ids; it MUST NOT become a hidden SQLite or PostgreSQL authority path.
...
## 3.7 Account-Isolated Local and Remote Infrastructure [ADD v02.202]

- Handshake-managed SurrealDB MUST persist LocalAccount, Principal, AuthenticatedSession, AccessSpace, ProtectedResource, ResourceGrant, membership, Persona, external-identity, revocation, and audit state as typed `SCHEMAFULL` records. Protected access MUST be proven under authenticated record-user table/field permissions plus ResourceBroker; privileged SurrealDB sessions do not count as isolation proof.
- Protected tables use Row-Level Security plus `FORCE ROW LEVEL SECURITY`; the runtime database role is neither table owner nor `SUPERUSER`/`BYPASSRLS`.
- The server derives transaction-local authorization context from the authenticated session. UI, model, tool, plugin, MCP, and remote-service inputs cannot set effective account or grant context directly.
- Files, folders, assets, artifacts, previews, caches, indexes, exports, backups, and external roots are opened through ResourceBroker/ArtifactStore handles. Canonicalized paths, junctions, symlinks, traversal, stale handles, and post-revocation use are denied.
- Local application isolation is mandatory. Windows Data Protection API or an equivalent OS-bound vault protects credential-adjacent secrets and account encryption keys. OS-level separation/encryption is an optional stronger mode and must be described honestly in the UserManual.
- Collaboration and SaaS/MCP sync carry stable remote identity/resource refs, audience, expiry, revocation epoch, provenance, hashes, and local import scope. Remote objects remain quarantined until locally linked and granted.
- Search, embedding, graph, CRDT, background jobs, projection rebuilds, diagnostics, and backup/restore paths consume the same account/Principal/Space context; “internal worker” is not a bypass identity.
