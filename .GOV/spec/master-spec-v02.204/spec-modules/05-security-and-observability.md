...
- [ADD v02.186] The constrained runner is implemented over the `SandboxAdapter` trait (§3.5) and every spawned process is recorded in the `ProcessOwnershipLedger` (§5.7); see those sections for the normative contract.
...
- â­ Works with any Handshake-owned backend or explicit compatibility adapter that exposes metrics
...
- **Adapter**: Code that translates generic benchmark calls to Handshake ModelRuntime APIs or explicit operator-configured compatibility adapters.
...
- Compare Handshake-native runtimes and explicit compatibility adapters
...
â”‚                                                              â”‚
â”‚  ADAPTERS                                                    â”‚
â”‚  â”œâ”€â”€ ModelRuntimeAdapter Talks to Handshake ModelRuntime      â”‚
â”‚  â”œâ”€â”€ NativeEngineAdapter Talks to managed native engines       â”‚
â”‚  â”œâ”€â”€ CompatAdapter      Explicit opt-in external engines       â”‚
â”‚  â””â”€â”€ ImageAdapter       Talks to native image graph runtime    â”‚
â”‚                                                              â”‚
â”‚  RUNNERS                                                     â”‚
...
class ModelRuntimeAdapter(LLMAdapter):
    async def generate(self, prompt: str, params: dict) -> Result:
        start = time.time()
        response = await httpx.post(
            f"{self.endpoint}/v1/model-runtime/generate",
...
|----------------|-------------------|-------------|--------|--------|------------|
| single_chat    | mistral-7b-native | 340ms       | 320ms  | 450ms  | 88         |
| single_chat    | mistral-7b-gpu    | 310ms       | 300ms  | 420ms  | 97         |
| concurrent_8   | mistral-7b-gpu    | 180ms       | 170ms  | 250ms  | 620        |

### 5.5.5 Findings

- GPU-native runtime is ~10% faster for single requests
- GPU-native runtime scales much better under load (620 vs ~100 tokens/sec at 8 concurrent)
- CPU/native runtime shows consistent latency regardless of load when batching is unavailable

### 5.5.6 Recommendations

- Use Handshake-native CPU/runtime-discovered engines for development and single-user proof
- Use Handshake-native GPU-capable runtimes for production or multi-user scenarios; external servers remain explicit compatibility adapters
```

---

**Key Takeaways**  
- Benchmark harness enables reproducible comparison of Handshake-native runtimes, models, configurations, and explicit compatibility adapters.
...
- Key insight from example: GPU-capable runtimes can scale much better under load; benchmark evidence decides residency and routing policy.
...
**Why**
Handshake Build Rules (HBRs) are the cross-cutting acceptance gates that hold every WP to the same minimum bar for sandbox quietness, visual debugging, multi-model parallel coordination, and in-product UserManual currency/self-consistency. Pre-v02.186 they were enforced ad-hoc through validator prose. This section [ADD v02.186] makes `HANDSHAKE_BUILD_RULES.json` the single source-of-truth and pins HBR gate evaluation to two concrete enforcement points (build time + every governed inter-role handoff). The cross-anchor in the Handshake Codex is CX-131 (added Stage 3).

**What**
Defines the HBR source-of-truth file, the two HARD evaluation points, the typed receipt requirement, the rule-change procedure, and the gov-check tooling.

---

### 5.6.1 Single source-of-truth

`HANDSHAKE_BUILD_RULES.json` is the single source-of-truth for HBR-IDs across the INT, SWARM, VIS, QUIET, MAN, and STOP families (31 active rules at v1.6.0). Authority lives in `.GOV/roles_shared/records/HANDSHAKE_BUILD_RULES.json`. Any prose mention of an HBR-ID in spec, refinement, or work-packet documents resolves against this file. The MAN family includes HBR-MAN-004, which makes same-change UserManual updates mandatory for implementations that change Handshake behaviors, tools, features, primitives, workflows, model lanes, diagnostics, storage/event contracts, or operator/model navigation paths; affected entries must also carry Flight Recorder/EventLedger linkage and HBR-INT-009 Flight Recorder/internal_diagnostics/Palmistry posture.

### 5.6.2 Two HARD evaluation points

HBR gate evaluation HARD runs at:

- **(a) Build time** inside `just gov-check` -- the `hbr-matrix-check.mjs` sub-check (Section 5.6.5) iterates the active rule set and asserts every required artifact + evidence row is present.
- **(b) Handoff time** on every governed inter-role transition: refinement -> coder, coder -> WP_VALIDATOR, WP_VALIDATOR -> INTEGRATION_VALIDATOR, INTEGRATION_VALIDATOR -> ORCHESTRATOR. Each transition emits an `HBR_HANDOFF_GATE` event; a failed rule blocks the handoff.

Skipping either evaluation point is a spec defect.

### 5.6.3 Typed receipts (no prose)

HBR violations MUST emit typed `HBR_VIOLATION` receipts per the CX-130 typed-receipt schema. Receipt fields: `hbr_id`, `wp_id`, `mt_id`, `role`, `evaluation_point` (build|handoff), `evidence_pointer`, `violation_class`. **Free-form prose explanations are NOT the wire contract** -- they may accompany a typed receipt as a `notes` field but never replace it.

### 5.6.4 Rule-change procedure

Adding, retiring, or modifying an HBR rule is a **CX-105A spec edit operation**. A rule change MUST update **in the same change**:

- `HANDSHAKE_BUILD_RULES.json` (the source-of-truth row).
- This Section 5.6 (if the gate authority text changes).
- The CX-131 anchor in `Handshake_Codex_v1.4.md` plus any rule-specific Codex companion anchor when the rule adds new cross-cutting product law (for example CX-982 for HBR-MAN-004 UserManual currency).

A rule change that updates only the JSON file without the spec + Codex companions is rejected by `just gov-check`.

### 5.6.5 Tooling

- `hbr-matrix-check.mjs` -- gov-check sub-check that walks the active rule set and verifies acceptance evidence per row.
- Packet hydration -- automatic emission of applicable HBR rows into `PACKET_ACCEPTANCE_MATRIX` at WP draft time (no manual copy-paste).
- Validator scan -- WP_VALIDATOR + INTEGRATION_VALIDATOR scan-and-verify pass against the matrix.

**Cross-references:** CX-130 typed-receipt discipline; CX-131 HBR gate anchor; CX-982 UserManual currency; CX-105A spec edit law; `.GOV/roles_shared/records/HANDSHAKE_BUILD_RULES.json`; HBR families INT/SWARM/VIS/QUIET/MAN/STOP; HBR-INT-009; HBR-MAN-004; WP-KERNEL-004 refinement acceptance criteria AC-HBR-AUTHORITY, AC-HBR-HANDOFF-GATE, AC-HBR-TYPED-RECEIPT.
...
**Why**
Pre-v02.186 Handshake had no canonical answer to "who spawned this process?" -- a problem that compounds under multi-engine model runtimes (Section 4.6), per-job sandbox adapters (Section 3.5), ASR workers, ComfyUI workers, plugin processes, and helper subprocesses. Orphan-process audits, post-mortem reconstructions, and reclaim hooks all need a single ledger. This section [ADD v02.186] introduces `ProcessOwnershipLedger` as that ledger, now backed by SurrealDB/EventLedger (no SQLite or PostgreSQL runtime path, per CX-503R and Section 2.3.13.0).

**What**
Defines the ledger primitive, the SurrealDB-backed typed schema, the HARD scope (every Handshake-spawned process), the perf-mitigated write path, the validator query surface, and the reclaim-hook integration.

---

### 5.7.1 Primitive + schema

Primitive: `ProcessOwnershipLedger`, backed by the `SCHEMAFULL` SurrealDB table `kernel_process_lifecycle`. Typed fields:

- `process_uuid` -- UUID v7 per HBR-INT-008 (time-ordered).
- `parent_session_id` -- governed session that spawned the process.
- `sandbox_adapter_id` -- which `SandboxAdapter` instance (Section 3.5.4) owns the isolation context.
- `engine_kind` -- enum (`llama_cpp_runtime`, `candle_runtime`, `mechanical_job`, `asr_worker`, `comfyui_worker`, `sandbox_container`, `plugin_process`, `helper_subprocess`, `abliteration_tool`, `distillation_candidate_generator`, ...).
- `started_at`, `stopped_at` -- timestamps.
- `exit_code` -- nullable.
- `model_artifact_sha256` -- nullable; populated for model-engine rows.
- `work_profile_id` -- governing Work Profile (Section 4.3.7).
- `owner_role` -- governed role that requested the spawn.
- `owner_wp` -- WP id.

### 5.7.2 HARD scope: every Handshake-spawned process

**HARD** (operator-binding, 2026-05-18; Q-PROCESS-LEDGER-SCOPE = ALL Handshake-spawned processes):

Every process spawned by Handshake -- model engine, mechanical job, ASR worker, ComfyUI worker, sandbox container, plugin process, helper subprocess -- MUST write START + STOP rows. **NO exceptions.** A spawn path that bypasses the ledger is a spec defect.

### 5.7.3 Perf safeguards (HARD)

The write path MUST be performance-safe (operator-binding):

- **Batched async writes** via a bounded mpsc channel (the spawn-thread never blocks on a SurrealDB round-trip).
- **Capped at N writes/sec** (configurable per Work Profile; default conservative).
- **Bypass-on-overflow** -- if the bounded channel saturates, the writer logs `FR-EVT-LEDGER-OVERFLOW` (degraded mode receipt) instead of silently dropping rows. Degraded-mode flag surfaces in the Diagnostics panel (Section 10.12).

HBR-SWARM-001 / HBR-SWARM-002 invariants apply (no unbounded loops, no silent loss).

### 5.7.4 Validator query surface

The ledger is the canonical answer for "who spawned what" in post-mortems and orphan-process audits. WP_VALIDATOR + INTEGRATION_VALIDATOR query patterns:

- "List all child processes of session X" -- `WHERE parent_session_id = X`.
- "Find orphan processes (started, never stopped, last_seen > T)" -- `WHERE stopped_at IS NULL AND started_at < T`.
- "Attribute resource cost per Work Profile" -- aggregate by `work_profile_id`.

### 5.7.5 Reclaim hooks

Reclaim hooks fire on session close, session failure, session staleness, and operator cancel (per CX-503D + CX-212G TERMINAL_HYGIENE). On fire, the reclaim hook:

- Queries `kernel_process_lifecycle` for sessions's non-stopped rows.
- Calls `SandboxAdapter.kill` for each.
- Writes STOP rows with `exit_code = -1` and `engine_kind` unchanged.

### 5.7.6 Modification of Section 5.2.5

Section 5.2.5 (Mechanical Runner Sandbox) is updated in v02.186 to add a 1-line pointer to Section 3.5 SandboxAdapter trait and Section 5.7 ProcessOwnershipLedger. The substantive content of Section 5.2.5 is unchanged; the pointer makes the cross-reference explicit so a no-context reader does not miss the normative contract.

**Cross-references:** Section 3.5 SandboxAdapter; Section 3.6 LocalModel boxing invariant; Section 5.2.5 Mechanical Runner Sandbox (1-line pointer added); Section 6.7 swarm harness (consumer); HBR-QUIET-003; HBR-SWARM-001 / HBR-SWARM-002; CX-212G TERMINAL_HYGIENE; CX-503D reclaim hooks; CX-503R no SQLite; HBR-INT-008 UUID v7; WP-KERNEL-004 refinement acceptance criteria AC-PROCESS-LEDGER-PRIMITIVE, AC-PROCESS-LEDGER-SCOPE, AC-PROCESS-LEDGER-PERF, AC-PROCESS-LEDGER-RECLAIM.

---

## 5.8 internal_diagnostics (Handshake-Native Internal Diagnostics) [ADD v02.196]

**Why**
The Flight Recorder (the backend business-event ledger; Section 5.3 observability and the replay/audit path) was historically expected to double as Handshake's internal self-diagnostics surface, but in practice it never filled that role: it records governed business events for replay and audit, not the in-process health of the native egui application itself. When a backend route was unreachable or the UI thread stalled, there was no first-party, in-process tool that captured the application's own liveness, panic, and resource state. This section [ADD v02.196] decouples the diagnostic responsibility into a three-tier model and introduces `internal_diagnostics` as the Handshake-native INTERNAL tier -- the in-process self-diagnostics role the Flight Recorder was meant to fill but never did. It SUPPLEMENTS the Flight Recorder; it does not change or replace it.

**What**
Defines the in-process diagnostics primitive that runs inside the native egui app, the components it owns (durable crash record, UI-thread heartbeat, frame-time and resource counters, and an open diagnostic-event API), the typed-allowlist privacy boundary, the in-app Diagnostics Panel consumer (Section 10.12), the backend-down graceful-degradation requirement, and the shared-memory ring it writes for the external Palmistry watcher (Section 6.13) to read.

---

### 5.8.1 Purpose and the three-tier decoupling

`internal_diagnostics` is the Handshake-native, in-process self-diagnostics tool for the native egui desktop application. It is the second of three diagnostic tiers:

- **Tier 1 -- Flight Recorder** (KEPT AS-IS): the backend BUSINESS-EVENT ledger for replay and audit. No schema change; it remains the authority for "what governed events happened."
- **Tier 2 -- internal_diagnostics** (NEW, this section): in-process application self-diagnostics -- liveness, panic, frame health, resource counters -- running inside the egui app.
- **Tier 3 -- Palmistry** (NEW, Section 6.13): an external, out-of-process watcher that survives Handshake freezes and crashes.

The decoupling is canonical in build_rule HBR-INT-009 (v1.5.0) and Codex anchor CX-981. Diagnostic responsibility is no longer overloaded onto the Flight Recorder.

### 5.8.2 Components

`internal_diagnostics` owns the following in-process components:

- **panic hook** -- a `panic::set_hook` handler that writes a durable local crash record (panic payload, location, backtrace, build id, last-N diagnostic events) to disk before the process tears down, so an in-process panic is never silent.
- **UI-thread heartbeat** -- a frame-loop counter plus a monotonic timestamp, written every frame into a shared-memory ring buffer. A stalled UI thread stops advancing the counter; the staleness is observable out-of-process without any cooperation from the (possibly frozen) thread.
- **Frame-time tracking** -- per-frame duration samples (min / max / percentile windows) to detect jank and slow frames.
- **Resource counters** -- CPU, RSS (resident memory), and GPU utilization counters sampled on a bounded cadence.
- **Open diagnostic-event API** -- any feature (backend call site, front-end widget, GUI action, mechanical job bridge) MAY call `internal_diagnostics.record(event)` to contribute a structured diagnostic event. The API is OPEN by design so nothing built is left un-instrumented.

### 5.8.3 Privacy: typed allowlist (no project or sensitive data)

`internal_diagnostics` records **NO project content and NO sensitive data**. Every diagnostic event is constrained to a **typed allowlist** of mechanical fields (counters, durations, enum states, standard mechanism names). The standard mechanism names are kept verbatim -- `heartbeat`, `minidump`, `watchdog`, `ring-buffer` -- because they are well-understood field-standard terms, not project data. A diagnostic-event API call that would carry project or sensitive payload is rejected at the allowlist boundary; this is a HARD invariant.

### 5.8.4 Consumption: in-app Diagnostics Panel

The in-app Diagnostics Panel (Section 10.12) is the operator-facing consumer of `internal_diagnostics` events. The panel surfaces the live heartbeat, frame-time stats, resource counters, and the last-N diagnostic events alongside the Tier 3 Palmistry freeze/crash records. `internal_diagnostics` is a producer; the panel is a projection over it and does not hold its own state authority.

### 5.8.5 Backend-down graceful degradation (HARD)

When a backend route is unreachable, the native app MUST **degrade, not freeze**. `internal_diagnostics` is the surface that makes this observable and enforceable: a backend-down condition is recorded as a typed diagnostic event, the UI thread keeps advancing its heartbeat (it never blocks the frame loop on a backend round-trip), and the affected surface renders a degraded state instead of hanging. A UI path that can freeze the frame loop on an unreachable backend is a spec defect.

### 5.8.6 Relationship to the Flight Recorder and Palmistry

`internal_diagnostics` **SUPPLEMENTS** the Flight Recorder; it does not modify or replace the Tier 1 business-event ledger. It writes the UI-thread heartbeat and the last-N diagnostic events into a **shared-memory ring buffer** that the external Palmistry watcher (Section 6.13) reads -- including while the UI thread is frozen, because the ring is written every frame and read with zero cooperation required from the writer. Recovered freeze/crash records captured by Palmistry are forwarded into the Flight Recorder once the app recovers (Section 6.13.7).

Built by WP-KERNEL-015 (the strong base) and retrofitted onto already-shipped work by WP-KERNEL-016.

**Cross-references:** Section 5.3 AI Observability (Flight Recorder business-event ledger; KEPT AS-IS); Section 6.13 Palmistry external watcher (ring-buffer reader + forwarder); Section 10.12 Diagnostics Panel (consumer); HBR-INT-009 v1.5.0 (three-tier decoupling, mandatory); Codex CX-981; HBR-QUIET-003 non-hijacking; HBR-SWARM-001 / HBR-SWARM-002 bounded-loop / no-silent-loss invariants; WP-KERNEL-015 (build), WP-KERNEL-016 (retrofit).

---

## 5.9 Authentication, Authorization, and Privacy Evidence [ADD v02.202]

Local credentials use a memory-hard password verifier with per-account salt and versioned parameters. Plaintext/reversible passwords are forbidden. Authentication, session renewal, sensitive account changes, break-glass access, service linking, and token issuance use rate limits, re-authentication where appropriate, bounded lifetime, revocation, and typed security events.

Authorization is default-deny and checked for every request. SurrealDB authenticated record-user table/field permissions and ResourceBroker enforcement protect data even when UI filters, API handlers, model prompts, background workers, or tool adapters are defective. Root, system, namespace-owner, database-owner, and other privileged sessions bypass table permissions and MUST NOT execute ordinary protected-resource flows or count as isolation proof. AccountRole, MembershipRole, AccessSpace, ResourceGrant, Persona, WorkProfile, and CapabilityProfile are distinct inputs and cannot substitute for one another.

Observability data is protected content. EventLedger, Flight Recorder, internal_diagnostics, Palmistry recovery records, logs, traces, metrics labels, debug bundles, screenshots, receipts, counts, and error payloads MUST carry account/Principal/session/Space/resource scope and be filtered before display, export, model retrieval, or remote transmission. Restricted names, paths, project existence, resource counts, thumbnails, and timing MUST not leak through a supposedly sanitized diagnostic surface.

Section 5.6 incorporates active HBR-PRIV-001 through HBR-PRIV-008 from `.GOV/roles_shared/records/HANDSHAKE_BUILD_RULES.json` v1.8.0. Packet/refinement/MT evidence must cover resource linkage, default-deny enforcement, same-project privacy, derivative non-widening, attribution, revocation/context switching, SaaS/MCP continuity, and UI/Argus visibility.

Required adversarial proof includes wrong account, wrong Space, project member without resource grant, disabled account, expired/revoked session, grant changed during use, stale search/cache/context, background worker without context, table/field-permission or ResourceBroker bypass attempt, privileged-session misuse, forged client scope, path traversal/junction/symlink escape, mixed-scope derivation, log/count/error inference, export/backup restore under a different account, and remote token audience/expiry/revocation. Database isolation proof MUST authenticate as record users; a privileged SurrealDB session is setup-only evidence and cannot prove privacy.
