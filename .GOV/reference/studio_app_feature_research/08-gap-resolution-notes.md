...
remaining_gaps:
  ...
  - id: "SFR-REMAINING-GAP-011"
    title: "Coverage matrix was blind to the deep-delta layer (silent-undercount mechanism)."
    status: "resolved"
    detail: "The coverage matrix (49-source-coverage-verification-matrix) previously consumed only the leaf indexes (2,730 rows); the 2,330 deep-delta rows (51-55) never entered it, so its missing_required_fields=0 was a green field-completeness signal about the leaf layer only. This was the single biggest silent-undercount mechanism (audit action A1)."
    resolution: "2026-07-21 (ACTION-A1): extended _tools/generate-source-coverage-verification-matrix.py to ingest the deep-delta records (51-55) with a schema-appropriate assessment (deep rows use a leaner id/name/app_behavior/primitive_domain/source_url/verification_status schema, NOT the leaf REQUIRED_FIELDS check). Regenerated 49 now audits 5,060 rows (2,730 leaf + 2,330 deep-delta), reports the two layers distinctly, and surfaces the 4 documented-unreachable deep rows as UNVERIFIED instead of hiding them. The summary carries a scope_and_honesty_note and a cannot_detect_missing_features caveat."
    residual_verification_needed: "None for layer coverage. A matrix over existing rows still cannot detect a MISSING feature (semantic omission); those remain tracked in 58 (feature gaps) and 59 (workflow needs)."
