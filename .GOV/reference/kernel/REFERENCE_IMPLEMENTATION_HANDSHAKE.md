...
Location note:
- This file lives under `/.GOV/reference/` because it is a non-authoritative reference implementation and inventory, not live kernel law.
...
5. Mechanical enforcement scripts + `justfile`
   - Deterministic checks and workflow gates implemented under role-owned/shared bundles (`.GOV/roles/*/{scripts,checks}/`, `.GOV/roles_shared/{scripts,checks}/`, `.GOV/roles_shared/scripts/hooks/`) and invoked via `just`.
...
- `.GOV/reference/PAST_WORK_INDEX.md`: archaeology pointers (note: this file currently contains stale references; see Section 8).
...
- `.GOV/roles_shared/scripts/spec-current-check.mjs`: enforces that SPEC_CURRENT points to the latest `Handshake_Master_Spec_v*.md` file by parsed version.
...
- `.GOV/roles_shared/checks/task-board-check.mjs`: enforces strict formatting for `## In Progress`, `## Done`, `## Superseded` entries.
...
- Mechanical enforcement: `.GOV/roles_shared/checks/refinement-check.mjs`
...
- `.GOV/roles_shared/validator_gates/{WP_ID}.json`: log of validation gate sequence (present -> acknowledge -> append -> commit).
...
Additionally, Validator uses a mechanical gate sequence (writes per WP to `.GOV/roles_shared/validator_gates/{WP_ID}.json`):
...
|---|---|---|---|
| `just record-refinement {WP_ID}` | `.GOV/roles/orchestrator/checks/orchestrator_gates.mjs refine` | refinement file, `.GOV/roles_shared/SPEC_CURRENT.md` (+ spec file) | `.GOV/roles/orchestrator/ORCHESTRATOR_GATES.json` |
| `just record-signature {WP_ID} {sig}` | `.GOV/roles/orchestrator/checks/orchestrator_gates.mjs sign` | `.GOV/roles/orchestrator/ORCHESTRATOR_GATES.json`, refinement file, `.GOV/roles_shared/SIGNATURE_AUDIT.md`, repo grep for one-time signature | refinement file (sets APPROVED + signature), `.GOV/roles_shared/SIGNATURE_AUDIT.md`, `.GOV/roles/orchestrator/ORCHESTRATOR_GATES.json` |
| `just worktree-add {WP_ID}` | `.GOV/roles_shared/scripts/topology/worktree-add.mjs` | git refs/worktree list | creates branch/worktree dir on disk (git operation) |
| `just record-prepare {WP_ID} ...` | `.GOV/roles/orchestrator/checks/orchestrator_gates.mjs prepare` | `.GOV/roles/orchestrator/ORCHESTRATOR_GATES.json`, git branch exists, `git worktree list --porcelain` | `.GOV/roles/orchestrator/ORCHESTRATOR_GATES.json` |
| `just create-task-packet {WP_ID}` | `.GOV/roles/orchestrator/scripts/create-task-packet.mjs` | refinement file, `.GOV/roles/orchestrator/ORCHESTRATOR_GATES.json`, `.GOV/roles_shared/SIGNATURE_AUDIT.md`, `.GOV/templates/TASK_PACKET_TEMPLATE.md`, `.GOV/roles_shared/SPEC_CURRENT.md` | `.GOV/task_packets/{WP_ID}.md` (or creates refinement scaffold and exits BLOCKED) |
| `just gate-check {WP_ID}` | `.GOV/roles_shared/checks/gate-check.mjs` | `.GOV/task_packets/{WP_ID}.md` | none |
| `just pre-work {WP_ID}` | `.GOV/roles/coder/checks/pre-work.mjs` (wraps `gate-check` + `pre-work-check` + CX-WT-001 evidence) | packet + refinement + signature audit; `.GOV/roles_shared/checks/cor701-spec.json`; git object DB for checkpoint commit gate | none |
| `just post-work {WP_ID}` | `.GOV/roles/coder/checks/post-work.mjs` (wraps `gate-check` + `post-work-check` + role mailbox export gate) | packet; git diff/index/worktree files; spec schema `cor701-spec.json` | none |
| `just cor701-sha <file>` | `.GOV/roles_shared/checks/cor701-sha.mjs` | git blobs (HEAD/INDEX) + worktree file | none |
| `just task-board-check` | `.GOV/roles_shared/checks/task-board-check.mjs` | `.GOV/roles_shared/TASK_BOARD.md` | none |
| `just task-packet-claim-check` | `.GOV/roles_shared/checks/task-packet-claim-check.mjs` | `.GOV/task_packets/*.md` | none |
| `just validator-gate-*` | `.GOV/roles/validator/checks/validator_gates.mjs` | `.GOV/roles_shared/validator_gates/{WP_ID}.json` (legacy archive: `.GOV/roles/validator/VALIDATOR_GATES.json`), (append gate checks packet exists) | `.GOV/roles_shared/validator_gates/{WP_ID}.json` |
...
- Spec schema: `.GOV/roles_shared/checks/cor701-spec.json`
- SHA helper: `.GOV/roles_shared/checks/cor701-sha.mjs`
- Enforcement: `.GOV/roles/coder/checks/post-work-check.mjs`
...
- `.GOV/roles_shared/scripts/topology/worktree-add.mjs` + `just worktree-add`: creates WP worktree/branch.
- `.GOV/roles_shared/checks/worktree-concurrency-check.mjs`: local-only check; requires linked worktrees when multiple WPs are in progress.
...
- `.GOV/roles_shared/scripts/hooks/pre-commit` messaging referenced "Codex v0.8" before the later governance-kernel conformance repair.
- `.GOV/task_packets/README.md` links to `Handshake Codex v0.8` (stale).
- `.GOV/reference/PAST_WORK_INDEX.md` references much older spec/codex versions (stale).
...
### 9.6 `.GOV/roles_shared/scripts/hooks/`
- `.GOV/roles_shared/scripts/hooks/pre-commit`
- root `.GOV/scripts/` is retired as an active implementation surface; role-owned tooling now lives under `.GOV/roles/*/{scripts,checks}/` and shared tooling under `.GOV/roles_shared/{scripts,checks}/`
...
- `.GOV/roles/coder/docs/CODER_IMPLEMENTATION_ROADMAP.md`
- `.GOV/roles/coder/CODER_PROTOCOL.md`
- `.GOV/roles/coder/docs/CODER_PROTOCOL_GAPS.md`
- `.GOV/roles/coder/docs/CODER_PROTOCOL_SCRUTINY.md`
- `.GOV/roles/coder/docs/CODER_RUBRIC.md`
- `.GOV/reference/master_spec/MASTER_SPEC_INTENT_AUDIT_v02.103.md`
- `.GOV/reference/master_spec/MASTER_SPEC_MVP_ROADMAP_AUDIT_v02.103.md`
- `.GOV/reference/master_spec/MASTER_SPEC_SECTION_DIGEST_v02.103.md`
- `.GOV/roles_shared/MIGRATION_GUIDE.md`
- `.GOV/roles/orchestrator/ORCHESTRATOR_GATES.json`
- `.GOV/roles/orchestrator/docs/ORCHESTRATOR_IMPLEMENTATION_ROADMAP.md`
- `.GOV/roles/orchestrator/docs/ORCHESTRATOR_PRIORITIES.md`
- `.GOV/roles/orchestrator/ORCHESTRATOR_PROTOCOL.md`
- `.GOV/roles/orchestrator/docs/ORCHESTRATOR_PROTOCOL_GAPS.md`
- `.GOV/roles/orchestrator/docs/ORCHESTRATOR_RUBRIC.md`
- `.GOV/roles_shared/OSS_REGISTER.md`
- `.GOV/roles_shared/OWNERSHIP.md`
- `.GOV/reference/PAST_WORK_INDEX.md`
- `.GOV/reference/master_spec/PHASE_1_EVIDENCE_MAP_v02.103.md`
- `.GOV/roles_shared/QUALITY_GATE.md`
- `.GOV/templates/REFINEMENT_TEMPLATE.md` (compat shim; canonical under `.GOV/templates/`)
- `.GOV/reference/master_spec/ROADMAP_SECTION_COVERAGE_MATRIX_v02.103.md`
- `.GOV/reference/master_spec/ROADMAP_SECTION_COVERAGE_MATRIX_v02.107.md`
- `.GOV/reference/master_spec/ROADMAP_VS_MASTER_SPEC_AUDIT_v02.102.md`
...
- `.GOV/reference/workflow_technical_paper.md`
