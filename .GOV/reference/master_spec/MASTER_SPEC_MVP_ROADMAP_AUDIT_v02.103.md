...
- Goal:
  ...
  2) Check whether the Roadmap (Spec Â§7.6) represents the Master Spec without loss of intent / hidden technical debt.
...
5) Deep inspection (non-Roadmap): section-by-section digest and normative scan:
   - Generated digest: `.GOV/roles_shared/MASTER_SPEC_SECTION_DIGEST_v02.103.md` (Purpose + line-numbered MUST/SHOULD requirements per `## X.Y` section; Roadmap pointer mentions by phase).
   - Generated Roadmap section coverage matrix: `.GOV/roles_shared/ROADMAP_SECTION_COVERAGE_MATRIX_v02.103.md` (All Main Body sections -> whether the Roadmap cites the literal section number).
   - Generated Phase 1 evidence map: `.GOV/roles_shared/PHASE_1_EVIDENCE_MAP_v02.103.md` (Phase 1 MUST deliver -> Main Body anchors -> Task Board WPs).

## 1) What is needed for an MVP (per Master Spec v02.103)

### 1.1 Product MVP definition location
- The spec's concrete definition of "Product MVP (Phase 1)" is in Spec Â§7.6.3 (Roadmap).
- Outside the Roadmap section, the only explicit "MVP + MUST/SHOULD/MUST NOT" clauses are in the ASR subsystem (Spec Â§6.2) and describe the ASR MVP boundaries (which are scheduled for Phase 3 in Â§7.6.5).

Implication:
- If you strictly ignore Â§7.6 (Roadmap), the spec does not currently provide a complete, MUST-level "Product MVP" checklist for the core product. The MVP checklist for the core product effectively lives in the Roadmap text.

### 1.2 Product MVP (Phase 1) - high-level MUST areas (Spec Â§7.6.3)
...
For the full normative checklist, refer directly to Spec Â§7.6.3.
...
- Spec Â§8.2 includes a "Technology Stack Summary" table that lists "Backend: Python (FastAPI)" and "AI Runtime: Ollama, ComfyUI".
...
- Spec Â§8.2's "Backend: Python (FastAPI)" does not match the current repo's backend language choice (Rust).
- Elsewhere, the spec does describe a Rust coordinator and allows for external services, so the simplest explanation is: Â§8.2 is stale or describes an optional/alternative component.
...
- If Â§8.2 is meant to be authoritative, it should be updated via the Spec Enrichment workflow (new spec version + `.GOV/roles_shared/SPEC_CURRENT.md` update + signature audit), not as an ad-hoc edit.
...
- All 6 WP IDs appear in the Roadmap (Â§7.6) and are represented in `.GOV/roles_shared/TASK_BOARD.md`.

Existing intent audits:
- `.GOV/roles_shared/ROADMAP_VS_MASTER_SPEC_AUDIT_v02.102.md` and `.GOV/roles_shared/MASTER_SPEC_INTENT_AUDIT_v02.103.md` document recent Roadmap pointer gap fixes (e.g., missing ANS-001 scheduling) and add a "no technical debt" phase-closure bullet to the Roadmap preamble.
...
- If a requirement exists only in Â§7.6 and not in the governing Main Body sections, then "ignore Roadmap" would lose intent.
...
- Sync topology diagram (Spec Â§3.3.2.2) uses:
  - "PHASE 1 (MVP): File Sync + CRDT"
  - "PHASE 2 (Multi-user): Central Sync Server"
- Plugin architecture appendix (Spec Â§8.6.3.4) uses:
  - "PHASE 1 (MVP): Internal Extension Points", "PHASE 2: User Scripts", "PHASE 3: Full Plugin System"

These are not necessarily wrong, but they are ambiguous because Â§7.6 defines Phase 1..4 for the whole product. This is a concrete place where roadmap-vs-spec planning can drift.

## 4) Are the phases utilized well? (high-level assessment)

Strengths:
- Â§7.6 defines an explicit Phase 0..4 sequence that covers the major technical subsystems (governance + diagnostics early; ingestion/ASR later; collaboration last).
...
## Addendum (v02.105) â€” Deeper inspection + Roadmap Coverage Matrix phase allocation

**Spec baseline updated:** `Handshake_Master_Spec_v02.105.md` (see `.GOV/roles_shared/SPEC_CURRENT.md`).

### What changed in v02.105 (roadmap governance)
- Â§7.6.1 Coverage Matrix rules tightened:
...
- Coverage Matrix now phase-allocates every non-Roadmap section (section-level determinism) across `P1â€“P4`.
- Roadmap phase Goal blocks now explicitly reference the Â§7.6.1 Coverage Matrix as the scheduling authority.

### Roadmap â†” Main Body coverage (global / all phases)
At section-level, **the Roadmap is now deterministically complete** because Â§7.6.1 lists every `## X.Y` outside Â§7.6 (plus top-level `# 9.`) exactly once and assigns it to one or more phases.

Deterministic matrix validation (v02.105; run 2026-01-11):
- Expected section rows: 68 (67 `## X.Y` excluding Â§7.6 + top-level `# 9.`)
- Matrix rows: 68
- Duplicates: 0
- Missing: 0
- Extra: 0
...
- The Roadmap section (Â§7.6) still contains many MUST-level statements (Phase "MUST deliver" lists). Governance currently treats Main Body (Â§1-Â§6, Â§9-Â§11) as the authority for "Done" (CX-598), and the Roadmap as scheduling/pointer. This means:
...
- Create an "Evidence Map" document that maps each Phase 1 Roadmap MUST-deliver item to:
  ...
  - the concrete WP ID(s) on `.GOV/roles_shared/TASK_BOARD.md`.
...
Status:
- Evidence map generated: `.GOV/roles_shared/PHASE_1_EVIDENCE_MAP_v02.103.md` (Phase 1 Roadmap MUST-deliver -> Main Body anchors -> Task Board WPs).
- Additive remediation stubs created for Ready-for-Dev FAIL items: see `.GOV/roles_shared/TASK_BOARD.md` + `.GOV/task_packets/stubs/`.

## 6) Deep inspection addendum (sections + normative density; Roadmap excluded)

### 6.1 Digest artifact
- `.GOV/roles_shared/MASTER_SPEC_SECTION_DIGEST_v02.103.md` is a deterministic extraction of:
