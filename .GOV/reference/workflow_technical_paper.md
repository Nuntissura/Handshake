...
- `.GOV/roles_shared/SPEC_CURRENT.md` - Pointer to active spec
- `Master_Spec_v1.0.md` - The authoritative specification
- `{Project} Codex v1.0.md` - Governance rules
- `.GOV/roles/orchestrator/ORCHESTRATOR_PROTOCOL.md` - Orchestrator workflow
- `.GOV/roles/coder/CODER_PROTOCOL.md` - Coder workflow
- `.GOV/roles/validator/VALIDATOR_PROTOCOL.md` - Validator workflow
- `.GOV/roles_shared/TASK_BOARD.md` - Master task tracking
- `.GOV/roles_shared/SIGNATURE_AUDIT.md` - Signature registry
- `.GOV/task_packets/TEMPLATE.md` - Packet template
- `.GOV/task_packets/WP-*.md` - Actual work packets
- `.GOV/scripts/validation/*.mjs` - Validator scripts (10-15 files)
- `.GOV/roles_shared/scripts/hooks/pre-commit` - Git hook
- `justfile` - Command wiring

## Core Principles

**1. Spec as Single Source of Truth**
- `.GOV/roles_shared/SPEC_CURRENT.md` is a pointer file naming the active Master Spec (e.g., `Master_Spec_v1.0.md`)
...
- Signatures are **one-time-use only**, recorded in `.GOV/roles_shared/SIGNATURE_AUDIT.md`, verified via grep: `grep -r "{signature}" . | grep -c signature_audit.md` must equal 1
...
**4. State Consistency (Packet â†” Task Board)**
...
- **Verification before delegation**: run `just pre-work WP-{id}` (blocks on failure); ensure `.GOV/roles_shared/TASK_BOARD.md` reflects Ready for Dev; lock packets with USER_SIGNATURE
...
### Macro 1: Ingest â†’ Normalize â†’ Index â†’ Enrich â†’ Present
...
### Macro 2: Extract â†’ Compute â†’ Visualize â†’ Export
...
### Macro 3: Detect â†’ Suggest â†’ Gate â†’ Apply
...
### Macro 4: Observe â†’ Diagnose â†’ Bundle â†’ Repair
...
- Updates `.GOV/roles_shared/TASK_BOARD.md` to Ready for Dev (atomic update: both packet and board)
...
**State Transitions**:
```
Backlog â†’ Ready-for-Dev â†’ In-Progress â†’ Ready-for-Validation â†’ Done (VALIDATED)
   â†“          â†“               â†“                  â†“                  â†“
...
1. DOES IT MODIFY CODE IN EXISTING PACKET?
   â”œâ”€ YES â†’ Is the packet still In-Progress (not yet VALIDATED)?
   â”‚        â”œâ”€ YES â†’ Add to existing packet as variant (WP-{id}-v2, new signature)
   â”‚        â””â”€ NO â†’ Create new packet (WP-{id+1})
   â””â”€ NO â†’ Proceed to step 2

2. IS IT A DEPENDENCY OF OTHER WORK?
   â”œâ”€ YES â†’ Create new packet; other packets reference it as blocker
   â”‚        Packet status starts as "Ready-for-Dev"
   â””â”€ NO â†’ Proceed to step 3

3. DOES IT REQUIRE SIGNIFICANT NEW FILES/MODULES?
   â”œâ”€ YES â†’ Create new packet (separates concerns)
   â””â”€ NO â†’ Could be part of existing packet (consolidate if same phase)

4. IS IT PART OF PHASE N CLOSURE GATES?
   â”œâ”€ YES â†’ Create new packet; list in TASK_BOARD Phase N section
   â””â”€ NO â†’ Create new packet or consolidate (estimate effort)
```

### Dependency Graph Representation

In `.GOV/roles_shared/TASK_BOARD.md`, explicitly list dependency edges:
...
**Script** `.GOV/scripts/validation/validator-dependency-graph.mjs`:
```javascript
...
if (cycles.length > 0) {
  console.error("âŒ Circular dependency detected:");
...
| User adds new requirement during WP-5-A implementation | Check if it modifies WP-5-A scope: if yes â†’ WP-5-A-v2 (new signature); if no â†’ new WP-5-B |
...
- âœ… WP-1-Storage-Abstraction-Layer (modifies src/storage/) + WP-2-UI-Frontend (modifies app/src/) â†’ No conflict
- âœ… WP-3-Auth (modifies src/api/auth.rs) + WP-4-Logging (modifies src/logging.rs) â†’ No conflict
- âœ… Same file but different functions: WP-5-Feature-A adds `fn feature_a()` + WP-6-Feature-B adds `fn feature_b()` in same file â†’ Git merge conflict-free if functions don't interact

### Conflicting (Serialize or Risk)
- âŒ WP-7-Database-Schema-v1 (adds columns) + WP-8-Database-Schema-v2 (modifies same columns) â†’ Serialize: WP-8 blocked until WP-7 VALIDATED
- âŒ WP-9-API-Contract (changes endpoint signature) + WP-10-Client (calls that endpoint) â†’ Serialize: WP-10 blocked until WP-9 VALIDATED
- âŒ Both WP-11 and WP-12 modify configuration structure â†’ Coordinate via Task Board; one becomes dependency
...
paths_wp1=$(grep -A 10 "^IN_SCOPE_PATHS:" .GOV/task_packets/WP-{id1}.md | grep "^  -" | awk '{print $2}')
paths_wp2=$(grep -A 10 "^IN_SCOPE_PATHS:" .GOV/task_packets/WP-{id2}.md | grep "^  -" | awk '{print $2}')

# Check for overlaps
overlap=$(comm -12 <(echo "$paths_wp1" | sort) <(echo "$paths_wp2" | sort))

if [ -n "$overlap" ]; then
  echo "âš ï¸  Conflict: Both packets modify:"
  echo "$overlap"
...
3. **Merge sequentially**: WP-1 â†’ main, then WP-2 â†’ main (or both â†’ feature branch â†’ main if high confidence)
...
|----------|--------|--------|--------|
| **Clarifying existing anchor** (e.g., "A3.2.1 OAuth: what counts as 'authenticated'?") | âœ… Yes | Removes ambiguity; doesn't change requirement | Enrich â†’ create v1.1 â†’ run validator-spec-regression â†’ obtain signature |
| **Adding missing anchor** (user says "we also need rate limiting" but spec doesn't cover it) | âœ… Yes | New requirement, not change to existing | Enrich â†’ add A5.1.2 Rate Limiting â†’ run validator-spec-regression â†’ obtain signature |
| **Rewriting existing anchor** (e.g., "A3.2.1 said 'email auth only', now add social login") | âŒ No | Changes requirement after user signed off on DONE_MEANS | Escalate to user; new signature required; check if existing packets affected |
| **Changing DONE_MEANS after signature** (Orchestrator adds new checkpoint mid-work) | âŒ No | Violates Signature Lock principle; scope creep | Create WP-{id}-v2 with new signature; don't modify original |
| **Adding out-of-scope requirement as "nice-to-have"** (packet says "no caching", user asks for "optional Redis support") | âŒ No | Introduces uncertainty; optional features are scope creep | Document in packet OUT_OF_SCOPE; create separate WP if needed |
| **Removing anchor** (spec v1.0 required feature X, now X is obsolete) | âœ… Yes | Spec evolution; old anchor becomes "deprecated" | Create v2.0 with deprecation notice; run validator-spec-regression; check dependent packets |
...
2. **Enrich spec**: Create new spec version (e.g., v1.0 â†’ v1.1 or v1.0 â†’ v2.0 if breaking)
...
**Minor enrichment** (v1.0 â†’ v1.1): Clarifications, new anchors, no anchor changes
...
**Major enrichment** (v1.0 â†’ v2.0): Rewrites, anchor changes, removals
...
When spec changes significantly (v1.0 â†’ v2.0), existing packets may reference old anchors. Use this process:

### Pre-Migration: Audit Impact
```bash
# Find all packets referencing v1.0 anchors
grep -r "v1.0" .GOV/task_packets/ | wc -l

# Identify anchors that will be removed/renamed
grep "^A2.3" .GOV/Master_Spec_v1.0.md > old_anchors.txt
grep "^A2.3" .GOV/Master_Spec_v2.0.md > new_anchors.txt
diff old_anchors.txt new_anchors.txt  # Shows gaps
```

### During Migration: Anchor Mapping
Create `.GOV/SPEC_MIGRATION_v1_to_v2.md`:
```markdown
## Anchor Mapping (v1.0 â†’ v2.0)

- A2.3.1 (old: "SQLite Storage") â†’ A2.4.1 (new: "Pluggable Storage Trait")
- A2.3.2 (old: "Direct SQL") â†’ DEPRECATED (no replacement; SQL access now via trait only)
- A3.2.1 (old: "Auth") â†’ A3.2.1 (unchanged)
```
...
- Create new anchor deprecation period (3 phases): DEPRECATED â†’ REMOVED â†’ gone
...
Scope Inputs:
- Task Packet: .GOV/task_packets/WP-{id}.md (status: {status})
...
- Target (after): <200ms
- Actual (after): 185ms âœ… PASS
- Benchmark command: `jq '.response_time_p99' perf_results.json`
- Proof of removability: Revert performance optimization â†’ benchmark shows >200ms âœ…
...
- Violations after: 0 âœ… PASS
- Test command: `pa11y --standard WCAG2AA https://app/feature`
- Proof of removability: Remove keyboard event handlers â†’ audit fails âœ…
...
- Files: .GOV/api/v1/users.md + src/main.rs doc comments
- Completeness checklist:
  âœ“ Endpoint URL and method
  âœ“ Required headers + authentication
  âœ“ Request body schema (with example)
  âœ“ Response body schema (with example)
  âœ“ Error codes (documented in src/errors.rs)
  âœ“ Rate limits (documented)
- Proof of removability: Remove doc comments â†’ CI check fails (missing docs) âœ…
- File evidence: .GOV/api/v1/users.md:1-50, src/api/handlers.rs:100-120
```
...
- Acceptance criteria met: âœ… All 5 acceptance criteria verified
  âœ“ Feature works as demoed
  âœ“ No blocking bugs
  âœ“ Performance acceptable
  âœ“ UX meets expectations
  âœ“ Ready for production
- Sign-off reference: Jira HSK-1234 comment #42
- Proof: Signed change log in .GOV/FEATURE_ACCEPTANCE.md
```
...
- Schema file: .GOV/openapi-3.0.yaml
- Valid schemas: 12/12 âœ… PASS
- Command: `swagger-cli validate .GOV/openapi-3.0.yaml`
- Implementation evidence: src/api/handlers/*.rs lines match response types
- Proof of removability: Change response struct â†’ schema validation fails âœ…
```
...
|-----------|---------|-----------|---------|--------------|----------|
| alice_25121512345 | Orchestrator | 2025-12-15 12:34:56 UTC | Packet creation WP-1 | v02.85 | .GOV/task_packets/WP-1.md |
```

### Justfile Skeleton
```
create-task-packet WP_ID:
  node .GOV/scripts/create-task-packet.mjs {{WP_ID}}

pre-work WP_ID:
  node .GOV/scripts/validation/pre-work-check.mjs {{WP_ID}}

post-work WP_ID:
  node .GOV/scripts/validation/post-work-check.mjs {{WP_ID}}
...
- Verifies task packet file exists: `.GOV/task_packets/WP-{ID}.md`
...
- Checks: `.GOV/roles_shared/SPEC_CURRENT.md` exists and is readable
...
- Reads `.GOV/roles_shared/TASK_BOARD.md` "Phase {N} Closure Gates (BLOCKING)"
...
Read: .GOV/roles_shared/SPEC_CURRENT.md sections A2.3.12.3 + A2.3.12.1
Read: .GOV/task_packets/WP-1-Storage-Layer.md (full packet)
DONE_MEANS:
...
**File**: `.GOV/roles_shared/scripts/hooks/pre-commit`
...
grep -r "TODO()" src/ app/ && \
  echo "âŒ TODOs must have tracking ID: TODO(HSK-####)" && exit 1

# 3. Forbid placeholder values in code
grep -r "{" src/ app/ --include="*.rs" --include="*.ts" --include="*.tsx" && \
  echo "âš ï¸  Found placeholder text in code" && exit 1
...
Create `.GOV/` directory with:

```
.GOV/
â”œâ”€â”€ START_HERE.md              # Entry point; repo map; AI agent workflow
â”œâ”€â”€ SPEC_CURRENT.md            # Pointer: "Current spec is Master_Spec_vX.Y.Z.md"
â”œâ”€â”€ ARCHITECTURE.md            # Module layout, responsibilities, entry points, RDD
â”œâ”€â”€ RUNBOOK_DEBUG.md           # Error codes, bug triage, debug patterns
â”œâ”€â”€ QUALITY_GATE.md            # Risk tier definitions, Gate 0/1 requirements
â”œâ”€â”€ TASK_BOARD.md              # Master task status + Phase gates + dependencies
â”œâ”€â”€ SIGNATURE_AUDIT.md         # Immutable registry of consumed signatures
â”œâ”€â”€ OWNERSHIP.md               # Path/area ownership (optional)
â”œâ”€â”€ task_packets/
â”‚   â”œâ”€â”€ README.md              # Packet naming convention, validation commands
â”‚   â”œâ”€â”€ TEMPLATE.md            # Copy-paste packet template
â”‚   â””â”€â”€ WP-1-*.md, WP-2-*.md   # (20-30 actual packets during active work)
â”œâ”€â”€ adr/                       # Architecture Decision Records
â”‚   â”œâ”€â”€ ADR-0001-workflow.md
â”‚   â””â”€â”€ ADR-0002-spec-governance.md
â””â”€â”€ agents/
    â””â”€â”€ AGENT_REGISTRY.md      # Map of contributing agents/models
```
...
**What It Does**:
1. Reads `.GOV/roles_shared/TASK_BOARD.md` section "Phase {PHASE_ID} Closure Gates (BLOCKING)"
...
# Output:
# âœ… Phase 1 Closure Gates check...
# âŒ BLOCKED: WP-1-Storage-Layer (status: Done, not VALIDATED)
# âŒ BLOCKED: WP-1-AppState-Refactor (status: In-Progress)
# Exit 1

# Once all gates pass:
# âœ… All blocking WPs VALIDATED
# âœ… Spec regression check passed
...
if just validator-phase-gate Phase-1; then
  echo "âœ… Phase 1 ready to close"
  git tag phase-1-complete
  git push origin phase-1-complete
else
  echo "âŒ Blockers remain; cannot close phase"
...
2. Create `.GOV/roles_shared/SPEC_CURRENT.md` (pointer file, 3 lines)
3. Create `.GOV/roles_shared/SIGNATURE_AUDIT.md` (empty table, 5 lines)
4. Create `.GOV/roles_shared/TASK_BOARD.md` (empty structure, 10 lines)
5. Create `.GOV/task_packets/TEMPLATE.md` (copy from Templates section below, 50 lines)
6. Create `.GOV/scripts/create-task-packet.mjs` (simple Node script: generate packet from template)
7. Test: `node .GOV/scripts/create-task-packet.mjs WP-0-Test` â†’ produces `.GOV/task_packets/WP-0-Test.md`
...
1. Create `.GOV/scripts/validation/pre-work-check.mjs` (verify 10 required fields, no placeholders)
2. Create `.GOV/scripts/validation/post-work-check.mjs` (verify VALIDATION results recorded)
3. Create `.GOV/scripts/validation/validator-spec-regression.mjs` (verify spec file exists + anchors)
4. Create `.GOV/scripts/validation/validator-scan.mjs` (grep for forbidden patterns)
5. Create `justfile` with commands:
   ...
   - `just validator-spec-regression`
6. Test:
   - `just create-task-packet WP-1-Feature` â†’ creates packet
   - `just pre-work WP-1-Feature` â†’ FAIL (has placeholders)
   - Edit packet, remove placeholders
   - `just pre-work WP-1-Feature` â†’ PASS
...
2. Create `.GOV/scripts/validation/codex-check.mjs` (grep for codex violations)
3. Create `.GOV/roles_shared/scripts/hooks/pre-commit` (wire codex-check + lint + tests)
4. Wire hook: `git config core.hooksPath .GOV/roles_shared/scripts/hooks`
5. Test: Try to commit code that violates a codex rule â†’ commit blocked
...
1. Create `.GOV/roles/orchestrator/ORCHESTRATOR_PROTOCOL.md` (copy template + customize, ~50KB)
   - Pre-orchestration gates checklist
   - Signature pause protocol
   - Packet creation workflow
2. Create `.GOV/roles/coder/CODER_PROTOCOL.md` (copy template + customize, ~30KB)
   - Pre-coding checklist (scope adequacy)
   - Validation order (TEST_PLAN â†’ manual review â†’ post-work)
   - Evidence recording format
3. Create `.GOV/roles/validator/VALIDATOR_PROTOCOL.md` (copy template + customize, ~20KB)
   - Pre-flight checks
...
1. Create `.GOV/scripts/validation/validator-dal-audit.mjs` (if storage-based)
   - Check trait boundaries, SQL portability, migration hygiene
2. Create `.GOV/scripts/validation/validator-error-codes.mjs`
   - Enforce typed errors, trace ID logging, no stringly errors
3. Create `.GOV/scripts/validation/validator-security.mjs`
   - RCE guardrails, input validation, secret detection
4. Create `.GOV/scripts/validation/validator-git-hygiene.mjs`
   - .gitignore coverage, no artifacts committed
5. Create `.GOV/scripts/validation/validator-phase-gate.mjs`
...
1. Create `.GOV/roles_shared/START_HERE.md` (entry point: repo map, AI agent workflow, links)
2. Create `.GOV/roles_shared/ARCHITECTURE.md` (module layout, responsibilities, entry points)
3. Create `.GOV/roles_shared/RUNBOOK_DEBUG.md` (error codes, debug patterns)
4. Create `.GOV/roles_shared/QUALITY_GATE.md` (risk tier definitions, Gate 0/1 requirements)
5. Create `.GOV/agents/AGENT_REGISTRY.md` (map of contributing agents/models)
6. Create first **real** task packet for your project's first feature
   ...
   - Run full workflow: create â†’ code â†’ validate â†’ merge
...
**File**: `.GOV/task_packets/WP-1-User-Authentication.md`
...
8. Integration test: login â†’ validate token â†’ request with Bearer â†’ success
...
- FILES_TO_OPEN:
  - .GOV/roles_shared/SPEC_CURRENT.md (section A3.2.1)
  - .GOV/task_packets/WP-1-User-Authentication.md (this packet)
  - src/api/handlers/mod.rs (entry point)
  - src/services/mod.rs (service structure)
  - tests/integration/auth_test.rs (test patterns)
  - Cargo.toml (dependencies)
  - .GOV/roles_shared/RUNBOOK_DEBUG.md (error codes section)
- SEARCH_TERMS:
...
- RISK_MAP:
  - "JWT library vulnerability" â†’ Security audit required (MEDIUM risk)
  - "Password stored plaintext" â†’ Must use bcrypt (HIGH risk)
  - "Token expiry not enforced" â†’ MEDIUM risk
  - "API contract mismatch" â†’ Integration test catches (LOW risk)
...
TASK_TYPE: FEATURE
FILES_TO_OPEN:
- .GOV/roles_shared/SPEC_CURRENT.md (A3.2.1)
...
Result: PASSED (8/8 cases)
  ...
  âœ“ login_invalid_password
  âœ“ login_invalid_email
  âœ“ token_decode_valid
  âœ“ token_expired
  âœ“ token_malformed
  âœ“ bearer_header_validation
  âœ“ bearer_token_not_found
...
## Scope Inputs
- Task Packet: .GOV/task_packets/WP-1-User-Authentication.md
...
| "Integration test: login â†’ Bearer â†’ success" | A3.2.1 | tests/integration/auth_test.rs:50-100 | cargo test auth_integration | PASS |
...
- [WP-1-User-Authentication] **Done (VALIDATED)** âœ…
  - Blocker: None
  - Unblocks: WP-2-Authorization-Roles, WP-3-Session-Management

- [WP-1-Database-Migrations] **Done (VALIDATED)** âœ…
  - Blocker: None
  - Unblocks: WP-1-Storage-Abstraction

- [WP-1-Error-Codes-Registry] **Done (VALIDATED)** âœ…
  - Blocker: None
...
**Rationale**: Dual-backend readiness (SQLite â†’ PostgreSQL), migration consistency, audit trail
...
- [ ] `.GOV/roles_shared/SPEC_CURRENT.md` exists and is readable
- [ ] Points to valid spec file (e.g., `Master_Spec_v1.0.md`)
- [ ] Run `just validator-spec-regression` â†’ PASS
- [ ] If FAIL: escalate to user; spec must be fixed before proceeding

### Gate 2: Task Board Fresh
- [ ] `.GOV/roles_shared/TASK_BOARD.md` exists
...
### Gate 4: Supply Chain
- [ ] Run `cargo deny` â†’ no blockers
- [ ] Run `npm audit` (if applicable) â†’ no critical
- [ ] Dependency check: no major version changes without review

### Gate 5: Signature Audit
- [ ] `.GOV/roles_shared/SIGNATURE_AUDIT.md` exists
...
1. Packet path: `.GOV/task_packets/WP-{id}.md`
2. WP_ID: `WP-{id}`
...
- [ ] Task packet file exists: `.GOV/task_packets/WP-{id}.md`
- [ ] All 10 required fields present
- [ ] No placeholder text (`{field_name}`, `TODO(TBD)`)
- [ ] Run `just pre-work WP-{id}` â†’ PASS (exit 0)
...
- [ ] Read `.GOV/roles_shared/SPEC_CURRENT.md` section on SPEC_ANCHOR
...
Run validators:
```
just validator-scan              â†’ 0 forbidden patterns
just validator-git-hygiene       â†’ 0 artifacts committed
just validator-error-codes       â†’ typed errors, codes documented
just validator-{custom}          â†’ domain-specific checks
```
...
Status: âœ… PASS
- Packet completeness: âœ… (all 10 fields)
- Spec match: âœ… (v1.0 anchors valid)
- USER_SIGNATURE: âœ… (alice_25121512345, not reused)
- Bootstrap present: âœ…
...
- Code removability verified: 5/5 âœ…

#### Validator 3: Hygiene Gates
Status: âš ï¸  WARNING (non-blocking)
- Forbidden patterns: âœ… (0 found)
- Git cleanliness: âœ…
- Coverage: 78% (below 80%, but documented waiver CX-EXC-001)
- Codex violations: âœ… (0)

#### Validator 4: Storage DAL Audit (custom)
Status: âœ… PASS
- Trait isolation: âœ…
- SQL portability check: âœ… (no SQLite-specific syntax outside storage/)
- Migration numbering: âœ… (migration_001.sql, migration_002.sql)
- Schema immutability: âœ…

#### Validator 5: Security Audit (custom)
Status: âœ… PASS
- RCE guardrails: âœ…
- Input validation: âœ…
- Secret detection: âœ… (0 secrets found)
- Dependency audit: âœ… (0 critical vulnerabilities)

#### Validator 6: Git Hygiene (custom)
Status: âœ… PASS
- .gitignore coverage: âœ…
- Artifacts committed: âœ… (0)
...
- Any FAIL from any validator â†’ overall FAIL
...
| Scenario | Resolution |
|----------|-----------|
| Storage auditor says âœ… PASS; Security auditor says âŒ FAIL | Overall = FAIL; rework required |
...
Scope:
- Packet: .GOV/task_packets/WP-{id}.md
...
**Action**: Append validation report to packet; update STATUS â†’ Done; update Task Board to Done (VALIDATED)
...
export async function validate(wpId, projectRoot) {
  ...
  if (!fs.existsSync(packetPath)) {
    console.error(`âŒ Packet not found: ${packetPath}`);
    process.exit(1);
  }
  ...
  for (const field of REQUIRED_FIELDS) {
    if (!content.includes(`## ${field}`) && !content.includes(`- ${field}`)) {
      console.error(`âŒ Missing required field: ${field}`);
      process.exit(1);
    }
  }
  ...
  for (const placeholder of PLACEHOLDERS) {
    ...
    if (matches) {
      console.error(`âŒ Found placeholder text: ${matches[0]}`);
      process.exit(1);
    }
  }

  console.log(`âœ… Pre-work check passed: ${wpId}`);
  process.exit(0);
}
...
for (const scanPath of SCAN_PATHS) {
  ...
  for (const { pattern, reason } of FORBIDDEN_PATTERNS) {
    try {
      ...
      if (result.trim()) {
        console.error(`âŒ FORBIDDEN: ${pattern}`);
  ...
  for (const placeholder of PLACEHOLDER_PATTERNS) {
    try {
      ...
      if (result.trim()) {
        console.error(`âš ï¸  PLACEHOLDER: ${placeholder}`);
        console.error(result);
        violations++;
      }
    } catch (e) {
...
if (violations > 0) {
  console.error(`\nâŒ Found ${violations} violations`);
  process.exit(1);
} else {
  console.log('âœ… validator-scan passed');
  process.exit(0);
}
```
...
```javascript
// .GOV/scripts/validation/validator-{your-concern}.mjs
// Template for project-specific validators

export async function validate(wpId, projectRoot) {
  // 1. Read task packet
  const packetPath = `${projectRoot}/.GOV/task_packets/${wpId}.md`;
  ...
  for (const filePath of inScopePaths) {
    ...
    if (content.includes('sqlx::query') && !filePath.includes('storage/')) {
      console.error(`âŒ CX-DBP-VAL-010: Direct DB access in ${filePath}`);
      process.exit(1);
    }

    // Check 2: SQL portability
    if (content.includes('strftime(') || content.includes('?1')) {
      console.error(`âŒ CX-DBP-VAL-011: SQLite-only SQL in ${filePath}`);
      process.exit(1);
    }
  }

  // 4. Return verdict
  console.log(`âœ… validator-{concern} passed`);
  process.exit(0);
}
...
4. Re-run `just pre-work WP-1` â†’ exit 0
...
2. Validator performs a docs-only status-sync commit on `main`:
   ...
   - Verify with: `git show main:.GOV/roles_shared/TASK_BOARD.md | rg "WP-1-Feature"` matches packet reality
...
2. Create `.GOV/roles_shared/SPEC_CURRENT.md` pointer
3. Create `.GOV/roles_shared/SIGNATURE_AUDIT.md` (empty)
4. Create `.GOV/roles_shared/TASK_BOARD.md` (list current in-flight work)
5. Create `.GOV/scripts/create-task-packet.mjs` (simple generator)
...
3. Run full workflow: pre-work â†’ code â†’ validate â†’ merge
...
# Verify: .GOV/task_packets/WP-0-Test.md created with template
```
...
# To test: add unwrap() to src/main.rs, re-run â†’ FAIL
```
...
# Remove println(), re-commit â†’ PASS
```
...
Tool outputs: 50KB of test logs â†’ embedded in next prompt
Prompt 2: "Here's test output... run next commands" (51KB)
Tool outputs: 40KB of build logs â†’ embedded in next prompt
Prompt 3: "Here's build output... run grep" (41KB)
...
Total tokens: ~150KB of raw I/O â†’ ~750K tokens for prompting (4:1 expansion)
```
...
Tool Agent executes:
  node_1: cargo test â†’ outputs handle "test_run_001" (log stored separately)
  node_2: cargo build â†’ outputs handle "build_001"
  node_3: grep search â†’ outputs handle "grep_001"
...
Total tokens: ~2.5KB actual prompts â†’ ~12K tokens
...
Create `.GOV/TOOL_AGENT_CATALOG.md`:
...
Parent Packet: WP-1-Storage-Layer (RISK_TIER: HIGH, needs security audit)
  â”œâ”€ Invokes: backend_validation_pack
  â”‚   Inherited capabilities:
  â”‚   - Test execution (restricted to IN_SCOPE_PATHS)
  â”‚   - Forbidden pattern scanning (inherits severity level)
  â”‚   - Lint rules (inherits codex rules)
  â”‚
  â””â”€ Validator applies MOST_RESTRICTIVE rule:
      - If parent = HIGH risk, child = MEDIUM risk â†’ child inherits HIGH
...
grep -r "api_test_pack" .GOV/task_packets/ | wc -l  # Find all users
...
test_and_build_WP:
  ...
  - Nodes:
    - run_cargo_tests(package, test_target) â†’ handle "test_run_*"
    - run_cargo_build(build_manifest) â†’ handle "build_*"
    - run_clippy(package) â†’ handle "clippy_*"
    - check_coverage(package, threshold: 80) â†’ handle "coverage_*"
...
backend_full_validation_WP:
  - Nodes:
    - lint_check (cargo clippy) â†’ handle "clippy_*"
    - format_check (cargo fmt --check) â†’ handle "fmt_*"
    - deny_check (cargo deny) â†’ handle "deny_*"
    - test_suite (all tests) â†’ handle "test_*"
...
validator_comprehensive_scan_WP:
  - Nodes:
    - forbidden_patterns_scan â†’ handle "scan_forbidden_*"
    - error_code_audit â†’ handle "audit_errors_*"
    - coverage_check â†’ handle "coverage_report_*"
    - security_scan â†’ handle "security_*"
...
| **Savings** | â€” | **99%** | **99%** | 100x token reduction |
...
1. Define error_workflow node: captures failure â†’ emits diagnostic + bundle handle
...
- Fetch handle_001 â†’ verify test counts, pass/fail
- Fetch handle_002 â†’ verify build succeeded
...
```bash
# .GOV/scripts/ci/token-budget-check.sh
# Measure actual token usage per packet; warn if exceed budget

WP_ID=$1
PHASE=$(echo $WP_ID | cut -d'-' -f1)  # WP-1-... â†’ WP-1
...
ACTUAL=$(cat .GOV/task_packets/$WP_ID.md | wc -c)
TOKENS=$((ACTUAL / 4))

echo "Packet $WP_ID:"
echo "  Budget: $BUDGET tokens"
echo "  Actual: $TOKENS tokens"
echo "  Status: $([ $TOKENS -le $BUDGET ] && echo 'âœ… PASS' || echo 'âŒ EXCEED')"
...
|---|---|---|---|---|---|
| WP-1-Storage-Layer | 2.1KB | 525 | 3K | âœ… 83% under | 96% (vs 15KB embedded) |
| WP-1-AppState-Refactor | 1.8KB | 450 | 3K | âœ… 85% under | 95% |
| WP-1-Migration-Framework | 2.3KB | 575 | 3K | âœ… 81% under | 97% |
| WP-1-Dual-Backend-Tests | 2.5KB | 625 | 3K | âœ… 79% under | 96% |
| **Phase 1 Total** | **9.7KB** | **2,175 tokens** | **12K budget** | âœ… PASS | **96% avg savings** |
...
...
```
**Total prompt**: 145KB â†’ ~725K tokens

Metadata: cargo test output = 50 test cases Ã— 1000 bytes = 50KB
...
```
**Total prompt**: 2.1KB â†’ ~10K tokens

**Savings**: 145KB â†’ 2.1KB = 98.6% reduction
**Token savings**: 725K â†’ 10K = 98.6% reduction (72.5x smaller)
```
  ...
  check:
    ...
    steps:
      ...
      - name: Check token budgets
        run: |
          for wp in .GOV/task_packets/WP-*.md; do
            bash .GOV/scripts/ci/token-budget-check.sh "$(basename $wp .md)" || exit 1
          done
      - name: Report phase budgets
        if: always()
        run: bash .GOV/scripts/ci/phase-token-report.sh
```
...
1. **Identify the breaking packet**:
   ...
   - Locate packet: `.GOV/task_packets/WP-{id}.md`
...
- .GOV/roles_shared/SPEC_CURRENT.md (clarify token format guarantee)
...
## BOOTSTRAP
FILES_TO_OPEN:
- .GOV/task_packets/WP-1-Auth-v2.md (original packet)
...
1. **Backward Compatibility Auditor**:
   ```javascript
   // .GOV/scripts/validation/validator-backward-compat.mjs
...
2. **Risk Assessment Auditor**:
   ```javascript
   // .GOV/scripts/validation/validator-risk-assessment.mjs
...
3. **Test Coverage Auditor (Stricter for HIGH Risk)**:
   ```javascript
   // .GOV/scripts/validation/validator-test-coverage.mjs
...
| Pre-prod validation | No staging env test | Add RISK_TIER=CRITICAL â†’ stage first |
...
## Follow-Up Actions
- [ ] Increase test coverage threshold for auth (85% â†’ 95%)
- [ ] Create custom validator for backward compatibility checks
- [ ] Require RISK_TIER=CRITICAL â†’ staging + extended QA
...
1) Create `.GOV/roles_shared/SPEC_CURRENT.md` pointing to your spec file.
2) Add packet template under `.GOV/task_packets/`.
3) Create `.GOV/roles_shared/TASK_BOARD.md` with columns and phase gates.
4) Add `.GOV/roles_shared/SIGNATURE_AUDIT.md` (empty table).
5) Add validator scripts under `.GOV/scripts/validation/`; wire into `justfile`.
6) Add `.GOV/scripts/create-task-packet.mjs` (or equivalent generator).
7) Add git hook (`.GOV/roles_shared/scripts/hooks/pre-commit`) for invariants and linting.
