...
- Data leakage: repo exports may contain secrets/PII unless content classification/redaction is enforced; treat `.GOV/ROLE_MAILBOX/` as potentially sensitive.
...
- EXCERPT_ASCII_ESCAPED:
  ...
  **Secret-leak prevention (HARD)**
  ...
  - The repo export under `.GOV/ROLE_MAILBOX/` MUST NOT contain mailbox message bodies. Only `body_ref` + `body_sha256` are permitted.
  ...
  **Repo export (machine-friendly; always-on for GOV_STANDARD and GOV_STRICT)**  
  ...
  - `.GOV/ROLE_MAILBOX/index.json` (one JSON object; thread inventory)
  - `.GOV/ROLE_MAILBOX/threads/<thread_id>.jsonl` (one JSON object per line; messages)
  - `.GOV/ROLE_MAILBOX/export_manifest.json` (export metadata + hashes)
  ...
  // .GOV/ROLE_MAILBOX/threads/<thread_id>.jsonl (one JSON object per line)
