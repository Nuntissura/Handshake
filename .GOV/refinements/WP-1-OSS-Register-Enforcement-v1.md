...
### GAPS_IDENTIFIED
- Enforcement gap: SPEC_CURRENT requires OSS Register Enforcement via a backend unit test that fails when dependencies are missing from `.GOV/roles_shared/OSS_REGISTER.md` or violate the copyleft isolation rule. This enforcement is not currently guaranteed by an automated test gate.
...
- A backend unit test in `src/backend/handshake_core/tests/` that:
  ...
  - Parses `.GOV/roles_shared/OSS_REGISTER.md` entries (component name + license + allowed integration mode).
...
- CLEARLY_COVERS_REASON: SPEC_CURRENT v02.100 explicitly requires maintaining `.GOV/roles_shared/OSS_REGISTER.md` and adding a backend unit test enforcing completeness and copyleft isolation.
