...
- Repo drift (kernel conformance failure): CI/hooks/docs still reference "Codex v0.8" and/or the file `Handshake Codex v0.8.md`, but the repo's authoritative governance reference is `Handshake Codex v1.4.md` (and `Handshake Codex v0.8.md` does not exist).
  - Evidence (current repo state):
    ...
    - `.GOV/scripts/validation/ci-traceability-check.mjs` prints "CI Traceability Check (Codex v0.8)" and hard-fails if `Handshake Codex v0.8.md` is missing.
    - `.GOV/scripts/hooks/pre-commit` prints "Pre-commit validation (Codex v0.8)" and points to `Handshake Codex v0.8.md` on failure.
    - `.GOV/task_packets/README.md` links to `Handshake Codex v0.8` (stale pointer).
    - `.GOV/roles/orchestrator/ORCHESTRATOR_PROTOCOL.md` still contains example references to "Handshake Codex v0.8.md" (stale guidance).
...
- NONE. This WP is governance enforcement drift remediation (CI/hooks/.GOV/scripts) and does not add or change runtime Flight Recorder events.
...
- Fix should prefer a single source of truth for the codex filename/version to avoid repeating this drift (e.g., derive from `.GOV/roles_shared/SPEC_CURRENT.md` governance reference or equivalent pointer).

### PRIMITIVES (traits/structs/enums)
- JavaScript/Node:
  - Add/adjust a single-source-of-truth resolver for the current codex filename/version used by CI/hook scripts (preferred: derive from `.GOV/roles_shared/SPEC_CURRENT.md`, which already states the authoritative Governance Reference).
  - Update `.GOV/scripts/validation/ci-traceability-check.mjs` to validate the correct codex filename exists (not `Handshake Codex v0.8.md`) and to report the correct codex version string.
- Shell/CI:
  - Update `.GOV/scripts/hooks/pre-commit` messaging to reference the correct codex filename and version.
