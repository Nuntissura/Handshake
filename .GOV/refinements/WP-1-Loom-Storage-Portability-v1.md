...
- USER_APPROVAL_EVIDENCE: APPROVE REFINEMENT WP-1-Loom-Storage-Portability-v1
- STUB_WP_IDS: NONE

### REQUIRED SECTIONS (per ORCHESTRATOR_PROTOCOL Part 2.5.2)

### GAPS_IDENTIFIED
- The current spec already defines Storage Backend Portability law, Loom storage-trait expectations, portable schema examples, and backend-agnostic search semantics; the gap is parity enforcement and conformance coverage, not missing normative text.
- Product code already contains SQLite and PostgreSQL Loom implementations, but there is no shared Loom storage conformance suite in `src/backend/handshake_core/src/storage/tests.rs` to prove both backends preserve the same block, edge, view, search, and source-anchor semantics.
- `api/loom.rs` couples filesystem writes, content-hash dedup, storage CRUD, preview job launch, and search/view endpoints. This WP must harden portability at the storage contract level without expanding into preview protocol redesign or workflow-runtime refactors.
- Provider-specific search implementations exist (`FTS5` in SQLite, text-search query logic in PostgreSQL), but the packet needs to guarantee stable API meaning for `LoomSearchFilters`, `LoomViewFilters`, `LoomBlockSearchResult`, and `LoomSourceAnchor` across both backends.
- The migration baseline exists for Loom tables, but replay-safe and down-migration behavior for Loom-specific tables is not currently paired with dedicated Loom parity assertions in the cross-backend tests.
- Asset blob layout in `loom_fs.rs` is part of the portability contract. Storage parity that ignores filesystem path stability would still break export, replay, or downstream Loom bridge packets.
...
- EXISTING_CAPABILITY_ALIGNMENT_REASON: The gap is real and implementation-oriented. Loom MVP and storage abstraction exist, but there is no dedicated packet that hardens Loom semantics, migrations, asset paths, and dual-backend conformance as one portable contract.
