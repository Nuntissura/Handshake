...
The governance kernel is the deterministic testbed for future Handshake Product governance. Governance workflow artifacts should be machine-readable, atomic, and deterministic first so external apps/tools and product runtime surfaces can ingest the same contracts directly. Markdown remains useful as generated projection or frozen legacy reference, but it must not become a second manually maintained authority when typed JSON, JSONL, schemas, hashes/provenance, and checks can carry the workflow truth.
...
- runtime artifacts use JSON/JSONL authority intended for app ingestion
...
- `roles/`
  - role-owned protocols, startup briefs, role-local state, and role-scoped implementation surfaces
  - `roles/orchestrator/`
...
- `docs_repo/`
  - repo-level governance docs and running governance logs
