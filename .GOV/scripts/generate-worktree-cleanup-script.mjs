...
import { loadJson, loadPacket, packetExists, parseClaimField } from "./role-resume-utils.mjs";
...
const SESSION_REGISTRY_PATH = path.join(".GOV", "roles_shared", "ROLE_SESSION_REGISTRY.json");
...
function usage() {
  fail("Usage: node .GOV/scripts/generate-worktree-cleanup-script.mjs <WP_ID> <CODER|WP_VALIDATOR>", [
    "Example: node .GOV/scripts/generate-worktree-cleanup-script.mjs WP-1-Example CODER",
  ]);
}
...
function buildPowerShellScript({
  ...
  return [
    ...
    "try {",
    "  & node '.GOV/scripts/delete-local-worktree.mjs' $WorktreeId `",
    "    --approve $Approval `",
