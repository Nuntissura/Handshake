...
export function buildOrchestratorRescuePrompt({ wpId = "", guardDecision = null } = {}) {
  ...
  return [
    ...
    "CLOSEOUT_PREP: before launching Integration Validator, verify final CODER_HANDOFF committed target evidence, run `just phase-check HANDOFF WP-{ID} WP_VALIDATOR --range <base>..<head>`, and use `just closeout-repair WP-{ID}` only for pre-verdict prep drift. Terminal `phase-check CLOSEOUT` runs only after Integration Validator resolves the final handoff with a review/verdict response.",
