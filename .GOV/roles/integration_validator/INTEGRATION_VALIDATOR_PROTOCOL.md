...
- [IV-OUT-003] When orchestrating, the IV does not change workflow, statuses or relay paths on its own (Codex CX-EXEC-011, CX-STATUS-001). It proposes the change to the Operator and waits.
- [IV-OUT-004] Every builder dispatch names the builder's target disk and target directory. That disk is never the validator's build disk, and one build runs per physical disk at a time (Codex CX-984-002).
- [IV-OUT-005] Before a round starts, the IV reads the validator's round script and build settings: no whole-run timeout (Codex CX-EXEC-005), runner exit handled as a result (CX-VAL-005), traced test environment (WPV-OUT-005), and target size within the grant cap (CX-984-014).
...
- Diagnostics/Flight-Recorder + Palmistry duty: block whole-WP PASS/merge unless every observable runtime behavior wired or DEFERRED (with reason) all three tiers of the three-tier diagnostic model and recorded the per-tier verdict — Tier 1 Flight Recorder (kept-as-is backend business-event ledger), Tier 2 internal_diagnostics (Handshake-native internal self-diagnostics: panic hook, UI-thread heartbeat, frame-time, CPU/RSS/GPU counters, open diagnostic-event API), and Tier 3 Palmistry (external out-of-process watcher that survives freezes/crashes). A missing per-tier outcome (not WIRED, NOT_APPLICABLE-with-reason, or DEFERRED-with-reason) is a silent skip and is a hard blocker. Per HBR-INT-009 + CX-981.
...
- SurrealDB/EventLedger duty: durable authority behavior requires real Handshake-managed SurrealDB/EventLedger proof in a fresh WP-scoped namespace/database. PostgreSQL, SQLite, caches, fixtures, compatibility paths, imports, reconciliation, examples, harnesses, temporary adapters, or in-memory substitutes are merge blockers unless the Operator created an explicit non-Handshake exception.
- Account-resource privacy duty: whole-WP PASS requires end-to-end proof that every primary and derived resource is linked to its owner and authorized scope and remains protected across authenticated SurrealDB record-user table/field permissions, filesystem/ResourceBroker, APIs, search/index, memory and model context, tools, UI, previews, exports, backups, diagnostics, EventLedger/Flight Recorder, and SaaS/MCP seams touched by the WP. Independently test different accounts inside the same project, unrelated projects, concurrent grant/revocation changes, context switches, mixed-scope derivation, and existence-metadata inference through counts/search/errors/logs/traces. A visible UI filter, fixture-only evaluator, or authorization check that omits a resource boundary cannot satisfy HBR-PRIV.
