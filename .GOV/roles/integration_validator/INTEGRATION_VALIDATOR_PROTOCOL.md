...
- Argus visual duty: UI/operator-surface, diagnostic-surface, frontend navigation, layout, style, panel, tab, button, input, or visible-state changes require Argus evidence per `.GOV/roles_shared/docs/ARGUS_VISUAL_INSPECTION_PROTOCOL.md`. If Argus cannot see, identify by stable `author_id`, steer, or re-observe an in-scope surface, require remediation before merge when the gap blocks product proof; otherwise record a blocking HBR-VIS gap rather than issuing PASS.
