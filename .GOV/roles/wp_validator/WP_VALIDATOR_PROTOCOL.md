...
- [WPV-CAD-003] The validator-executed focused Cargo proof for each MT targets the MT's changed behavior plus any concrete reviewer finding; bundle the focused proof of all READY MTs at the same `validate_at_commit` into one build and one test invocation per [VPX-007]/[VPX-008]. Do not substitute a broad suite for review judgment, and do not substitute implementer-run focused proof for the validator's own run.
