...
- [WPV-CAD-001] (rewritten per [VPX-005]) An intermediate per-MT review that is not also the declared session-batch or final-WP boundary does not run the broad/full Cargo test suite. The Coder or Kernel Builder must declare an exact `SESSION_MT_BATCH` and supply compile/static plus focused behavior proof records for the MT as triage input ([VPX-002]); the WP Validator executes the MT's compile/static and focused proof itself in its own target ([VPX-001], [VPX-003]). A missing or malformed batch declaration is a governance/evidence blocker to correct; it does not authorize a per-MT broad-suite run. When the last-MT review overlaps a declared session-batch or final-WP boundary, the WP Validator executes the broad suite itself at that boundary; implementer broad-suite evidence does not satisfy the boundary.
- [WPV-CAD-002] Accept `FULL_CARGO_SUITE=DEFERRED_TO_SESSION_MT_BATCH` as a valid non-final evidence state for the BROAD suite only, only until the declared boundary, and only when it names the declared batch and exact MT IDs. Focused proof is never deferred. Continue to review code quality, scope, boundaries, compilation, and focused proof normally.
- [WPV-CAD-003] The validator-executed focused Cargo proof for each MT targets the MT's changed behavior plus any concrete reviewer finding. Do not substitute a broad suite for review judgment, and do not substitute implementer-run focused proof for the validator's own run.
- [WPV-CAD-004] The WP Validator executes one broad/full Cargo suite at the end of the declared session MT batch or at final WP implementation, whichever boundary is reached first, and records it as a proof record whose `covers` binds the covered MT IDs to the exact commit/tree state.
- [WPV-CAD-005] Any product-code change after that PASS makes the validator's broad-suite proof record stale for the changed tree ([VPX-004]). Execute affected focused proof immediately and a fresh broad/full suite at the next required boundary. Never confirm final WP evidence without a validator-executed full-suite PASS on the final unchanged implementation state.
...
- [WPV-ART-003] A batch spanning MTs MUST declare one actual owning MT and every covered MT in existing typed evidence; artifacts stay below the owning WP/MT. Reuse compatible builds and unchanged proof per [CX-503I1]; this hierarchy alone MUST NOT cause duplicate builds/tests or invalidate product proof. (Reuse clause superseded for verdict purposes by [VPX-004]: validator verdicts may reuse only the validator's own proof records under identical commit, clean-tree state, command, features, and environment knobs.)
...
- Cite implementer-executed proof, result lines, logs, or summaries as the basis of PASS, or reuse the implementer's target directory / warm cache for validator proof ([VPX-001], [VPX-002])
