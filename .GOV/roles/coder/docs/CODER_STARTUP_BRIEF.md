...
- DO: per MT, capture-and-compare through Argus. Use the native Rust MCP/AccessKit path in `src/frontend/handshake_native/`: canonical JSON-RPC methods `argus.inspect`, `argus.click`, `argus.set_value`, and `argus.screenshot`, backed by `src/mcp/argus.rs`, `src/mcp/tools.rs`, `src/mcp/session.rs`, `src/mcp/server.rs`, `src/mcp/screenshot.rs`, and egui/AccessKit kittest harnesses. Include request-level `agent_label` when parallel clients share one binding token. Compare the rendered/AccessKit result against the MT's expected UI state before requesting review.
- DO_NOT: use the Tauri CDP path; use foreground desktop automation; steal keyboard/mouse/focus; assume pixel screenshots work headless — `Harness::render()` readback can crash `0xc0000005` on headless-GPU, so run pixel screenshots on a real-GPU host and fall back to `argus.inspect`/AccessKit-tree assertions on headless hosts
...
- SOURCE: HANDSHAKE_BUILD_RULES.json HBR-VIS-001..005, `.GOV/roles_shared/docs/ARGUS_VISUAL_INSPECTION_PROTOCOL.md`, native `src/mcp/argus.rs` + `src/mcp/tools.rs` + `src/mcp/session.rs` + `src/mcp/server.rs` + `src/mcp/screenshot.rs`, CX-503D1, CX-006-VIS
