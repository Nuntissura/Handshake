...
## PHASE 1: Critical Foundations (82 â†’ 88/100) â€” 3-4 hours
...
IF INCOMPLETE:
â†’ BLOCK with specific reason
â†’ "Packet missing/incomplete: {field name}. Orchestrator: please complete before I proceed."
```

**Where:** Insert after Step 2 current content (line 75)

**Impact:** 82 â†’ 85/100 (removes 30% of Coder rejection rate due to incomplete packets)
...
- [ ] **FILES_TO_OPEN**: 5-15 files minimum
  - Include: .GOV/roles_shared/docs/START_HERE.md, .GOV/spec/SPEC_CURRENT.md, .GOV/roles_shared/docs/ARCHITECTURE.md
...
- [ ] **RISK_MAP**: 3-8 failure modes minimum
  - Format: "{failure mode}" â†’ "{subsystem}"
  - Example: "Database migration fails" â†’ "Storage layer"

IF INCOMPLETE:
â†’ BLOCK: "BOOTSTRAP incomplete: missing {field}. Cannot start without full understanding."
```

**Where:** Replace Step 5 template (lines 123-159) with clearer structure + checklist

**Impact:** 85 â†’ 87/100 (100% clarity on BOOTSTRAP format; Validator never says "incomplete" again)
...
â†’ BLOCK: "TEST_PLAN has placeholders. Orchestrator: please make commands concrete."

Example INCOMPLETE TEST_PLAN:
```bash
pnpm test          # âŒ Which directory?
cargo test         # âŒ Which manifest path?
```
...
**Impact:** 87 â†’ 89/100 (prevents Coder from running incomplete validation)
...
2. Identify error type:
   - Compilation error? â†’ Fix syntax, re-test
   - Assertion failure? â†’ Fix logic, re-test
   - Missing dependency? â†’ Escalate (should be in packet)
...
2. Identify issue type:
   - Hard invariant violation? â†’ Fix code (unwrap, println, etc.)
   - Security issue? â†’ Fix immediately (Validator will reject)
   - Test coverage? â†’ Add tests or document waiver
   - Hollow code? â†’ Implement full logic, not stubs
...
**Recovery if error occurs:**
1. Identify conflict:
   - Packet scope incomplete? â†’ Missing prerequisite work
   - Scope contradicts itself? â†’ IN_SCOPE includes file, OUT_OF_SCOPE forbids it
   - Implementation needs changed? â†’ Found bug/refactoring required

2. Document conflict in packet NOTES with specific examples

3. Escalate to Orchestrator:
   ```
   âš ï¸ SCOPE CONFLICT: {WP-ID}
...
1. If packet changed (VALIDATOR_PROTOCOL Part 5.6):
   - Original packet is locked â†’ immutable
   - Changes require new variant (WP-{ID}-v2)

2. Identify what changed:
   - DONE_MEANS added? â†’ New work, needs new packet
   - IN_SCOPE_PATHS expanded? â†’ New work, needs new packet
   - OUT_OF_SCOPE items removed? â†’ Scope change, needs clarification
...
2. Identify issue:
   - Missing test? â†’ Add test, re-run
   - Build error? â†’ Fix code, re-run
   - Validation error? â†’ Fix issue, re-run
...
**Impact:** 89 â†’ 91/100 (Coder has recovery playbook for common issues)
...
**Impact:** 91 â†’ 92/100 (100% clarity on validation order; eliminates confusion)

---

## PHASE 2: Quality Systems (88 â†’ 93/100) â€” 2-3 hours
...
**Enforcement:**
- âœ… New code: MUST go through /src/backend/llm/
- âŒ Existing violation in IN_SCOPE_PATHS: Fix it (code cleanup)
- âš ï¸ Existing violation outside IN_SCOPE_PATHS: Document, don't fix (out of scope)
...
**Enforcement:**
- âœ… New code: MUST use API layer
- âŒ Violation in new code: Fix it
- âš ï¸ Existing violation: Document in packet NOTES
...
**Enforcement:**
- âœ… New code: MUST use logging, no println!
- âŒ Violation in new code: Replace with tracing::info!() or log::info!()
- âš ï¸ Existing violation: Document, consider future cleanup task
...
**Enforcement:**
- âœ… New code: Format as `// TODO(HSK-####): description`
- âŒ Unformatted TODO: Add HSK-#### prefix
- âš ï¸ Existing unformatted TODOs outside scope: Document for future task
```

**Where:** Add as new section after "Hard invariants to respect" (line 188)

**Impact:** 89 â†’ 91/100 (removes 50% of manual review blocks due to hard invariant violations)
...
**If coverage <80%:**
â†’ Add tests until >80% OR get explicit user waiver
â†’ Document waiver in packet WAIVERS_GRANTED section
...
**Impact:** 91 â†’ 92/100 (no more "coverage too low" validation failures)
...
```markdown
### Step 1.5: Scope Adequacy Check âœ‹ STOP

**Before implementing, verify scope is adequate:**

1. Do you understand what "done" means?
   â†’ Read DONE_MEANS carefully
   â†’ If vague: BLOCK and escalate

2. Do IN_SCOPE_PATHS have all files you'll need to change?
   â†’ Mentally walk through implementation
   â†’ If missing files: BLOCK and escalate

3. Does implementation require OUT_OF_SCOPE work?
   â†’ Example: Implementing storage API but schema is OUT_OF_SCOPE
   â†’ If blocking: BLOCK and escalate

**IF ANY ANSWER IS "NO":**

Escalate to Orchestrator:
```
âš ï¸ SCOPE INADEQUATE: {WP-ID}
...
**Impact:** 92 â†’ 93/100 (prevents scope creep and scope conflicts mid-work)

---

## PHASE 3: Polish (93 â†’ 99/100) â€” 2-3 hours
...
5. Append to packet VALIDATION block: "âœ… {criterion} at {file:line}"
...
**Your Verification:**
- âœ… Database trait defined at src/backend/handshake_core/src/storage/mod.rs:10-50 (6 methods: get_blocks, save_blocks, ...)
- âœ… SqliteDatabase impl at src/backend/handshake_core/src/storage/sqlite.rs:80-150
- âœ… PostgresDatabase stub at src/backend/handshake_core/src/storage/postgres.rs:10-30
- âœ… Tests: cargo test returned 5 passed

IF any criterion has no file:line evidence:
â†’ BLOCK: "Cannot claim done without evidence for {criterion}"
```

**Where:** Insert after Step 6 implementation (before line 195)

**Impact:** 93 â†’ 95/100 (Validator spends <5% time re-checking your work)
...
**Impact:** 95 â†’ 96/100 (reduces manual review confusion; Coder knows when to stop)
...
- Clear BOOTSTRAP â†’ "I read the right files, understand the problem"
- Complete VALIDATION â†’ "I tested thoroughly, here's the proof"
- File:line evidence â†’ "This code implements that requirement"
```

**Where:** Add after main role definition (after line 12)

**Impact:** 96 â†’ 97/100 (Coder understands their place in the system; writes better validation blocks)
...
- âŒ SCOPE (packet scope is locked)
- âŒ IN_SCOPE_PATHS (locked by Orchestrator)
- âŒ OUT_OF_SCOPE (locked by Orchestrator)
- âŒ DONE_MEANS (locked by Orchestrator)
- âŒ TEST_PLAN (locked by Orchestrator)
- âŒ Any other existing content

**If packet needs changes:**
â†’ Escalate to Orchestrator
â†’ Orchestrator creates WP-{ID}-v2 (variant packet)
â†’ You work from new packet
...
**Impact:** 97 â†’ 98/100 (Coder understands governance; no accidental packet edits)
...
|-------|--------|-------|-------|-------|
| **Baseline** | â€” | 18 gaps | B+ (82) | â€” |
| **P0 (Foundations)** | 3-4h | 5 items | **A- (88)** | +6 pts |
| **P1 (Quality)** | 2-3h | 4 items | **A (93)** | +5 pts |
| **P2 (Polish)** | 2-3h | 4 items | **A+ (99)** | +6 pts |
| **TOTAL** | 7-10h | 18 gaps â†’ âœ… | **A+ (99/100)** | +17 pts |

---

## Implementation Roadmap (Next Steps)

### Phase 1 (P0 - Week 1): Critical Foundations [82 â†’ 88]
...
### Phase 2 (P1 - Week 2): Quality Systems [88 â†’ 93]
...
### Phase 3 (P2 - Week 3): Polish [93 â†’ 99]
