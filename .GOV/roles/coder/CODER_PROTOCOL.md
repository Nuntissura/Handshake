...
- [CODER-ART-003] A batch spanning MTs MUST declare one actual owning MT and every covered MT in existing typed evidence; artifacts stay below the owning WP/MT. Reuse compatible builds and unchanged proof per [CX-503I1]; this hierarchy alone MUST NOT cause duplicate builds/tests or invalidate product proof. Build once per commit and bundle the batch's tests into one invocation; the shared warm `CARGO_TARGET_DIR` may be reused by the validator, but the validator executes its own test runs and never cites Coder runs as PASS evidence ([CODER-PROOF-001], [VPX-002], [VPX-007], [VPX-008]).
...
- Tell live lanes whenever you land a shared-file change, per the shared-file batching rule, so a forced recompile is not misread as a stall.
- Never end an orchestration turn in "waiting". If every lane is genuinely building, do work that touches no compile-graph file.
