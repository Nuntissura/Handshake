...
- **Conversation memory (MUST — `just repomem`):** Cross-session conversational memory. **HARD rules:**
  ...
  - **CONCERN when spotting a risk (SHOULD):** When you notice a potential regression, a scope creep risk, a missing test, or a design smell: `just repomem concern "<risk or issue flagged>" --wp WP-{ID}`. Min 80 chars. These are imported into the Workflow Dossier at closeout.
