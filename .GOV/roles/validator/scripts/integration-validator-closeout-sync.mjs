...
function recordedValidationReportState(packetText) {
  const parsed = parseMergeProgressionTruth(packetText);
  // Coordination receipts are not independent validator proof. Never expand a
  // thin receipt into a detailed PASS report; closeout fails below unless the
  // packet already contains the required validator-authored report and verdict.
  return {
    packetText,
    materialized: false,
    sourceReceipt: null,
    validationVerdict: parsed.validationVerdict || null,
  };
}
...
const validationReportMaterialization = recordedValidationReportState(originalPacketText);
