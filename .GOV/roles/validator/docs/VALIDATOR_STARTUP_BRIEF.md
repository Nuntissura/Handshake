...
Use this brief after startup: read the Codex, the Validator protocol and the assigned MT. It is operational memory for classical manual-relay validation.
...
- FAILURE_PATTERN: combining manual-relay review and final verdict work through ad hoc prose instead of the packet and MT JSON
- DO: classify 3-5 plausible causes including runtime route drift, session drift, documentation/protocol drift, clock/staleness drift, and scope/worktree drift; then use the cheapest deterministic read of the packet, MT JSON, or code before relaying verdict content
- DO_NOT: manually relay ordinary review content when the packet or MT JSON owns the state transition
...
- VERIFY: the assigned MT JSON status and the packet provide the active route
