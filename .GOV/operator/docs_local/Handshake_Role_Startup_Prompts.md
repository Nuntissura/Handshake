...
ARTIFACTS: all build/test/tooling outputs must be written to `../Handshake_Artifacts/` (full path `D:\\Projects\\LLM projects\\Handshake\\Handshake Worktrees\\Handshake_Artifacts`) and cleaned after WP validation passes before merge-to-main.
...
SPEC_WRITE_AUTHORITY: allowed current Master Spec writer for MANUAL_RELAY pre-launch/enrichment. Resolve `.GOV/spec/SPEC_CURRENT.md` JSON -> indexed manifest -> `spec-modules/`; patch modules, update manifest hashes/metadata, and run spec-current/spec-regression/spec-eof/gov-check.
...
MECHANICAL_INTERVENTION [CX-218K]: before patching, steering, relaying, declaring a stall, or treating handoff/documentation/protocol drift as blocked, classify 3-5 plausible causes including runtime route drift, notification/cursor drift, session/ACP drift, documentation/protocol drift, clock/staleness drift, and scope/worktree drift; then use the cheapest deterministic read or typed helper. For ORCHESTRATOR_MANAGED, use `.GOV/roles_shared/docs/ORCHESTRATOR_MANAGED_WORKFLOW_PLAYBOOK.md`.
SPEC_WRITE_AUTHORITY: allowed current Master Spec writer for approved ORCHESTRATOR_MANAGED pre-launch enrichment. Resolve `.GOV/spec/SPEC_CURRENT.md` JSON -> indexed manifest -> `spec-modules/`; patch modules, update manifest hashes/metadata, and run spec-current/spec-regression/spec-eof/gov-check.
...
MECHANICAL_INTERVENTION [CX-218K]: before patching, steering, relaying, declaring a stall, or treating handoff/documentation/protocol drift as blocked, classify 3-5 plausible causes including runtime route drift, notification/cursor drift, session/ACP drift, documentation/protocol drift, clock/staleness drift, and scope/worktree drift; then use the cheapest deterministic read or typed helper. For ORCHESTRATOR_MANAGED, use `.GOV/roles_shared/docs/ORCHESTRATOR_MANAGED_WORKFLOW_PLAYBOOK.md`.
SPEC_WRITE_AUTHORITY: none. Coder reads resolved SPEC_CURRENT only; if spec text is wrong, missing, or underspecified, STOP and escalate to an allowed spec-writer role.
...
MECHANICAL_INTERVENTION [CX-218K]: before patching, steering, relaying, declaring a stall, or treating handoff/documentation/protocol drift as blocked, classify 3-5 plausible causes including runtime route drift, notification/cursor drift, session/ACP drift, documentation/protocol drift, clock/staleness drift, and scope/worktree drift; then use the cheapest deterministic read or typed helper. For ORCHESTRATOR_MANAGED, use `.GOV/roles_shared/docs/ORCHESTRATOR_MANAGED_WORKFLOW_PLAYBOOK.md`.
SPEC_WRITE_AUTHORITY: none. WP Validator may emit SPEC_GAP/CONCERN/remediation receipts, but must not edit `.GOV/spec/**`.
...
ARTIFACTS: fail per-MT review if runtime/build/artifact output appears in the WP worktree and should be in `../Handshake_Artifacts/` (full path `D:\\Projects\\LLM projects\\Handshake\\Handshake Worktrees\\Handshake_Artifacts`); repo-local `target/`, `dist/`, `coverage/`, `.parcel-cache/`, `node_modules/` are invalid there.
...
ARTIFACTS: repo-local `target/` is invalid; fail if build/test/tool outputs that belong in `../Handshake_Artifacts/` (full path `D:\\Projects\\LLM projects\\Handshake\\Handshake Worktrees\\Handshake_Artifacts`) remain in the WP worktree or product tree.
...
ARTIFACTS: all runtime/build/tool outputs for validator work must live at `../Handshake_Artifacts/` (full path `D:\\Projects\\LLM projects\\Handshake\\Handshake Worktrees\\Handshake_Artifacts`) and must not be committed or left under the WP worktree or product worktree.
...
```text
.GOV/spec/SPEC_CURRENT.md           - machine-readable current spec entrypoint (`handshake.spec_current@1`)
.GOV/spec/indexed_spec/             - current indexed Master Spec manifest and ordered modules
.GOV/spec/Handshake_Master_Spec_*   - source baseline/provenance during indexed spec migration
