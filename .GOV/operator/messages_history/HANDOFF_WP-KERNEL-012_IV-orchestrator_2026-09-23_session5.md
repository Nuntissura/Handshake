...
- **Operator decision 2026-09-23 16:10 (session 6): builders do NOT run expensive tests.** Builder proof = `cargo check` / clippy of the affected feature sets, plus at most ONE cheap focused test when its result decides the next edit. All test execution for acceptance is the validator's (a single independent run). This overrides KB-PROOF-001 / KB-CAD-001 implementer proof runs for WP-KERNEL-012 (Codex CX-021). Builders fix from the validator's failure lists in one batch per round.
- **OUTCOME 2026-09-23 16:30 (session 6), supersedes the two session-6 runtime notes above:** with runtime roots on C: (HANDSHAKE_ARTIFACTS_ROOT relocated) the schema bootstrap tests and mt154 route tests HANG at ~0 CPU. The same tests PASS with runtime on D: and default sync (validator run 11 at e9973e8c). A static audit found no code deadlock and ruled out SURREAL_DATASTORE_SYNC as a waiter; likely an environment-triggered engine retry loop (surrealdb-core-3.2.0 kvs/sequences.rs:562-590, no engine timeout by default; Defender on C: temp paths UNVERIFIED). Rule: runtime roots back on D:, SURREAL_DATASTORE_SYNC unset; targets may stay on C:. The earlier slowness was mostly host load (Steam downloads to C:, Obsidian vault agents on D:).
- **ROOT CAUSE 2026-09-23 17:25 (session 6), supersedes the runtime notes above:** the intermittent zero-CPU test hang was stack-walked from a ProcDump dump (Handshake_Artifacts/WP-KERNEL-012/MT-109/wpv-c3x/logs/hang-run15.stacks.txt, tools gov_runtime/tools minidump-stackwalk + dump_syms): Thread 100 is blocked in `surrealdb_core::kvs::rocksdb::commit_coordinator::CommitBatcher::run` → rocksdb `SyncWAL` → `WinWritableFile::Sync` (OS file sync never returns under disk load); all other threads wait on it. The commit coordinator exists only in the default `SyncMode::Every`. Rule: runtime roots on D:, `SURREAL_DATASTORE_SYNC=never` for all tests except crash/restart/reopen/durability tests (default sync); record the env in proof records; on any new hang take a ProcDump before stopping.
