...
The CLI's local memory endpoint disables authentication; root-only results cannot establish record-user permission equivalence. A small owned loopback server is required for advisory authentication checks. No diagnostic dependency or synthetic fixture substitutes for the real product union.
