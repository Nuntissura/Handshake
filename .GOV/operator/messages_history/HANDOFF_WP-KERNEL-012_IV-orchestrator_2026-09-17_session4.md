...
- **Session-4 end state (2026-09-18 ~00:55Z):** the wpv-v3 set-A `--tests` build (`cargo.exe` PID 19588, started 18:36Z, `-j 4`) had been linking for ~4.5 h and was still advancing (393 targets; late-alphabet targets like `wp_kernel_012_*`, `user_manual_*`, `loom_pins_*` were linking). Before doing anything: (a) `Get-CimInstance Win32_Process` for `cargo.exe`/`rustc.exe`/`link.exe` — if that cargo is STILL RUNNING it belongs to the dead session's validator; do NOT kill it and do NOT start another cargo; wait for it to exit (poll each minute), then check `logs/10-norun-setA.log.exit`; (b) if it is gone and no `.exit` file exists, the build was cut off — the fresh validator re-runs the identical `--no-run` command (incremental; only un-linked targets remain, minutes not hours); (c) then continue the lane plan (lib → set B → set C → exec). Weekly provider limit was the reason for the handoff, not a failure.
