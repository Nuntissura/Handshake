<!-- HANDSHAKE_GENERATED_PROJECTION schema_id=hsk.work_packet_contract@1 source_file=.GOV/task_packets/WP-1-Migration-Framework/packet.json source_hash=26cff621aaad2b9a projection_hash=fd291f3f374308bd generated_at_utc=2026-05-06T15:57:13.225Z generator=wp-contract-import.mjs -->
...
Rewrite all database migrations to use DB-agnostic SQL and implement a professional migration framework (`sqlx::migrate!`). This enforces Pillar 2 of Master Spec Â§2.3.12 (Portable Schema & Migrations).
...
- .GOV/roles_shared/MIGRATION_GUIDE.md (New doc)

### OUT_OF_SCOPE
- PostgreSQL CI infrastructure (â†’ WP-1-Dual-Backend-Tests)
...
- **TEST_PLAN**:
  ...
  ```
- **DONE_MEANS**:
  - âœ… All migrations refactored to use `$n` placeholders (Â§2.3.12.2).
  - âœ… SQLite datetime functions (`strftime`) replaced with `CURRENT_TIMESTAMP` (Â§2.3.12.2).
  - âœ… SQLite-specific triggers removed; logic moved to `src/storage/` (Â§2.3.12.1).
  - âœ… `sqlx::migrate!` integrated into `main.rs` (Â§2.3.12.4).
  - âœ… `.GOV/roles_shared/MIGRATION_GUIDE.md` created with portability rules.
...
## AUTHORITY
- **SPEC_ANCHOR**: Â§2.3.12.1, Â§2.3.12.2, Â§2.3.12.4
...
## VALIDATION (Coder)
- Command: `cargo test --manifest-path src/backend/handshake_core/Cargo.toml` â†’ PASS (warnings: existing dead_code warning in workflows.rs unchanged)
- Command: `just validator-dal-audit` â†’ PASS
- Command: `just validator-hygiene-full` â†’ PASS (traceability WARN only; tool reports warning-level)
- Command: `just post-work WP-1-Migration-Framework` â†’ PASS (tool indicated AI review not required)

---

## VALIDATION REPORT â€” WP-1-Migration-Framework
**Verdict: PASS**

**Scope Inputs:**
- Task Packet: .GOV/task_packets/WP-1-Migration-Framework.md (status: Done)
- Spec: Handshake_Master_Spec_v02.84.md (Â§2.3.12)
...
**Findings:**
- **Requirement Â§2.3.12.2 (Placeholders):** All migrations refactored to use `$n` placeholders. Verified in `0001_init.sql` and `0002_create_ai_core_tables.sql`.
- **Requirement Â§2.3.12.2 (Timestamps):** SQLite datetime functions (`strftime`) replaced with `TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP`. Verified in both migration files.
- **Requirement Â§2.3.12.1 (Triggers):** Zero SQLite-specific triggers found. Application-layer mutation tracking is now the authority.
- **Requirement Â§2.3.12.4 (Framework):** `sqlx::migrate!` integrated into `SqliteDatabase::run_migrations` and invoked in `main.rs`.
...
- **Risk:** Differences in `CURRENT_TIMESTAMP` precision between SQLite (ms) and Postgres (Âµs).
...
- **Protocol:** Added `.GOV/roles_shared/MIGRATION_GUIDE.md` as "LAW" to prevent regression.
