<!-- HANDSHAKE_GENERATED_PROJECTION schema_id=hsk.work_packet_contract@1 source_file=.GOV/task_packets/WP-1-AppState-Refactoring-v2/packet.json source_hash=e97ea8eb5cea6c46 projection_hash=3c6bd4a075037b8b generated_at_utc=2026-05-14T21:13:39.391Z generator=wp-contract-import.mjs -->
...
- **Why**: Remediate Strategic Audit failure in WP-1-AppState-Refactoring. Direct pool exposure (`fr_pool`, `sqlite_pool`) violates the One Storage API boundary mandate of Â§2.3.12.
- **IN_SCOPE_PATHS**:
...
- **TEST_PLAN**:
  ...
  ```
- **DONE_MEANS**:
  * âœ… `AppState` exposes only `Arc<dyn Database>` and `Arc<dyn FlightRecorder>`.
  * âœ… `Database` trait no longer exposes `sqlite_pool()`.
  * âœ… `Janitor` refactored to use `Arc<dyn Database>` and `Arc<dyn FlightRecorder>` instead of concrete pools.
  * âœ… `main.rs` initialization returns trait objects (`Arc<dyn ...>`).
  * âœ… All API handlers consume only trait-based APIs.
...
- **FILES_TO_OPEN**:
  ...
  * .GOV/roles_shared/SPEC_CURRENT.md (v02.93 Â§2.3.12.3)
- **SEARCH_TERMS**:
  * "pub struct AppState"
  * "fn sqlite_pool"
  * "fr_pool"
...
- **SPEC_ANCHOR**: Â§2.3.12.3 [CX-DBP-040], Â§2.3.11.2 [HSK-GC-005]
...
**REASON FOR PASS:** All DONE_MEANS criteria verified with file:line evidence. Implementation aligns with Master Spec v02.93 Â§2.3.12.3 [CX-DBP-040]. No forbidden patterns in production code.
...
Scope Inputs:
- Task Packet: .GOV/task_packets/WP-1-AppState-Refactoring-v2.md
...
- node .GOV/scripts/validation/post-work-check.mjs WP-1-AppState-Refactoring-v2: FAIL (non-ASCII + missing COR-701 validation manifest fields/gates)
