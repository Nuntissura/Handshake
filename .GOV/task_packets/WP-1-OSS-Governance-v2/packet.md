<!-- HANDSHAKE_GENERATED_PROJECTION schema_id=hsk.work_packet_contract@1 source_file=.GOV/task_packets/WP-1-OSS-Governance-v2/packet.json source_hash=96d893a42897017a projection_hash=c5e3c6c296ab87c4 generated_at_utc=2026-05-14T21:13:39.657Z generator=wp-contract-import.mjs -->
...
- REFINEMENT_FILE: .GOV/refinements/WP-1-OSS-Governance-v2.md
- Rule: Task packet creation is blocked until refinement is complete and signed.

## SCOPE
- What: Align OSS governance artifacts to SPEC_CURRENT v02.113 by migrating `.GOV/roles_shared/OSS_REGISTER.md` to the required register schema and updating the backend enforcement test to parse the new schema and enforce dependency coverage + copyleft isolation.
...
- IN_SCOPE_PATHS:
  ...
  - .GOV/task_packets/WP-1-OSS-Governance-v2.md (evidence + validation manifest updates only)
- OUT_OF_SCOPE:
  ...
  - Do not edit `.GOV/roles_shared/TASK_BOARD.md` or `.GOV/roles_shared/WP_TRACEABILITY_REGISTRY.md` (orchestrator maintains).
  - Do not edit `.GOV/refinements/WP-1-OSS-Governance-v2.md` (locked).
...
### DONE_MEANS
- `.GOV/roles_shared/OSS_REGISTER.md` uses the Spec v02.113 register schema required columns (11.7.5.7.1) for its OSS tables; no remaining legacy 5-column header format is used.
...
- `src/backend/handshake_core/tests/oss_register_enforcement_tests.rs` parses the new schema and enforces (11.10.4 item 2):
  - Every crate in `src/backend/handshake_core/Cargo.lock` exists in `.GOV/roles_shared/OSS_REGISTER.md` (match against `name`, case-insensitive).
  - Every npm package in `app/package.json` (dependencies + devDependencies) exists in `.GOV/roles_shared/OSS_REGISTER.md` (match against `name`, case-insensitive).
...
- Prior packet (historical FAIL, outdated): .GOV/task_packets/WP-1-OSS-Governance.md
- Stub backlog item (planning only): .GOV/task_packets/stubs/WP-1-OSS-Governance-v2.md
...
- FILES_TO_OPEN:
  ...
  - .GOV/refinements/WP-1-OSS-Governance-v2.md
  - .GOV/task_packets/WP-1-OSS-Governance-v2.md
...
- Proposed interfaces/types/contracts:
  - OSS Register schema (Spec v02.113 11.7.5.7.1):
    - All OSS tables in `.GOV/roles_shared/OSS_REGISTER.md` use the same 10-column Markdown header (exact match, case + order):
...
Warnings:
  1. Manifest[1]: pre_sha1 matches merge-base(9fb57b2572b2ee933784e69e13da486337daae2c) for .GOV\\\OSS_REGISTER.md (common after WP commits); prefer LF blob SHA1=086fb4516c1d1e492cbc88f567018751c61473a0
...
Scope Inputs:
- Task Packet: .GOV/task_packets/WP-1-OSS-Governance-v2.md
- Spec (resolved via .GOV/roles_shared/SPEC_CURRENT.md): Handshake_Master_Spec_v02.113.md
- Spec anchors used:
  - Handshake_Master_Spec_v02.113.md:46642 (11.7.4.3 Copyleft isolation rule)
  - Handshake_Master_Spec_v02.113.md:46764 (11.7.5.7.1 Register schema required columns)
...
- .GOV/scripts/validation/post-work-check.mjs (read-only; gate behavior reference)

Findings (evidence-based):
- Register schema compliance (11.7.5.7.1):
  - Spec required columns listed at Handshake_Master_Spec_v02.113.md:46764.
  - Register tables use 10-column schema header in .GOV/roles_shared/OSS_REGISTER.md:11 and .GOV/roles_shared/OSS_REGISTER.md:434.
...
- Traceability (active packet mapping):
  - Base WP mapped to active packet at .GOV/roles_shared/WP_TRACEABILITY_REGISTRY.md:98.
...
Workflow / Gates:
- Coder provided pre-commit post-work evidence under .GOV/task_packets/WP-1-OSS-Governance-v2.md:221 showing:
  - just post-work WP-1-OSS-Governance-v2: PASS with warnings (merge-base pre_sha advisory from .GOV/scripts/validation/post-work-check.mjs).
- Validator Gate state recorded in .GOV/validator_gates/WP-1-OSS-Governance-v2.json (Gate 1: report presented; Gate 2: user acknowledged).
