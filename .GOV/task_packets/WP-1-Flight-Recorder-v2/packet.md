<!-- HANDSHAKE_GENERATED_PROJECTION schema_id=hsk.work_packet_contract@1 source_file=.GOV/task_packets/WP-1-Flight-Recorder-v2/packet.json source_hash=50ab8f8a6472d7cb projection_hash=cf165193f836cc29 generated_at_utc=2026-05-14T21:13:39.534Z generator=wp-contract-import.mjs -->
...
- **Why**: Existing implementation is fragmented or non-normative. We need a central, durable, and queryable audit log to meet Phase 1 closure requirements [Â§11.5].
- **IN_SCOPE_PATHS**:
...
- **TEST_PLAN**:
  ...
  ```
- **DONE_MEANS**:
  * âœ… `FlightRecorder` trait implemented with `append_event(event: FlightRecorderEvent)` method per Â§11.5.1.
  * âœ… DuckDB storage sink correctly creates required tables on startup if missing.
  * âœ… Mandatory 7-day rolling retention policy implemented (automatic pruning of older logs).
  * âœ… Canonical `FR-EVT-*` JSON shapes are successfully persisted and queryable by `trace_id`.
  * âœ… All existing tests pass and new tests cover the DuckDB write/read path.
- **HARDENED_INVARIANTS**:
...
## Authority
- **SPEC_ANCHOR**: Â§11.5 (Flight Recorder Event Shapes & Retention)
...
Scope Inputs:
- Task Packet: .GOV/task_packets/WP-1-Flight-Recorder-v2.md (STATUS: Done)
- Spec: Handshake_Master_Spec_v02.96 Â§11.5 (Flight Recorder)
...
Reason for PASS: Trait + DuckDB implementation align with Â§11.5 (event schema, normalization, retention) and tests pass.
...
Scope Inputs:
- Task Packet: .GOV/task_packets/WP-1-Flight-Recorder-v2.md
- Spec Pointer: .GOV/roles_shared/SPEC_CURRENT.md -> Handshake_Master_Spec_v02.98.md (11.5 Flight Recorder Event Shapes & Retention)
- Codex: Handshake Codex v1.4.md
...
- node .GOV/scripts/validation/gate-check.mjs WP-1-Flight-Recorder-v2: FAIL (Implementation detected without SKELETON APPROVED marker.)
- node .GOV/scripts/validation/post-work-check.mjs WP-1-Flight-Recorder-v2: FAIL (non-ASCII packet + missing COR-701 manifest fields/gates)
