<!-- HANDSHAKE_GENERATED_PROJECTION schema_id=hsk.work_packet_contract@1 source_file=.GOV/task_packets/WP-1-Gate-Check-Tool/packet.json source_hash=c0d376b4b29c056e projection_hash=f73ea6eb1fed8461 generated_at_utc=2026-05-06T15:57:13.095Z generator=wp-contract-import.mjs -->
...
**End State:**
- `.GOV/scripts/validation/gate-check.mjs` exists and correctly parses task packets for checkpoint signatures.
- `justfile` includes a `gate-check` command.
- The script prevents merging of phases into a single turn.

### IN_SCOPE_PATHS
- .GOV/scripts/validation/gate-check.mjs (New)
- justfile (Update)
...
- **TEST_PLAN**:
  ...
  node .GOV/scripts/validation/gate-check.mjs WP-1-Migration-Framework
...
- **FILES_TO_OPEN**:
  ...
  * .GOV/scripts/validation/pre-work-check.mjs (as template)
- **SEARCH_TERMS**:
  * "BOOTSTRAP"
  * "SKELETON"
  * "SKELETON APPROVED"
- **RUN_COMMANDS**:
  ```bash
  node .GOV/scripts/validation/gate-check.mjs WP-Test-Sample
  ```
- **RISK_MAP**:
...
## AUTHORITY
- **SPEC_ANCHOR**: [CX-GATE-001] in .GOV/roles/validator/VALIDATOR_PROTOCOL.md
...
- CLI: `node .GOV/scripts/validation/gate-check.mjs {wp-id}` and `just gate-check {wp-id}`.

SKELETON APPROVED: 2025-12-25 (Orchestrator message)


## VALIDATION (Coder)
- Command: `node .GOV/scripts/validation/gate-check.mjs WP-1-Gate-Check-Tool` -> PASS
- Command: `node .GOV/scripts/validation/gate-check.mjs WP-1-Migration-Framework` -> PASS
...
**Scope Inputs:**
- Task Packet: .GOV/task_packets/WP-1-Gate-Check-Tool.md
...
**Tests:**
- `node .GOV/scripts/validation/gate-check.mjs WP-1-Gate-Check-Tool`: **PASS**.
...
Scope Inputs:
- Task Packet: .GOV/task_packets/WP-1-Gate-Check-Tool.md (STATUS: Done [VALIDATED])
- Spec: [CX-GATE-001] Binary Phase Gate (.GOV/roles/validator/VALIDATOR_PROTOCOL.md)
- Codex: Handshake Codex v1.4.md

Files Checked:
- .GOV/scripts/validation/gate-check.mjs:5-55 (checks BOOTSTRAP/SKELETON/APPROVAL markers and sequencing)
...
Scope Inputs:
- Task Packet: .GOV/task_packets/WP-1-Gate-Check-Tool.md
- Spec Anchor: .GOV/roles/validator/VALIDATOR_PROTOCOL.md (CX-GATE-001 Binary Phase Gate)
- Codex: Handshake Codex v1.4.md
- Validator Protocol: .GOV/roles/validator/VALIDATOR_PROTOCOL.md

Commands (evidence):
- just validator-spec-regression: PASS
- node .GOV/scripts/validation/gate-check.mjs WP-1-Gate-Check-Tool: FAIL ("SKELETON APPROVED marker must follow SKELETON")
- node .GOV/scripts/validation/post-work-check.mjs WP-1-Gate-Check-Tool: FAIL (non-ASCII packet + missing COR-701 manifest fields/gates)
- just validator-packet-complete WP-1-Gate-Check-Tool: PASS

Blocking Findings:
1) Phase gate FAIL: `node .GOV/scripts/validation/gate-check.mjs WP-1-Gate-Check-Tool` fails due to an unanchored APPROVAL regex match.
   - First "SKELETON APPROVED" occurrence is in prose (.GOV/task_packets/WP-1-Gate-Check-Tool.md:18) before the SKELETON section (.GOV/task_packets/WP-1-Gate-Check-Tool.md:79).
   - gate-check uses pattern /SKELETON APPROVED/i (not anchored to headings), so prose can trip ordering (.GOV/scripts/validation/gate-check.mjs:23-26).
2) Deterministic manifest gate FAIL: `node .GOV/scripts/validation/post-work-check.mjs WP-1-Gate-Check-Tool` fails.
...
- Harden .GOV/scripts/validation/gate-check.mjs to match only explicit headings/markers (e.g., '^## BOOTSTRAP', '^## SKELETON', and a dedicated 'SKELETON APPROVED' marker line) and avoid matching prose (code change required; not performed in this revalidation).
