<!-- HANDSHAKE_GENERATED_PROJECTION schema_id=hsk.work_packet_contract@1 source_file=.GOV/task_packets/WP-1-Validator-Error-Codes-v1/packet.json source_hash=7b5609523f456dad projection_hash=6e91af04dd1937d7 generated_at_utc=2026-05-06T15:57:13.531Z generator=wp-contract-import.mjs -->
...
- Governance / validators:
  ...
  - `.GOV/task_packets/WP-1-Debug-Bundle-v3.md` (append-only note: dependency)
...
- **Validator Protocol**: `.GOV/roles/validator/VALIDATOR_PROTOCOL.md` (typed errors + determinism enforcement)
...
- Target File: `.GOV/scripts/validation/validator-error-codes.mjs`
- Start: 9
- End: 207
- Line Delta: 135
...
- 2025-12-29: `node .GOV/scripts/validation/validator-error-codes.mjs` ran (FAIL). Findings: `ace/validators/cache.rs:27,31,36`; `api/bundles.rs:110,117`; `main.rs:173`.
...
Scope Inputs:
- Task Packet: .GOV/task_packets/WP-1-Validator-Error-Codes-v1.md (status: Done)
- Spec: .GOV/roles_shared/SPEC_CURRENT.md -> Handshake_Master_Spec_v02.98.md (anchor: 4.2.3 / HSK-TRAIT-004)
- Codex: Handshake Codex v1.4.md
...
Findings:
- DONE_MEANS: `just validator-error-codes` passes (evidence: .GOV/task_packets/WP-1-Validator-Error-Codes-v1.md:178).
...
- Validator is sandbox-compatible (no child_process usage) and waiver-aware (evidence: .GOV/scripts/validation/validator-error-codes.mjs:13, .GOV/scripts/validation/validator-error-codes.mjs:130).

Tests:
- `just validator-error-codes`: PASS (.GOV/task_packets/WP-1-Validator-Error-Codes-v1.md:178)
- `cargo test --manifest-path src/backend/handshake_core/Cargo.toml`: PASS (.GOV/task_packets/WP-1-Validator-Error-Codes-v1.md:179)
...
- Waiver-aware validator: .GOV/scripts/validation/validator-error-codes.mjs:13 and .GOV/scripts/validation/validator-error-codes.mjs:130
...
- `just validator-scan`: PASS (.GOV/task_packets/WP-1-Validator-Error-Codes-v1.md:180)
- `just cargo-clean`: PASS (.GOV/task_packets/WP-1-Validator-Error-Codes-v1.md:181)
...
- Git warns about LF->CRLF normalization for `.GOV/scripts/validation/validator-error-codes.mjs`; align `.gitattributes` or `core.autocrlf` if this becomes noisy.
- `.GOV/scripts/validation/post-work-check.mjs` requires subprocess spawning; sandboxed runs need escalation (or a future refactor to avoid child_process).
