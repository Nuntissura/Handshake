...
- IN_SCOPE:
  ...
  - A single â€œMaterializeâ€ API for all save/export-to-path writes (atomic tmp+rename; no UI bypass; capability-gated; Flight Recorder logged).
...
- Risk: Multiple competing â€œexport/materializeâ€ implementations cause drift and bypass policies; must centralize.
...
- [ ] Create `.GOV/refinements/WP-1-Artifact-System-Foundations-v1.md` (approved/signed).
- [ ] Create the official task packet via `just create-task-packet WP-1-Artifact-System-Foundations-v1` (in `.GOV/task_packets/`).
