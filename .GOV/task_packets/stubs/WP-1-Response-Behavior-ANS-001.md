...
- Brainstorm mode can omit determinism-only fields but must still respect the contractâ€™s allowed-operations constraints.
...
- Risk of scope overlap with Spec Router/governance session log; activation should confirm boundary between â€œmode selectionâ€ and â€œresponse contract enforcementâ€.
...
- [ ] Create `.GOV/refinements/WP-1-Response-Behavior-ANS-001.md` (approved/signed).
- [ ] Create official task packet via `just create-task-packet WP-1-Response-Behavior-ANS-001` (in `.GOV/task_packets/`).
