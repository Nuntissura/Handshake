...
# Work Packet Stub: WP-1-Charts-Dashboards-Backfill-v1

## STUB_METADATA
- WP_ID: WP-1-Charts-Dashboards-Backfill-v1
- BASE_WP_ID: WP-1-Charts-Dashboards-Backfill
...
- BUILD_ORDER_DEPENDS_ON: WP-1-Docs-Sheets-Runtime-Backfill, WP-1-Dev-Command-Center-MVP
- BUILD_ORDER_BLOCKS: WP-1-Presentations-Decks-Backfill
...
## INTENT (DRAFT)
- What: Backfill Charts & Dashboards as explicit runtime outputs with chart specs, source-table lineage, and operator evidence.
- Why: Charts are a high-ROI combination surface for docs, decks, DCC, and analytics, but current appendix coverage is still missing.

## SCOPE_SKETCH (DRAFT)
- IN_SCOPE:
  - ChartSpec-driven runtime coverage.
  - Source-table / trace lineage and export posture.
- OUT_OF_SCOPE:
  - Full charting feature build-out.
  - Advanced BI/dashboard implementation.
...
- Charts feature row exists with explicit runtime mapping.
- Chart source lineage is preserved in appendix/runtime guidance.

## DEPENDENCIES / BLOCKERS (DRAFT)
- Depends on Docs & Sheets runtime mapping for source-table semantics.

## RISKS / UNKNOWNs (DRAFT)
- Risk: charts are treated as static presentation garnish instead of governed derived artifacts.
...
- [ ] Create `.GOV/refinements/WP-1-Charts-Dashboards-Backfill-v1.md` (approved/signed).
- [ ] Create the official task packet via `just create-task-packet WP-1-Charts-Dashboards-Backfill-v1` (in `.GOV/task_packets/`).
