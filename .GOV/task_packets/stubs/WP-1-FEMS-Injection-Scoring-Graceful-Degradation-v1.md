...
- IN_SCOPE:
  ...
  - Progressive retrieval tiers: Tier 1 SurrealDB exact/indexed lookup (~0ms) → Tier 2 SurrealDB search/graph query (~5ms target) → Tier 3 SurrealDB vector similarity (~50ms) → Tier 4 bounded SurrealDB graph traversal (~100ms). Each tier is optional; pack builds from whatever tiers complete within a configurable time budget.
  - Graceful degradation: under load, skip Tier 3-4 and build from SurrealDB exact/search/graph capabilities only. Log degradation tier in MemoryPack warnings. No hard failure — always produce a pack.
  ...
  - Cross-encoder reranking (research §4.1): optional Tier 2.5 — after SurrealDB broad retrieval, narrow with BGE-reranker (self-hosted cross-encoder). +50-100ms but significantly better precision. Skipped under load (progressive degradation), used when latency budget allows.
...
- TOUCHED_OR_UNKNOWN_PILLARS:
  ...
  - PILLAR: RAG | STATUS: TOUCHED | NOTES: shares SurrealDB retrieval infrastructure (exact/search, vector, typed relations/graph); progressive retrieval pattern applies to both | Stub follow-up: NONE
...
- MemoryPack builds succeed under simulated heavy CPU load by degrading to SurrealDB exact/search/graph capabilities only.
