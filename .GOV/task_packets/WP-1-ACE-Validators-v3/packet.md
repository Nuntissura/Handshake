<!-- HANDSHAKE_GENERATED_PROJECTION schema_id=hsk.work_packet_contract@1 source_file=.GOV/task_packets/WP-1-ACE-Validators-v3/packet.json source_hash=ac0a1f4bff0b0d23 projection_hash=ab90df2883462883 generated_at_utc=2026-05-06T15:57:12.819Z generator=wp-contract-import.mjs -->
...
- **TEST_PLAN**:
  ...
  ```
- **DONE_MEANS**:
  * âœ… `AceRuntimeValidator` trait implemented with `resolve_content()` support per Â§2.6.6.7.11.0.
  * âœ… `PromptInjectionGuard` successfully detects injection in NFC-normalized substrings per Â§2.6.6.7.11.6.
  * âœ… `CloudLeakageGuard` performs recursive classification checks on composite `SourceRefs` per Â§2.6.6.7.11.5.
  * âœ… Every safety violation emits `FR-EVT-SEC-VIOLATION` to Flight Recorder.
- **ROLLBACK_HINT**:
  ```bash
...
- **FILES_TO_OPEN**:
  ...
  * .GOV/roles_shared/SPEC_CURRENT.md (Master Spec v02.91 Â§2.6.6.7.11)
...
## Authority
- **SPEC_ANCHOR**: Â§2.6.6.7.11 (ACE Security Guards)
...
Scope Inputs:
- Task Packet: .GOV/task_packets/WP-1-ACE-Validators-v3.md (STATUS: Done)
- Spec: Handshake_Master_Spec_v02.93 (Â§2.6.6.7.11)
- Codex: Handshake Codex v1.4.md
...
- Pipeline includes all Â§2.6.6.7.11 guards; PromptInjectionGuard enforces substring patterns and triggers poisoning; CloudLeakageGuard blocks non-exportable/high-sensitivity refs recursively.
...
REASON FOR PASS: Implementation conforms to Master Spec v02.93 Â§2.6.6.7.11 with content-aware scanning, NFC normalization, and atomic poisoning; targeted validator tests pass.

## VALIDATION REPORT â€” 2025-12-27 (Revalidation)
Verdict: FAIL

Scope Inputs:
- Task Packet: .GOV/task_packets/WP-1-ACE-Validators-v3.md (STATUS: Done/Validated)
- Spec: Packet references Handshake_Master_Spec_v02.91 (A2.6.6.7.11); .GOV/roles_shared/SPEC_CURRENT.md now points to Handshake_Master_Spec_v02.93.
- Codex: Handshake Codex v1.4.md
...
Scope Inputs:
- Task Packet: .GOV/task_packets/WP-1-ACE-Validators-v3.md
...
- node .GOV/scripts/validation/post-work-check.mjs WP-1-ACE-Validators-v3: FAIL (non-ASCII + missing COR-701 validation manifest fields/gates)
...
- Spec mismatch: this packet asserts Master Spec v02.93 (and earlier v02.91), but .GOV/roles_shared/SPEC_CURRENT.md points to v02.98. Spec-to-code alignment is not established against the current spec.
