<!-- HANDSHAKE_GENERATED_PROJECTION schema_id=hsk.work_packet_contract@1 source_file=.GOV/task_packets/WP-1-Terminal-LAW/packet.json source_hash=1360d597f5afb52e projection_hash=a870c584fb6f8371 generated_at_utc=2026-05-06T16:33:39.607Z generator=wp-contract-import.mjs -->
# Task Packet: WP-1-Terminal-LAW

## Metadata
- TASK_ID: WP-1-Terminal-LAW
- ROLE: Orchestrator
...
**Procedure:** 
1. Validator/Coder must search for terminal execution hardening.
2. Verify implementation matches **100% of the technical rules, schemas, and invariants** found in the Main Body (Â§7.6.3.14 v02.36 / Â§11.8).
3. Surface-level compliance with roadmap bullets is insufficient. Every line of text in the Main Body section must be implemented (Timeout, kill_grace, max_output_bytes).
4. If 100% alignment exists -> **PASS**. Otherwise -> **FAIL**.

---

## SCOPE
- **What**: Implement the Terminal LAW (hardened execution, timeout, session binding).
- **Why**: Ensure terminal command execution cannot bypass governance or exhaust resources.
- **SPEC_ANCHOR**: Â§7.6.3.14, Â§11.8
...
just validator-scan WP-1-Terminal-LAW
...
## VALIDATION REPORT — WP-1-Terminal-LAW
Verdict: FAIL

Scope Inputs:
- Task Packet: .GOV/task_packets/WP-1-Terminal-LAW.md (status: Ready for Dev)
- Spec: - **SPEC_ANCHOR**: Â§7.6.3.14, Â§11.8
