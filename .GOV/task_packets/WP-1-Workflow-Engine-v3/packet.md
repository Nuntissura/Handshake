<!-- HANDSHAKE_GENERATED_PROJECTION schema_id=hsk.work_packet_contract@1 source_file=.GOV/task_packets/WP-1-Workflow-Engine-v3/packet.json source_hash=8c0d40a037ee2b48 projection_hash=15598d7ba6adcac9 generated_at_utc=2026-05-14T21:13:39.889Z generator=wp-contract-import.mjs -->
...
## Scope
- **What**: Implement mandatory boot-time crash recovery for interrupted workflows per Â§2.6.1.
...
- **TEST_PLAN**:
  ...
  ```
- **DONE_MEANS**:
  * âœ… `main.rs` calls `mark_stalled_workflows` (or equivalent) during initialization.
  * âœ… Recovery loop is non-blocking (does not delay API startup).
  * âœ… Transitions are logged to Flight Recorder with event type `FR-EVT-WF-RECOVERY`.
  * âœ… All existing workflow tests pass.
...
- **FILES_TO_OPEN**:
  ...
  * .GOV/roles_shared/SPEC_CURRENT.md (v02.93)
- **SEARCH_TERMS**:
...
- **SPEC_ANCHOR**: Â§2.6.1 [HSK-WF-002], [HSK-WF-003]
...
Scope Inputs:
- Task Packet: .GOV/task_packets/WP-1-Workflow-Engine-v3.md (STATUS: VALIDATED)
- Spec: Handshake_Master_Spec_v02.93 (A2.6.1 [HSK-WF-002], [HSK-WF-003]) via .GOV/roles_shared/SPEC_CURRENT.md
...
Scope Inputs:
- Task Packet: .GOV/task_packets/WP-1-Workflow-Engine-v3.md
...
- node .GOV/scripts/validation/post-work-check.mjs WP-1-Workflow-Engine-v3: FAIL (non-ASCII + missing COR-701 validation manifest fields/gates)
