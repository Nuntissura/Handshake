<!-- HANDSHAKE_GENERATED_PROJECTION schema_id=hsk.refinement_contract@1 source_file=.GOV/task_packets/WP-1-Governance-Workflow-Mirror-v1/packet.json source_hash=237a7a818ef1b37a projection_hash=b4994520a52d60e6 generated_at_utc=2026-05-06T15:57:13.114Z generator=wp-contract-import.mjs -->
...
- CREATED_AT: 2026-04-09T18:17:34.004Z
...
- USER_APPROVAL_EVIDENCE: APPROVE REFINEMENT WP-1-Governance-Workflow-Mirror-v1
- STUB_WP_IDS: NONE

### REQUIRED SECTIONS (per the current ORCHESTRATOR_PROTOCOL refinement workflow)

### GAPS_IDENTIFIED
- `runtime_governance.rs` exposes runtime task-board/work-packet/governance-decision/auto-signature paths, but not workflow-mirror paths for per-WP validator gates or activation traceability.
- Locus already owns canonical work-tracking identity/state. Widening base Locus families to absorb overlay-only gate ledgers and activation traceability would mix canonical work tracking with imported governance overlay state.
- `governance_artifact_registry.rs` and `governance_check_runner.rs` exist, but no durable workflow-mirror layer currently ties registry entries, check runs, task-board projections, and activation traceability into one runtime-owned governance view.
- `role_mailbox.rs` already persists `spec_session_log_entries`, but the current append path is mailbox-owned rather than workflow-facing.
- `storage/mod.rs` already defines governance check run structs, but default trait methods remain `NotImplemented`; the concept exists, the durability path is partial.
- The local boundary docs reject a general repo-governance mirror. The viable resolution is a narrow runtime-owned workflow-mirror layer only for software-delivery governance state.

### LANDSCAPE_SCAN (prior art / better approaches)
- TIMEBOX: 2h
- SEARCH_SCOPE: Local-first only. Reviewed the current Master Spec, local governance docs, task board, traceability registry, target stub, and current `../handshake_main` backend code. Inspected only two directly analogous refinements: artifact registry and check runner.
...
- `FR-EVT-GOV-GATES-001` must fire on every runtime validator-gate transition written by the workflow mirror.
- `FR-EVT-GOV-WP-001` must fire when a stub is activated into an active packet mapping inside the runtime mirror.
...
- CODE_REALITY_EVIDENCE:
  ...
  - Path: ../handshake_main/src/backend/handshake_core/src/role_mailbox.rs | Artifact: WP-1-Role-Mailbox-v1 | Covers: execution | Verdict: PARTIAL | Notes: `spec_session_log_entries` and `append_spec_session_log` exist, but the access pattern is mailbox-owned instead of workflow-mirror-oriented.
