<!-- HANDSHAKE_GENERATED_PROJECTION schema_id=hsk.work_packet_contract@1 source_file=.GOV/task_packets/WP-1-Capability-Enforcement/packet.json source_hash=6ae1e4edb587d539 projection_hash=a0493c15a3518980 generated_at_utc=2026-05-06T16:33:39.103Z generator=wp-contract-import.mjs -->
...
2. Verify implementation matches **100% of the technical rules, schemas, and invariants** found in the Main Body (Â§11.1 Capability Registry / Â§11.3 MCP Gate).
3. Surface-level compliance with roadmap bullets (Â§7.6.3.4) is insufficient. Implementation must use the Central Registry (SSoT) and follow axis/scope resolution.
...
- **TEST_PLAN**:
  ...
  pnpm -C app test
  node .GOV/scripts/validation/post-work-check.mjs WP-1-Capability-Enforcement
  ```
- **DONE_MEANS**:
...
- **FILES_TO_OPEN**:
  * Handshake_Master_Spec_v02.50.md (Â§7.6.3 Item 4)
  * src/backend/handshake_core/src/workflows.rs
  * src/backend/handshake_core/src/jobs.rs
  * src/backend/handshake_core/src/models.rs
- **SEARCH_TERMS**:
...
## VALIDATION REPORT â€” WP-1-Capability-Enforcement
Verdict: FAIL

Scope Inputs:
- Task Packet: .GOV/task_packets/WP-1-Capability-Enforcement.md (status: In Progress)
...
Findings:
- **Architectural Gap (SSoT) [HSK-4001]**: Implementation in `workflows.rs` uses a local `Lazy<HashMap>` profile map. Spec v02.84 Â§11.1 mandates a centralized `CapabilityRegistry` (SSoT).
- **Axis/Scope Logic**: Implementation lacks the `axis:scope` resolution logic defined in Â§11.1.3.
...
- **Spec Regression**: Packet references v02.50. MUST align with v02.84 Â§11.1.
