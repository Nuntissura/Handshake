<!-- HANDSHAKE_GENERATED_PROJECTION schema_id=hsk.work_packet_contract@1 source_file=.GOV/task_packets/WP-1-Terminal-Integration-Baseline/packet.json source_hash=976ba86764076900 projection_hash=88be9fc65327ebe4 generated_at_utc=2026-05-06T15:57:13.504Z generator=wp-contract-import.mjs -->
...
- **TEST_PLAN**:
  ...
  ```
- **DONE_MEANS**:
  - âœ… `TerminalService` exists and wraps `tokio::process::Command` (not `std::process`).
  - âœ… Placeholder "terminal jobs are disabled" in `workflows.rs` is REMOVED.
  - âœ… Terminal jobs ONLY run if the job profile contains the `term.exec` capability.
  - âœ… Command stdout/stderr is captured and logged to Flight Recorder.
  - âœ… Success and failure (missing capability) are verified by integration tests.
  - âœ… Evidence mapping block is complete and accurate.
...
- **FILES_TO_OPEN**:
  ...
  * Handshake_Master_Spec_v02.84.md (Â§2.3.12, Â§7.6.3)
...
## AUTHORITY
- **SPEC_ANCHOR**: Â§7.6.3 Item 14 (Terminal LAW)
- **Codex**: Handshake Codex v1.4.md
- RE-OPENED: Validation failed. Feature is hard-blocked in code and evidence mapping is missing.

## VALIDATION REPORT â€” WP-1-Terminal-Integration-Baseline
Verdict: PASS

Scope Inputs:
- Task Packet: .GOV/task_packets/WP-1-Terminal-Integration-Baseline.md
...
## VALIDATION REPORT â€” WP-1-Terminal-Integration-Baseline (Final PASS)
Verdict: PASS

Scope Inputs:
- Task Packet: .GOV/task_packets/WP-1-Terminal-Integration-Baseline.md
- Spec: Handshake_Master_Spec_v02.84.md (Â§7.6.3 Terminal LAW)
- Codex: Handshake Codex v1.4.md
