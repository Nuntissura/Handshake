...
[CX-113] HARD_MAIN_CANONICAL_BACKUP_MODEL (HARD): `main` is the sole canonical integrated branch on disk and on GitHub. Role/user branches (`user_ilja`, `role_orchestrator`, `gov_kernel`) are backup branches. They MAY diverge from `main` and MUST NOT be treated as canonical integration targets. Permanent non-main worktrees (`wt-ilja`, `wt-orchestrator`, `wtc-*`) take their non-`.GOV/` base from local `main`; their matching GitHub branches are safety copies, not the refresh source for product code or root-level LLM files. The `role_orchestrator` branch MUST NOT be merged into `main` — its `.GOV/` is a junction (files absent from git tracking) which creates irreconcilable modify/delete conflicts. Non-`.GOV/` changes from the orchestrator worktree (`AGENTS.md`, `justfile`) reach `main` by file copy + commit on main, not by merge or cherry-pick. The `gov_kernel` branch reaches `main` only through `just sync-gov-to-main` [CX-212D].

[CX-113A] MAIN_ONLY_ROOT_REPO_CONTROL_FILES (HARD): Root-level repo control files inherited from `main`, currently `AGENTS.md` and the root `justfile`, are main-only authoring surfaces. Orchestrator, Integration Validator, or any other role MAY change them only from the `handshake_main` worktree on local `main`, then commit on `main` and refresh/reseed non-main worktrees from that canonical base. Do NOT author or commit these files from `wt-orchestrator`, `wt-ilja`, or any `wtc-*` WP worktree. Backup branches may carry earlier copies, but they are not valid authoring surfaces for new root-file changes.
...
[CX-964] CHANGE_SUMMARY_V08_5: v0.8 adds [CX-213] requiring `.GOV/task_packets/` directory and [CX-906] requiring role protocol files under `.GOV/roles/`.
