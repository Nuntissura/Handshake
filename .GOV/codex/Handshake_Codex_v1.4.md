...
[CX-908] HARD_DETERMINISTIC_ATOMIC_GOVERNANCE_FILES (HARD): New or substantially refactored repo-governance workflow surfaces MUST move toward a single deterministic machine-readable authority file, not manually maintained Markdown-plus-JSON sidecars. Work packets, Activation Manager refinements, microtasks, startup capsules, runtime state, receipts, validator reports, workflow dossiers, and templates SHOULD use typed JSON/JSONL/YAML-compatible contracts that ACP, apps, checks, and other tools can ingest without parsing narrative Markdown. When a machine-readable contract exists for a surface, that contract is the authority; Markdown is a generated human-readable projection, generated role-facing reference, or legacy frozen reference and MUST NOT be edited as a second source of truth. Sidecars are allowed only as a short migration bridge, generated cache, or read-only projection with source hash/provenance. During migration, legacy Markdown remains authoritative only until the machine contract is generated, validated, and declared current. Any governance change that touches packet, refinement, microtask, startup, dossier, or protocol surfaces MUST update the single authoritative machine contract or record explicit migration debt with a concrete RGF/task-board item.
...
The governance kernel worktree is the deterministic testbed for Handshake Product governance. Governance workflow artifacts must move toward machine-readable, atomic, deterministic contracts that ACP, external apps/tools, and later the Handshake Product can ingest directly. Markdown/prose is projection/reference only when explicitly requested or required by an explicit report/projection contract. New governance workflow behavior should be designed as reusable product-governance primitives, not as repo-local prose rituals. Non-Coder roles must address drift risks autonomously by adding typed contracts, schemas, generated projections, hashes/provenance, and checks instead of waiting for Operator input when the choice is governance hardening rather than product scope.
...
Deterministic atomic governance files are the direction of travel: where machine contracts exist, consume the single authoritative typed packet/refinement/microtask/startup/runtime truth before generated Markdown projections [CX-908]. Legacy Markdown is a migration safety rail only and MUST NOT be copied forward as the future model-created artifact pattern [CX-914]. Keep workflow playbooks current as projections over machine workflow contracts and role protocols [CX-909].
