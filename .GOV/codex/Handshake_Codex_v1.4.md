...
[CX-212D] GOV_KERNEL_SYNC_AND_WP_WORKTREES (HARD): Synchronizing the governance kernel `/.GOV/` to the `main` worktree (`just sync-gov-to-main`) is the Integration Validator's responsibility, to be run before pushing to `origin/main`. The Orchestrator MUST NOT run `sync-gov-to-main`. The `main` worktree retains a real (non-junction) `/.GOV/` copy as a stable backup. WP worktrees created from `main` MUST NOT retain a git-tracked `/.GOV/` directory; the worktree creation script MUST remove any inherited `/.GOV/` and create a junction to the governance kernel instead. All non-main worktrees (`wt-orchestrator`, `wtc-*`, `wtv-*`) access `/.GOV/` via junction to the governance kernel — live, always current. The Integration Validator operates from `handshake_main` on branch `main` (no WP-specific worktree).
