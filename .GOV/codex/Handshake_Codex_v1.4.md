...
[CX-VAL-007] Test entrypoint: every proof run uses only the checked-in test command and the version-controlled config files named in HBR `test_environment_prerequisites.test_entrypoint`; agents add no environment variables, flags or wrappers at run time. A setting stated in a handoff, chat or brief is never authority; changing the environment is a commit to that config. Where a WP has a canary ([CX-VAL-005]), the canary is that entrypoint on one test per package.

[CX-VAL-008] Closure loop: after a FAIL verdict the next claim names that verdict's remediation (failed acceptance refs, failed checks, artifact refs, affected code refs, repair direction; all refs, each text field at most 120 chars, gameplan writing style). A resubmission after a FAIL must change the affected code or cite a new diagnosis id, else it is not admitted. A diagnosis is recorded once, as a blocker record with a diagnosis id, a cause of at most 120 chars, evidence refs and `distinct_from`, and is cited by `diagnosis_refs` and by gameplan step sources. Two FAIL verdicts on the same `failure_class` without a new diagnosis set NEEDS_NEW_APPROACH ([CX-EXEC-013]).

[CX-VAL-009] Proof binding: a verdict binds to the exact candidate SHA and this round's results (`results_ref`, `candidate_commit`, `started_at`, `completed_at`, an `artifact_ref` per failure). Results from another commit or older than the round start are infrastructure, never a verdict. At round freeze every PASS MT whose relevant input paths intersect the candidate diff returns to READY_FOR_VALIDATION and is included in the round ([CX-VAL-002]). A FAIL verdict requires at least one product failure ([CX-VAL-006]); an implementer's compile or static result counts only when the input digest before the check equals the digest after it.
