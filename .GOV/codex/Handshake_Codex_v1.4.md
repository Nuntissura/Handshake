...
[CX-503K] FAILURE_MEMORY (RECOMMENDED): Error patterns and fix patterns from smoketest reviews and validator findings SHOULD be stored in a searchable index. Future coder sessions query this index at startup to avoid repeating known mistakes. This enables cross-WP learning (pattern: A-MEM, desplega-ai Agent Swarm). [RGF-103]
...
[CX-503M] LOCAL_MODEL_ROUTING (RECOMMENDED): When Ollama or other local model providers are available, simple MTs (struct definitions, import fixes, test scaffolding) SHOULD route to local models. Complex MTs (cross-module integration, security-sensitive code) route to cloud models. The MT task board indicates complexity tier per MT. Auto-escalate to cloud on local model failure (pattern: OpenClaw Gateway, K2.5 frozen sub-agents). [RGF-109]
