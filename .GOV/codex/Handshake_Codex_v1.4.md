...
[CX-105B] HARD_INDEXED_MASTER_SPEC_WRITE_PATH (HARD): Current Master Spec edits target the indexed module set, not the legacy monolith filename. A spec-writing role MUST resolve `.GOV/spec/SPEC_CURRENT.md`, inspect `.GOV/spec/indexed_spec/spec-modules/module-index.md` and the manifest, patch the smallest governing module(s), and update manifest hashes/metadata and reconstruction metadata in the same change. `SPEC_CURRENT.md` is edited only when the active entrypoint, version, human index path, manifest path, or source-baseline metadata changes.
...
[CX-205F] EXTERNAL_BUILD_ARTIFACT_ROOT: Build/test/tool outputs SHOULD live outside the repo working tree at the external sibling root `../Handshake_Artifacts/` unless a tool requires another explicitly documented location.
...
[CX-407] SPEC_VERSIONING: Durable modifications to the Master Spec (LAW_2) MUST update the indexed spec version metadata when they change product law, architecture, primitives, shared contracts, or other durable Main Body truth. The normal edit path is to patch indexed modules, update manifest hashes/line counts/byte counts/heading metadata plus reconstruction metadata, and update `SPEC_CURRENT.md` only when its entrypoint/version/source-baseline fields change. Do not rename legacy monolith files as the routine versioning mechanism.
[CX-407A] SPEC_INDEXED_MANIFEST_INTEGRITY: A spec-writing role that changes indexed modules MUST leave `.GOV/spec/indexed_spec/indexed-spec-manifest.json` internally consistent and run the shared spec-current validation checks before closeout. Module content, manifest metadata, `module-index.md`, and reconstruction provenance MUST agree.
...
[CX-503K] GOVERNANCE_MEMORY_SYSTEM: Governance memory is a cross-session, cross-WP knowledge system stored in `gov_runtime/roles_shared/GOVERNANCE_MEMORY.db` (SQLite, WAL mode, busy_timeout=5s for concurrent writer safety). It stores three memory types: episodic (session events), semantic (distilled facts and patterns), and procedural (fix recipes and workflows â€” the fail log). Memory is populated mechanically from WP receipts, smoketest findings, check failures, and session-end flushes. Refreshed automatically at every role startup and during `just gov-check`. Memory is NOT a source of truth â€” work packets, receipts, and governance ledgers remain authoritative. Memory is supplementary context that helps roles avoid repeating known mistakes. The `gov_runtime/` directory is included in backup snapshots; `just memory-export` provides git-trackable JSONL archival. [RGF-103, RGF-115â€“143]
...
- **Entrypoint/Manifest Update:** When a spec version metadata update occurs, `.GOV/spec/SPEC_CURRENT.md`, `.GOV/spec/indexed_spec/indexed-spec-manifest.json`, and `.GOV/spec/indexed_spec/spec-modules/module-index.md` MUST stay mutually consistent. `SPEC_CURRENT.md` changes only when its current version, active manifest path, human index path, or source-baseline metadata changes.
...
[CX-908] HARD_DETERMINISTIC_ATOMIC_GOVERNANCE_FILES (HARD): New or substantially refactored repo-governance workflow surfaces MUST move toward a single deterministic machine-readable authority file, not manually maintained Markdown-plus-JSON sidecars. Work packets, Activation Manager refinements, microtasks, startup capsules, runtime state, receipts, validator reports, workflow dossiers, and templates SHOULD use typed JSON/JSONL/YAML-compatible contracts that ACP, apps, checks, and other tools can ingest without parsing narrative Markdown. When a machine-readable contract exists for a surface, that contract is the authority; Markdown is a generated human/operator projection or legacy frozen reference and MUST NOT be edited as a second source of truth. Sidecars are allowed only as a short migration bridge, generated cache, or read-only projection with source hash/provenance. During migration, legacy Markdown remains authoritative only until the machine contract is generated, validated, and declared current. Any governance change that touches packet, refinement, microtask, startup, dossier, or protocol surfaces MUST update the single authoritative machine contract or record explicit migration debt with a concrete RGF/task-board item.

[CX-909] HARD_WORKFLOW_PLAYBOOK_UPKEEP (HARD): Workflow playbooks are projection/reference surfaces over machine-readable workflow contracts and role protocols, not independent authority. Any governance change that modifies workflow contracts, role startup/session-control behavior, role protocol workflow law, packet/refinement/MT lifecycle rules, or ACP-consumed workflow capsules MUST update or regenerate the affected workflow playbook projection in the same change. If the playbook projection cannot be updated immediately, the change MUST record explicit migration/projection debt in the governance refactor task board or changelog with the exact stale surface, owner, and intended repair. Do not let workflow playbooks silently rot behind machine-readable contract truth.
...
The governance kernel worktree is the deterministic testbed for Handshake Product governance. Governance workflow artifacts must move toward machine-readable, atomic, deterministic contracts that ACP, external apps/tools, and later the Handshake Product can ingest directly. Operator-facing Markdown is projection/reference only when a contract exists. New governance workflow behavior should be designed as reusable product-governance primitives, not as repo-local prose rituals. Non-Coder roles must address drift risks autonomously by adding typed contracts, schemas, generated projections, hashes/provenance, and checks instead of waiting for Operator input when the choice is governance hardening rather than product scope.

[CX-912] HARD_GOVERNANCE_TOPOLOGY_LEDGER (HARD)
The repo governance surface MUST maintain a machine-readable topology ledger at `.GOV/roles_shared/records/GOVERNANCE_TOPOLOGY.json`. This ledger is the intended single inventory for governance roles, public scripts, checks, tests, Just recipes, phase/checkpoint bundles, workflow artifacts, authority owners, side-effect classes, primary debug artifacts, replacement/sunset status, and machine-readable contract surfaces. All non-Coder roles MUST keep this ledger current when they add, rename, retire, expose, or materially change governance scripts, public Just recipes, checks, workflow artifacts, role protocols, phase bundles, topology surfaces, or session/runtime authority surfaces. If a non-Coder role is not allowed to write `.GOV/` directly from its current lane, it MUST emit a typed blocker/proposal that names the exact topology update required and the owning coordinator MUST update the ledger before closeout. The Coder role is excluded from governance-topology maintenance and must route any discovered drift to Orchestrator, Classic Orchestrator, Validator, Integration Validator, Activation Manager, WP Validator, or Memory Manager as appropriate. New public governance entrypoints are illegal unless represented in the topology ledger with owner, phase, side-effect class, invocation path, replacement bundle, primary debug artifact, and validation/check coverage.
## 10. Versioning
...
Deterministic atomic governance files are the direction of travel: where machine contracts exist, consume the single authoritative typed packet/refinement/microtask/startup/runtime truth before generated Markdown projections [CX-908]. Keep workflow playbooks current as projections over machine workflow contracts and role protocols [CX-909].
