...
async function loadTokenLib(governanceRuntimeRoot) {
  ...
  try {
    return await import(`${tokenLibHref}?runtime_root=${encodeURIComponent(governanceRuntimeRoot)}&t=${Date.now()}`);
  } finally {
...
test("syncWpTokenUsageLedger rolls up commands by WP and role", async () => withTempRepo(async (repoRoot) => {
  ...
  const first = syncWpTokenUsageLedger(repoRoot, {
  ...
  const second = syncWpTokenUsageLedger(repoRoot, {
