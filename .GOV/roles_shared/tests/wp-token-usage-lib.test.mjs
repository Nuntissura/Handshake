...
test("syncWpTokenUsageLedger rolls up commands by WP and role", async () => withTempRepo(async (repoRoot) => {
  ...
  assert.equal(second.ledger.ledger_health.severity, "PASS");
  assert.equal(second.ledger.ledger_health.drift_class, "NONE");
...
test("readWpTokenUsageLedger reports minor raw-output drift as WARN", async () => withTempRepo(async (repoRoot) => {
  ...
  fs.writeFileSync(coderOutput, `${JSON.stringify({ timestamp: "2026-03-29T10:00:01Z", type: "turn.completed", usage: { input_tokens: 101, cached_input_tokens: 40, output_tokens: 12 } })}\n`, "utf8");
  ...
  assert.equal(ledger.summary.command_count, 1);
  ...
  assert.equal(ledger.raw_scan.summary.command_count, 1);
  assert.equal(ledger.ledger_health.status, "DRIFT");
  assert.equal(ledger.ledger_health.severity, "WARN");
  assert.equal(ledger.ledger_health.drift_class, "MINOR");
  assert.equal(ledger.ledger_health.metrics.input_token_delta, 1);
  assert.equal(ledger.ledger_health.invalidity_code, "");
}));

test("readWpTokenUsageLedger reports material raw-output drift as FAIL", async () => withTempRepo(async (repoRoot) => {
  const { syncWpTokenUsageLedger, readWpTokenUsageLedger } = await loadTokenLib(path.join(repoRoot, "gov_runtime"));
  const coderOutput = path.join(repoRoot, "gov_runtime", "roles_shared", "SESSION_CONTROL_OUTPUTS", "CODER_WP-TEST-v1", "cmd-coder.jsonl");
  const validatorOutput = path.join(repoRoot, "gov_runtime", "roles_shared", "SESSION_CONTROL_OUTPUTS", "WP_VALIDATOR_WP-TEST-v1", "cmd-wpval.jsonl");
  fs.mkdirSync(path.dirname(coderOutput), { recursive: true });
  ...
  assert.equal(ledger.ledger_health.status, "DRIFT");
  assert.equal(ledger.ledger_health.severity, "FAIL");
  assert.equal(ledger.ledger_health.drift_class, "MATERIAL");
  assert.equal(ledger.ledger_health.blocker_class, "POLICY_CONFLICT");
  assert.equal(ledger.ledger_health.invalidity_code, "TOKEN_LEDGER_DRIFT");
