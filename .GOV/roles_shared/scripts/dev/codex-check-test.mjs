...
run("node .GOV/roles_shared/scripts/spec-current-check.mjs");

// Validate that the fetch guard is active by running it against a known test fixture.
shouldFail(
  'rg -n "\\bfetch\\s*\\(" .GOV/roles_shared/scripts/dev/fixtures/forbidden_fetch.ts && exit 1 || exit 0',
  "fetch guard fixture",
);
...
shouldFail(
  'node -e "const out = require(\'child_process\').execSync(\'rg -n TODO .GOV/roles_shared/scripts/dev/fixtures/forbidden_todo.txt\').toString(); if (out.split(\'\\n\').filter(Boolean).some(l => !/TODO\\(HSK-\\d+\\)/.test(l))) process.exit(1)"',
  "TODO guard fixture",
);
