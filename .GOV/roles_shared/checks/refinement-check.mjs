...
export function validateRefinementFile(refinementPath, { expectedWpId, requireSignature } = {}) {
  ...
  const parsed = {
    ...
    clauseProofPlan: [],
    contractSurfaces: [],
    coderImplementationOrder: [],
    coderHotFiles: [],
    coderTripwireTests: [],
    coderCarryForwardWarnings: [],
    validatorClausesToInspect: [],
    validatorFilesToRead: [],
    validatorCommandsToRun: [],
    validatorPostMergeSpotchecks: [],
    refinementNotProven: [],
  ...
  const hasRefinementHandoffSections = isVersionAtLeast(refinementFormatVersion, '2026-03-15');
  const hasSemanticProofSections = isVersionAtLeast(refinementFormatVersion, '2026-03-16');
  ...
  if (isHydratedResearchProfile) {
    ...
    if (hasRefinementHandoffSections) {
      requiredSections.splice(requiredSections.indexOf('CLEARLY_COVERS'), 0, 'CLAUSE_PROOF_PLAN');
      requiredSections.splice(requiredSections.indexOf('CLEARLY_COVERS'), 0, 'CONTRACT_SURFACES');
      if (hasSemanticProofSections) {
        requiredSections.splice(requiredSections.indexOf('CLEARLY_COVERS'), 0, 'SEMANTIC_PROOF_PLAN');
      }
      requiredSections.splice(requiredSections.indexOf('CLEARLY_COVERS'), 0, 'CODER_HANDOFF_BRIEF');
      requiredSections.splice(requiredSections.indexOf('CLEARLY_COVERS'), 0, 'VALIDATOR_HANDOFF_BRIEF');
      requiredSections.splice(requiredSections.indexOf('CLEARLY_COVERS'), 0, 'NOT_PROVEN_AT_REFINEMENT_TIME');
    }
  }
  ...
  if (isModernRefinement) {
    ...
    if (isHydratedResearchProfile && hasRefinementHandoffSections) {
      const clauseRows = extractIndentedListAfterLabel(lines, 'CLAUSE_ROWS');
      const contractRows = extractIndentedListAfterLabel(lines, 'CONTRACT_ROWS');
      const semanticTripwireTests = extractIndentedListAfterLabel(lines, 'SEMANTIC_TRIPWIRE_TESTS');
      const canonicalContractExamples = extractIndentedListAfterLabel(lines, 'CANONICAL_CONTRACT_EXAMPLES');
      const implementationOrder = extractIndentedListAfterLabel(lines, 'IMPLEMENTATION_ORDER');
      const hotFiles = extractIndentedListAfterLabel(lines, 'HOT_FILES');
      const tripwireTests = extractIndentedListAfterLabel(lines, 'TRIPWIRE_TESTS');
      const carryForwardWarnings = extractIndentedListAfterLabel(lines, 'CARRY_FORWARD_WARNINGS');
      const clausesToInspect = extractIndentedListAfterLabel(lines, 'CLAUSES_TO_INSPECT');
      const filesToRead = extractIndentedListAfterLabel(lines, 'FILES_TO_READ');
      const commandsToRun = extractIndentedListAfterLabel(lines, 'COMMANDS_TO_RUN');
      const postMergeSpotchecks = extractIndentedListAfterLabel(lines, 'POST_MERGE_SPOTCHECKS');
      const notProvenItems = extractIndentedListAfterLabel(lines, 'NOT_PROVEN_ITEMS');

      parsed.clauseProofPlan = clauseRows.items.filter((item) => !/^NONE$/i.test(item));
      parsed.contractSurfaces = contractRows.items.filter((item) => !/^NONE$/i.test(item));
      parsed.semanticTripwireTests = semanticTripwireTests.items.filter((item) => !/^NONE$/i.test(item));
      parsed.canonicalContractExamples = canonicalContractExamples.items.filter((item) => !/^NONE$/i.test(item));
      parsed.coderImplementationOrder = implementationOrder.items.filter((item) => !/^NONE$/i.test(item));
      parsed.coderHotFiles = hotFiles.items.filter((item) => !/^NONE$/i.test(item));
      parsed.coderTripwireTests = tripwireTests.items.filter((item) => !/^NONE$/i.test(item));
      parsed.coderCarryForwardWarnings = carryForwardWarnings.items.filter((item) => !/^NONE$/i.test(item));
      parsed.validatorClausesToInspect = clausesToInspect.items.filter((item) => !/^NONE$/i.test(item));
      parsed.validatorFilesToRead = filesToRead.items.filter((item) => !/^NONE$/i.test(item));
      parsed.validatorCommandsToRun = commandsToRun.items.filter((item) => !/^NONE$/i.test(item));
      parsed.validatorPostMergeSpotchecks = postMergeSpotchecks.items.filter((item) => !/^NONE$/i.test(item));
      parsed.refinementNotProven = notProvenItems.items.filter((item) => !/^NONE$/i.test(item));

      const requireConcreteList = (listInfo, label, { allowNone = false } = {}) => {
        if (!listInfo.found) {
          errors.push(`${label} is required for REFINEMENT_FORMAT_VERSION >= 2026-03-15`);
          return;
        }
        const hasNone = listInfo.items.length === 1 && /^NONE$/i.test(listInfo.items[0] || '');
        const concreteItems = listInfo.items.filter((item) => !/^NONE$/i.test(item || '') && !isPlaceholderValue(item));
        if (!allowNone && (hasNone || concreteItems.length === 0)) {
          errors.push(`${label} must include one or more concrete entries`);
        }
        if (!allowNone && listInfo.items.some((item) => isPlaceholderValue(item))) {
          errors.push(`${label} contains placeholders; fill every row before approval`);
        }
        if (allowNone && hasNone) return;
        if (listInfo.items.some((item) => !/^NONE$/i.test(item || '') && isPlaceholderValue(item))) {
          errors.push(`${label} contains placeholders; fill every row or use NONE where allowed`);
        }
      };

      requireConcreteList(clauseRows, 'CLAUSE_PROOF_PLAN CLAUSE_ROWS');
      requireConcreteList(contractRows, 'CONTRACT_SURFACES CONTRACT_ROWS');
      if (hasSemanticProofSections) {
        if (!semanticTripwireTests.found) {
          errors.push('SEMANTIC_PROOF_PLAN SEMANTIC_TRIPWIRE_TESTS is required for REFINEMENT_FORMAT_VERSION >= 2026-03-16');
        }
        if (!canonicalContractExamples.found) {
          errors.push('SEMANTIC_PROOF_PLAN CANONICAL_CONTRACT_EXAMPLES is required for REFINEMENT_FORMAT_VERSION >= 2026-03-16');
        }

        const tripwireConcreteItems = semanticTripwireTests.items.filter((item) => !/^NONE$/i.test(item || '') && !isPlaceholderValue(item));
        const exampleConcreteItems = canonicalContractExamples.items.filter((item) => !/^NONE$/i.test(item || '') && !isPlaceholderValue(item));

        if (semanticTripwireTests.items.some((item) => !/^NONE$/i.test(item || '') && isPlaceholderValue(item))) {
          errors.push('SEMANTIC_PROOF_PLAN SEMANTIC_TRIPWIRE_TESTS contains placeholders; fill every row or use NONE');
        }
        if (canonicalContractExamples.items.some((item) => !/^NONE$/i.test(item || '') && isPlaceholderValue(item))) {
          errors.push('SEMANTIC_PROOF_PLAN CANONICAL_CONTRACT_EXAMPLES contains placeholders; fill every row or use NONE');
        }
        if (tripwireConcreteItems.length === 0 && exampleConcreteItems.length === 0) {
          errors.push('SEMANTIC_PROOF_PLAN must include one or more semantic tripwire tests or canonical contract examples');
        }
      }
      requireConcreteList(implementationOrder, 'CODER_HANDOFF_BRIEF IMPLEMENTATION_ORDER');
      requireConcreteList(hotFiles, 'CODER_HANDOFF_BRIEF HOT_FILES');
      requireConcreteList(tripwireTests, 'CODER_HANDOFF_BRIEF TRIPWIRE_TESTS');
      requireConcreteList(carryForwardWarnings, 'CODER_HANDOFF_BRIEF CARRY_FORWARD_WARNINGS');
      requireConcreteList(clausesToInspect, 'VALIDATOR_HANDOFF_BRIEF CLAUSES_TO_INSPECT');
      requireConcreteList(filesToRead, 'VALIDATOR_HANDOFF_BRIEF FILES_TO_READ');
      requireConcreteList(commandsToRun, 'VALIDATOR_HANDOFF_BRIEF COMMANDS_TO_RUN');
      requireConcreteList(postMergeSpotchecks, 'VALIDATOR_HANDOFF_BRIEF POST_MERGE_SPOTCHECKS', { allowNone: true });
      requireConcreteList(notProvenItems, 'NOT_PROVEN_AT_REFINEMENT_TIME NOT_PROVEN_ITEMS', { allowNone: true });

      const validatePipeRows = (rawItems, label, requiredKeys) => {
        rawItems.forEach((item, idx) => {
          if (/^NONE$/i.test(item || '')) return;
          const record = parsePipeRecord(item);
          for (const key of requiredKeys) {
            if (isPlaceholderValue(record[key] || '')) {
              errors.push(`${label} row ${idx + 1} must fill ${key}`);
            }
          }
        });
      };

      validatePipeRows(clauseRows.items, 'CLAUSE_PROOF_PLAN CLAUSE_ROWS', [
        'CLAUSE',
        'WHY_IN_SCOPE',
        'EXPECTED_CODE_SURFACES',
        'EXPECTED_TESTS',
        'RISK_IF_MISSED',
      ]);
      validatePipeRows(contractRows.items, 'CONTRACT_SURFACES CONTRACT_ROWS', [
        'CONTRACT',
        'PRODUCER',
        'CONSUMER',
