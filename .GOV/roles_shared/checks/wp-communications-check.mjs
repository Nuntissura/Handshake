...
export function isPacketlessSyntheticWp(wpId) {
  return /^WP-MEMORY-HYGIENE_/i.test(String(wpId || "").trim());
}
...
function main() {
  ...
  if (fs.existsSync(COMM_ROOT)) {
    ...
    for (const entry of entries) {
      ...
      if (isPacketlessSyntheticWp(entry.name)) {
        for (const requiredName of [THREAD_FILE_NAME, RECEIPTS_FILE_NAME, NOTIFICATIONS_FILE_NAME, NOTIFICATION_CURSOR_FILE_NAME]) {
          const requiredPath = path.join(COMM_ROOT, entry.name, requiredName);
          if (!fs.existsSync(requiredPath)) {
            violations.push(`${COMM_ROOT}/${entry.name}: packetless synthetic lane missing required artifact ${requiredName}`);
          }
        }
        const receiptsPath = path.join(COMM_ROOT, entry.name, RECEIPTS_FILE_NAME);
        if (fs.existsSync(receiptsPath)) {
          try {
            ...
            receipts.forEach((receipt, index) => {
              const receiptErrors = validateReceipt(receipt);
              for (const error of receiptErrors) {
                violations.push(`${COMM_ROOT}/${entry.name}: ${RECEIPTS_FILE_NAME} line ${index + 1} invalid -> ${error}`);
              }
            });
          } catch (error) {
            violations.push(`${COMM_ROOT}/${entry.name}: ${RECEIPTS_FILE_NAME} parse/validation failure -> ${error.message}`);
          }
        }
        continue;
      }
