...
- Amendment 2026-09-12 (same RGF-328): operator clarified that build-cache reuse and test bundling are intended and encouraged; `[VPX-001]` relaxed so a shared warm `CARGO_TARGET_DIR` is allowed (execution + captured result must be the validator's own), `[VPX-005]` reworded to caching + bundling, `[VPX-007]` (bundle READY MTs at one `validate_at_commit` into one build + one test invocation, `covers[]`) and `[VPX-008]` (build once per commit) added; implementer cadence text in Kernel Builder/Coder protocols aligned. Added typed JSON `waivers_granted` ledger path (`buildPolicyWaiverLedgerFromEntries`, `parsePolicyWaiverLedgerFromPacket`) so `NONE_JSON_AUTHORITY_ONLY` packets can carry signed waivers; signed WP-KERNEL-012 kb-ready waiver `WP-KERNEL-012-KB-READY-RECEIPT-WAIVER-20260910` (signature consumed in `SIGNATURE_AUDIT.md`).
