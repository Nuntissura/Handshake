...
**Date:** 2026-04-29
**Scope:** Governance-only implementation guide for formal `RGF-255` through `RGF-264` board rows
...
## Malformed And Drift-Prone Surfaces

In this guide, "malformed" does not only mean invalid syntax. It also means a governance artifact is structurally valid but no longer trustworthy because it has stale values, duplicate projections, placeholder text, wrong enum/status values, missing proof fields, wrong artifact paths, or prose claims that contradict machine truth.

The repair objective is to stop Orchestrator from manually reconciling these surfaces. Each surface must either derive from a canonical source, fail early with a specific blocker key, or be reduced to read-only narrative generated from structured truth.

| Surface | Failure Mode Seen In WP-1 | Required Repair Direction | Primary RGF Coverage |
| --- | --- | --- | --- |
| Task packet and scope packet | Acceptance criteria, validation report sections, candidate target fields, signed-scope references, and status headers could be missing, stale, or prose-only. | Convert acceptance criteria into executable rows with stable IDs and evidence requirements. Block WP Validator PASS when required rows are unresolved. | `RGF-256`, `RGF-255` |
| Candidate target and signed-scope truth | Closeout reported malformed target/scope truth, including zero unique patch artifact references and stale target diff entries from old files. | Derive candidate identity, patch artifact reference, diff range, and signed-scope status from one canonical terminal record or truth bundle input. Do not let stale packet prose override current commit truth. | `RGF-233`, `RGF-234`, `RGF-255`, `RGF-259` |
| Runtime projection | Runtime still projected `active_microtask=MT-001:DECLARED` and `waiting_on=CODER_HANDOFF` after final validation had moved past that state. | Receipt/verdict publication must update runtime progression once, atomically, and leave a terminal projection when terminal verdict exists. | `RGF-237`, `RGF-257`, `RGF-260` |
| Receipts and notifications | Duplicate handoffs, missing acknowledgement chains, and stale pending work can leave the next legal action ambiguous. | Use named verbs, correlation IDs, duplicate suppression, and receipt-driven next-action derivation. Pending notifications must reconcile against terminal verdicts. | `RGF-245`, `RGF-248`, `RGF-257` |
| Validator reports and gates | A PASS can be narratively correct while gate metadata lacks exact proof closure, candidate identity, or counterfactual evidence. | Validator reports must include machine-readable closure fields for candidate commit, branch, diff range, acceptance matrix, counterfactuals, and unresolved blockers. | `RGF-250`, `RGF-256`, `RGF-259` |
| Workflow dossier | Live sections can duplicate, narrative can lag behind terminal truth, rubric placeholders can remain, and imported logs can become too large to review. | Dossier closeout should auto-fill mechanical metrics from structured sources, fail on placeholders/stale claims, and keep large raw logs as linked artifacts rather than inline narrative. | `RGF-222`, `RGF-223`, `RGF-224`, `RGF-261` |
| Refactor task board and mirrored guide tables | Board status, row table, execution briefs, follow-on sequence, and mirrored guide rows can drift from each other. | Add a board integrity check that validates row uniqueness, status consistency, sequence references, and mirrored guide references before `gov-check` passes. | `RGF-254`, `RGF-264` |
| Repomem sessions | Sessions can be opened without durable decisions, closed without useful proof, or fragmented across many repair attempts. | Session closeout should require bounded decisions/evidence for governed mutations and feed a compact WP truth bundle instead of forcing manual transcript reconstruction. | `RGF-255`, `RGF-260` |
| ACP session registry and broker ledger | Stale `READY`, active, or `SESSION_RUNTIME_FAILED` residue can survive after terminal verdicts and confuse topology checks. | Terminal PASS/FAIL publication must close or quarantine role sessions, record proof, and make stale session residue governance debt instead of live work. | `RGF-236`, `RGF-240`, `RGF-260` |
| Artifact roots and target directories | `Handshake Artifacts` and `Handshake_Artifacts` ambiguity caused late final-lane failure even after product proof was intact. | Run artifact-root preflight before final validation. Noncanonical roots should fail as environment debt with a bounded remediation path. | `RGF-235`, `RGF-244`, `RGF-262` |
| Baseline compile blockers and waivers | Out-of-scope baseline failures can cause repeated informal Operator interruptions and unclear product-versus-environment blame. | Record bounded waivers/unblockers with owner, scope, expiry, and proof. Coder and validators must compare failures against active waiver scope. | `RGF-09`, `RGF-12`, `RGF-263` |

Implementers should prefer structured JSON/JSONL/SQLite readers over Markdown scraping whenever both exist. Markdown documents remain useful for operator review, but they should not be the source of truth for terminal verdict, candidate identity, scope closure, next actor, or containment.
...
3. Verify the latest `RGF-*` IDs in `.GOV/roles_shared/records/REPO_GOVERNANCE_REFACTOR_TASK_BOARD.md`; `RGF-255` through `RGF-264` were verified free, promoted to formal board rows, implemented, and verified on 2026-04-29.
...
These rows now exist in the Post-Refactor Follow-On Board. Keep this mirrored table aligned with the board if a future renumbering or status change is required.
