...
- Broker `session_key` and receipt `actor_session` are separate identifiers. Direct-review helpers must use the exact receipt/session string from the current route, notification, or open review item; helpers and prompts must not synthesize `CODER:<WP_ID>` / `WP_VALIDATOR:<WP_ID>` when the active session reports a different value.
