...
| `gov-flush` stops immediately on artifact-root drift | one or more discovered worktrees have a stale Cargo `target-dir` or a repo-local `target/` directory, so cleanup and NAS backup would be guaranteed to fail later anyway | Run `just artifact-hygiene-check`, fix each `.cargo/config.toml` so Cargo resolves to the canonical external artifact root, remove repo-local `target/` residue if present, then rerun `just gov-flush` |
