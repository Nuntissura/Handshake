...
| Data/schema-rollout problems | Active typed SurrealDB storage boundary and SurrealKit rollout manifests named by the current WP; legacy SQL migrations are removal/translation debt only | `rg "SurrealKit|surrealdb|SurrealDB|SCHEMAFULL" src/backend/handshake_core`, inspect rollout stages and typed schema diffs |
...
| `kbstart.cmd` output is truncated but its injected output still shows `KBSTART COMPLETE` | the shell caller preserved a success-looking tail while dropping authority-file bodies; command exit proves injection only, not reading | Read the required binding files directly in bounded chunks, then issue the exact `AUTHORITY_CONTRACT_ACK`; the root launcher must end with `KBSTART_FINAL_STATE ... authority_read=UNVERIFIED role_startup_complete=NO` |
...
| `phase-check CLOSEOUT` or context status shows a real `verdict_of_record`, but the lane still looks "not settled" | closeout support surfaces or `TERMINAL_CLOSEOUT_RECORD.json` still carry settlement debt even though product judgment is already known | Check `just phase-check CLOSEOUT WP-{ID}`, `just integration-validator-context-brief WP-{ID}`, and `just session-registry-status WP-{ID}` for `terminal_record_status`, `terminal_state`, `product_outcome_blockers`, `governance_debt`, and closeout provenance. If `product_outcome_blockers` is empty, repair only the named settlement debt; do not rerun validation just to restate the same verdict |
