...
| Build/test fails | package configs (`app/package.json`, Rust `Cargo.toml`) | Re-run `pnpm -C app test`, `cargo test --manifest-path src/backend/handshake_core/Cargo.toml` |
...
| Shared coder/WP-validator worktree is dirty or invalid but the branch must be repaired to packet truth | governed cleanup/reseed path is required; manual reuse risks stale evidence or mixed scope | Make an out-of-repo backup copy first, use `git worktree remove <path>`, then recreate from the packet baseline instead of hand-cleaning or reusing the dirty worktree |

### Windows long-path recovery

`Filename too long` / `ENAMETOOLONG` is a common Windows error when worktrees contain deeply nested `node_modules`, `target/`, or `.GOV/` paths exceeding 260 characters.
...
1. **Enable long paths system-wide (preferred, one-time):**
   ...
   ```
   Restart the shell, then retry `git worktree remove`.
...
After any manual filesystem recovery, always run `git worktree prune` to verify consistency.
