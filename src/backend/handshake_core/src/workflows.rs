...
async fn execute_locus_sync_task_board(
    ...
    let task_board_index_path = runtime_governance_paths.task_board_index_path();
    let task_board_default_view_path = runtime_governance_paths
        .task_board_view_path("default")
        .map_err(|e| WorkflowError::Terminal(format!("invalid task board view path: {e}")))?;
    ...
    > = async {
        ...
        let structured_updated_at = Utc::now().to_rfc3339();
        let task_board_id = runtime_governance_paths.task_board_display();
        let task_board_authority_refs = vec![runtime_governance_paths.task_board_index_display()];
        let task_board_evidence_refs = vec![runtime_governance_paths.task_board_display()];
        let task_board_view_ids = vec!["default".to_string()];
        let task_board_lane_ids = vec![
            "stub".to_string(),
            "ready".to_string(),
        ...
        let mut display_order = 0u64;
        for status in statuses_for_projection {
            for entry in canonical.entries_for_status(status) {
                structured_rows.push(locus::task_board::build_structured_task_board_entry(
                    task_board_id.as_str(),
                    structured_updated_at.as_str(),
                    &task_board_authority_refs,
                    &task_board_evidence_refs,
                    display_order,
                    &task_board_view_ids,
                    entry,
                ));
        ...
        let structured_index = locus::task_board::build_structured_task_board_index(
            task_board_id.as_str(),
            structured_updated_at.as_str(),
            &task_board_authority_refs,
            &task_board_evidence_refs,
            &task_board_view_ids,
            structured_rows.clone(),
        );
        let structured_view = locus::task_board::build_structured_task_board_view(
            task_board_id.as_str(),
            "default",
            structured_updated_at.as_str(),
            &task_board_authority_refs,
        ...
        let mut task_board_validation =
            locus::task_board::validate_structured_task_board_index(&structured_index);
        task_board_validation.merge(locus::task_board::validate_structured_task_board_view(
            &structured_view,
        ));
        let invalid_task_board_authority_refs =
            runtime_governance_paths.invalid_runtime_authority_refs(&task_board_authority_refs);
        if !invalid_task_board_authority_refs.is_empty() {
            task_board_validation.push_issue(
                locus::StructuredCollaborationValidationCode::AuthorityScopeMismatch,
                "authority_refs",
                Some(runtime_governance_paths.governance_root_display()),
                Some(invalid_task_board_authority_refs.join(",")),
                "task board structured projections must keep authority_refs inside .handshake/gov",
            );
        }
        if !task_board_validation.ok {
            return Err(WorkflowError::Terminal(format!(
                "task board structured projection validation failed: {}",
                serde_json::to_string(&task_board_validation)
                    .unwrap_or_else(|_| "serialization_failed".to_string())
            )));
        }
        let work_packet_artifacts_written = if dry_run {
            0u64
        } else {
            let mut wp_ids: Vec<String> = after_map.keys().cloned().collect();
            wp_ids.sort();
            for wp_id in &wp_ids {
        ...
        let structured_projection_written = if dry_run {
            false
        } else {
            write_json_atomic(&workspace_root, &task_board_index_path, &structured_index)?;
            write_json_atomic(
                &workspace_root,
                &task_board_default_view_path,
                &structured_view,
            )?;
            true
        };
        ...
        let output = json!({
            ...
            "work_packet_artifacts_written": work_packet_artifacts_written,
            "structured_projection_written": structured_projection_written,
            "entries_added": entries_added_count,
            "entries_removed": entries_removed_count,
            "status_changes": status_changes,
            "validation_result": task_board_validation,
        });
...
async fn emit_runtime_structured_work_packet_artifacts(
    ...
    let packet_path = runtime_paths.work_packet_packet_path(wp_id).map_err(|e| {
        WorkflowError::Terminal(format!("invalid work-packet packet path for {wp_id}: {e}"))
    })?;
    let summary_path = runtime_paths.work_packet_summary_path(wp_id).map_err(|e| {
        WorkflowError::Terminal(format!("invalid work-packet summary path for {wp_id}: {e}"))
    })?;
    let tracked_wp =
...
async fn emit_runtime_structured_micro_task_artifacts(
    ...
    let workspace_root = runtime_paths.workspace_root().to_path_buf();
    let packet_path = runtime_paths
        .micro_task_packet_path(wp_id, mt_id)
        .map_err(|e| {
            WorkflowError::Terminal(format!("invalid micro-task packet path for {mt_id}: {e}"))
        })?;
    let summary_path = runtime_paths
        .micro_task_summary_path(wp_id, mt_id)
        .map_err(|e| {
            WorkflowError::Terminal(format!("invalid micro-task summary path for {mt_id}: {e}"))
        })?;
    let summary_display = runtime_paths
        .micro_task_summary_display(wp_id, mt_id)
        .map_err(|e| {
            WorkflowError::Terminal(format!(
                "invalid micro-task summary display path for {mt_id}: {e}"
            ))
        })?;
    let mut tracked_mt =
...
async fn load_tracked_work_packet_from_sqlite(
    ...
    let packet_display = runtime_paths
        .work_packet_packet_display(&wp_id)
        .map_err(|e| {
            WorkflowError::Terminal(format!(
                "invalid work-packet packet display path for {wp_id}: {e}"
            ))
        })?;
    let summary_display = runtime_paths
        .work_packet_summary_display(&wp_id)
        .map_err(|e| {
            WorkflowError::Terminal(format!(
                "invalid work-packet summary display path for {wp_id}: {e}"
            ))
        })?;
...
fn read_existing_work_packet_type(
    ...
    let packet_path = runtime_paths.work_packet_packet_path(wp_id).ok()?;
...
fn apply_runtime_structured_micro_task_registry(
    ...
    let packet_display = runtime_paths
        .micro_task_packet_display(wp_id, &tracked_mt.mt_id)
        .map_err(|e| {
            WorkflowError::Terminal(format!(
                "invalid micro-task packet display path for {}: {e}",
                tracked_mt.mt_id
            ))
        })?;
    let summary_display = runtime_paths
        .micro_task_summary_display(wp_id, &tracked_mt.mt_id)
        .map_err(|e| {
            WorkflowError::Terminal(format!(
                "invalid micro-task summary display path for {}: {e}",
                tracked_mt.mt_id
            ))
        })?;
