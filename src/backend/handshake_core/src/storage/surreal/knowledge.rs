...
async fn delete_owned_document_execute(
    ...
    // Do not call mt120_document_access directly as a record user: it traverses
    // protected_resources.external_resource_id, whose SELECT permission is NONE.
    // The mandatory tombstone UPDATE invokes mt120_knowledge_rich_documents_update_guard
    // with event permissions, checking mt120_document_delete on BOTH row versions.
