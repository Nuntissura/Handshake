...
//! store.
//!
//! Every `Database` method is overridden with a native SurrealDB operation.
//! The source-level surface audit at the end of this module prevents a trait
//! default that rejects production work from being inherited accidentally.
...
impl Database for SurrealDatabase {
    ...
    async fn append_kernel_crdt_update_with_event_atomic(
        ...
        super::kernel_crdt_store::append_update_with_event_atomic(&self.storage, request).await
    }
    ...
    async fn run_migrations(&self) -> StorageResult<()> {
        super::bootstrap_production_schemas(&self.storage)
            .await
            .map(|_| ())
...
mod not_implemented_surface {
    ...
    fn not_implemented_surface_is_declared() {
        ...
        assert_eq!(
            inherited_methods,
            Vec::<&str>::new(),
            "SurrealDatabase must override every Database method"
        );
