...
// The block and its protected resource are created with RETURN NONE: a record user's CREATE output is
// filtered by the table's SELECT permission, and neither row is readable until the creator grant
// exists (the next statement), so a `RETURN VALUE id` count check would deny every owned create.
// Silent denies are still caught by the final verification SELECT, which covers every bundle row.
