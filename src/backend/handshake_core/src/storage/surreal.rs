...
pub enum SurrealStorageError {
    ...
    #[error("embedded cloud model-lane record has an invalid shape: {reason}")]
    InvalidCloudModelLaneRecord { reason: &'static str },
}
...
struct SurrealStorageInner {
    ...
    /// Consumed by the typed outer-module ports (workspaces/documents/blocks)
    /// in the reference worktree; retained here so `open_with_guard` keeps its
    /// contract while those ports are still to be imported.
    #[allow(dead_code)]
