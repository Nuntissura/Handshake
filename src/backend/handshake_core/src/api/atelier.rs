...
use crate::atelier::prompt_feedback::adapter::{CuippRow, LeeseoImportRequest};
use crate::atelier::prompt_feedback::adapter as prompt_feedback_adapter;
...
// --- MT-020 prompt-feedback kernel routes ---------------------------------

fn bad_request_static() -> (StatusCode, Json<ErrorResponse>) {
    (
...
async fn import_prompt_feedback_cases(
...
) -> Result<(StatusCode, Json<PromptFeedbackImportResponse>), (StatusCode, Json<ErrorResponse>)> {
    ...
    let new_cases =
        prompt_feedback_adapter::import_leeseo(&req).map_err(|err| atelier_error(err.into()))?;
    let cases = store
        .import_prompt_cases(&new_cases)
        .await
        .map_err(atelier_error)?;
    let seed_rule_pack = store.ensure_seed_rule_pack(&actor).await.map_err(atelier_error)?;
    Ok((
        StatusCode::CREATED,
...
) -> Result<Json<Vec<PromptCase>>, (StatusCode, Json<ErrorResponse>)> {
    let store = atelier_store(&state);
    let cases = store
...
) -> Result<(StatusCode, Json<ReviewVerdict>), (StatusCode, Json<ErrorResponse>)> {
    let actor = calling_actor(&headers)?;
    let store = atelier_store(&state);
    let reviewer_kind =
        ReviewerKind::from_token(&payload.reviewer_kind).map_err(|err| atelier_error(err.into()))?;
    let verdict_kind =
...
async fn preview_prompt_feedback_rewrite(
...
) -> Result<Json<RewritePlan>, (StatusCode, Json<ErrorResponse>)> {
    let actor = calling_actor(&headers)?;
    let store = atelier_store(&state);
    ...
    if payload.rule_pack_id == SEED_RULE_PACK_ID && version == SEED_RULE_PACK_VERSION {
        store.ensure_seed_rule_pack(&actor).await.map_err(atelier_error)?;
    }
    let plan = store
...
) -> Result<(StatusCode, Json<PromptExport>), (StatusCode, Json<ErrorResponse>)> {
    let actor = calling_actor(&headers)?;
    let store = atelier_store(&state);
    if payload.rule_pack_id.trim().is_empty() {
        return Err(bad_request_static());
...
async fn list_prompt_feedback_rulepacks(
    State(state): State<AppState>,
) -> Result<Json<Vec<RulePack>>, (StatusCode, Json<ErrorResponse>)> {
    let store = atelier_store(&state);
