...
const SURFACES: &[SurfaceDescriptor] = &[
    ...
    surface!("loom.journals.open", SurfaceGroup::NotesLoom, "PUT",
        "/workspaces/:workspace_id/loom/journals/:journal_date",
        "Open or create the daily journal block for one ISO date.",
        "workspace_id + journal_date path params.",
        "JSON journal block projection."),
    surface!("loom.blocks.transclusion", SurfaceGroup::NotesLoom, "GET",
        "/workspaces/:workspace_id/loom/blocks/:block_id/transclusion",
        "Resolve a Loom block to its source rich document for non-copying transclusion.",
        "workspace_id + block_id path params.",
        "JSON source-document transclusion projection."),
    surface!("loom.assets.tiers.list", SurfaceGroup::NotesLoom, "GET",
        "/workspaces/:workspace_id/assets/:asset_id/tiers",
        "Inspect the persisted media-cache tier state for an asset.",
        "workspace_id + asset_id path params.",
        "JSON tier-state rows."),
    surface!("loom.assets.tiers.retry", SurfaceGroup::NotesLoom, "POST",
        "/workspaces/:workspace_id/assets/:asset_id/tiers/:tier/retry",
        "Retry one failed media-cache tier operation.",
        "workspace_id + asset_id + tier path params.",
        "JSON updated tier/retry state."),
    surface!("loom.collections.create", SurfaceGroup::NotesLoom, "POST",
        "/workspaces/:workspace_id/loom/collections",
        "Create a persisted Loom media collection.",
        "workspace_id path param; JSON collection input.",
        "JSON created collection."),
    surface!("loom.collections.get", SurfaceGroup::NotesLoom, "GET",
        "/workspaces/:workspace_id/loom/collections/:collection_id",
        "Load one persisted Loom media collection.",
        "workspace_id + collection_id path params.",
        "JSON collection with its ordered source list."),
    surface!("loom.collections.order", SurfaceGroup::NotesLoom, "PUT",
        "/workspaces/:workspace_id/loom/collections/:collection_id/order",
        "Persist the exact item order for a Loom media collection.",
        "workspace_id + collection_id path params; JSON ordered ids.",
        "JSON updated collection order."),
    surface!("loom.search.v2", SurfaceGroup::NotesLoom, "POST",
        "/workspaces/:workspace_id/loom/search-v2",
        "Run PostgreSQL-native graph-blended hybrid Loom search.",
        "workspace_id path param; JSON query, optional content type, graph boost, and limit.",
        "JSON ranked hits, facets, and semantic-availability state."),
    surface!("loom.quick_switcher.recents.list", SurfaceGroup::NotesLoom, "GET",
        "/workspaces/:workspace_id/loom/quick-switcher/recents",
        "List the workspace's persisted Quick Switcher recents.",
        "workspace_id path param.",
        "JSON recent navigation targets."),
    surface!("loom.quick_switcher.recents.record", SurfaceGroup::NotesLoom, "POST",
        "/workspaces/:workspace_id/loom/quick-switcher/recents",
        "Record one Quick Switcher navigation target.",
        "workspace_id path param; JSON recent-target input.",
        "JSON persisted recent entry."),
    surface!("loom.ai.jobs.run", SurfaceGroup::NotesLoom, "POST",
        "/workspaces/:workspace_id/loom/ai-jobs",
        "Run a Loom AI proposal job such as auto-tag, auto-caption, or link-suggest.",
        "workspace_id path param; JSON job request.",
        "JSON job and pending suggestion state."),
    surface!("loom.ai.jobs.accept_all", SurfaceGroup::NotesLoom, "POST",
        "/workspaces/:workspace_id/loom/ai-jobs/:job_id/accept-all",
        "Promote every still-pending suggestion from one Loom AI job.",
        "workspace_id + job_id path params.",
        "JSON promotion results."),
    surface!("loom.ai.suggestions.list", SurfaceGroup::NotesLoom, "GET",
        "/workspaces/:workspace_id/loom/ai-suggestions",
        "List Loom AI suggestions and their proposal state.",
        "workspace_id path param; optional filters.",
        "JSON suggestion rows."),
    surface!("loom.ai.suggestions.accept", SurfaceGroup::NotesLoom, "POST",
        "/workspaces/:workspace_id/loom/ai-suggestions/:suggestion_id/accept",
        "Confirm and promote one pending Loom AI suggestion.",
        "workspace_id + suggestion_id path params.",
        "JSON accepted suggestion state."),
    surface!("loom.ai.suggestions.reject", SurfaceGroup::NotesLoom, "POST",
        "/workspaces/:workspace_id/loom/ai-suggestions/:suggestion_id/reject",
        "Reject one pending Loom AI suggestion without promotion.",
        "workspace_id + suggestion_id path params.",
        "JSON rejected suggestion state."),
    surface!("loom.canvas.boards.create", SurfaceGroup::NotesLoom, "POST",
        "/workspaces/:workspace_id/loom/canvas-boards",
        "Create a persisted Loom CanvasBoard.",
        "workspace_id path param; JSON board input.",
        "JSON created board."),
    surface!("loom.canvas.boards.get", SurfaceGroup::NotesLoom, "GET",
        "/workspaces/:workspace_id/loom/canvas-boards/:block_id",
        "Load one CanvasBoard and its persisted view state.",
        "workspace_id + board block_id path params.",
        "JSON CanvasBoard view."),
    surface!("loom.canvas.boards.viewport", SurfaceGroup::NotesLoom, "PUT",
    ...
    surface!("loom.canvas.placements.create", SurfaceGroup::NotesLoom, "POST",
        "/workspaces/:workspace_id/loom/canvas-boards/:block_id/placements",
        "Place an existing Loom block on a CanvasBoard.",
        "workspace_id + board block_id path params; JSON placement input.",
        "JSON created placement."),
    surface!("loom.canvas.cards.create", SurfaceGroup::NotesLoom, "POST",
        "/workspaces/:workspace_id/loom/canvas-boards/:block_id/cards",
        "Create a card and place it on a CanvasBoard.",
        "workspace_id + board block_id path params; JSON card input.",
        "JSON created card placement."),
    surface!("loom.canvas.stage_cards.compensate", SurfaceGroup::NotesLoom, "POST",
        "/workspaces/:workspace_id/loom/canvas-boards/:block_id/stage-cards/:placement_id/compensate",
        "Compensate a failed staged CanvasBoard card placement.",
        "workspace_id + board block_id + placement_id path params.",
        "JSON compensation result."),
    surface!("loom.canvas.placements.patch", SurfaceGroup::NotesLoom, "PATCH",
        "/workspaces/:workspace_id/loom/canvas-placements/:placement_id",
        "Persist a CanvasBoard placement update.",
        "workspace_id + placement_id path params; JSON placement patch.",
        "JSON updated placement."),
    surface!("loom.canvas.placements.delete", SurfaceGroup::NotesLoom, "DELETE",
        "/workspaces/:workspace_id/loom/canvas-placements/:placement_id",
        "Remove a block placement from a CanvasBoard.",
        "workspace_id + placement_id path params.",
        "Empty success; 404 when absent."),
    surface!("loom.canvas.visual_edges.create", SurfaceGroup::NotesLoom, "POST",
        "/workspaces/:workspace_id/loom/canvas-boards/:block_id/visual-edges",
        "Create a visual-only edge on a CanvasBoard.",
        "workspace_id + board block_id path params; JSON edge input.",
        "JSON created visual edge."),
    surface!("loom.canvas.visual_edges.delete", SurfaceGroup::NotesLoom, "DELETE",
        "/workspaces/:workspace_id/loom/canvas-visual-edges/:visual_edge_id",
        "Delete a visual-only CanvasBoard edge.",
        "workspace_id + visual_edge_id path params.",
        "Empty success; 404 when absent."),
    surface!("loom.views.definitions.create", SurfaceGroup::NotesLoom, "POST",
        "/workspaces/:workspace_id/loom/views/definitions",
        "Create a saved Block Collection View as one atomic view_def authority mutation.",
        "Identity headers; JSON {block_id, title, definition}. block_id is required and retained across retries.",
        "JSON saved view definition; an identical same-id retry converges and changed evidence returns 409."),
    surface!("loom.views.definitions.get", SurfaceGroup::NotesLoom, "GET",
        "/workspaces/:workspace_id/loom/views/definitions/:block_id",
        "Load one saved Block Collection View definition.",
        "workspace_id + block_id path params.",
        "JSON {block, definition}; 404 when absent."),
    surface!("loom.views.definitions.patch", SurfaceGroup::NotesLoom, "PATCH",
        "/workspaces/:workspace_id/loom/views/definitions/:block_id",
        "Persist a saved Block Collection View definition update and its recoverable recorder intent.",
        "Identity headers; workspace_id + block_id path params; JSON {definition}.",
        "JSON updated saved view definition."),
    surface!("loom.views.definitions.results", SurfaceGroup::NotesLoom, "POST",
        "/workspaces/:workspace_id/loom/views/definitions/:block_id/results",
        "Execute a saved Block Collection View against the authoritative Loom query backend.",
        "workspace_id + block_id path params; required JSON body (use {} for defaults, or {limit, offset}).",
        "JSON typed table, Kanban, or calendar result projection."),
    ...
    surface!("loom.tags.get", SurfaceGroup::NotesLoom, "GET",
        ...
        "Load one complete tag hub with direct sub-tags, tagged blocks, and the all-edge backlink count.",
        "workspace_id and tag_block_id path params; tag_block_id must identify a tag_hub block.",
        "JSON LoomTagHub row: block, sub_tags, tagged_blocks, backlink_count."),
