...
const SURFACES: &[SurfaceDescriptor] = &[
    ...
    surface!(
        "knowledge.code.workspace_index",
        SurfaceGroup::CodeNavigation,
        "POST",
        "/workspaces/:workspace_id/code-nav/index",
        "Index a configured local workspace root through the real ingestion and code-index pipelines; bounded per-file fanout keeps the embedded SurrealDB store responsive and every run finishes with a terminal receipt.",
        "Required identity headers; JSON {root_path} for the operator-configured local root. Per-file fanout uses bounded local concurrency while reserving control-plane capacity; a single available worker remains serial.",
    ...
    surface!(
        ...
        "Run embedded-SurrealDB-native graph-blended hybrid Loom search.",
    ...
    surface!(
        "loom.canvas.stage_cards.compensate",
        SurfaceGroup::NotesLoom,
        "POST",
    ...
    surface!(
        "loom.canvas.visual_edges.delete",
        SurfaceGroup::NotesLoom,
        "DELETE",
    ...
    surface!(
        ...
        "Set/clear the block's ordinal in the reorderable Pins grid. The set/clear \
         ...
         in one embedded SurrealDB transaction (MT-024 atomic-receipt boundary). Tier 1 \
    ...
    surface!(
        ...
        "Atomically remove a pin: clear pin_order AND unpin the block in ONE \
         embedded SurrealDB transaction alongside its durable KNOWLEDGE_LOOM_BLOCK_MUTATED \
    ...
    surface!(
        ...
        "Attach an editable overlay to a wiki page (operator edits survive \
         ...
         EventLedger receipt commit in ONE embedded SurrealDB transaction (MT-025 FAIL_V2 \
