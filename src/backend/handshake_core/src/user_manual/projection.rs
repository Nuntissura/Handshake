...
//!
//! Two formats (projections only — embedded SurrealDB records stay authority):
...
pub fn render_page_markdown(
    ...
    out.push_str(&format!("# {}\n\n", page.title));
    out.push_str(
        "This is an on-demand projection. The embedded SurrealDB UserManual records remain canonical.\n\n",
    );
    for section in sections {
        out.push_str(&format!(
