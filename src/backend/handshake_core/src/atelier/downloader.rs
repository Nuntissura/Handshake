...
const WRITE_OUTPUT_ROOT: &str = concat!("RETURN { LET $row = (UPSERT $domain.record SET root_id = $domain.root_id, configured_root = $domain.configured_root, materialization_mode = $domain.materialization_mode, per_mode_subdirs = $domain.per_mode_subdirs RETURN AFTER)[0]; ", atelier_event_sql!(), " RETURN $row; };");
const WRITE_ALLOWLIST: &str = concat!("RETURN { LET $row = (UPSERT $domain.record SET allowlist_policy_id = $domain.allowlist_policy_id, name = $domain.name, allowed_domains = $domain.allowed_domains, explicit_url_lists = $domain.explicit_url_lists, default_decision = $domain.default_decision, rate_limit = $domain.rate_limit, max_pages = $domain.max_pages, robots_posture = $domain.robots_posture RETURN AFTER)[0]; ", atelier_event_sql!(), " RETURN $row; };");
const WRITE_AUTH: &str = concat!("RETURN { LET $row = (UPSERT $domain.record SET auth_context_ref = $domain.auth_context_ref, label = $domain.label, auth_mode = $domain.auth_mode, session_ref = $domain.session_ref, cookie_jar_artifact_ref = $domain.cookie_jar_artifact_ref, header_secret_refs = $domain.header_secret_refs RETURN AFTER)[0]; ", atelier_event_sql!(), " RETURN $row; };");
