...
//!
//! Plain data records for the deterministic prompt-feedback kernel. PostgreSQL/
//! EventLedger is the authority (see `super`); these structs are the typed shapes
//! the store reads/writes and the API serializes. No `sqlx` row wiring lives here
//! (that is in `super`), so this module stays a clean data contract.
