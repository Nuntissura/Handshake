//! WP-KERNEL-005 MT-155 (Visual Diff Baseline Contract) embedded SurrealDB
//! round-trip proof.
//!
...
//! reference contract, both RE-READ from the embedded store through the real
...
async fn mt155_visual_diff_baseline_persists_re_reads_and_tracks_latest() {
    ...
    // RE-READ from the embedded store by id: full fidelity.
    let reloaded = store
    ...
    assert_eq!(
        latest.baseline_id, second.baseline_id,
        "newest capture wins"
    );
    assert_ne!(
...
async fn mt155_visual_diff_request_persists_threshold_and_metadata_for_both_reference_kinds() {
    ...
    // RE-READ from the embedded store: threshold config and metadata round-trip.
    let reloaded = store
    ...
    assert_eq!(
        ...
        VisualDiffReference::PreviousScreenshot {
            previous_screenshot_ref: format!("artifact://screenshots/{surface_id}/previous-1.png"),
        }
    );
    assert_eq!(
...
async fn mt155_visual_diff_request_rejects_unknown_baseline_and_nonportable_refs() {
    ...
    let err = store
        .record_visual_diff_request(&NewVisualDiffRequest {
            ...
            candidate_screenshot_ref: format!("artifact://screenshots/{surface_id}/candidate.png"),
            comparison_mode: VisualComparisonMode::PixelDiff,
            threshold_config: threshold_config(),
            metadata: json!({}),
            requested_by: "mt155-proof".to_string(),
