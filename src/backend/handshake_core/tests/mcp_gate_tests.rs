...
use handshake_core::storage::{
    tests::embedded_test_backend, AccessMode, Database, ModelSessionState, NewModelSession,
};
...
async fn mcp_tool_call_allows_when_session_scoped_grants_satisfy_required_caps(
    ...
    db.upsert_model_session(NewModelSession {
        ...
        // ASSERT record::exists); the gate resolves the session by session_id,
        // so the seeded session carries no fabricated job link.
        job_id: None,
