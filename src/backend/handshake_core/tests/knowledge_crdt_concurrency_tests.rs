...
/// Build a structurally valid Yjs v1 update from a deterministic actor client.
/// The text is only a test sentinel; the bytes are the real frontend/backend
/// wire format enforced by the bridge.
...
/// Build a structurally valid Yjs update envelope for tests.
