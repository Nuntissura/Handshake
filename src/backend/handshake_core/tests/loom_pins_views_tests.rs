...
use handshake_core::storage::{
    ...
    LoomEdgeType, LoomViewFilters, LoomViewResponse, LoomViewType, NewLoomBlock, NewLoomEdge,
    WriteContext,
};
...
async fn pins_view_respects_user_pin_order() {
    ...
    pg.db.set_loom_block_pin_order(&ctx, &ws, &c, Some(0)).await.expect("order c");
    pg.db.set_loom_block_pin_order(&ctx, &ws, &a, Some(1)).await.expect("order a");
    pg.db.set_loom_block_pin_order(&ctx, &ws, &b, Some(2)).await.expect("order b");
    ...
    pg.db.set_loom_block_pin_order(&ctx, &ws, &b, Some(-1)).await.expect("reorder b");
    let pins2 = pg
        .db
        .query_loom_view(&ws, LoomViewType::Pins, LoomViewFilters::default(), 100, 0)
        .await
...
async fn clearing_pin_order_sends_block_to_the_end() {
    ...
    pg.db.set_loom_block_pin_order(&ctx, &ws, &a, Some(0)).await.expect("order a");
    pg.db.set_loom_block_pin_order(&ctx, &ws, &b, Some(1)).await.expect("order b");

    // Clear B's order (NULL) -> NULLS LAST puts it after the ordered A.
    let cleared = pg.db.set_loom_block_pin_order(&ctx, &ws, &b, None).await.expect("clear b");
    ...
    let pins = pg
        ...
        .await
        .expect("pins");
    assert_eq!(pin_ids(&pins), vec![a.clone(), b.clone()], "cleared pin trails ordered pin");
}
...
async fn unlinked_view_excludes_linked_blocks() {
    ...
    let unlinked = pg
        .db
        .query_loom_view(&ws, LoomViewType::Unlinked, LoomViewFilters::default(), 100, 0)
        .await
        .expect("unlinked");
    let ids: Vec<String> = match unlinked {
        LoomViewResponse::Unlinked { blocks } => blocks.iter().map(|b| b.block_id.clone()).collect(),
        _ => panic!("expected Unlinked"),
    };
    assert!(ids.contains(&lonely), "lonely block is unlinked");
    assert!(!ids.contains(&src) && !ids.contains(&tgt), "linked blocks are excluded");
}
