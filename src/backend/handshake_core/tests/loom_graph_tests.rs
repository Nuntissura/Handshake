//! WP-KERNEL-009 MT-179 LocalGraphApi + MT-180 GlobalGraphApi — real embedded
//! authority proof.
//!
...
macro_rules! embedded_or_skip {
    () => {{
        match open_embedded_store().await {
            Some(store) => store,
            None => {
                eprintln!("SKIP MT-179/180 loom graph proof: embedded store unavailable");
                return;
            }
        }
    }};
...
async fn bridge(db: &handshake_core::storage::surreal::SurrealDatabase, ws: &str, block_id: &str) {
    let ctx = WriteContext::human(None);
...
async fn local_graph_is_undirected_with_stale_markers_and_citations() {
    ...
    let a = block(&store.db, &ws, "Alpha", LoomBlockContentType::Note).await;
    let b = block(&store.db, &ws, "Beta", LoomBlockContentType::Note).await;
    let c = block(&store.db, &ws, "Gamma", LoomBlockContentType::TagHub).await;
    let d = block(&store.db, &ws, "Delta", LoomBlockContentType::Note).await;

    // Bridge A and B (indexed); leave C and D un-bridged so they are STALE.
    bridge(&store.db, &ws, &a).await;
    bridge(&store.db, &ws, &b).await;
    ...
    assert!(
        node_ids.contains(&a.as_str()),
        "incoming neighbor A present (undirected)"
    );
    assert!(
        node_ids.contains(&c.as_str()),
        "outgoing neighbor C present"
    );
    ...
    assert!(
        start_node.entity_id.is_some(),
        "B carries a ProjectKnowledgeIndex citation"
    );
    ...
    assert!(
        !m_ids.contains(&c.as_str()),
        "C not reachable when only mention edges are followed"
    );
}
...
async fn local_graph_depth_one_is_immediate_neighborhood() {
    ...
    let a = block(&store.db, &ws, "A1", LoomBlockContentType::Note).await;
    let b = block(&store.db, &ws, "B1", LoomBlockContentType::Note).await;
    let c = block(&store.db, &ws, "C1", LoomBlockContentType::Note).await;
    edge(
        &store.db,
        &ws,
        &a,
...
async fn global_graph_returns_all_and_suppresses_hubs() {
    ...
    let hub = block(&store.db, &ws, "Hub", LoomBlockContentType::Note).await;
    ...
    let x = block(&store.db, &ws, "X", LoomBlockContentType::Note).await;
    let y = block(&store.db, &ws, "Y", LoomBlockContentType::Note).await;
    edge(
        &store.db,
        &ws,
        &x,
    ...
    let full_ids: Vec<&str> = full
        ...
        .collect();
    assert!(
        full_ids.contains(&hub.as_str()),
        "hub present when suppression disabled"
    );
    ...
    let suppressed = store
        .db
        .global_graph(&ws, &[], 500, 5)
        .await
        .expect("suppressed global");
    let s_ids: Vec<&str> = suppressed
        .nodes
        .iter()
        .map(|n| n.block.block_id.as_str())
        .collect();
...
async fn global_graph_node_limit_sets_truncated() {
    ...
    let ws = store.create_workspace().await;
    for i in 0..5 {
        block(&store.db, &ws, &format!("N{i}"), LoomBlockContentType::Note).await;
    }
