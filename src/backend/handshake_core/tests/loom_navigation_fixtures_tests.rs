//! WP-KERNEL-009 MT-192 LoomNavigationFixtures — real embedded-store proof.
//!
...
//! wiki projection that regenerates. Authority = the embedded store; no parallel store.
//!
...
use handshake_core::storage::{
    ...
    LoomEdgeType, LoomFolderSortMode, LoomViewFilters, LoomViewResponse, LoomViewType,
    NewLoomBlock, NewLoomEdge, NewLoomFolder, WriteContext,
};
...
macro_rules! embedded_or_skip {
    () => {{
        match open_embedded_store().await {
            Some(store) => store,
            None => {
                eprintln!("SKIP MT-192 loom navigation fixtures: embedded store unavailable");
                return;
            }
        }
    }};
...
struct Db<'a>(
    &'a handshake_core::storage::surreal::SurrealDatabase,
    String,
);
...
async fn loom_navigation_fixture_exercises_every_path() {
    ...
    let q3 = db
        .block(
            "Q3 Plan",
            Some("align the Roadmap with budget"),
            LoomBlockContentType::Note,
        )
        .await;
    let lonely = db
        .block(
            "Stray Idea",
            Some("mentions the Roadmap in passing"),
            LoomBlockContentType::Note,
        )
        .await;
    let tag_project = db
        .block("project", None, LoomBlockContentType::TagHub)
        .await;
    ...
    db.edge(&tag_alpha, &tag_project, LoomEdgeType::SubTag)
        .await; // alpha is child of project

    // === FIXTURE 1: exact LoomBlock open (direct id load) ==============
    let opened = store
        .db
        .get_loom_block(&ws, &roadmap)
        .await
        .expect("exact open");
    ...
    let backlinks = store
        .db
        .get_backlinks_with_context(&ws, &roadmap)
        .await
        .expect("backlinks");
    assert!(
        backlinks.iter().any(|b| b.source_block.block_id == q3),
        "Q3 backlinks Roadmap via the mention edge"
    );
    assert!(
        backlinks.iter().any(|b| b
            .context_snippet
            .as_deref()
    ...
    let unlinked = store
        .db
        .scan_unlinked_mentions(&ws, &roadmap, &[], 100)
        .await
        .expect("unlinked");
    assert!(
        unlinked.iter().any(|m| m.source_block.block_id == lonely),
        "Stray Idea is an unlinked mention of Roadmap"
    );
    assert!(
        !unlinked.iter().any(|m| m.source_block.block_id == q3),
    ...
    let hub = store
        .db
        .get_tag_hub(&ws, &tag_project)
        .await
        .expect("tag hub");
    assert!(
        hub.sub_tags.iter().any(|b| b.block_id == tag_alpha),
        "alpha is a sub-tag of project"
    );
    let nested = store
        .db
        .list_blocks_for_tag(&ws, &tag_project, true, 100, 0)
        .await
        .expect("nested tag");
    let nested_ids: Vec<&str> = nested.iter().map(|b| b.block_id.as_str()).collect();
    assert!(
        nested_ids.contains(&q3.as_str()),
        "Q3 tagged #project directly"
    );
    assert!(
        nested_ids.contains(&roadmap.as_str()),
        "Roadmap tagged #alpha, nested under #project"
    );
    ...
    store
        .db
        .add_block_to_loom_folder(&ws, &work, &roadmap, None)
        .await
        .expect("file roadmap");
    let folders = store.db.list_loom_folders(&ws).await.expect("folders");
    let work_folder = folders.iter().find(|f| f.folder_id == work).unwrap();
    assert_eq!(
        work_folder.color.as_deref(),
        Some("#3366ff"),
        "folder color label persisted"
    );

    // === FIXTURE 6: breadcrumbs across the spine =======================
    let trail = store
        .db
        .loom_block_breadcrumbs(&ws, &roadmap)
        .await
        .expect("breadcrumbs");
    ...
    let local = store
        .db
        .local_graph(&ws, &roadmap, 2, &[], 200)
        .await
        .expect("local graph");
    assert!(
        local.nodes.iter().any(|n| n.block.block_id == q3),
        "Q3 in Roadmap's neighborhood (undirected)"
    );
    // Edge-type filter: tags only -> the Q3->Roadmap mention is excluded.
    let local_tags = store
        .db
        .local_graph(&ws, &roadmap, 1, &[LoomEdgeType::Tag], 200)
        .await
        .expect("tag graph");
    assert!(
        local_tags
            .nodes
            .iter()
            .any(|n| n.block.block_id == tag_alpha),
        "Roadmap -> #alpha tag edge present in tag-filtered graph"
    );
    let global = store
        .db
        .global_graph(&ws, &[], 500, 0)
        .await
        .expect("global graph");
    assert!(
        global.nodes.len() >= 5,
        "global graph spans the fixture blocks"
    );
    ...
    store
        .db
        .set_loom_block_pin_order(&ctx, &ws, &roadmap, Some(0))
        .await
        .expect("order");
    let pins = store
        .db
    ...
    let proj = store
        .db
        .compile_loom_wiki_projection(&ws, "Roadmap Topic", &[roadmap.clone(), q3.clone()])
        .await
        .expect("compile");
    assert!(!store
        .db
        .loom_wiki_projection_is_stale(&ws, &proj.projection_id)
        .await
        .expect("fresh"));
    ...
    assert!(
        store
            ...
            .await
            .expect("stale"),
        "wiki page is stale after source edit"
    );
    let regen = store
        .db
    ...
    let after =
        serde_json::to_string(&store.db.get_loom_block(&ws, &roadmap).await.expect("after"))
            .unwrap();
    assert_eq!(
        before, after,
        "deleting the wiki projection leaves the LoomBlock authority byte-identical"
    );
}
