...
use handshake_core::storage::{
    AccessMode, AiJob, Database, JobKind, JobMetrics, JobState, ModelSession, ModelSessionState,
    NewAiJob, NewModelSession, SafetyMode, SessionMessageRole, StorageError,
};
...
async fn model_run_persists_session_and_artifact_first_messages(
    ...
    let (state, _harness) = setup_state().await?;
