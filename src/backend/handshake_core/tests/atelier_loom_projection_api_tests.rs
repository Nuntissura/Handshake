//! WP-KERNEL-012 MT-033: real PostgreSQL proof for the canonical
...
use handshake_core::atelier::intake::{
    intake_event_family, IntakeBatchMode, IntakeProfileMode, NewIntakeBatch, NewIntakeItem,
};
...
async fn real_atelier_endpoint_returns_durable_canonical_loom_identity() {
    let Some(pg) = knowledge_pg().await else {
        eprintln!("SKIP MT-033 Atelier/Loom projection proof: no PostgreSQL");
        return;
    };
    ...
    let batch = store
        .open_intake_batch(&NewIntakeBatch {
            idempotency_key: format!("mt033-loom-projection-{}", Uuid::new_v4()),
            source_label: "MT-033 canonical Loom projection".to_string(),
            source_ref: None,
        ...
        .await
        .expect("create intake batch");
    let item = store
        .add_intake_item(
            batch.batch_id,
            &NewIntakeItem {
                source_path: format!("artifact://mt033/{}.png", Uuid::new_v4()),
                file_name: "mt033-canonical.png".to_string(),
                byte_len: 33,
                content_hash: None,
            },
        )
    ...
    let (base, http) = serve(state).await;
    let link_url = format!(
    ...
    assert_eq!(linked_json["item_id"], item.item_id.to_string());
    ...
    let listed_item = listed_json["items"]
        ...
        .expect("items array")
        .iter()
        .find(|row| row["item_id"] == item.item_id.to_string())
