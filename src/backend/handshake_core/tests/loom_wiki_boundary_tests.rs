...
//! Overlay / MT-187 ObsidianImportBoundary — real embedded authority proof.
//!
...
macro_rules! embedded_or_skip {
    () => {{
        match open_embedded_store().await {
            ...
            None => {
                eprintln!(
                    "SKIP MT-184/185/187 loom wiki boundary proof: embedded store unavailable"
                );
                return;
            }
        }
...
async fn wiki_projection_compiles_with_citations_and_staleness() {
    ...
    let a = note(&store.db, &ws, "Alpha", "alpha body").await;
    let b = note(&store.db, &ws, "Beta", "beta body").await;
...
async fn deleting_projection_leaves_block_authority_byte_identical() {
    ...
    let store = embedded_or_skip!();
    let ws = store.create_workspace().await;
    let a = note(&store.db, &ws, "Authority Block", "the canonical content").await;
    ...
    let before = block_fingerprint(&store.db.get_loom_block(&ws, &a).await.expect("before"));
    let bridge_before = store
        .db
    ...
    assert!(
        store
            .db
            .get_loom_wiki_projection(&ws, &proj.projection_id)
            .await
            .is_err(),
        "projection is gone"
    );
    ...
    let after = block_fingerprint(&store.db.get_loom_block(&ws, &a).await.expect("after"));
    assert_eq!(
        before, after,
        "block authority is byte-identical after projection delete"
    );
    let bridge_after = store
        .db
        .get_loom_block_knowledge_bridge(&ws, &a)
        .await
        .expect("bridge after")
...
async fn overlay_is_own_authority_independent_of_projection() {
    let store = embedded_or_skip!();
    let ws = store.create_workspace().await;
    let a = note(&store.db, &ws, "Src", "body").await;
    let proj = store
        .db
    ...
    let overlays = store
        .db
        .list_loom_wiki_overlays(&ws, &proj.projection_id)
        .await
        .expect("list");
    ...
    store
        .db
        .delete_loom_wiki_overlay(&ws, &overlay.overlay_id)
        .await
        .expect("del overlay");
    assert!(
        store
...
async fn markdown_import_creates_authority_never_vault() {
    ...
    // A real authority LoomBlock backed by a real RichDocument exists in the embedded store.
    ...
    // The block is queryable as authority through a typed embedded-store round-trip.
    let reloaded = store
        .db
...
async fn wiki_apis_fail_closed() {
    ...
    assert!(store
        .db
        .get_loom_wiki_projection(&ws, "LWPmissing")
        .await
        .is_err());
}
