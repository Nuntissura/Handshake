-- WP-KERNEL-005 MT-041 hardening v6: current-source validation must parse
-- the whole source and SQL marker trimming must match Rust's edge-whitespace
-- normalization for projection labels.

CREATE OR REPLACE FUNCTION atelier_bracket_link_trim_marker_text(value TEXT)
RETURNS TEXT
LANGUAGE sql
IMMUTABLE
AS $$
...
BEGIN
    IF expected_seq <= 0 OR source_text IS NULL THEN
        RETURN NULL;
    END IF;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'chk_atelier_bracket_link_projection_guard_v6'
          AND conrelid = 'atelier_bracket_link_projection'::regclass
    ) THEN
        ALTER TABLE atelier_bracket_link_projection
        ADD CONSTRAINT chk_atelier_bracket_link_projection_guard_v6 CHECK (TRUE);
    END IF;
END $$;
