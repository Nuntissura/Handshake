...
fn diff_engine_identical_texts_all_equal() {
    ...
    assert!(!blocks.is_empty(), "a non-empty diff must produce at least one block");
    assert!(
    ...
    println!("AC-001 identical: {} Equal block(s) covering 3 lines", blocks.len());
}
...
fn diff_engine_added_line_one_added_block() {
    ...
    assert_eq!(added.len(), 1, "AC-001: added line -> exactly one Added block; got {blocks:?}");
    assert_eq!(added[0].left_lines.len(), 0, "AC-001: Added block empty on the left");
    assert_eq!(added[0].right_lines.len(), 1, "AC-001: Added block covers one right line");
    println!("AC-001 added: one Added block at right lines {:?}", added[0].right_lines);
}
...
fn diff_engine_removed_line_one_removed_block() {
    ...
    assert_eq!(removed.len(), 1, "AC-001: removed line -> exactly one Removed block; got {blocks:?}");
    assert_eq!(removed[0].right_lines.len(), 0, "AC-001: Removed block empty on the right");
    assert_eq!(removed[0].left_lines.len(), 1, "AC-001: Removed block covers one left line");
    println!("AC-001 removed: one Removed block at left lines {:?}", removed[0].left_lines);
}
...
fn diff_engine_modified_line_one_modified_block() {
    ...
    assert_eq!(modified[0].left_lines.len(), 1, "AC-001: Modified covers one left line");
    assert_eq!(modified[0].right_lines.len(), 1, "AC-001: Modified covers one right line");
    // And there is NO separate Added or Removed block for the modify (it is fused).
    assert!(
        !blocks.iter().any(|b| b.status == DiffStatus::Added || b.status == DiffStatus::Removed),
