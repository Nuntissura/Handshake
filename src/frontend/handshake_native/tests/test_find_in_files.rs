...
use handshake_native::find_in_files::{
    ...
    KindFilter, MatchOptions, MatchPreview, ReplacementPlan, APPLY_AUTHOR_ID, KIND_FILTER_AUTHOR_ID,
...
fn hit(source_kind: &str, ref_id: &str, title: &str, excerpt: &str, doc_id: Option<&str>) -> LoomGraphSearchHit {
    LoomGraphSearchHit {
...
fn seeded_state() -> FindInFilesPanelState {
    ...
    s.results = vec![
        hit("loom_block", "blk-1", "First Note", "has FIND_TARGET here", Some("KRD-1")),
        hit("loom_block", "blk-2", "Second Note", "FIND_TARGET twice FIND_TARGET", Some("KRD-2")),
        hit("file", "blk-3", "Some File", "no match excerpt", None),
    ];
    s.result_set_key = Some(s.current_search_key());
    s.preview_plans = vec![mock_plan("KRD-1", "First Note", 1), mock_plan("KRD-2", "Second Note", 2)];
...
fn harness_for<'a>(
    ...
    Harness::builder()
        ...
        .build_ui(move |ui| {
            ...
            let mut cbs = FindInFilesCallbacks { on_open_hit: &mut on_open };
            show(
                ui,
...
fn accesskit_tree_has_all_contract_author_ids() {
    ...
    let mut harness = harness_for(state, opened, search_client, doc_client, Some("ws-1".to_owned()));
    harness.run();
    ...
    for required in [
        ...
        assert!(ids.contains(required), "PT-5: required author_id '{required}' missing from {ids:?}");
    }
...
fn toggle_buttons_flip_state() {
    ...
    let search_client = WorkspaceSearchClient::new(TEST_BASE, r.handle().clone());
    let doc_client = RichDocClient::new(TEST_BASE, r.handle().clone());
    let mut harness = harness_for(Arc::clone(&state), opened, search_client, doc_client, Some("ws-1".to_owned()));
    harness.run();

    assert!(!state.lock().unwrap().case_sensitive, "case off initially");
    click_author_id(&harness, TOGGLE_CASE_AUTHOR_ID);
    harness.run();
    assert!(state.lock().unwrap().case_sensitive, "AC-3: case toggle flipped on");

    click_author_id(&harness, TOGGLE_REGEX_AUTHOR_ID);
    harness.run();
    assert!(state.lock().unwrap().is_regex, "AC-3: regex toggle flipped on");
    click_author_id(&harness, TOGGLE_WORD_AUTHOR_ID);
    harness.run();
    assert!(state.lock().unwrap().whole_word, "AC-3: whole-word toggle flipped on");
...
fn preview_and_apply_gating() {
    ...
    assert!(preview_disabled, "AC-5: Preview Replace disabled with no search");
    ...
    let preview_enabled = h2
        ...
        .map(|n| !n.accesskit_node().is_disabled())
        .expect("preview node present");
    assert!(preview_enabled, "AC-5: Preview Replace enabled after a search");
    let apply_enabled = h2
        ...
        .map(|n| !n.accesskit_node().is_disabled())
        .expect("apply node present");
    assert!(apply_enabled, "AC-8: Apply enabled with a non-stale preview");
...
fn stale_result_guard_blocks_preview() {
    ...
    assert!(s.preview_plans.is_empty(), "no preview computed under stale results");
...
fn search_page_query_uses_verified_params() {
    ...
    assert!(params.contains(&("limit".to_owned(), "500".to_owned())), "AC-2: page size 500");
    ...
    let params2 = c.search_page_query(
        ...
        "tag-1, tag-2",
        "src/app",
        SearchMatchOptions { case_sensitive: true, whole_word: true, is_regex: true },
        500,
    );
    assert!(params2.contains(&("source_kinds".to_owned(), "document".to_owned())), "AC-4: source_kinds passed");
    ...
    assert!(params2.contains(&("offset".to_owned(), "500".to_owned())), "pagination offset forwarded");
    println!("AC-2/AC-4: graph-search params = verified q/limit/offset/source_kinds/tag_ids/path/regex");
}
...
fn bookmark_save_request_wraps_blob() {
    ...
    assert_eq!(spec.url, "http://127.0.0.1:37501/workspaces/ws-1/search-bookmarks");
    let body = spec.body.expect("bookmark body");
    assert_eq!(body.get("bookmark_state").unwrap(), &blob, "PUT wraps the blob under bookmark_state");
    // RISK-6: the blob carries the EXACT backend-validated schema_id.
    assert_eq!(
        body["bookmark_state"]["schema_id"],
        "hsk.workspace_search_bookmark_state@1",
...
fn replace_preserves_non_text_nodes_and_walks_code() {
    ...
    let re = handshake_native::find_in_files::compile_search_regex("FIND_TARGET", MatchOptions::default()).unwrap();
...
fn document_id_extraction_krd_prefix() {
    ...
    assert_eq!(document_id_from_hit(&bad), None, "RISK-5: non-KRD id rejected");
...
fn pane_opens_via_registry_and_renders_real_panel() {
    ...
    let sc = WorkspaceSearchClient::new(TEST_BASE, r.handle().clone());
    let dc = RichDocClient::new(TEST_BASE, r.handle().clone());
    let shared = Arc::new(Mutex::new(FindInFilesPaneShared::new(HsTheme::Dark.palette())));
    {
        let mut g = shared.lock().unwrap();
        g.workspace_id = Some("ws-1".to_owned());
    }
    let factory: Box<dyn PaneFactory> =
        Box::new(FindInFilesPaneFactory::with_state(sc, dc, Arc::clone(&shared), seeded_state()));
    ...
    let ids = author_ids(&harness);
    for required in [QUERY_AUTHOR_ID, SEARCH_AUTHOR_ID, PREVIEW_REPLACE_AUTHOR_ID, APPLY_AUTHOR_ID] {
        assert!(
            ids.contains(required),
            "registry-dispatched pane rendered the placeholder, not the real panel — '{required}' missing ({ids:?})"
        );
    }
    assert!(ids.contains(&result_author_id("loom_block", "blk-1")), "result row missing from registry pane");
...
fn find_in_files_screenshot() {
    ...
    let mut harness = Harness::builder()
        ...
        .build_ui(move |ui| {
            let pal = HsTheme::Dark.palette();
            let opened_cb = Arc::clone(&opened);
            let mut on_open = move |hit: &LoomGraphSearchHit| opened_cb.lock().unwrap().push(hit.ref_id.clone());
            let mut cbs = FindInFilesCallbacks { on_open_hit: &mut on_open };
            show(
                ui,
                &mut state.lock().unwrap(),
...
fn find_in_files_search_live_pg() {
    ...
    let workspace_id =
        std::env::var("HSK_TEST_WORKSPACE_ID").expect("set HSK_TEST_WORKSPACE_ID to a seeded workspace");
    ...
    let (hits, _key) = delivered.expect("PT-1: real graph-search succeeded against the live backend");
    assert!(!hits.is_empty(), "PT-1: expected >= 1 hit for query '{query}'");
    for h in &hits {
        assert!(!h.title.is_empty() || !h.ref_id.is_empty(), "PT-1: each hit carries a title or ref_id");
        assert!(!h.source_kind.is_empty(), "PT-1: each hit carries a source_kind");
    }
    println!("PT-1 PASS: {} hits from real PG /loom/graph-search for query '{query}'", hits.len());
}
...
fn find_in_files_replace_cycle_live_pg() {
    ...
    if std::env::var("HSK_TEST_REPLACE").as_deref() != Ok("1") {
        eprintln!("skipping replace-cycle: set HSK_TEST_REPLACE=1 to run the MUTATING replace test");
        return;
    }
    let workspace_id =
        std::env::var("HSK_TEST_WORKSPACE_ID").expect("set HSK_TEST_WORKSPACE_ID to a seeded workspace");
    ...
    assert!(!doc_ids.is_empty(), "PT-2: expected >= 1 KRD- doc containing FIND_TARGET");
    ...
    for doc_id in &doc_ids {
        ...
        let replaced = replace_in_content(&doc.content_json, &re, "REPLACED", MatchOptions::default());
        if replaced.count == 0 {
            continue;
        }
...
fn find_in_files_bookmark_roundtrip_live_pg() {
    ...
    use handshake_native::find_in_files::{bookmark_state_blob, parse_bookmark_state, SearchBookmark};
    use std::time::{Duration, Instant};

    let workspace_id =
        std::env::var("HSK_TEST_WORKSPACE_ID").expect("set HSK_TEST_WORKSPACE_ID to a real workspace");
    ...
    let found = parsed.iter().find(|b| b.id == "pt3-fixture").expect("PT-3: saved bookmark present");
    assert_eq!(found.query, "PT3_QUERY", "PT-3: bookmark query round-trips");
    assert_eq!(found.kind, KindFilter::Document, "PT-3: bookmark kind round-trips");
    assert!(found.case_sensitive, "PT-3: bookmark options round-trip");
    println!("PT-3 PASS: bookmark saved + read back via real PG /search-bookmarks with options intact");
}
