...
use handshake_native::loom_search_v2::{
    parse_highlight_segments, sorted_facets, LoomSearchV2PanelState,
};
...
fn parity_full_text_search_native() {
    ...
    assert!(
        spec.url.ends_with("/workspaces/ws-1/loom/search-v2"),
        "E4-37: native search URL (got {})",
        spec.url
    );
    let body = spec.body.expect("E4-37: search carries a body");
    assert_eq!(
        body["query"], "the",
        "E4-37: the FTS query is sent in the LoomSearchV2Body"
    );
    assert!(
        body.get("content_type").is_none(),
        "E4-37: an unfaceted FTS omits content_type"
    );
    ...
    assert!(
        segs.iter().any(|s| s.marked && s.text == "the"),
        "E4-37: the native highlighter marks the matched run"
    );
...
fn parity_semantic_search_native() {
    ...
    assert_eq!(
        state.status_text(),
        "3 results (semantic on)",
        "E4-39: the native panel reports semantic ON when pgvector contributed"
    );
    state.response = Some(LoomSearchV2Response {
        hits: vec![],
        content_type_facets: BTreeMap::new(),
    ...
    assert_eq!(
        state.status_text(),
        "1 result (keyword/fuzzy only)",
        "E4-39: the native panel NEVER claims semantic when it is off"
    );
    println!(
        "E4-39 NATIVE PASS: LoomSearchV2PanelState::status_text honestly reports semantic on/off"
    );
}
...
fn parity_semantic_search() {
    ...
    let resp: LoomSearchV2Response = serde_json::from_value(resp_json)
        .expect("E4-39: response deserializes through the native LoomSearchV2Response");
    ...
    assert!(
        state.status_text().contains("semantic on"),
        "E4-39: the native panel confirms semantic ON"
    );
    assert!(
        hit_count > 0,
        "E4-39: semantic search (pgvector) must return a non-empty hits list (got 0)"
    );
...
fn parity_faceted_filter_native() {
    ...
    assert_eq!(
        body["content_type"], "note",
        "E4-40: the active facet is sent as the content_type filter"
    );
    ...
    assert_eq!(
        sorted[0],
        ("code".to_string(), 5),
        "E4-40: the native facet ordering is count-desc (code=5 first)"
    );
...
fn parity_faceted_filter() {
    ...
    println!(
        "E4-40 PASS: native faceted request -> {} hits all match content_type '{content_type}'",
        hits.len()
    );
    mark_pass("E4-40");
}
...
fn parity_save_results_as_view_native() {
    ...
    assert!(
        ...
        "E4-41: native save-view URL (got {})",
        spec.url
    );
    let body = spec.body.expect("E4-41: save-view carries a body");
    assert_eq!(
        body["definition"]["kind"], "table",
        "E4-41: the saved search is born as a table view_def"
    );
    assert_eq!(
        body["definition"]["query"]["content_type"], "annotated_file",
        "E4-41: the search facet is embedded in the saved view query"
    );
...
fn parity_find_in_files_native() {
    ...
    let regex = compile_search_regex(needle, opts)
        .expect("E4-42: the native find engine compiles the query");
    ...
    assert_eq!(
        matched.len(),
        3,
        "E4-42: the native find-in-files engine must match all 3 files (got {matched:?})"
    );
...
fn parity_find_in_files() {
    ...
    println!(
        "E4-42 PASS: native file-search request surfaced {} distinct file paths for '{needle}'",
        distinct_paths.len()
    );
    mark_pass("E4-42");
}
...
fn parity_quick_switcher_native() {
    ...
    assert_eq!(
        recent_key, "loom_block:blk-2",
        "E4-43: the native recents key is source_kind:ref_id"
    );
    let ordered = ordered_results(&hits, std::slice::from_ref(&recent_key));
    assert_eq!(
        ordered[0].ref_id, "blk-2",
        "E4-43: the recorded recent surfaces FIRST in the native quick-switcher order"
    );
...
fn parity_quick_switcher() {
    ...
    let ordered = ordered_results(
        std::slice::from_ref(&recorded),
        std::slice::from_ref(&recent_key),
    );
    assert_eq!(
        ordered[0].ref_id, block_id,
        "E4-43: the native quick-switcher ranks the recorded block first"
    );
