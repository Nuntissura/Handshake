...
use handshake_native::backend_client::{
    ...
    LoomFolderClient, LoomGraphClient, LoomSidebarClient, LoomTagClient, LoomWikiClient, RequestSpec,
};
...
fn parity_local_graph_native() {
    ...
    assert!(spec.url.ends_with("/workspaces/ws-1/loom/graph-search"), "E3-23: native local-graph URL (got {})", spec.url);
    assert!(has_query(&spec, "backlink_depth", "2"), "E3-23: depth-2 local graph query");
    assert!(has_query_key(&spec, "q"), "E3-23: the local graph queries by focus title");
    println!("E3-23 NATIVE PASS: LoomGraphClient::local_request_with_depth built {} (depth 2)", spec.url);
}
...
fn parity_local_graph() {
    ...
    let graph = be.get_json(&format!("/workspaces/{}/loom/graph/local?start_block_id={block_id}&max_depth=2", be.workspace_id));
...
fn parity_global_graph_native() {
    ...
    assert_eq!(spec.method, HttpMethod::Get);
    assert!(spec.url.ends_with("/workspaces/ws-1/loom/views/all"), "E3-24: native global-graph URL (got {})", spec.url);
    println!("E3-24 NATIVE PASS: LoomGraphClient::global_request built {}", spec.url);
}
...
fn parity_global_graph() {
    ...
    println!("E3-24 PASS: native global-graph request + live graph -> {} nodes", nodes.len());
    mark_pass("E3-24");
}
...
fn parity_folder_tree_native() {
    ...
    assert_eq!(spec.method, HttpMethod::Get);
    assert!(spec.url.ends_with("/workspaces/ws-1/loom/folders"), "E3-25: native folders URL (got {})", spec.url);
    println!("E3-25 NATIVE PASS: LoomFolderClient::list_folders_request built {}", spec.url);
}
...
fn parity_folder_tree() {
    ...
    let path = spec.url.strip_prefix(NATIVE_BASE).unwrap_or(&spec.url).to_owned();
...
fn parity_color_labels_native() {
    ...
    assert_eq!(spec.method, HttpMethod::Patch, "E3-26: recolor is a merge-PATCH (never clobbers name/sort)");
    assert!(spec.url.ends_with("/workspaces/ws-1/loom/folders/folder-1"), "E3-26: native recolor URL (got {})", spec.url);
    let body = spec.body.expect("E3-26: recolor carries a body");
    assert_eq!(body["color"], "#ff8800", "E3-26: the recolor body carries ONLY the color key");
    assert_eq!(body.as_object().map(|o| o.len()), Some(1), "E3-26: recolor is a pure merge-patch (color only)");
    println!("E3-26 NATIVE PASS: LoomFolderClient::recolor_request built PATCH {} {{color}}", spec.url);
}
...
fn parity_color_labels() {
    let be = require_live_backend();
    let folder_id = std::env::var("HSK_TEST_FOLDER_ID").expect("E3-26 requires_pg: set HSK_TEST_FOLDER_ID to a real folder id");
    ...
    assert_eq!(spec.body.as_ref().and_then(|b| b.get("color")).and_then(|c| c.as_str()), Some("#ff8800"));
    // Live round-trip on the audited route, then confirm the color is preserved on reload.
    be.put_json(&format!("/workspaces/{}/loom/folders/{folder_id}", be.workspace_id), &serde_json::json!({ "color": "#ff8800" }));
    let reloaded = be.get_json(&format!("/workspaces/{}/loom/folders", be.workspace_id));
    assert!(serde_json::to_string(&reloaded).unwrap().contains("#ff8800"), "E3-26: the folder color label must be preserved after reload");
...
fn parity_tags_and_hubs_native() {
    ...
    assert!(spec.url.ends_with("/workspaces/ws-1/loom/edges"), "E3-27: native tag-edge URL (got {})", spec.url);
    let body = spec.body.expect("E3-27: tag edge carries a body");
    assert_eq!(body["source_block_id"], "blk-1", "E3-27: the tagged block is the edge source");
    assert_eq!(body["target_block_id"], "hub-1", "E3-27: the tag hub is the edge target");
    assert_eq!(body["edge_type"], "tag", "E3-27: the edge is a typed tag edge");
...
fn parity_tags_and_hubs() {
    ...
    assert_eq!(spec.body.as_ref().and_then(|b| b.get("edge_type")).and_then(|e| e.as_str()), Some("tag"));
    // Live round-trip on the audited tag + tag-hub routes.
    be.put_json(&format!("/workspaces/{}/loom/blocks/{block_id}", be.workspace_id), &serde_json::json!({ "tags": [tag] }));
    let hub = be.get_json(&format!("/workspaces/{}/loom/tags/{tag}", be.workspace_id));
    assert!(serde_json::to_string(&hub).unwrap().contains(&block_id), "E3-27: the tagged block {block_id} must appear in the tag hub for '{tag}'");
...
fn parity_pins_native() {
    ...
    assert!(set.url.ends_with("/workspaces/ws-1/loom/blocks/blk-1/pin-order"), "E3-28: native pin-order URL (got {})", set.url);
    assert_eq!(set.body.expect("E3-28: pin-order body")["pin_order"], 0, "E3-28: the pin ordinal is set to 0 (top)");
    let view = e3_28_pins_view_spec("ws-1");
    assert_eq!(view.method, HttpMethod::Get);
    assert!(view.url.ends_with("/workspaces/ws-1/loom/views/pins"), "E3-28: native pins-view URL (got {})", view.url);
...
fn parity_pins() {
    ...
    let set_path = set.url.strip_prefix(NATIVE_BASE).unwrap_or(&set.url).to_owned();
    be.put_json(&set_path, &set.body.clone().unwrap_or(serde_json::Value::Null));
    let view = e3_28_pins_view_spec(&be.workspace_id);
    let view_path = format!("{}?limit=100", view.url.strip_prefix(NATIVE_BASE).unwrap_or(&view.url));
    let pinned = be.get_json(&view_path);
    assert!(serde_json::to_string(&pinned).unwrap().contains(&block_id), "E3-28: the pinned block {block_id} must appear in the pins view (got {pinned})");
...
fn parity_backlinks_native() {
    ...
    assert_eq!(spec.method, HttpMethod::Get);
    assert!(spec.url.ends_with("/workspaces/ws-1/loom/blocks/blk-1/backlinks"), "E3-29: native backlinks URL (got {})", spec.url);
    println!("E3-29 NATIVE PASS: LoomSidebarClient::backlinks_request built {}", spec.url);
}
...
fn parity_backlinks() {
    ...
    println!("E3-29 PASS: native backlinks request -> {block_id} has {count} backlink(s) from real PG");
    mark_pass("E3-29");
}
...
fn parity_unlinked_mentions_native() {
    ...
    assert_eq!(spec.method, HttpMethod::Get);
    assert!(spec.url.ends_with("/workspaces/ws-1/loom/blocks/blk-1/unlinked-mentions"), "E3-30: native unlinked URL (got {})", spec.url);
    println!("E3-30 NATIVE PASS: LoomSidebarClient::unlinked_request built {}", spec.url);
}
...
fn parity_breadcrumbs_native() {
    ...
    let path: Vec<&str> = panel.breadcrumbs.iter().map(|b| b.block_id.as_str()).collect();
    assert_eq!(path, vec!["root", "parent", "child"], "E3-31: the native breadcrumb trail records the parent->child path");
    assert_eq!(panel.breadcrumbs.last().map(|b| b.title.as_str()), Some("Child"), "E3-31: the last crumb is the active block");
...
fn parity_breadcrumbs() {
    ...
    assert_eq!(trail.breadcrumbs.len(), 3, "E3-31: the native breadcrumb trail has the built path");
    // Live round-trip on the audited breadcrumbs route.
    let crumbs = be.get_json(&format!("/workspaces/{}/loom/blocks/{block_id}/breadcrumbs", be.workspace_id));
    let count = crumbs
        .as_array()
        .map(|a| a.len())
...
fn parity_wiki_page_projection_native() {
    ...
    assert_eq!(spec.method, HttpMethod::Get);
    assert!(spec.url.ends_with("/workspaces/ws-1/loom/wiki/proj-1"), "E3-32: native wiki-projection URL (got {})", spec.url);
    println!("E3-32 NATIVE PASS: LoomWikiClient::load_request built {}", spec.url);
}
...
fn parity_wiki_page_projection() {
    let be = require_live_backend();
    let projection_id = std::env::var("HSK_TEST_WIKI_PROJECTION_ID").expect("E3-32 requires_pg: set HSK_TEST_WIKI_PROJECTION_ID to a real compiled wiki page id");
    let spec = e3_32_wiki_spec(&be.workspace_id, &projection_id);
    let path = spec.url.strip_prefix(NATIVE_BASE).unwrap_or(&spec.url).to_owned();
    let wiki = be.get_json(&path);
    let has_body = wiki.get("rendered_content").and_then(|c| c.as_str()).map(|s| !s.is_empty()).unwrap_or(false);
    assert!(has_body, "E3-32: the wiki-page projection must return non-empty rendered_content (got {wiki})");
    println!("E3-32 PASS: native wiki-load request -> projection {projection_id} resolved wikilinks");
    mark_pass("E3-32");
}
...
fn e3_33_place_spec(ws: &str, board: &str, block: &str) -> RequestSpec {
    let rt = native_rt();
    CanvasBoardClient::new(NATIVE_BASE, rt.handle().clone()).place_block_request(ws, board, block, 100.0, 100.0, 200.0, 120.0)
}
...
fn parity_canvas_board_placement_native() {
    ...
    assert_eq!(spec.method, HttpMethod::Post);
    assert!(spec.url.ends_with("/workspaces/ws-1/loom/canvas-boards/board-1/placements"), "E3-33: native placement URL (got {})", spec.url);
    let body = spec.body.expect("E3-33: placement carries a body");
    assert_eq!(body["placed_block_id"], "blk-1", "E3-33: the placement references the real Loom block");
...
fn parity_canvas_board_placement() {
    let be = require_live_backend();
    let block_id = be.require_block_id();
    let board_id = std::env::var("HSK_TEST_BOARD_ID").expect("E3-33 requires_pg: set HSK_TEST_BOARD_ID to a real canvas board id");
    ...
    let placement_id = placement["placement_id"].as_str().expect("E3-33: placement returns a placement_id");
    let board = be.get_json(&format!("/workspaces/{}/loom/canvas-boards/{board_id}", be.workspace_id));
    assert!(serde_json::to_string(&board).unwrap().contains(placement_id), "E3-33: the new placement {placement_id} must appear in the board view");
...
fn e3_34_create_spec(ws: &str, kind: BlockViewKind) -> RequestSpec {
    let rt = native_rt();
    BlockViewClient::new(NATIVE_BASE, rt.handle().clone()).create_view_request(ws, "parity-view", &BlockViewDefinition::of_kind(kind))
}
...
fn parity_block_collection_table_native() {
    ...
    assert!(create.url.ends_with("/workspaces/ws-1/loom/views/definitions"), "E3-34: native create-view URL (got {})", create.url);
    let body = create.body.expect("E3-34: create-view body");
    assert_eq!(body["definition"]["kind"], "table", "E3-34: the created view is a table view_def");
    let results = e3_34_results_spec("ws-1", "view-1");
    assert_eq!(results.method, HttpMethod::Post, "E3-34: the results query is a POST (not a GET)");
    assert!(results.url.ends_with("/workspaces/ws-1/loom/views/definitions/view-1/results"), "E3-34: native results URL (got {})", results.url);
...
fn parity_block_collection_table() {
    ...
    let create_path = create.url.strip_prefix(NATIVE_BASE).unwrap_or(&create.url).to_owned();
    let view = be.post_json(&create_path, &create.body.clone().unwrap_or(serde_json::Value::Null));
    let view_id = view_block_id(&view);
    let results = e3_34_results_spec(&be.workspace_id, &view_id);
    let results_path = results.url.strip_prefix(NATIVE_BASE).unwrap_or(&results.url).to_owned();
    let results_body = be.post_json(&results_path, &results.body.clone().unwrap_or(serde_json::Value::Null));
    let rows = results_body.get("blocks").and_then(|r| r.as_array()).map(|a| a.len()).unwrap_or(0);
...
fn parity_block_collection_kanban_native() {
    ...
    assert_eq!(def.kind, BlockViewKind::Kanban, "E3-35: the native Kanban view is grouped");
    assert!(def.group_by.is_some(), "E3-35: a native Kanban view carries a group_by so it renders lanes");
    let spec = e3_35_card_move_spec("ws-1", "blk-1", &["done".to_string()], &["todo".to_string()]);
    assert_eq!(spec.method, HttpMethod::Patch);
    assert!(spec.url.ends_with("/workspaces/ws-1/loom/blocks/blk-1"), "E3-35: native card-move URL (got {})", spec.url);
    let body = spec.body.expect("E3-35: card-move body");
    assert_eq!(body["add_tags"][0], "done", "E3-35: the card moves into the 'done' lane (add tag)");
    assert_eq!(body["remove_tags"][0], "todo", "E3-35: the card leaves the 'todo' lane (remove tag)");
    println!("E3-35 NATIVE PASS: BlockViewClient::card_move_request built a lane-move tag mutation");
}
...
fn parity_block_collection_kanban() {
    let be = require_live_backend();
    let block_id = be.require_block_id();
    let view_id = std::env::var("HSK_TEST_VIEW_ID").expect("E3-35 requires_pg: set HSK_TEST_VIEW_ID to a real Kanban view_def id");
    let target_column = "parity-e3-35-done".to_string();
    // CTRL-2: build + send the native card-move (add the target lane's tag).
    let spec = e3_35_card_move_spec(&be.workspace_id, &block_id, std::slice::from_ref(&target_column), &[]);
    let path = spec.url.strip_prefix(NATIVE_BASE).unwrap_or(&spec.url).to_owned();
    ...
    assert!(s.contains(&block_id) && s.contains(&target_column), "E3-35: after the move, card {block_id} must appear in column '{target_column}'");
...
fn parity_block_collection_calendar_native() {
    ...
    assert_eq!(def.kind, BlockViewKind::Calendar, "E3-36: the native calendar view buckets by date");
    let results = e3_34_results_spec("ws-1", "cal-1");
    assert_eq!(results.method, HttpMethod::Post);
    assert!(results.url.ends_with("/workspaces/ws-1/loom/views/definitions/cal-1/results"), "E3-36: native calendar results URL (got {})", results.url);
...
fn parity_block_collection_calendar() {
    let be = require_live_backend();
    let view_id = std::env::var("HSK_TEST_VIEW_ID").expect("E3-36 requires_pg: set HSK_TEST_VIEW_ID to a real calendar view_def id");
    let today = "2026-06-26";
    // CTRL-2: build + send the native calendar results request.
    let results = e3_34_results_spec(&be.workspace_id, &view_id);
    let path = results.url.strip_prefix(NATIVE_BASE).unwrap_or(&results.url).to_owned();
    let results_body = be.post_json(&path, &results.body.clone().unwrap_or(serde_json::Value::Null));
    assert!(serde_json::to_string(&results_body).unwrap().contains(today), "E3-36: the calendar view for {today} must surface the daily journal block");
    println!("E3-36 PASS: native calendar results request -> {today} surfaced the daily journal block");
    mark_pass("E3-36");
}
