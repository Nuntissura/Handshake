...
fn sticky_scroll_enclosing_stack_is_outermost_first() {
    ...
    assert_eq!(headers[0].line, 0, "AC-002: outermost header is the impl (line 0)");
    assert!(headers[0].text.contains("impl Thing"), "outermost text is the impl decl");
    assert_eq!(headers[1].line, 1, "AC-002: second is the fn (line 1)");
    assert!(headers[1].text.contains("fn deep"), "second text is the fn decl");
    assert_eq!(headers[2].line, 2, "AC-002: innermost is the if-block (line 2)");
    assert!(headers[2].text.contains("if true"), "innermost text is the if decl");
    for (i, h) in headers.iter().enumerate() {
        assert_eq!(h.depth, i, "depth is the shown-stack position (0 = outermost)");
    }
...
fn sticky_scroll_caps_at_max_keeping_innermost() {
    ...
    assert_eq!(headers[0].line, 1, "after the cap, the fn is the outermost shown (impl dropped)");
    assert_eq!(headers[1].line, 2, "the if-block (innermost) is kept");

    // The default cap is 5 (VS Code parity).
    assert_eq!(StickyScroll::new().max_sticky_lines(), 5, "default cap is 5 (VS Code parity)");
...
fn sticky_scroll_empty_and_scrolled_past_yield_no_pin() {
    ...
    assert!(sticky.compute(3, &[], &buffer).is_empty(), "no regions -> no headers");
...
fn code_editor_sticky_scroll_renders_pinned_while_scrolled() {
    ...
    assert!(!painted.contains(&0), "after scrolling to line 60, line 0 (fn decl) is off-screen");
    ...
    assert!(
        harness.query_all_by_label("Code editor sticky scroll").count() >= 1,
    ...
    match harness.render() {
        Ok(image) => {
            ...
            assert!(image.width() > 0 && image.height() > 0, "sticky-scroll image non-empty");
            println!(
...
fn clicking_sticky_header_scrolls_to_its_line() {
    ...
    assert!(!before.contains(&0), "scrolled away from the top before the click");
    ...
    harness.get_by_label("Sticky header: fn outer_fn() {").click();
    harness.run();
    harness.run();
...
fn sticky_scroll_source_has_no_reparse_or_backend() {
    let full = std::fs::read_to_string("src/code_editor/sticky_scroll.rs")
        .expect("read sticky_scroll.rs");
