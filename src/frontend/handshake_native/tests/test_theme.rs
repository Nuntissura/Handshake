...
fn light_palette_tokens_match_react_root() {
    ...
    assert_eq!(
        p.accent_soft,
        Color32::from_rgb(0xe0, 0xf2, 0xfe),
        "light.accent_soft"
    );
    assert_eq!(
        p.success_bg,
...
fn dark_palette_tokens_match_react_dark() {
    ...
    assert_eq!(
        p.surface,
        Color32::from_rgb(0x1d, 0x23, 0x2b),
        "dark.surface"
    );
    assert_eq!(
        p.surface_strong,
    ...
    assert_eq!(
        p.text_subtle,
        Color32::from_rgb(0xba, 0xc5, 0xd1),
        "dark.text_subtle"
    );
...
fn divider_tokens_exist_for_both_palettes_and_differ() {
    ...
    assert_ne!(
        ...
        "divider_idle differs dark vs light"
    );
    assert_ne!(
        dark.divider_grab, light.divider_grab,
        "divider_grab differs dark vs light"
    );
}
...
fn divider_token_overrides_apply() {
    ...
    assert_eq!(
        ...
        "divider_idle override applies"
    );
    // other divider tokens unchanged
    assert_eq!(
        p.divider_hover,
        HsPalette::dark().divider_hover,
        "divider_hover untouched"
    );
}
...
fn dark_rgba_tokens_are_premultiplied() {
    ...
    assert_eq!(
        p.accent_soft,
        Color32::from_rgba_unmultiplied(34, 197, 94, 46),
        "dark.accent_soft"
    );
    assert_eq!(
        p.accent_soft.to_array(),
        [6, 36, 17, 46],
        "dark.accent_soft premultiplied bytes"
    );
    // rgba(34,197,94,0.2) -> alpha8 51
    assert_eq!(
        p.success_bg,
        Color32::from_rgba_unmultiplied(34, 197, 94, 51),
        "dark.success_bg"
    );
    // rgba(248,113,113,0.2) -> alpha8 51
    assert_eq!(
        p.error_bg,
        Color32::from_rgba_unmultiplied(248, 113, 113, 51),
        "dark.error_bg"
    );
}
...
fn dark_syntax_tokens() {
    ...
    assert_eq!(
        ...
        "dark.syntax.type_name"
    );
    // punctuation/background are derived from the base palette tokens.
    assert_eq!(
        s.punctuation,
        HsPalette::dark().text_subtle,
        "dark.syntax.punctuation==text_subtle"
    );
    assert_eq!(
        s.background,
        HsPalette::dark().bg,
        "dark.syntax.background==bg"
    );
}
...
fn light_syntax_tokens() {
    ...
    assert_eq!(
        s.string,
        Color32::from_rgb(0xa3, 0x15, 0x15),
        "light.syntax.string"
    );
    assert_eq!(
        s.comment,
    ...
    assert_eq!(
        s.number,
        Color32::from_rgb(0x09, 0x86, 0x58),
        "light.syntax.number"
    );
    assert_eq!(
        s.type_name,
        Color32::from_rgb(0x26, 0x7f, 0x99),
        "light.syntax.type_name"
    );
    assert_eq!(
        s.punctuation,
        HsPalette::light().text_subtle,
        "light.syntax.punctuation==text_subtle"
    );
    assert_eq!(
        s.background,
        HsPalette::light().bg,
        "light.syntax.background==bg"
    );
}
...
fn dark_diagnostic_tokens_match_mt007_contract() {
    ...
    assert_eq!(
        ...
        "dark.diagnostics.error==RED"
    );
    assert_eq!(
        d.warning,
        Color32::from_rgb(0xff, 0xff, 0x00),
        "dark.diagnostics.warning==YELLOW"
    );
    assert_eq!(
        d.info,
        Color32::from_rgb(100, 149, 237),
        "dark.diagnostics.info==cornflower"
    );
    assert_eq!(
        d.hint,
        Color32::from_rgb(100, 149, 237),
        "dark.diagnostics.hint==cornflower"
    );
    assert_eq!(
        d.breakpoint,
        Color32::from_rgb(229, 60, 60),
        "dark.diagnostics.breakpoint"
    );
    // Error must equal egui's RED so the AC-003 gutter red-pixel screenshot stays valid.
    assert_eq!(
        d.error,
        Color32::RED,
        "dark.diagnostics.error==Color32::RED"
    );
    assert_eq!(
        d.warning,
        Color32::YELLOW,
        "dark.diagnostics.warning==Color32::YELLOW"
    );
}
...
fn light_diagnostic_tokens_match_mt007_contract() {
    ...
    assert_eq!(
        d.error,
        Color32::from_rgb(0xff, 0x00, 0x00),
        "light.diagnostics.error==RED"
    );
    assert_eq!(
        d.warning,
        Color32::from_rgb(0xff, 0xff, 0x00),
        "light.diagnostics.warning==YELLOW"
    );
    assert_eq!(
        d.info,
        Color32::from_rgb(100, 149, 237),
        "light.diagnostics.info==cornflower"
    );
    assert_eq!(
        d.hint,
        Color32::from_rgb(100, 149, 237),
        "light.diagnostics.hint==cornflower"
    );
    assert_eq!(
        d.breakpoint,
        Color32::from_rgb(229, 60, 60),
        "light.diagnostics.breakpoint"
    );
}
