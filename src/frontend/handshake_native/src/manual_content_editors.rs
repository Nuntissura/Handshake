...
left viewport atelier-pose-3d-viewport shows the REAL decoded source image (its node value reports source_image=loaded dimensions=WxH when source bytes are present, source_image=empty with an explicit empty-state message otherwise, and source_image=decode_error on a bad decode) and never paints a fabricated placeholder tile. Those source bytes are fetched at runtime from the native ArtifactStore: when atelier-pose-source-ref is a canonical atelier://media/<uuid> media asset (the PoseRig.source_asset_id), the panel calls GET /atelier/media-assets/<uuid>/bytes and feeds the returned image bytes into the left viewport; the route reads the ArtifactStore payload the asset's artifact_ref points at, returns the DAM MIME as Content-Type, and fails closed (404 on an unknown asset, hard error on a payload/manifest/DAM content-hash mismatch) so a broken source can never serve fabricated or partial bytes. Watch atelier-pose-source-status for the fetch state: no source set, an unresolvable ref (not atelier://media/<uuid>), backend offline, fetching, loaded (N bytes), or a fetch failure — and on any failure the viewport keeps its explicit empty state, never a placeholder. The right viewport atelier-pose-openpose-viewport renders the REAL generated OpenPose keypoints (node value bound_openpose=real_keypoints with keypoint_source=backend_export when a backend export result exists, else keypoint_source=live_preview for the offline preview): body-18 bones plus face-70 and hand-21 joints, exactly as the exported PNG would draw them, and its keypoint_signature changes whenever the keypoints move so a rotation is provably re-rendered from the real keypoints. Drive rotation with atelier-pose-yaw-minus, atelier-pose-yaw-plus, \
