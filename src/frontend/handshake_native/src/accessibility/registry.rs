...
pub const DECLARED_IDENTITIES: &[DeclaredIdentity] = &[
    ...
    // container (91), strictly below the pane id base (100). The menu count is FIXED at seven (WP-1
    // added the MODELS menu for the multi-model orchestration operator surfaces), so each
    // top-level menu button gets a fixed id (MENU_BAR_NODE_ID_BASE + index) and a fixed author_id (its
    // `menu-{name}` key), both enumerated here so the collision test proves the seven ids are disjoint
    // from every other declared identity. (Const-context: the indices are spelled out because a const slice
