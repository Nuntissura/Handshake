...
pub struct CodeEditorPanel {
    ...
    /// Action-specific acknowledgement for the stable editor-context opener. Argus may activate this
    /// persistent node repeatedly; every handled AccessKit click advances the generation only after the
    /// production context-menu request has been issued and its snapshot-open latch has been set.
...
impl CodeEditorPanel {
    ...
    fn render_editor_context_menu(&self, ui: &mut egui::Ui, rect: egui::Rect) {
        ...
        let context_menu_open_generation =
            self.context_menu_open_generation.load(Ordering::Relaxed);
        let context_menu_open_completion = serialize_same_target_click_completion(
            "code-editor.open-context-menu",
            &self.suffixed(CODE_EDITOR_CONTEXT_SURFACE_AUTHOR_ID),
            context_menu_open_generation,
            if context_menu_open_generation == 0 {
                ClickCompletionState::Ready
            } else {
                ClickCompletionState::Applied
            },
        )
        .expect("editor-context completion fields are bounded and valid");
        ui.ctx().accesskit_node_builder(node_id, move |node| {
            node.set_role(accesskit::Role::MenuItem);
            node.set_author_id(author.clone());
            node.set_label("Rename Symbol".to_owned());
            node.set_value(context_menu_open_completion.clone());
            node.add_action(accesskit::Action::Click);
        });
