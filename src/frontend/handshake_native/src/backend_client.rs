...
impl AtelierClient {
    ...
    pub fn create_ckc_media_album(
        ...
        let spec = self.create_media_album_actor_request(
            character_internal_id,
            name,
            notes,
            sheet_version_id,
    ...
    pub fn add_ckc_media_album_items(
        ...
        let spec = self.add_media_album_items_actor_request(
            collection_id,
            asset_ids,
            source_path_ref,
            source_url_ref,
    ...
    pub fn export_posekit_openpose(
        ...
        let spec = self.posekit_openpose_export_actor_request(
            source_ref,
            yaw_deg,
            pitch_deg,
            zoom,
    ...
    pub fn export_contact_sheet(
        ...
        let spec = self.contact_sheet_export_actor_request(
            source_kind,
            source_ref,
            rows,
            columns,
    ...
    pub fn analyze_ingest_facial(
        ...
        let spec = self.facial_ingest_analysis_actor_request(batch_id, profile, actor_id);
        let client = self.client.clone();
        self.runtime.spawn(async move {
            let result = post_json_with_actor(&client, &spec)
                .await
    ...
    pub fn save_ckc_media_notes_tags(
        ...
        let spec =
            self.media_notes_tags_actor_request(asset_id, notes, tags, review_status, actor_id);
        let client = self.client.clone();
        self.runtime.spawn(async move {
            let result = post_json_with_actor(&client, &spec)
                .await
...
mod tests {
    ...
    #[test]
    fn atelier_client_builds_posekit_openpose_export_request() {
        let rt = rt();
        let c = AtelierClient::new_with_actor_id(BASE, rt.handle().clone(), "posekit-agent-014");
        let spec = c.posekit_openpose_export_actor_request(
            "atelier://media/source.png",
            90.0,
            0.0,
            1.25,
            true,
        ...
        assert_eq!(spec.method, HttpMethod::Post);
        assert_eq!(spec.url, format!("{BASE}/atelier/posekit/openpose-export"));
        assert_eq!(
            spec.headers,
            vec![(
                HSK_HEADER_ACTOR_ID.to_owned(),
                "posekit-agent-014".to_owned()
            )]
        );
        let body = spec.body.expect("posekit export body");
        assert_eq!(body["source_ref"], "atelier://media/source.png");
        assert_eq!(body["rig_id"], "018f7848-1111-7000-9000-00000000f014");
        assert_eq!(body["yaw_deg"], 90.0);
        assert_eq!(body["pitch_deg"], 0.0);
        assert_eq!(body["zoom"], 1.25);
    ...
    #[test]
    fn atelier_client_builds_contact_sheet_export_request() {
        let rt = rt();
        let c = AtelierClient::new_with_actor_id(BASE, rt.handle().clone(), "ingest-agent-018");
        let items = vec![
            AtelierContactSheetItem {
                item_id: "018f7848-1111-7000-9000-00000000a001".to_owned(),
                label: "frame-a.png".to_owned(),
                source_ref: "dataset://source/frame-a.png".to_owned(),
                media_ref: Some("atelier://media/a".to_owned()),
            },
            AtelierContactSheetItem {
                item_id: "018f7848-1111-7000-9000-00000000a002".to_owned(),
                label: "frame-b.png".to_owned(),
                source_ref: "dataset://source/frame-b.png".to_owned(),
                media_ref: None,
            },
        ];
        let spec = c.contact_sheet_export_actor_request(
            "ingest_batch",
            "018f7848-1111-7000-9000-00000000b018",
            3,
            4,
            300,
            true,
            "cover",
            Some("artifact://atelier/contact-sheets/test.svg"),
            &items,
            c.actor_id(),
        );
        assert_eq!(spec.method, HttpMethod::Post);
        assert_eq!(spec.url, format!("{BASE}/atelier/contact-sheets/export"));
        assert_eq!(
            spec.headers,
            vec![(
                HSK_HEADER_ACTOR_ID.to_owned(),
                "ingest-agent-018".to_owned()
            )]
        );
        let body = spec.body.expect("contact sheet body");
        assert_eq!(body["source_kind"], "ingest_batch");
        assert_eq!(body["source_ref"], "018f7848-1111-7000-9000-00000000b018");
        assert_eq!(body["rows"], 3);
        assert_eq!(body["columns"], 4);
        assert_eq!(body["dpi"], 300);
        assert_eq!(body["include_labels"], true);
        assert_eq!(body["thumbnail_fit"], "cover");
        assert_eq!(
            body["output_path"],
            "artifact://atelier/contact-sheets/test.svg"
        );
        assert_eq!(
        ...
        assert_eq!(
            body["items"][0]["source_ref"],
            "dataset://source/frame-a.png"
        );
        assert_eq!(body["items"][0]["media_ref"], "atelier://media/a");
        assert_eq!(body["items"][1]["media_ref"], serde_json::Value::Null);
    }

    #[test]
    fn atelier_contact_sheet_export_parser_requires_layout_artifacts_and_lineage() {
        let artifact = serde_json::json!({
            "artifact_ref": "artifact://.handshake/artifacts/L1/contact/payload",
            "manifest_ref": "artifact://.handshake/artifacts/L1/contact/artifact.json",
            "content_hash": "hash-a",
            "byte_len": 128,
            "mime": "image/svg+xml",
        ...
        let row = serde_json::json!({
            ...
            "source_ref": "018f7848-1111-7000-9000-00000000b018",
            "thumbnail_fit": "cover",
            "output_path": "artifact://atelier/contact-sheets/test.svg",
            "layout": {"rows": 3, "columns": 4, "dpi": 300, "cell_count": 12},
            "source_items": [
                {
                    "item_id": "018f7848-1111-7000-9000-00000000a001",
                    "label": "frame-a.png",
                    "source_ref": "dataset://source/frame-a.png",
                    "media_ref": null
                }
            ],
            "item_count": 1,
            ...
            "content_hash": "export-hash",
            "receipt_ref": "artifact://.handshake/artifacts/L1/contact-receipt/payload",
            "svg_artifact": artifact,
            "receipt_artifact": {
                "artifact_ref": "artifact://.handshake/artifacts/L1/contact-receipt/payload",
                "manifest_ref": "artifact://.handshake/artifacts/L1/contact-receipt/artifact.json",
                "content_hash": "receipt-sha",
                "byte_len": 256,
        ...
        let parsed =
            parse_atelier_contact_sheet_export_row(&row).expect("complete contact sheet export");
        assert_eq!(parsed.schema_id, "hsk.atelier.contact_sheet_export@1");
        assert_eq!(parsed.layout["rows"], 3);
        assert_eq!(parsed.thumbnail_fit, "cover");
        assert_eq!(
            parsed.output_path.as_deref(),
            Some("artifact://atelier/contact-sheets/test.svg")
        );
        assert_eq!(
            parsed.source_items[0].item_id,
            "018f7848-1111-7000-9000-00000000a001"
        );
        assert_eq!(
            parsed.source_items[0].source_ref,
            "dataset://source/frame-a.png"
        );
        assert_eq!(parsed.svg_artifact.mime, "image/svg+xml");
        assert_eq!(parsed.receipt_artifact.mime, "application/json");

        let mut wrong_schema = row.clone();
        wrong_schema["schema_id"] = serde_json::json!("hsk.atelier.contact_sheet_export@0");
        assert!(parse_atelier_contact_sheet_export_row(&wrong_schema).is_none());

        let mut missing_rows = row.clone();
        missing_rows["layout"]
            .as_object_mut()
            .expect("layout object")
            .remove("rows");
        assert!(parse_atelier_contact_sheet_export_row(&missing_rows).is_none());

        let mut missing_source_items = row;
        missing_source_items
            .as_object_mut()
            .expect("row object")
            .remove("source_items");
        assert!(parse_atelier_contact_sheet_export_row(&missing_source_items).is_none());

        let mut bad_cell_count = serde_json::json!({
            "schema_id": "hsk.atelier.contact_sheet_export@1",
            "source_kind": "ingest_batch",
            "source_ref": "018f7848-1111-7000-9000-00000000b018",
            "thumbnail_fit": "cover",
            "output_path": null,
            "layout": {"rows": 3, "columns": 4, "dpi": 300, "cell_count": 99},
            "source_items": [],
            "item_count": 0,
            ...
            "svg_artifact": {
                "artifact_ref": "artifact://svg",
                "manifest_ref": "artifact://svg/manifest",
                "content_hash": "svg-sha",
                "byte_len": 1,
                "mime": "image/svg+xml",
                "file_name": "atelier-contact-sheet.svg"
            },
            "receipt_artifact": {
                "artifact_ref": "artifact://receipt",
                "manifest_ref": "artifact://receipt/manifest",
                "content_hash": "receipt-sha",
                "byte_len": 1,
                "mime": "application/json",
                "file_name": "atelier-contact-sheet-receipt.json"
        ...
        assert!(parse_atelier_contact_sheet_export_row(&bad_cell_count).is_none());
        bad_cell_count["layout"]["cell_count"] = serde_json::json!(12);
        bad_cell_count["svg_artifact"]["mime"] = serde_json::json!("text/plain");
        assert!(parse_atelier_contact_sheet_export_row(&bad_cell_count).is_none());
    }

    #[test]
    fn atelier_client_builds_facial_ingest_analysis_request() {
        let rt = rt();
        let c = AtelierClient::new_with_actor_id(BASE, rt.handle().clone(), "facial-agent-019");
        let spec = c.facial_ingest_analysis_actor_request(
            "018f7848-1111-7000-9000-00000000b019",
            "quality+dedupe+identity",
            c.actor_id(),
        );
        assert_eq!(spec.method, HttpMethod::Post);
        assert_eq!(
            spec.url,
            format!(
                "{BASE}/atelier/intake/batches/018f7848-1111-7000-9000-00000000b019/facial/analyze"
            )
        );
        assert_eq!(
            spec.headers,
    ...
    fn atelier_facial_ingest_analysis_parser_requires_summary_artifacts_and_schema() {
        ...
        let row = serde_json::json!({
            ...
            "receipt_ref": "artifact://.handshake/artifacts/L1/facial-receipt/payload",
            "analysis_artifact": {
                "artifact_ref": "artifact://.handshake/artifacts/L1/facial-analysis/payload",
                "manifest_ref": "artifact://.handshake/artifacts/L1/facial-analysis/artifact.json",
                "content_hash": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
                "byte_len": 4096,
                "mime": "application/json",
            ...
            "receipt_artifact": {
                "artifact_ref": "artifact://.handshake/artifacts/L1/facial-receipt/payload",
                "manifest_ref": "artifact://.handshake/artifacts/L1/facial-receipt/artifact.json",
                "content_hash": "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
                "byte_len": 2048,
                "mime": "application/json",
        ...
        let parsed =
            parse_atelier_facial_ingest_analysis_row(&row).expect("complete Facial analysis row");
        assert_eq!(parsed.schema_id, "hsk.atelier.facial_ingest_analysis@1");
        assert_eq!(parsed.item_count, 3);
        assert_eq!(parsed.summary["decoded_count"], 2);
        assert_eq!(
            parsed.analysis_artifact.artifact_ref,
            "artifact://.handshake/artifacts/L1/facial-analysis/payload"
        );
        assert_eq!(parsed.receipt_artifact.mime, "application/json");

        let mut wrong_schema = row.clone();
        wrong_schema["schema_id"] = serde_json::json!("hsk.atelier.facial_ingest_analysis@0");
        assert!(parse_atelier_facial_ingest_analysis_row(&wrong_schema).is_none());

        let mut bad_summary_count = row.clone();
        bad_summary_count["summary"]["item_count"] = serde_json::json!(2);
        assert!(parse_atelier_facial_ingest_analysis_row(&bad_summary_count).is_none());

        let mut missing_provenance = row.clone();
        ...
        let mut missing_counts = row.clone();
        missing_counts["summary"]
            .as_object_mut()
            .expect("summary object")
            .remove("review_recommendation_counts");
        assert!(parse_atelier_facial_ingest_analysis_row(&missing_counts).is_none());
        ...
        assert!(parse_atelier_facial_ingest_analysis_row(&bad_artifact_hash).is_none());

        let mut bad_receipt_mime = row;
        bad_receipt_mime["receipt_artifact"]["mime"] = serde_json::json!("text/plain");
        assert!(parse_atelier_facial_ingest_analysis_row(&bad_receipt_mime).is_none());
    }
